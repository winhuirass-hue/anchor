...
G_DEFINE_TYPE (AnchorFloatingBar, anchor_floating_bar,
               GTK_TYPE_BOX);

static void
action_button_clicked_cb (GtkButton *button,
			  AnchorFloatingBar *self)
{
	gint action_id;
...
static void
anchor_floating_bar_finalize (GObject *obj)
{
	AnchorFloatingBar *self = NAUTILUS_FLOATING_BAR (obj);

	g_free (self->priv->primary_label);
	g_free (self->priv->details_label);

	G_OBJECT_CLASS (anchor_floating_bar_parent_class)->finalize (obj);
}

static void
anchor_floating_bar_get_property (GObject *object,
				    guint property_id,
				    GValue *value,
				    GParamSpec *pspec)
{
	AnchorFloatingBar *self = NAUTILUS_FLOATING_BAR (object);
...
static void
anchor_floating_bar_set_property (GObject *object,
				    guint property_id,
				    const GValue *value,
				    GParamSpec *pspec)
{
	...
	switch (property_id) {
	case PROP_PRIMARY_LABEL:
		anchor_floating_bar_set_primary_label (self, g_value_get_string (value));
		break;
	case PROP_DETAILS_LABEL:
		anchor_floating_bar_set_details_label (self, g_value_get_string (value));
		break;
	case PROP_SHOW_SPINNER:
		anchor_floating_bar_set_show_spinner (self, g_value_get_boolean (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
...
static void
update_labels (AnchorFloatingBar *self)
{
...
static void
anchor_floating_bar_parent_set (GtkWidget *widget,
...
static void
anchor_floating_bar_show (GtkWidget *widget)
{
	AnchorFloatingBar *self = NAUTILUS_FLOATING_BAR (widget);

	GTK_WIDGET_CLASS (anchor_floating_bar_parent_class)->show (widget);
...
static void
anchor_floating_bar_hide (GtkWidget *widget)
{
	AnchorFloatingBar *self = NAUTILUS_FLOATING_BAR (widget);

	GTK_WIDGET_CLASS (anchor_floating_bar_parent_class)->hide (widget);

	gtk_spinner_stop (GTK_SPINNER (self->priv->spinner));
}

static gboolean
anchor_floating_bar_draw (GtkWidget *widget,
			    cairo_t *cr)
{
	...
	return GTK_WIDGET_CLASS (anchor_floating_bar_parent_class)->draw (widget, cr);;
}

static void
anchor_floating_bar_constructed (GObject *obj)
{
	AnchorFloatingBar *self = NAUTILUS_FLOATING_BAR (obj);
	GtkWidget *w, *box, *labels_box;

	G_OBJECT_CLASS (anchor_floating_bar_parent_class)->constructed (obj);
...
static void
anchor_floating_bar_init (AnchorFloatingBar *self)
{
	GtkStyleContext *context;

	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ANCHOR_TYPE_FLOATING_BAR,
						  AnchorFloatingBarDetails);
...
static void
anchor_floating_bar_class_init (AnchorFloatingBarClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);

	oclass->constructed = anchor_floating_bar_constructed;
	oclass->set_property = anchor_floating_bar_set_property;
	oclass->get_property = anchor_floating_bar_get_property;
	oclass->finalize = anchor_floating_bar_finalize;

	wclass->draw = anchor_floating_bar_draw;
	wclass->show = anchor_floating_bar_show;
	wclass->hide = anchor_floating_bar_hide;
	wclass->parent_set = anchor_floating_bar_parent_set;
	...
	g_type_class_add_private (klass, sizeof (AnchorFloatingBarDetails));
	g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
}

void
anchor_floating_bar_set_primary_label (AnchorFloatingBar *self,
					 const gchar *label)
{
...
void
anchor_floating_bar_set_details_label (AnchorFloatingBar *self,
					 const gchar *label)
{
...
void
anchor_floating_bar_set_labels (AnchorFloatingBar *self,
				  const gchar *primary_label,
				  const gchar *details_label)
{
	anchor_floating_bar_set_primary_label (self, primary_label);
	anchor_floating_bar_set_details_label (self, details_label);
}

void
anchor_floating_bar_set_show_spinner (AnchorFloatingBar *self,
					gboolean show_spinner)
{
...
GtkWidget *
anchor_floating_bar_new (const gchar *primary_label,
			   const gchar *details_label,
			   gboolean show_spinner)
{
	return g_object_new (ANCHOR_TYPE_FLOATING_BAR,
...
void
anchor_floating_bar_add_action (AnchorFloatingBar *self,
...
void
anchor_floating_bar_cleanup_actions (AnchorFloatingBar *self)
{
	GtkWidget *widget;
	GList *children, *l;
	gpointer data;
