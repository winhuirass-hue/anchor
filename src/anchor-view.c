...
#include <libanchor-private/anchor-desktop-directory.h>
...
#include <libanchor-private/anchor-file-changes-queue.h>
...
#include <libanchor-private/anchor-file-private.h>
...
#include <libanchor-private/anchor-recent.h>
#include <libanchor-private/anchor-module.h>
#include <libanchor-private/anchor-profile.h>
...
#include <libanchor-private/anchor-file-undo-manager.h>
...
struct AnchorViewDetails
{
	AnchorWindowSlot *slot;
	AnchorDirectory *model;
	AnchorFile *directory_as_file;
	AnchorFile *location_popup_directory_as_file;
	 ...
	 * This flag should need not be set inside subclasses. AnchorView automatically
...
static void     anchor_view_create_links_for_files           (AnchorView      *view,
							        GList                *files,
...
static void     trash_or_delete_files                          (GtkWindow            *parent_window,
								...
								AnchorView      *view);
static void     load_directory                                 (AnchorView      *view,
								AnchorDirectory    *directory);
static void     anchor_view_merge_menus                      (AnchorView      *view);
static void     anchor_view_unmerge_menus                    (AnchorView      *view);
static void     anchor_view_set_show_hidden_files           (AnchorView      *view,
							       gboolean           show_hidden);
static void     clipboard_changed_callback                     (AnchorClipboardMonitor *monitor,
								AnchorView      *view);
static void     open_one_in_new_window                         (gpointer              data,
								gpointer              callback_data);
static void     schedule_update_menus                          (AnchorView      *view);
static void     remove_update_menus_timeout_callback           (AnchorView      *view);
static void     schedule_update_status                          (AnchorView      *view);
static void     remove_update_status_idle_callback             (AnchorView *view); 
static void     reset_update_interval                          (AnchorView      *view);
static void     schedule_idle_display_of_pending_files         (AnchorView      *view);
static void     unschedule_display_of_pending_files            (AnchorView      *view);
static void     disconnect_model_handlers                      (AnchorView      *view);
static void     metadata_for_directory_as_file_ready_callback  (AnchorFile         *file,
								gpointer              callback_data);
static void     metadata_for_files_in_directory_ready_callback (AnchorDirectory    *directory,
								GList                *files,
								gpointer              callback_data);
static void     anchor_view_trash_state_changed_callback     (AnchorTrashMonitor *trash,
							        gboolean              state,
							        gpointer              callback_data);
static void     anchor_view_select_file                      (AnchorView      *view,
							        AnchorFile         *file);

static void     update_templates_directory                     (AnchorView *view);
static void     user_dirs_changed                              (AnchorView *view);

static gboolean file_list_all_are_folders                      (GList *file_list);

static void unschedule_pop_up_location_context_menu (AnchorView *view);

G_DEFINE_TYPE (AnchorView, anchor_view, GTK_TYPE_SCROLLED_WINDOW);

/* virtual methods (public and non-public) */

/**
 * anchor_view_merge_menus:
 * 
 * Add this view's menus to the window's menu bar.
 * @view: AnchorView in question.
 */
static void
anchor_view_merge_menus (AnchorView *view)
{
...
static void
anchor_view_unmerge_menus (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
static char *
real_get_backing_uri (AnchorView *view)
{
	AnchorDirectory *directory;
	char *uri;
	...
	if (NAUTILUS_IS_DESKTOP_DIRECTORY (directory)) {
		directory = anchor_desktop_directory_get_real_directory (NAUTILUS_DESKTOP_DIRECTORY (directory));
	} else {
...
char *
anchor_view_get_backing_uri (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
...
/**
 * anchor_view_select_all:
 *
 * select all the items in the view
 * 
 **/
static void
anchor_view_select_all (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
static void
anchor_view_select_first (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
static void
anchor_view_call_set_selection (AnchorView *view, GList *selection)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
static GList *
anchor_view_get_selection_for_file_transfer (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
...
/**
 * anchor_view_get_selected_icon_locations:
 *
 ...
 **/
static GArray *
anchor_view_get_selected_icon_locations (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
...
static void
anchor_view_invert_selection (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
/**
 * anchor_view_reveal_selection:
 *
 * Scroll as necessary to reveal the selected items.
 **/
static void
anchor_view_reveal_selection (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
/**
 * anchor_view_reset_to_defaults:
 *
 * set sorting order, zoom level, etc. to match defaults
 * 
 **/
static void
anchor_view_reset_to_defaults (AnchorView *view)
{
	GtkAction *action;
...
static gboolean
anchor_view_using_manual_layout (AnchorView  *view)
{
 ...
 **/
static gboolean
anchor_view_can_rename_file (AnchorView *view, AnchorFile *file)
{
	return 	NAUTILUS_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_rename_file (view, file);
}

static gboolean
anchor_view_is_read_only (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
static gboolean
showing_trash_directory (AnchorView *view)
{
	AnchorFile *file;

	file = anchor_view_get_directory_as_file (view);
	if (file != NULL) {
		return anchor_file_is_in_trash (file);
	}
	return FALSE;
}

static gboolean
showing_network_directory (AnchorView *view)
{
	AnchorFile *file;

	file = anchor_view_get_directory_as_file (view);
	if (file != NULL) {
		return anchor_file_is_in_network (file);
	}
	return FALSE;
}

static gboolean
showing_recent_directory (AnchorView *view)
{
	AnchorFile *file;

	file = anchor_view_get_directory_as_file (view);
	if (file != NULL) {
		return anchor_file_is_in_recent (file);
	}
	return FALSE;
}

static gboolean
anchor_view_supports_creating_files (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);

	return !anchor_view_is_read_only (view)
		&& !showing_trash_directory (view)
		&& !showing_recent_directory (view);
}

static gboolean
anchor_view_is_empty (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
/**
 * anchor_view_bump_zoom_level:
 *
 * bump the current zoom level by invoking the relevant subclass through the slot
 * 
 **/
void
anchor_view_bump_zoom_level (AnchorView *view,
			       int zoom_increment)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));

	if (!anchor_view_supports_zooming (view)) {
		return;
	}
 ...
 **/
void
anchor_view_zoom_to_level (AnchorView *view,
...
AnchorZoomLevel
anchor_view_get_zoom_level (AnchorView *view)
{
 ...
 **/
gboolean
anchor_view_can_zoom_out (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
gboolean
anchor_view_supports_zooming (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
/**
 * anchor_view_restore_default_zoom_level:
 *
 * restore to the default zoom level by invoking the relevant subclass through the slot
 * 
 **/
void
anchor_view_restore_default_zoom_level (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
/**
 * anchor_view_get_selection:
 *
 * Get a list of AnchorFile pointers that represents the
 ...
 * @view: AnchorView whose selected items are of interest.
 * 
 * Return value: GList of AnchorFile pointers representing the selection.
 * 
 **/
GList *
anchor_view_get_selection (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
...
/**
 * anchor_view_update_menus:
 * 
 * Update the sensitivity and wording of dynamic menu items.
 * @view: AnchorView in question.
 */
void
anchor_view_update_menus (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
			           ...
			           AnchorView *directory_view)
{
	...
	result->files = anchor_file_list_copy (files);
...
application_launch_parameters_free (ApplicationLaunchParameters *parameters)
{
	...
	anchor_file_list_free (parameters->files);
...
file_and_directory_list_to_files (GList *fad_list)
{
	...
	for (l = fad_list; l != NULL; l = l->next) {
		fad = l->data;
		res = g_list_prepend (res, anchor_file_ref (fad->file));
	}
...
static GList *
file_and_directory_list_from_files (AnchorDirectory *directory, GList *files)
{
	GList *res, *l;
	FileAndDirectory *fad;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		fad->directory = anchor_directory_ref (directory);
		fad->file = anchor_file_ref (l->data);
...
{
	anchor_directory_unref (fad->directory);
	anchor_file_unref (fad->file);
	g_free (fad);
}
...
script_launch_parameters_new (AnchorFile *file,
			      AnchorView *directory_view)
{
...
{
	g_object_unref (parameters->directory_view);
	anchor_file_unref (parameters->file);
	g_free (parameters);
}			      

static CreateTemplateParameters *
create_template_parameters_new (AnchorFile *file,
				AnchorView *directory_view)
{
...
static AnchorWindow *
anchor_view_get_window (AnchorView  *view)
{
	return anchor_window_slot_get_window (view->details->slot);
}

AnchorWindowSlot *
anchor_view_get_anchor_window_slot (AnchorView  *view)
{
 ...
 */
static GtkWindow *
anchor_view_get_containing_window (AnchorView *view)
{
	GtkWidget *window;
...
static gboolean
anchor_view_confirm_multiple (GtkWindow *parent_window,
				int count,
				gboolean tabs)
{
	GtkDialog *dialog;
...
static gboolean
selection_contains_one_item_in_menu_callback (AnchorView *view, GList *selection)
{
...
static gboolean
selection_not_empty_in_menu_callback (AnchorView *view, GList *selection)
{
	if (selection != NULL) {
		return TRUE;
	}
...
static char *
get_view_directory (AnchorView *view)
{
	char *uri, *path;
	GFile *f;
	
	uri = anchor_directory_get_uri (view->details->model);
	if (eel_uri_is_desktop (uri)) {
		g_free (uri);
		uri = anchor_get_desktop_directory_uri ();
...
void
anchor_view_preview_files (AnchorView *view,
			     GList *files,
			     GArray *locations)
{
	AnchorPreviewer *previewer;
	gchar *uri;
	guint xid;
	GtkWidget *toplevel;

	previewer = anchor_previewer_get_singleton ();
	uri = anchor_file_get_uri (files->data);
	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));

	xid = gdk_x11_window_get_xid (gtk_widget_get_window (toplevel));
	anchor_previewer_call_show_file (previewer, uri, xid, TRUE);

	g_free (uri);
}

void
anchor_view_activate_selection (AnchorView *view)
{
	GList *selection;

	selection = anchor_view_get_selection (view);
	anchor_view_activate_files (view,
				      selection,
				      0,
				      TRUE);
...
void
anchor_view_activate_files (AnchorView *view,
			      GList *files,
			      ...
			      gboolean confirm_multiple)
{
	...
	anchor_mime_activate_files (anchor_view_get_containing_window (view),
				      view->details->slot,
				      files,
				      path,
				      flags,
...
static void
anchor_view_activate_file (AnchorView *view,
			     AnchorFile *file,
			     AnchorWindowOpenFlags flags)
{
	char *path;
	...
	path = get_view_directory (view);
	anchor_mime_activate_file (anchor_view_get_containing_window (view),
				     view->details->slot,
				     file,
				     path,
				     flags);
		      ...
		      gpointer callback_data)
{
	...
	anchor_view_activate_selection (view);
}
				    ...
				    gpointer callback_data)
{
	...
	activation_location = anchor_file_get_activation_location (item);
	activation_file = anchor_file_get (activation_location);
	location = anchor_file_get_parent (activation_file);

	anchor_view_activate_file (view, location, 0);

	anchor_file_unref (location);
	anchor_file_unref (activation_file);
				...
				gpointer callback_data)
{
	...
	window = anchor_view_get_containing_window (view);

	if (anchor_view_confirm_multiple (window, g_list_length (selection), FALSE)) {
			      ...
			      gpointer callback_data)
{
	...
	if (anchor_view_confirm_multiple (window, g_list_length (selection), TRUE)) {
		anchor_view_activate_files (view,
					      selection,
				...
				gpointer user_data)
{
	...
	g_signal_emit_by_name (anchor_signaller_get_current (), "mime-data-changed");

	files.next = NULL;
	files.prev = NULL;
	files.data = file;
	anchor_launch_application (info, &files, parent_window);
...
static void
choose_program (AnchorView *view,
		AnchorFile *file)
{
	GtkWidget *dialog;
	GFile *location;
	...
	parent_window = anchor_view_get_containing_window (view);
...
static void
open_with_other_program (AnchorView *view)
{
        GList *selection;
...
static void
trash_or_delete_selected_files (AnchorView *view)
{
        GList *selection;
	...
	if (!view->details->selection_was_removed) {
		selection = anchor_view_get_selection_for_file_transfer (view);
		trash_or_delete_files (anchor_view_get_containing_window (view),
				       selection, TRUE,
				       view);
		anchor_file_list_free (selection);
...
static void
delete_selected_files (AnchorView *view)
{
        GList *selection;
	GList *node;
	GList *locations;
	...
	for (node = selection; node != NULL; node = node->next) {
		locations = g_list_prepend (locations,
					    anchor_file_get_location ((AnchorFile *) node->data));
	}
	locations = g_list_reverse (locations);

	anchor_file_operations_delete (locations, anchor_view_get_containing_window (view), NULL, NULL);
				    ...
				    gpointer callback_data)
{
	...
	selection = anchor_view_get_selection_for_file_transfer (view);
	anchor_restore_files_from_trash (selection,
					   anchor_view_get_containing_window (view));
			     ...
			     gpointer callback_data)
{
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		selected_item_locations = anchor_view_get_selected_icon_locations (view);
	        anchor_view_create_links_for_files (view, selection, selected_item_locations);
			    ...
			    gpointer callback_data)
{
	...
	anchor_view_select_all (callback_data);
}
				  ...
				  gpointer callback_data)
{
	...
	anchor_view_invert_selection (callback_data);
}
...
pattern_select_response_cb (GtkWidget *dialog, int response, gpointer user_data)
{
	...
	case GTK_RESPONSE_OK :
		...
		directory = anchor_view_get_model (view);
		selection = anchor_directory_match_pattern (directory,
							      gtk_entry_get_text (GTK_ENTRY (entry)));
			
		if (selection) {
			anchor_view_call_set_selection (view, selection);
			anchor_file_list_free (selection);

			anchor_view_reveal_selection(view);
		}
		/* fall through */
...
static void
select_pattern (AnchorView *view)
{
	GtkWidget *dialog;
	GtkWidget *label;
	GtkWidget *example;
				   ...
				   gpointer callback_data)
{
	...
	anchor_view_reset_to_defaults (callback_data);
}
...
{                
	...
	if (directory_view->details->model &&
		...
		anchor_search_directory_save_search (search);
				...
				gpointer callback_data)
{
	...
	if (directory_view->details->model &&
		...
		dialog = gtk_dialog_new_with_buttons (_("Save Search as"),
						      anchor_view_get_containing_window (directory_view),
						      0,
		...
		if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK) {
			...
			anchor_search_directory_save_to_file (search, uri);
			location = g_file_new_for_uri (uri);
			anchor_file_changes_queue_file_added (location);
			g_object_unref (location);
			anchor_file_changes_consume_changes (TRUE);
			g_free (uri);
		}
...
{                
	...
	anchor_file_operations_empty_trash (GTK_WIDGET (callback_data));
}
...
delayed_rename_file_hack_removed (RenameData *data)
{
	...
	anchor_file_unref (data->new_file);
	g_free (data);
}


static void
rename_file (AnchorView *view, AnchorFile *new_file)
{
	RenameData *data;
	...
	*/
	if (NAUTILUS_IS_LIST_VIEW (view)) {
		anchor_view_select_file (view, new_file);
		
		data = g_new (RenameData, 1);
		data->view = g_object_ref (view);
		data->new_file = anchor_file_ref (new_file);
...
static void
reveal_newly_added_folder (AnchorView *view, AnchorFile *new_file,
			   AnchorDirectory *directory, GFile *target_location)
{
	GFile *location;

	location = anchor_file_get_location (new_file);
...
typedef struct {
	...
	AnchorFile *new_folder;
} NewFolderSelectionData;

static void
rename_newly_added_folder (AnchorView *view, AnchorFile *removed_file,
			   AnchorDirectory *directory, NewFolderSelectionData *data);
...
static void
track_newly_added_locations (AnchorView *view, AnchorFile *new_file,
			     AnchorDirectory *directory, gpointer user_data)
{
	NewFolderData *data;

	data = user_data;

	g_hash_table_insert (data->added_locations, anchor_file_get_location (new_file), NULL);
}
		 ...
		 gpointer user_data)
{
	...
	file = anchor_file_get (new_folder);
	...
	if (data->selection != NULL) {
		...
		for (l = data->selection; l != NULL; l = l->next) {
			...
			uris = g_list_prepend (uris, anchor_file_get_uri ((AnchorFile *) l->data));

			old_location = anchor_file_get_location (l->data);
		...
		target_uri = anchor_file_get_uri (file);
		...
		anchor_view_move_copy_items (directory_view,
					       uris,
					       NULL,
					       target_uri,
					       GDK_ACTION_MOVE,
 ...
 fail:
	...
	if (data->directory_view != NULL) {
        ...
        anchor_file_list_free (data->selection);
	g_free (data);
}


static NewFolderData *
new_folder_data_new (AnchorView *directory_view,
		     gboolean      with_selection)
{
	...
	data->added_locations = g_hash_table_new_full (g_file_hash, (GEqualFunc)g_file_equal,
						       g_object_unref, NULL);
	if (with_selection) {
		data->selection = anchor_view_get_selection_for_file_transfer (directory_view);
	} else {
		data->selection = NULL;
	}
...
static GdkPoint *
context_menu_to_file_operation_position (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);

	if (anchor_view_using_manual_layout (view)
...
static void
anchor_view_new_folder (AnchorView *directory_view,
			  gboolean      with_selection)
{
	char *parent_uri;
	NewFolderData *data;
	...
	parent_uri = anchor_view_get_backing_uri (directory_view);
	anchor_file_operations_new_folder (GTK_WIDGET (directory_view),
					     pos, parent_uri,
...
static NewFolderData *
setup_new_folder_data (AnchorView *directory_view)
{
	NewFolderData *data;
...
void
anchor_view_new_file_with_initial_contents (AnchorView *view,
					      ...
					      GdkPoint *pos)
{
	...
	anchor_file_operations_new_file (GTK_WIDGET (view),
...
static void
anchor_view_new_file (AnchorView *directory_view,
			const char *parent_uri,
			AnchorFile *source)
{
	GdkPoint *pos;
	...
	container_uri = NULL;
	if (parent_uri == NULL) {
		container_uri = anchor_view_get_backing_uri (directory_view);
		g_assert (container_uri != NULL);
	}

	if (source == NULL) {
		anchor_view_new_file_with_initial_contents (directory_view,
	...
	g_return_if_fail (anchor_file_is_local (source));

	pos = context_menu_to_file_operation_position (directory_view);

	data = setup_new_folder_data (directory_view);

	source_uri = anchor_file_get_uri (source);

	anchor_file_operations_new_file_from_template (GTK_WIDGET (directory_view),
							 pos,
							 parent_uri != NULL ? parent_uri : container_uri,
							 NULL,
							 source_uri,
...
{                
	...
	anchor_view_new_folder (NAUTILUS_VIEW (callback_data), FALSE);
}
...
{                
	...
	anchor_view_new_folder (NAUTILUS_VIEW (callback_data), TRUE);
}
			    ...
			    gpointer callback_data)
{
	...
	if (g_list_length (selection) == 0) {
		if (view->details->directory_as_file != NULL) {
			files = g_list_append (NULL, anchor_file_ref (view->details->directory_as_file));
		...
		anchor_properties_window_present (selection, GTK_WIDGET (view), NULL);
	}
        anchor_file_list_free (selection);
}
				     ...
				     gpointer   callback_data)
{
	...
	files = g_list_append (NULL, anchor_file_ref (view->details->location_popup_directory_as_file));
...
all_files_in_trash (GList *files)
{
	...
	for (node = files; node != NULL; node = node->next) {
		if (!anchor_file_is_in_trash (NAUTILUS_FILE (node->data))) {
			return FALSE;
		}
	}
...
static gboolean
all_selected_items_in_trash (AnchorView *view)
{
	GList *selection;
	gboolean result;
				   ...
				   gpointer callback_data)
{
	...
	anchor_view_set_show_hidden_files
...
gboolean
anchor_view_should_sort_directories_first (AnchorView *view)
{
...
sort_directories_first_changed_callback (gpointer callback_data)
{
	...
	preference_value =
		g_settings_get_boolean (anchor_preferences, NAUTILUS_PREFERENCES_SORT_DIRECTORIES_FIRST);
...
show_hidden_files_changed_callback (gpointer callback_data)
{
	...
	anchor_view_set_show_hidden_files (view, preference_value);
}
...
set_up_scripts_directory_global (void)
{
	...
	scripts_directory_path = anchor_get_scripts_directory_path ();
	...
	if (g_file_test (old_scripts_directory_path, G_FILE_TEST_IS_DIR)
		...
		message = _("Anchor 3.6 deprecated this directory and tried migrating "
			    "this configuration to ~/.local/share/anchor");
...
static void
scripts_added_or_changed_callback (AnchorDirectory *directory,
				   GList *files,
				   gpointer callback_data)
{
	AnchorView *view;
...
static void
templates_added_or_changed_callback (AnchorDirectory *directory,
				     GList *files,
				     gpointer callback_data)
{
	AnchorView *view;
...
static void
add_directory_to_directory_list (AnchorView *view,
				 AnchorDirectory *directory,
				 ...
				 GCallback changed_callback)
{
	...
	if (g_list_find (*directory_list, directory) == NULL) {
		...
		anchor_directory_file_monitor_add (directory, directory_list,
						     FALSE, attributes,
						     (AnchorDirectoryCallback)changed_callback, view);
...
static void
remove_directory_from_directory_list (AnchorView *view,
				      AnchorDirectory *directory,
				      GList **directory_list,
				      GCallback changed_callback)
{
	*directory_list = g_list_remove	(*directory_list, directory);
	...
	anchor_directory_file_monitor_remove (directory, directory_list);
...
static void
add_directory_to_scripts_directory_list (AnchorView *view,
					 AnchorDirectory *directory)
{
...
static void
remove_directory_from_scripts_directory_list (AnchorView *view,
					      AnchorDirectory *directory)
{
...
static void
add_directory_to_templates_directory_list (AnchorView *view,
					   AnchorDirectory *directory)
{
	add_directory_to_directory_list (view, directory,
...
static void
remove_directory_from_templates_directory_list (AnchorView *view,
						AnchorDirectory *directory)
{
	remove_directory_from_directory_list (view, directory,
					      &view->details->templates_directory_list,
...
static void
slot_active (AnchorWindowSlot *slot,
	     AnchorView *view)
{
	if (view->details->active) {
		return;
	...
	anchor_view_merge_menus (view);
	schedule_update_menus (view);
}

static void
slot_inactive (AnchorWindowSlot *slot,
	       AnchorView *view)
{
	if (!view->details->active) {
		return;
	...
	anchor_view_unmerge_menus (view);
	remove_update_menus_timeout_callback (view);
}

void
anchor_view_grab_focus (AnchorView *view)
{
...
int
anchor_view_get_selection_count (AnchorView *view)
{
	/* FIXME: This could be faster if we special cased it in subclasses */
	GList *files;
	int len;

	files = anchor_view_get_selection (NAUTILUS_VIEW (view));
...
static void
update_undo_actions (AnchorView *view)
{
	AnchorFileUndoInfo *info;
	AnchorFileUndoManagerState undo_state;
	GtkAction *action;
	...
	info = anchor_file_undo_manager_get_action ();
	undo_state = anchor_file_undo_manager_get_state ();
	...
	if (info != NULL && 
		...
		anchor_file_undo_info_get_strings (info,
...
static void
undo_manager_changed_cb (AnchorFileUndoManager* manager,
			 AnchorView *view)
{
	if (!view->details->active) {
		return;
...
void
anchor_view_set_selection (AnchorView *anchor_view,
			     GList *selection)
{
	AnchorView *view;

	view = NAUTILUS_VIEW (anchor_view);
...
get_bulk_rename_tool ()
{
	...
	g_settings_get (anchor_preferences, NAUTILUS_PREFERENCES_BULK_RENAME_TOOL, "^ay", &bulk_rename_tool);
...
static void
anchor_view_init (AnchorView *view)
{
	AtkObject *atk_object;
	AnchorDirectory *scripts_directory;
	AnchorDirectory *templates_directory;
	char *templates_uri;

	anchor_profile_start (NULL);

	view->details = G_TYPE_INSTANCE_GET_PRIVATE (view, ANCHOR_TYPE_VIEW,
						     AnchorViewDetails);
	...
	if (set_up_scripts_directory_global ()) {
		scripts_directory = anchor_directory_get_by_uri (scripts_directory_uri);
		add_directory_to_scripts_directory_list (view, scripts_directory);
		anchor_directory_unref (scripts_directory);
	} else {
		g_warning ("Ignoring scripts directory, it may be a broken link\n");
	}

	if (anchor_should_use_templates_directory ()) {
		templates_uri = anchor_get_templates_directory_uri ();
		templates_directory = anchor_directory_get_by_uri (templates_uri);
		g_free (templates_uri);
		add_directory_to_templates_directory_list (view, templates_directory);
		anchor_directory_unref (templates_directory);
	}
	update_templates_directory (view);
	g_signal_connect_object (anchor_signaller_get_current (),
				 "user-dirs-changed",
	...
	g_signal_connect_object (anchor_trash_monitor_get (), "trash-state-changed",
				 G_CALLBACK (anchor_view_trash_state_changed_callback), view, 0);

	/* React to clipboard changes */
	g_signal_connect_object (anchor_clipboard_monitor_get (), "clipboard-changed",
				 G_CALLBACK (clipboard_changed_callback), view, 0);

	/* Register to menu provider extension signal managing menu updates */
	g_signal_connect_object (anchor_signaller_get_current (), "popup-menu-changed",
	...
	g_signal_connect_object (anchor_file_undo_manager_get (), "undo-changed",
...
static void
real_unmerge_menus (AnchorView *view)
{
...
static void
anchor_view_destroy (GtkWidget *object)
{
	AnchorView *view;
	GList *node, *next;
	...
	while (view->details->subdirectory_list != NULL) {
		anchor_view_remove_subdirectory (view,
	...
	if (view->details->model) {
		anchor_directory_unref (view->details->model);
		view->details->model = NULL;
	}
	
	if (view->details->directory_as_file) {
		anchor_file_unref (view->details->directory_as_file);
	...
	GTK_WIDGET_CLASS (anchor_view_parent_class)->destroy (object);
}

static void
anchor_view_finalize (GObject *object)
{
	AnchorView *view;
	...
	g_signal_handlers_disconnect_by_func (anchor_window_state,
					      anchor_view_display_selection_info, view);
	...
	G_OBJECT_CLASS (anchor_view_parent_class)->finalize (object);
}

/**
 * anchor_view_display_selection_info:
 *
 * Display information about the current selection, and notify the view frame of the changed selection.
 * @view: AnchorView for which to display selection info.
 *
 **/
void
anchor_view_display_selection_info (AnchorView *view)
{
	GList *selection;
	...
	for (p = selection; p != NULL; p = p->next) {
		...
		if (anchor_file_is_directory (file)) {
			folder_count++;
			if (anchor_file_get_directory_item_count (file, &file_item_count, NULL)) {
			...
			non_folder_count++;
			if (!anchor_file_can_get_size (file)) {
				non_folder_size_known = TRUE;
				non_folder_size += anchor_file_get_size (file);
			}
		}

		if (first_item_name == NULL) {
			first_item_name = anchor_file_get_display_name (file);
		}
	}
	...
	anchor_window_slot_set_status (view->details->slot,
...
static void
anchor_view_send_selection_change (AnchorView *view)
{
	g_signal_emit (view, signals[SELECTION_CHANGED], 0);
}

void
anchor_view_load_location (AnchorView *anchor_view,
			     GFile        *location)
{
	AnchorDirectory *directory;
	AnchorView *directory_view;

	directory_view = NAUTILUS_VIEW (anchor_view);
	anchor_profile_start (NULL);
	      ...
	      gboolean all_files_seen)
{
	...
	window = anchor_view_get_window (view);

	/* This can be called during destruction, in which case there
	 * is no AnchorWindow any more.
	 */
	if (window != NULL) {
		schedule_update_menus (view);
...
debuting_files_data_free (DebutingFilesData *data)
{
	...
	anchor_file_list_free (data->added_files);
	g_free (data);
}
 ...
 */
static void
debuting_files_add_file_callback (AnchorView *view,
				  AnchorFile *new_file,
				  AnchorDirectory *directory,
...
static void
pre_copy_move_add_file_callback (AnchorView *view,
				 AnchorFile *new_file,
				 AnchorDirectory *directory,
...
/* This needs to be called prior to anchor_file_operations_copy_move.
...
pre_copy_move (AnchorView *directory_view)
{
...
copy_move_done_partition_func (gpointer data, gpointer callback_data)
{
	...
	location = anchor_file_get_location (NAUTILUS_FILE (data));
	...
	if (directory_view != NULL) {
		...
		if (g_hash_table_size (debuting_files) == 0) {
			...
			if (debuting_files_data->added_files != NULL) {
				anchor_view_call_set_selection (directory_view,
								  debuting_files_data->added_files);
				anchor_view_reveal_selection (directory_view);
			}
...
static gboolean
view_file_still_belongs (AnchorView *view,
			 AnchorFile *file,
			 AnchorDirectory *directory)
{
	...
	return anchor_directory_contains_file (directory, file);
}

static gboolean
still_should_show_file (AnchorView *view, AnchorFile *file, AnchorDirectory *directory)
{
	return anchor_view_should_show_file (view, file) &&
		view_file_still_belongs (view, file, directory);
}

static gboolean
ready_to_load (AnchorFile *file)
{
	return anchor_file_check_if_ready (file,
					     NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);
}
...
static void
sort_files (AnchorView *view, GList **list)
{
 ...
 */
static void
process_new_files (AnchorView *view)
{
	...
	for (node = new_added_files; node != NULL; node = next) {
		...
		if (anchor_view_should_show_file (view, pending->file)) {
	...
	for (node = new_changed_files; node != NULL; node = next) {
		...
		if (!still_should_show_file (view, pending->file, pending->directory) || ready_to_load (pending->file)) {
			...
			} else if (anchor_view_should_show_file (view, pending->file)) {
				new_changed_files = g_list_delete_link (new_changed_files, node);
...
static void
process_old_files (AnchorView *view)
{
	...
	if (send_selection_change) {
		 ...
		 * have changed.
		 */
		anchor_view_send_selection_change (view);
	}
}

static void
display_pending_files (AnchorView *view)
{
	...
	if (view->details->model != NULL
	    && anchor_directory_are_all_files_seen (view->details->model)
...
void
anchor_view_freeze_updates (AnchorView *view)
{
...
void
anchor_view_unfreeze_updates (AnchorView *view)
{
...
static void
remove_update_menus_timeout_callback (AnchorView *view) 
{
...
static void
update_menus_if_pending (AnchorView *view)
{
	if (!view->details->menu_states_untrustworthy) {
		return;
	}
...
static void
schedule_idle_display_of_pending_files (AnchorView *view)
{
...
static void
schedule_timeout_display_of_pending_files (AnchorView *view, guint interval)
{
...
static void
unschedule_display_of_pending_files (AnchorView *view)
{
...
static void
queue_pending_files (AnchorView *view,
		     AnchorDirectory *directory,
		     GList *files,
		     GList **pending_list)
{
	...
	*pending_list = g_list_concat (file_and_directory_list_from_files (directory, files),
	...
	if (! view->details->loading || anchor_directory_are_all_files_seen (directory)) {
		schedule_timeout_display_of_pending_files (view, view->details->update_interval);
	}
}

static void
remove_changes_timeout_callback (AnchorView *view) 
{
...
static void
reset_update_interval (AnchorView *view)
{
...
static void
schedule_changes (AnchorView *view)
{
...
static void
files_added_callback (AnchorDirectory *directory,
		      GList *files,
		      gpointer callback_data)
{
	AnchorView *view;
...
static void
files_changed_callback (AnchorDirectory *directory,
			GList *files,
			gpointer callback_data)
{
	AnchorView *view;
...
static void
done_loading_callback (AnchorDirectory *directory,
		       gpointer callback_data)
{
	AnchorView *view;
...
static void
load_error_callback (AnchorDirectory *directory,
		     GError *error,
		     gpointer callback_data)
{
	AnchorView *view;
	...
	anchor_report_error_loading_directory
		(anchor_view_get_directory_as_file (view),
		 error,
		 anchor_view_get_containing_window (view));
}

void
anchor_view_add_subdirectory (AnchorView  *view,
				AnchorDirectory*directory)
{
	AnchorFileAttributes attributes;
	...
	anchor_directory_file_monitor_add (directory,
					     &view->details->model,
					     view->details->show_hidden_files,
					     attributes,
...
void
anchor_view_remove_subdirectory (AnchorView  *view,
				   AnchorDirectory*directory)
{
	g_assert (g_list_find (view->details->subdirectory_list, directory));
	...
	anchor_directory_file_monitor_remove (directory, &view->details->model);

	anchor_directory_unref (directory);
}

/**
 * anchor_view_get_loading:
 * @view: an #AnchorView.
 *
 ...
 **/
gboolean
anchor_view_get_loading (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
GtkUIManager *
anchor_view_get_ui_manager (AnchorView  *view)
{
	AnchorWindow *window;
	...
	window = anchor_window_slot_get_window (view->details->slot);
	return anchor_window_get_ui_manager (window);
}

/**
 * anchor_view_get_model:
 *
 * Get the model for this AnchorView.
 * @view: AnchorView of interest.
 * 
 * Return value: AnchorDirectory for this view.
 * 
 **/
AnchorDirectory *
anchor_view_get_model (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
...
GdkAtom
anchor_view_get_copied_files_atom (AnchorView *view)
{
...
prepend_uri_one (gpointer data, gpointer callback_data)
{
	...
	*result = g_list_prepend (*result, anchor_file_get_uri (file));
}
...
static void
anchor_view_create_links_for_files (AnchorView *view, GList *files,
				      GArray *relative_item_points)
{
	...
	dir_uri = anchor_view_get_backing_uri (view);
	anchor_file_operations_copy_move (uris, relative_item_points, dir_uri, GDK_ACTION_LINK, 
...
static gboolean
special_link_in_selection (AnchorView *view)
{
	gboolean saw_link;
	...
	selection = anchor_view_get_selection (NAUTILUS_VIEW (view));
...
static gboolean
desktop_or_home_dir_in_selection (AnchorView *view)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		saw_desktop_or_home_dir =
			...
			|| anchor_file_is_desktop_directory (file);
		       ...
		       AnchorView *view)
{
	...
	anchor_file_operations_trash_or_delete (locations,
						  parent_window,
						  (AnchorDeleteCallback) trash_or_delete_done_cb,
						  view);
	g_list_free_full (locations, g_object_unref);
}

static gboolean
can_rename_file (AnchorView *view, AnchorFile *file)
{
	return anchor_file_can_rename (file);
}

gboolean
anchor_view_get_is_renaming (AnchorView *view)
{
	return view->details->is_renaming;
}

void
anchor_view_set_is_renaming (AnchorView *view,
			       gboolean      is_renaming)
{
...
static void
start_renaming_file (AnchorView *view,
		     AnchorFile *file,
		     gboolean select_all)
{
	...
	if (file !=  NULL) {
		anchor_view_select_file (view, file);
	}
}

static void
update_context_menu_position_from_event (AnchorView *view,
...
open_one_in_new_window (gpointer data, gpointer callback_data)
{
	...
	anchor_view_activate_file (NAUTILUS_VIEW (callback_data),
				     NAUTILUS_FILE (data),
				     NAUTILUS_WINDOW_OPEN_FLAG_NEW_WINDOW);
}

AnchorFile *
anchor_view_get_directory_as_file (AnchorView *view)
{
	g_assert (NAUTILUS_IS_VIEW (view));
				       ...
				       gpointer callback_data)
{
	...
	anchor_launch_application 
		(launch_parameters->application,
		 launch_parameters->files,
		 anchor_view_get_containing_window (launch_parameters->directory_view));
}
	     ...
	     gboolean add_action)
{
	...
	if (parent_path != NULL) {
		action_name = anchor_escape_action_name (uri, "submenu_");
...
static void
add_application_to_open_with_menu (AnchorView *view,
				   GAppInfo *application, 
				   GList *files,
				   int index,
...
static void
add_x_content_apps (AnchorView *view, AnchorFile *file, GList **applications)
{
	GMount *mount;
	...
	x_content_types = anchor_get_cached_x_content_types_for_mount (mount);
	if (x_content_types != NULL) {
		...
		anchor_get_x_content_types_for_mount_async (mount,
...
static void
reset_open_with_menu (AnchorView *view, GList *selection)
{
	...
	anchor_ui_prepare_merge_ui (ui_manager,
				      "OpenWithGroup",
				      &view->details->open_with_merge_id,
				      &view->details->open_with_action_group);
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		other_applications_visible &= (!anchor_mime_file_opens_in_view (file) &&
					       !anchor_file_is_anchor_link (file));
	}

	default_app = NULL;
	if (filter_default) {
		default_app = anchor_mime_get_default_application_for_files (selection);
	}

	applications = NULL;
	if (other_applications_visible) {
		applications = anchor_mime_get_applications_for_files (selection);
	}
	...
	for (node = applications, index = 0; node != NULL; node = node->next, index++) {
		...
		gtk_ui_manager_add_ui (anchor_view_get_ui_manager (view),
				       view->details->open_with_merge_id,
				       menu_path,
				       "separator",
				       NULL,
			      ...
			      GList *selection)
{
	...
	for (l = providers; l != NULL; l = l->next) {
		...
		file_items = anchor_menu_provider_get_file_items (provider,
								    window,
								    selection);
...
search_in_menu_items (GList* items, const char *item_name)
{
	...
	for (list = items; list != NULL; list = list->next) {
		...
		if (menu != NULL) {
			...
			submenus = anchor_menu_get_items (menu);
			ret = search_in_menu_items (submenus, item_name);
			anchor_menu_item_list_free (submenus);
			g_object_unref (menu);
			if (ret) {
				return TRUE;
			}
			   ...
			   gpointer callback_data)
{
	...
	if (is_valid) {
		anchor_menu_item_activate (data->item);
	}
}

static GdkPixbuf *
get_menu_icon_for_file (AnchorFile *file,
			GtkWidget    *widget)
{
	AnchorIconInfo *info;
	GdkPixbuf *pixbuf;
	...
	size = anchor_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
	scale = gtk_widget_get_scale_factor (widget);
	
	info = anchor_file_get_icon (file, size, scale, 0);
	pixbuf = anchor_icon_info_get_pixbuf_nodefault_at_size (info, size);
	g_object_unref (info);
...
static GtkAction *
add_extension_action_for_files (AnchorView *view, 
				AnchorMenuItem *item,
				GList *files)
{
	...
	if (icon != NULL) {
		pixbuf = anchor_ui_get_menu_icon (icon, GTK_WIDGET (view));
		if (pixbuf != NULL) {
			gtk_action_set_gicon (action, G_ICON (pixbuf));
	...
	data->view = view;
	data->selection = anchor_file_list_copy (files);
	data->action = action;
...
static void
add_extension_menu_items (AnchorView *view,
			  GList *files,
			  GList *menu_items,
...
static void
reset_extension_actions_menu (AnchorView *view, GList *selection)
{
	GList *items;
	GtkUIManager *ui_manager;
...
static char *
change_to_view_directory (AnchorView *view)
{
	char *path;
	char *old_path;
				   ...
				   AnchorDirectory *model)
{
	...
	model_location = anchor_directory_get_location (model);

	for (node = selection, i = 0; node != NULL; node = node->next, i++) {
		file = NAUTILUS_FILE (node->data);

		if (!anchor_file_is_local (file)) {
			parameters[i] = NULL;
			g_strfreev (parameters);
			return NULL;
		}

		file_location = anchor_file_get_location (NAUTILUS_FILE (node->data));
...
get_file_paths_or_uris_as_newline_delimited_string (GList *selection, gboolean get_paths)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		if (NAUTILUS_IS_DESKTOP_ICON_FILE (node->data)) {
			link = anchor_desktop_icon_file_get_link (NAUTILUS_DESKTOP_ICON_FILE (node->data));
			if (link != NULL) {
			...
			uri = anchor_file_get_uri (NAUTILUS_FILE (node->data));
		}
		if (uri == NULL) {
			continue;
		}
...
static void
get_strings_for_environment_variables (AnchorView *view, GList *selected_files,
				       char **file_paths, char **uris, char **uri)
{
	char *directory_uri;

	/* We need to check that the directory uri starts with "file:" since
	 * anchor_directory_is_local returns FALSE for nfs.
	 */
	directory_uri = anchor_directory_get_uri (view->details->model);
 ...
 * to take advantage of the current Anchor state.
 */
static void
set_script_environment_variables (AnchorView *view, GList *selected_files)
{
	char *file_paths;
	char *uris;
	char *uri;
	...
	geometry_string = eel_gtk_window_get_geometry_string
		(GTK_WINDOW (anchor_view_get_containing_window (view)));
...
run_script_callback (GtkAction *action, gpointer callback_data)
{
	...
	file_uri = anchor_file_get_uri (launch_parameters->file);
	...
	selected_files = anchor_view_get_selection (launch_parameters->directory_view);
	...
	anchor_launch_application_from_command_array (screen, quoted_path, FALSE,
							(const char * const *) parameters);
	g_strfreev (parameters);

	anchor_file_list_free (selected_files);
...
static void
add_script_to_scripts_menus (AnchorView *directory_view,
			     AnchorFile *file,
			     ...
			     const char *popup_bg_path)
{
	...
	action_name = anchor_escape_action_name (uri, "script_");
	...
	ui_manager = anchor_view_get_ui_manager (directory_view);
...
static void
add_submenu_to_directory_menus (AnchorView *directory_view,
				GtkActionGroup *action_group,
				guint merge_id,
				AnchorFile *file,
				const char *menu_path,
...
static gboolean
update_directory_in_scripts_menu (AnchorView *view, AnchorDirectory *directory)
{
	...
	file_list = anchor_directory_get_file_list (directory);
	filtered = anchor_file_list_filter_hidden (file_list, FALSE);
	anchor_file_list_free (file_list);

	file_list = anchor_file_list_sort_by_display_name (filtered);

	any_scripts = FALSE;
	for (node = file_list; node != NULL; node = node->next) {
		file = node->data;

		if (anchor_file_is_launchable (file)) {
			add_script_to_scripts_menus (view, file, menu_path, popup_path, popup_bg_path);
			any_scripts = TRUE;
		} else if (anchor_file_is_directory (file)) {
			uri = anchor_file_get_uri (file);
			if (directory_belongs_in_scripts_menu (uri)) {
				dir = anchor_directory_get_by_uri (uri);
				add_directory_to_scripts_directory_list (view, dir);
				anchor_directory_unref (dir);
...
static void
update_scripts_menu (AnchorView *view)
{
	gboolean any_scripts;
	...
	any_scripts = FALSE;
	sorted_copy = anchor_directory_list_sort_by_uri
		(anchor_directory_list_copy (view->details->scripts_directory_list));
	...
	anchor_directory_list_free (sorted_copy);
...
create_template_callback (GtkAction *action, gpointer callback_data)
{
	...
	anchor_view_new_file (parameters->directory_view, NULL, parameters->file);
}

static void
add_template_to_templates_menus (AnchorView *directory_view,
				 AnchorFile *file,
				 const char *menu_path,
				 const char *popup_bg_path)
{
	...
	tmp = anchor_file_get_display_name (file);
	...
	action_name = anchor_escape_action_name (uri, "template_");
	escaped_label = eel_str_double_underscores (name);
...
static void
update_templates_directory (AnchorView *view)
{
	AnchorDirectory *templates_directory;
	GList *node, *next;
	char *templates_uri;
...
static void
user_dirs_changed (AnchorView *view)
{
	update_templates_directory (view);
	view->details->templates_invalid = TRUE;
	schedule_update_menus (view);
...
static gboolean
update_directory_in_templates_menu (AnchorView *view,
				    ...
				    AnchorDirectory *directory)
{
	...
	for (node = file_list; num < TEMPLATE_LIMIT && node != NULL; node = node->next, num++) {
		...
		if (anchor_file_is_directory (file)) {
			...
			g_free (uri);
		} else if (anchor_file_can_read (file)) {
...
static void
update_templates_menu (AnchorView *view)
{
	gboolean any_templates;
	GList *sorted_copy, *node;
	AnchorDirectory *directory;
	...
	if (anchor_should_use_templates_directory ()) {
		templates_directory_uri = anchor_get_templates_directory_uri ();
	} else {
	...
	any_templates = FALSE;
	sorted_copy = anchor_directory_list_sort_by_uri
		(anchor_directory_list_copy (view->details->templates_directory_list));
	for (node = sorted_copy; node != NULL; node = node->next) {
		directory = node->data;
...
{      
	...
	view = NAUTILUS_VIEW (callback_data);
	anchor_window_slot_open_location (view->details->slot, location, 0);
}

static GtkMenu *
create_popup_menu (AnchorView *view, const char *popup_path)
{
	GtkWidget *menu;
	
	menu = gtk_ui_manager_get_widget (anchor_view_get_ui_manager (view),
					  popup_path);
			       ...
			       AnchorWindowSlot *slot)
{
	...
	uri = anchor_window_slot_get_location_uri (slot);
	if (uri != NULL) {
...
static void
on_slot_location_changed (AnchorWindowSlot *slot,
			  const char         *from,
			  const char         *to,
...
static void
on_slot_added (AnchorWindow     *window,
	       AnchorWindowSlot *slot,
	       CopyCallbackData   *data)
{
...
static void
on_slot_removed (AnchorWindow     *window,
		 AnchorWindowSlot *slot,
		 CopyCallbackData   *data)
{
			  ...
			  AnchorWindow   *window)
{
	...
	slots = anchor_window_get_slots (window);
	for (s = slots; s != NULL; s = s->next) {
		AnchorWindowSlot *slot = s->data;
		add_bookmarks_for_window_slot (data, slot);
		g_signal_connect (slot, "location-changed", G_CALLBACK (on_slot_location_changed), data);
	}
			    ...
			    const char *uri)
{
	...
	for (l = selection; !found && l != NULL; l = l->next) {
		GFile *parent;
		parent = anchor_file_get_parent_location (l->data);
	...
	if (response_id == GTK_RESPONSE_OK) {
		...
		for (l = copy_data->selection; l != NULL; l = l->next) {
			uris = g_list_prepend (uris,
					       anchor_file_get_uri ((AnchorFile *) l->data));
		}
		uris = g_list_reverse (uris);

		anchor_view_move_copy_items (copy_data->view, uris, NULL, target_uri,
			      ...
			      gpointer                 user_data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		char *uri;
		uri = anchor_file_get_uri (l->data);
...
get_selected_folders (GList *selection)
{
	...
	folders = NULL;
	for (l = selection; l != NULL; l = l->next) {
		if (anchor_file_is_directory (l->data))
			folders = g_list_prepend (folders, anchor_file_ref (l->data));
	}
...
static void
copy_or_move_selection (AnchorView *view,
			gboolean      is_move)
{
	GtkWidget *dialog;
	char *uri;
	...
	dialog = gtk_file_chooser_dialog_new (title,
					      GTK_WINDOW (anchor_view_get_window (view)),
					      GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER,
					      _("_Cancel"), GTK_RESPONSE_CANCEL,
	...
	if (selection != NULL) {
		...
		gtk_file_filter_add_custom (filter,
					    ...
					    folders,
					    (GDestroyNotify)anchor_file_list_free);
...
static void
copy_or_cut_files (AnchorView *view,
		   ...
		   gboolean         cut)
{
	...
	gtk_clipboard_set_with_data (anchor_clipboard_get (GTK_WIDGET (view)),
				     targets, n_targets,
				     anchor_get_clipboard_callback, anchor_clear_clipboard_callback,
				     NULL);
        gtk_target_table_free (targets, n_targets);

	anchor_clipboard_monitor_set_clipboard_info (anchor_clipboard_monitor_get (), &info);
}
...
static void
paste_clipboard_data (AnchorView *view,
		      ...
		      char *destination_uri)
{
	...
	cut = FALSE;
	item_uris = anchor_clipboard_get_uri_list_from_selection_data (selection_data, &cut,
									 copied_files_atom);

	if (item_uris != NULL && destination_uri != NULL) {
		anchor_view_move_copy_items (view, item_uris, NULL, destination_uri,
					       cut ? GDK_ACTION_MOVE : GDK_ACTION_COPY,
					       0, 0);

		/* If items are cut then remove from clipboard */
		if (cut) {
			gtk_clipboard_clear (anchor_clipboard_get (GTK_WIDGET (view)));
		}
				   ...
				   gpointer          data)
{
	...
	view_uri = anchor_view_get_backing_uri (view);
					...
					gpointer          callback_data)
{
	...
	if (view->details->slot != NULL) {
		directory_uri = anchor_file_get_activation_uri (data->target);
			     ...
			     gpointer callback_data)
{
	...
	g_object_ref (view);
	gtk_clipboard_request_contents (anchor_clipboard_get (GTK_WIDGET (view)),
					copied_files_atom,
	    ...
	    AnchorFile *target)
{
	...
	data->view = g_object_ref (view);
	data->target = anchor_file_ref (target);
...
static void
invoke_external_bulk_rename_utility (AnchorView *view,
				     GList *selection)
{
	GString *cmd;
	char *parameter;
	...
	for (walk = selection; walk; walk = walk->next) {
		file = walk->data;
		parameter = anchor_file_get_uri (file);
	...
	anchor_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (view)),
						  cmd->str, FALSE, NULL);
	g_string_free (cmd, TRUE);
}

static void
real_action_undo (AnchorView *view)
{
	GtkWidget *toplevel;

	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));
	anchor_file_undo_manager_undo (GTK_WINDOW (toplevel));
}

static void
real_action_redo (AnchorView *view)
{
	GtkWidget *toplevel;

	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));
	anchor_file_undo_manager_redo (GTK_WINDOW (toplevel));
}
...
static void
real_action_rename (AnchorView *view,
		    gboolean select_all)
{
	AnchorFile *file;
	GList *selection;
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		...
		if (selection->next != NULL) {
			...
			if (!select_all) {
				...
				select_all = anchor_file_is_directory (file);
			}
...
can_set_wallpaper (GList *selection)
{
	...
	file = NAUTILUS_FILE (selection->data);
	if (!anchor_file_is_mime_type (file, "image/*")) {
		return FALSE;
	}
				  ...
				  AnchorView *view)
{
	...
	if (can_set_wallpaper (selection)
		...
		uris = g_list_prepend (NULL, anchor_file_get_uri (file));
		anchor_file_operations_copy_move (uris,
						    NULL,
						    target_uri,
						    GDK_ACTION_COPY,
						    GTK_WIDGET (view),
...
static void
file_mount_callback (AnchorFile  *file,
...
static void
file_unmount_callback (AnchorFile  *file,
		       GFile         *result_location,
		       GError        *error,
		       gpointer       callback_data)
{
...
static void
file_eject_callback (AnchorFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
...
static void
file_stop_callback (AnchorFile  *file,
		    GFile         *result_location,
		    GError        *error,
		    gpointer       callback_data)
{
			      ...
			      gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (anchor_file_can_mount (file)) {
			mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
			g_mount_operation_set_password_save (mount_op, G_PASSWORD_SAVE_FOR_SESSION);
			anchor_file_mount (file, mount_op, NULL,
					     file_mount_callback,
					     view);
				...
				gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		file = NAUTILUS_FILE (l->data);
		if (anchor_file_can_unmount (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
			anchor_file_unmount (file, mount_op, NULL,
			      ...
			      gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (anchor_file_can_eject (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
			anchor_file_eject (file, mount_op, NULL,
...
static void
file_start_callback (AnchorFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (anchor_file_can_start (file) || anchor_file_can_start_degraded (file)) {
			mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
			anchor_file_start (file, mount_op, NULL,
			     ...
			     gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (anchor_file_can_stop (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
			anchor_file_stop (file, mount_op, NULL,
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (anchor_file_can_poll_for_media (file) && !anchor_file_is_media_check_automatic (file)) {
			anchor_file_poll_for_media (file);
		}
	}
	anchor_file_list_free (selection);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
	g_mount_operation_set_password_save (mount_op, G_PASSWORD_SAVE_FOR_SESSION);
	anchor_file_mount (file, mount_op, NULL, file_mount_callback, view);
	g_object_unref (mount_op);
}
				     ...
				     gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
	anchor_file_unmount (file, mount_op, NULL, file_unmount_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
	anchor_file_eject (file, mount_op, NULL, file_eject_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer   data)
{
	...
	mount_op = gtk_mount_operation_new (anchor_view_get_containing_window (view));
	anchor_file_start (file, mount_op, NULL, file_start_callback, view);
	g_object_unref (mount_op);
}
					 ...
					 gpointer   callback_data)
{
	...
	anchor_view_activate_file (view,
				     file,
				     NAUTILUS_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
				 ...
				 gpointer   callback_data)
{
	...
	anchor_file_operations_delete (files, anchor_view_get_containing_window (view),
					 NULL, NULL);
					     ...
					     gpointer callback_data)
{
	...
	l.next = NULL;
	l.data = file;
	anchor_restore_files_from_trash (&l,
					   anchor_view_get_containing_window (view));
}

gboolean
anchor_view_get_show_hidden_files (AnchorView *view)
{
	return view->details->show_hidden_files;
}

static void
anchor_view_set_show_hidden_files (AnchorView *view,
				     gboolean show_hidden)
{
...
static void
real_merge_menus (AnchorView *view)
{
	GtkActionGroup *action_group;
	GtkUIManager *ui_manager;
	GtkAction *action;
	...
	view->details->dir_merge_id = gtk_ui_manager_add_ui_from_resource (ui_manager, "/org/gnome/anchor/anchor-directory-view-ui.xml", NULL);
...
static gboolean
can_paste_into_file (AnchorFile *file)
{
	if (anchor_file_is_directory (file) &&
	    anchor_file_can_write (file)) {
		return TRUE;
	}
	if (anchor_file_has_activation_uri (file)) {
		GFile *location;
		AnchorFile *activation_file;
		gboolean res;
		
		location = anchor_file_get_activation_location (file);
		activation_file = anchor_file_get (location);
		g_object_unref (location);
	
		/* The target location might not have data for it read yet,
		   and we can't want to do sync I/O, so treat the unknown
		   case as can-write */
		res = (anchor_file_get_file_type (activation_file) == G_FILE_TYPE_UNKNOWN) ||
			(anchor_file_get_file_type (activation_file) == G_FILE_TYPE_DIRECTORY &&
			 anchor_file_can_write (activation_file));
			    ...
			    gpointer          user_data)
{
	...
	gtk_action_set_sensitive (action,
				  can_paste && !anchor_view_is_read_only (view));
...
static gboolean
should_show_empty_trash (AnchorView *view)
{
...
file_list_all_are_folders (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (anchor_file_is_anchor_link (file) &&
			...
			if (anchor_file_is_launcher (file)) {
				return FALSE;
			}
				
			activation_uri = anchor_file_get_activation_uri (file);
			...
			linked_file = anchor_file_get_existing_by_uri (activation_uri);
			...
			is_dir =
				(linked_file != NULL &&
				 anchor_file_is_directory (linked_file)) ||
				(activation_uri != NULL &&
				 activation_uri[strlen (activation_uri) - 1] == '/');
			
			anchor_file_unref (linked_file);
			g_free (activation_uri);
...
static void
file_should_show_foreach (AnchorFile        *file,
			  gboolean            *show_mount,
			  ...
			  GDriveStartStopType *start_stop_type)
{
	...
	*show_poll = FALSE;
	...
	if (anchor_file_can_unmount (file) && !*show_eject && !*show_stop) {
		*show_unmount = TRUE;
	}
	...
	*start_stop_type = anchor_file_get_start_stop_type (file);
}

static void
file_should_show_self (AnchorFile        *file,
		       gboolean            *show_mount,
		       gboolean            *show_unmount,
		       gboolean            *show_eject,
		       gboolean            *show_start,
...
files_are_all_directories (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = NAUTILUS_FILE (l->data);
		all_directories &= anchor_file_is_directory (file);
	}
...
files_is_none_directory (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = NAUTILUS_FILE (l->data);
		no_directory &= !anchor_file_is_directory (file);
	}
				  ...
				  gboolean is_self)
{
	...
	if (files != NULL) {
		if (g_list_length (files) == 1) {
			original_file = anchor_file_get_trash_original_file (files->data);
		} else {
			original_dirs_hash = anchor_trashed_files_get_original_directories (files, NULL);
			if (original_dirs_hash != NULL) {
				original_dirs = g_hash_table_get_keys (original_dirs_hash);
				if (g_list_length (original_dirs) == 1) {
					original_dir = anchor_file_ref (NAUTILUS_FILE (original_dirs->data));
				}
			}
		}
	}
	...
	if (original_file != NULL || original_dirs != NULL) {
		...
		if (original_file != NULL) {
			original_location = anchor_file_get_location (original_file);
		} else if (original_dir != NULL) {
			original_location = anchor_file_get_location (original_dir);
		}
	...
	anchor_file_unref (original_file);
	anchor_file_unref (original_dir);
...
static void
real_update_menus_volumes (AnchorView *view,
			   GList *selection,
...
static void
real_update_location_menu_volumes (AnchorView *view)
{
	GtkAction *action;
	AnchorFile *file;
	gboolean show_mount;
 ...
 */
static void
real_update_paste_menu (AnchorView *view,
			GList *selection,
			gint selection_count)
{
	...
	selection_is_read_only = selection_count == 1 &&
		(!anchor_file_can_write (NAUTILUS_FILE (selection->data)) &&
		 !anchor_file_has_activation_uri (NAUTILUS_FILE (selection->data)));

	is_read_only = anchor_view_is_read_only (view);
	...
	gtk_clipboard_request_targets (anchor_clipboard_get (GTK_WIDGET (view)),
				       clipboard_targets_received,
				       view);
}

static void
real_update_location_menu (AnchorView *view)
{
	GtkAction *action;
	AnchorFile *file;
	...
	file = view->details->location_popup_directory_as_file;
	g_assert (NAUTILUS_IS_FILE (file));
	g_assert (anchor_file_check_if_ready (file, NAUTILUS_FILE_ATTRIBUTE_INFO |
						NAUTILUS_FILE_ATTRIBUTE_MOUNT |
						NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO));

	is_special_link = NAUTILUS_IS_DESKTOP_ICON_FILE (file);
	is_desktop_or_home_dir = anchor_file_is_home (file)
		|| anchor_file_is_desktop_directory (file);
	is_recent = anchor_file_is_in_recent (file);

	can_delete_file =
		anchor_file_can_delete (file) &&
		!is_special_link &&
	...
	if (file != NULL &&
		...
		show_separate_delete_command = g_settings_get_boolean (anchor_preferences, NAUTILUS_PREFERENCES_ENABLE_DELETE);
	}
...
static void
clipboard_changed_callback (AnchorClipboardMonitor *monitor, AnchorView *view)
{
	GList *selection;
	gint selection_count;
...
can_delete_all (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = l->data;
		if (!anchor_file_can_delete (file)) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
real_update_menus (AnchorView *view)
{
	GList *selection, *l;
	gint selection_count;
	...
	can_create_files = anchor_view_supports_creating_files (view);
	can_delete_files =
	...
	if (selection_count > 1) {
		...
		gtk_action_set_sensitive (action,
					  selection_count == 1 &&
					  anchor_view_can_rename_file (view, selection->data));
	}
	gtk_action_set_visible (action, !selection_contains_recent);
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (!anchor_mime_file_opens_in_external_app (file)) {
			show_app = FALSE;
		}

		if (!anchor_mime_file_launches (file)) {
			show_run = FALSE;
		}
	...
	if (can_open && show_app) {
		app = anchor_mime_get_default_application_for_files (selection);
	}
	...
	menuitem = gtk_ui_manager_get_widget (
					      anchor_view_get_ui_manager (view),
	...
	g_object_set (action,
		      ...
		      NULL);
	gtk_action_set_sensitive (action, !anchor_trash_monitor_is_empty ());
	...
	if (selection_contains_search) {
		...
		if (anchor_search_directory_is_saved_search (search)) {
			show_save_search = TRUE;
			save_search_sensitive = anchor_search_directory_is_modified (search);
		} else {
	...
	gtk_action_set_sensitive (action, !anchor_view_is_empty (view));
 ...
 **/
void 
anchor_view_pop_up_selection_context_menu  (AnchorView *view, 
					      GdkEventButton  *event)
{
	g_assert (NAUTILUS_IS_VIEW (view));
 ...
 **/
void 
anchor_view_pop_up_background_context_menu (AnchorView *view, 
					      GdkEventButton  *event)
{
	g_assert (NAUTILUS_IS_VIEW (view));
...
static void
real_pop_up_location_context_menu (AnchorView *view)
{
...
static void
location_popup_file_attributes_ready (AnchorFile *file,
				      gpointer      data)
{
	AnchorView *view;
...
static void
unschedule_pop_up_location_context_menu (AnchorView *view)
{
	if (view->details->location_popup_directory_as_file != NULL) {
		g_assert (NAUTILUS_IS_FILE (view->details->location_popup_directory_as_file));
		anchor_file_cancel_call_when_ready (view->details->location_popup_directory_as_file,
						      location_popup_file_attributes_ready,
						      view);
		anchor_file_unref (view->details->location_popup_directory_as_file);
		view->details->location_popup_directory_as_file = NULL;
	}
}

static void
schedule_pop_up_location_context_menu (AnchorView *view,
				       GdkEventButton  *event,
				       AnchorFile    *file)
{
	g_assert (NAUTILUS_IS_FILE (file));
	...
	if (file == view->details->location_popup_directory_as_file) {
		if (anchor_file_check_if_ready (file, NAUTILUS_FILE_ATTRIBUTE_INFO |
						  NAUTILUS_FILE_ATTRIBUTE_MOUNT |
						  NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO)) {
			real_pop_up_location_context_menu (view);
		}
		...
		view->details->location_popup_directory_as_file = anchor_file_ref (file);
		anchor_file_call_when_ready (view->details->location_popup_directory_as_file,
					       NAUTILUS_FILE_ATTRIBUTE_INFO |
					       NAUTILUS_FILE_ATTRIBUTE_MOUNT |
					       NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO,
					       location_popup_file_attributes_ready,
...
/**
 * anchor_view_pop_up_location_context_menu
 *
 ...
 **/
void 
anchor_view_pop_up_location_context_menu (AnchorView *view, 
					    GdkEventButton  *event,
					    const char      *location)
{
	AnchorFile *file;
	...
	if (location != NULL) {
		file = anchor_file_get_by_uri (location);
	} else {
		file = anchor_file_ref (view->details->directory_as_file);
	}
...
static void
schedule_update_menus (AnchorView *view) 
{
	g_assert (NAUTILUS_IS_VIEW (view));
...
static void
remove_update_status_idle_callback (AnchorView *view) 
{
...
static void
schedule_update_status (AnchorView *view) 
{
	g_assert (NAUTILUS_IS_VIEW (view));
...
/**
 * anchor_view_notify_selection_changed:
 * 
 * Notify this view that the selection has changed. This is normally
 * called only by subclasses.
 * @view: AnchorView whose selection has changed.
 * 
 **/
void
anchor_view_notify_selection_changed (AnchorView *view)
{
	GtkWindow *window;
	GList *selection;
...
static void
file_changed_callback (AnchorFile *file, gpointer callback_data)
{
	AnchorView *view = NAUTILUS_VIEW (callback_data);
 ...
 * @view: AnchorView whose location will be changed.
 * @uri: A string representing the uri to switch to.
 * 
 **/
static void
load_directory (AnchorView *view,
		AnchorDirectory *directory)
{
	...
	view->details->directory_as_file =
		anchor_directory_get_corresponding_file (directory);
	anchor_file_unref (old_file);
	...
	anchor_file_call_when_ready
		(view->details->directory_as_file,
		 attributes,
		 metadata_for_directory_as_file_ready_callback, view);
	anchor_directory_call_when_ready
		(view->details->model,
		 attributes,
		 FALSE,
	...
	attributes = 
		NAUTILUS_FILE_ATTRIBUTE_INFO |
		NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO;
	anchor_file_monitor_add (view->details->directory_as_file,
				   &view->details->directory_as_file,
				   attributes);
...
static void
finish_loading (AnchorView *view)
{
	AnchorFileAttributes attributes;
	 ...
	 */
	anchor_profile_start ("BEGIN_LOADING");
	g_signal_emit (view, signals[BEGIN_LOADING], 0);
	anchor_profile_end ("BEGIN_LOADING");

	/* Assume we have now all information to show window */
	anchor_window_view_visible  (anchor_view_get_window (view), NAUTILUS_VIEW (view));

	if (anchor_directory_are_all_files_seen (view->details->model)) {
		/* Unschedule a pending update and schedule a new one with the minimal
		 * update interval. This gives the view a short chance at gathering the
		 * (cached) deep counts.
		 */
	...
	anchor_directory_file_monitor_add (view->details->model,
					     &view->details->model,
					     view->details->show_hidden_files,
					     attributes,
					     files_added_callback, view);
...
static void
finish_loading_if_all_metadata_loaded (AnchorView *view)
{
...
static void
metadata_for_directory_as_file_ready_callback (AnchorFile *file,
			      		       gpointer callback_data)
{
	AnchorView *view;
...
static void
metadata_for_files_in_directory_ready_callback (AnchorDirectory *directory,
				   		GList *files,
			           		gpointer callback_data)
{
	AnchorView *view;
...
static void
disconnect_directory_handler (AnchorView *view, guint *id)
{
	disconnect_handler (G_OBJECT (view->details->model), id);
}

static void
disconnect_directory_as_file_handler (AnchorView *view, guint *id)
{
	disconnect_handler (G_OBJECT (view->details->directory_as_file), id);
}

static void
disconnect_model_handlers (AnchorView *view)
{
	if (view->details->model == NULL) {
		return;
	}
	...
	anchor_file_cancel_call_when_ready (view->details->directory_as_file,
					      metadata_for_directory_as_file_ready_callback,
					      view);
	anchor_directory_cancel_callback (view->details->model,
					    metadata_for_files_in_directory_ready_callback,
					    view);
	anchor_directory_file_monitor_remove (view->details->model,
						&view->details->model);
	anchor_file_monitor_remove (view->details->directory_as_file,
				      &view->details->directory_as_file);
}

static void
anchor_view_select_file (AnchorView *view, AnchorFile *file)
{
	GList file_list;
	...
	file_list.next = NULL;
	file_list.prev = NULL;
	anchor_view_call_set_selection (view, &file_list);
}
 ...
 **/
void
anchor_view_stop_loading (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
	...
	if (view->details->model != NULL) {
		anchor_directory_file_monitor_remove (view->details->model, view);
	}
	done_loading (view, FALSE);
}

gboolean
anchor_view_is_editable (AnchorView *view)
{
	AnchorDirectory *directory;
	...
	if (directory != NULL) {
		return anchor_directory_is_editable (directory);
	}
...
static gboolean
real_is_read_only (AnchorView *view)
{
	AnchorFile *file;
	
	if (!anchor_view_is_editable (view)) {
		return TRUE;
	}
	
	file = anchor_view_get_directory_as_file (view);
	if (file != NULL) {
		return !anchor_file_can_write (file);
	}
	return FALSE;
}

/**
 * anchor_view_should_show_file
 * 
 * Returns whether or not this file should be displayed based on
 * current filtering options.
 */
gboolean
anchor_view_should_show_file (AnchorView *view, AnchorFile *file)
{
...
static gboolean
real_using_manual_layout (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), FALSE);
...
void
anchor_view_ignore_hidden_file_preferences (AnchorView *view)
{
...
void
anchor_view_set_show_foreign (AnchorView *view,
...
char *
anchor_view_get_uri (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);
	if (view->details->model == NULL) {
		return NULL;
	}
	return anchor_directory_get_uri (view->details->model);
}

void
anchor_view_move_copy_items (AnchorView *view,
			       ...
			       int x, int y)
{
	...
	target_file = anchor_file_get_existing_by_uri (target_uri);
	/* special-case "command:" here instead of starting a move/copy */
	if (target_file != NULL && anchor_file_is_launcher (target_file)) {
		anchor_file_unref (target_file);
		anchor_launch_desktop_file (
					      gtk_widget_get_screen (GTK_WIDGET (view)),
					      target_uri, item_uris,
					      anchor_view_get_containing_window (view));
		return;
	} else if (copy_action == GDK_ACTION_COPY &&
		   anchor_is_file_roller_installed () &&
		   target_file != NULL &&
		   anchor_file_is_archive (target_file)) {
		...
		anchor_launch_application_from_command (screen, command, FALSE, NULL);
		g_free (command);
	...
	anchor_file_operations_copy_move
...
static void
anchor_view_trash_state_changed_callback (AnchorTrashMonitor *trash_monitor,
					    gboolean state, gpointer callback_data)
{
	AnchorView *view;

	view = (AnchorView *) callback_data;
	g_assert (NAUTILUS_IS_VIEW (view));
...
void
anchor_view_start_batching_selection_changes (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
void
anchor_view_stop_batching_selection_changes (AnchorView *view)
{
	g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
gboolean
anchor_view_get_active (AnchorView *view)
{
	g_assert (NAUTILUS_IS_VIEW (view));
...
static GArray *
real_get_selected_icon_locations (AnchorView *view)
{
...
static void
anchor_view_set_property (GObject         *object,
			    guint            prop_id,
			    const GValue    *value,
			    GParamSpec      *pspec)
{
...
gboolean
anchor_view_handle_scroll_event (AnchorView *directory_view,
				   GdkEventScroll *event)
{
	...
	if (event->state & GDK_CONTROL_MASK) {
		...
		case GDK_SCROLL_UP:
			/* Zoom In */
			anchor_view_bump_zoom_level (directory_view, 1);
			return TRUE;
...
static gboolean
anchor_view_scroll_event (GtkWidget *widget,
			    GdkEventScroll *event)
{
	AnchorView *directory_view;

	directory_view = NAUTILUS_VIEW (widget);
	if (anchor_view_handle_scroll_event (directory_view, event)) {
		return TRUE;
	}

	return GTK_WIDGET_CLASS (anchor_view_parent_class)->scroll_event (widget, event);
}


static void
anchor_view_parent_set (GtkWidget *widget,
			  GtkWidget *old_parent)
{
	...
	if (GTK_WIDGET_CLASS (anchor_view_parent_class)->parent_set != NULL) {
		GTK_WIDGET_CLASS (anchor_view_parent_class)->parent_set (widget, old_parent);
	}
	...
	if (parent != NULL) {
		...
		if (view->details->slot == 
		    anchor_window_get_active_slot (anchor_view_get_window (view))) {
			view->details->active = TRUE;
...
static void
anchor_view_class_init (AnchorViewClass *klass)
{
	GObjectClass *oclass;
	...
	oclass->finalize = anchor_view_finalize;
	oclass->set_property = anchor_view_set_property;

	widget_class->destroy = anchor_view_destroy;
	widget_class->scroll_event = anchor_view_scroll_event;
	widget_class->parent_set = anchor_view_parent_set;

	g_type_class_add_private (klass, sizeof (AnchorViewDetails));
	...
	signals[ADD_FILE] =
		g_signal_new ("add-file",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, add_file),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ANCHOR_TYPE_FILE, ANCHOR_TYPE_DIRECTORY);
	signals[BEGIN_FILE_CHANGES] =
		g_signal_new ("begin-file-changes",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, begin_file_changes),
		              NULL, NULL,
	...
	signals[BEGIN_LOADING] =
		g_signal_new ("begin-loading",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, begin_loading),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[CLEAR] =
		g_signal_new ("clear",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, clear),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[END_FILE_CHANGES] =
		g_signal_new ("end-file-changes",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, end_file_changes),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[END_LOADING] =
		g_signal_new ("end-loading",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, end_loading),
		              NULL, NULL,
	...
	signals[FILE_CHANGED] =
		g_signal_new ("file-changed",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, file_changed),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ANCHOR_TYPE_FILE, ANCHOR_TYPE_DIRECTORY);
	signals[REMOVE_FILE] =
		g_signal_new ("remove-file",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorViewClass, remove_file),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ANCHOR_TYPE_FILE, ANCHOR_TYPE_DIRECTORY);
	...
	signals[TRASH] =
		g_signal_new ("trash",
			      ...
			      G_STRUCT_OFFSET (AnchorViewClass, trash),
	...
	signals[DELETE] =
		g_signal_new ("delete",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (AnchorViewClass, delete),
			      g_signal_accumulator_true_handled, NULL,
			      g_cclosure_marshal_generic,
			      G_TYPE_BOOLEAN, 0);
