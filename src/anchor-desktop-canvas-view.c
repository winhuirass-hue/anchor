...
/* anchor-desktop-canvas-view.c - implementation of canvas view for managing the desktop.
...
#include <libanchor-private/anchor-directory-notify.h>
...
struct AnchorDesktopCanvasViewDetails
{
...
static void     real_merge_menus                                  (AnchorView        *view);
static void     real_update_menus                                 (AnchorView        *view);
static void     anchor_desktop_canvas_view_update_canvas_container_fonts  (AnchorDesktopCanvasView      *view);
static void     font_changed_callback                             (gpointer                callback_data);

G_DEFINE_TYPE (AnchorDesktopCanvasView, anchor_desktop_canvas_view, ANCHOR_TYPE_CANVAS_VIEW)

static char *desktop_directory;
static time_t desktop_dir_modify_time;

#define get_canvas_container(w) anchor_canvas_view_get_canvas_container(NAUTILUS_CANVAS_VIEW (w))

#define POPUP_PATH_CANVAS_APPEARANCE		"/selection/Canvas Appearance Items"

static void
canvas_container_set_workarea (AnchorCanvasContainer *canvas_container,
			     GdkScreen             *screen,
			     long                  *workareas,
			     int                    n_items)
{
	...
	anchor_canvas_container_set_margins (canvas_container,
					     left, right, top, bottom);
}

static void
net_workarea_changed (AnchorDesktopCanvasView *canvas_view,
		      GdkWindow         *window)
{
				   ...
				   gpointer data)
{
	...
	AnchorDesktopCanvasView *canvas_view;
...
static const char *
real_get_id (AnchorView *view)
{
	return NAUTILUS_DESKTOP_CANVAS_VIEW_ID;
}

static void
anchor_desktop_canvas_view_dispose (GObject *object)
{
	AnchorDesktopCanvasView *canvas_view;
	...
	G_OBJECT_CLASS (anchor_desktop_canvas_view_parent_class)->dispose (object);
}

static void
anchor_desktop_canvas_view_class_init (AnchorDesktopCanvasViewClass *class)
{
	AnchorViewClass *vclass;

	vclass = NAUTILUS_VIEW_CLASS (class);

	G_OBJECT_CLASS (class)->dispose = anchor_desktop_canvas_view_dispose;

	vclass->merge_menus = real_merge_menus;
	vclass->update_menus = real_update_menus;
	vclass->get_view_id = real_get_id;

	g_type_class_add_private (class, sizeof (AnchorDesktopCanvasViewDetails));
}

static void
unrealized_callback (GtkWidget *widget, AnchorDesktopCanvasView *desktop_canvas_view)
{
...
static void
realized_callback (GtkWidget *widget, AnchorDesktopCanvasView *desktop_canvas_view)
{
	GdkWindow *root_window;
	GdkScreen *screen;
...
desktop_canvas_container_realize (GtkWidget *widget,
				  AnchorDesktopCanvasView *desktop_canvas_view)
{
...
default_zoom_level_changed (gpointer user_data)
{
	...
	anchor_canvas_container_set_zoom_level (get_canvas_container (desktop_canvas_view),
						new_level);
}
...
do_desktop_rescan (gpointer data)
{
	...
	anchor_directory_force_reload
		(anchor_view_get_model (NAUTILUS_VIEW (desktop_canvas_view)));

	return TRUE;
}

static void
done_loading (AnchorDirectory *model,
	      AnchorDesktopCanvasView *desktop_canvas_view)
{
	struct stat buf;
...
/* This function is used because the AnchorDirectory model does not
 * exist always in the desktop_canvas_view, so we wait until it has been
 * instantiated.
 */
static void
delayed_init (AnchorDesktopCanvasView *desktop_canvas_view)
{
	/* Keep track of the load time. */
	g_signal_connect_object (anchor_view_get_model (NAUTILUS_VIEW (desktop_canvas_view)),
				 "done-loading",
...
font_changed_callback (gpointer callback_data)
{
	...
	anchor_desktop_canvas_view_update_canvas_container_fonts (NAUTILUS_DESKTOP_CANVAS_VIEW (callback_data));
}

static void
anchor_desktop_canvas_view_update_canvas_container_fonts (AnchorDesktopCanvasView *canvas_view)
{
	AnchorCanvasContainer *canvas_container;
	char *font;
	...
	font = g_settings_get_string (anchor_desktop_preferences,
				      NAUTILUS_PREFERENCES_DESKTOP_FONT);

	anchor_canvas_container_set_font (canvas_container, font);

	g_free (font);
}

static void
anchor_desktop_canvas_view_init (AnchorDesktopCanvasView *desktop_canvas_view)
{
	AnchorCanvasContainer *canvas_container;
	...
	desktop_canvas_view->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_canvas_view,
								  ANCHOR_TYPE_DESKTOP_CANVAS_VIEW,
								  AnchorDesktopCanvasViewDetails);
	...
	anchor_canvas_container_set_use_drop_shadows (canvas_container, TRUE);
	anchor_canvas_view_container_set_sort_desktop (NAUTILUS_CANVAS_VIEW_CONTAINER (canvas_container), TRUE);

	/* Do a reload on the desktop if we don't have FAM, a smarter
	 * way to keep track of the items on the desktop.
	 */
	if (!anchor_monitor_active ()) {
	...
	anchor_canvas_container_set_is_fixed_size (canvas_container, TRUE);
	anchor_canvas_container_set_is_desktop (canvas_container, TRUE);
	anchor_canvas_container_set_store_layout_timestamps (canvas_container, TRUE);
	...
	anchor_view_ignore_hidden_file_preferences
		(NAUTILUS_VIEW (desktop_canvas_view));

	anchor_view_set_show_foreign (NAUTILUS_VIEW (desktop_canvas_view),
					FALSE);
	...
	anchor_desktop_canvas_view_update_canvas_container_fonts (desktop_canvas_view);

	g_signal_connect_swapped (gnome_lockdown_preferences,
				  "changed::" NAUTILUS_PREFERENCES_LOCKDOWN_COMMAND_LINE,
				  G_CALLBACK (anchor_view_update_menus),
				  desktop_canvas_view);
}
				   ...
				   gpointer data)
{
	...
	anchor_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (data)),
						  control_center_cmd,
						  FALSE,
						  params, NULL);
}
					 ...
					 gpointer data)
{
	...
	anchor_file_operations_empty_trash (GTK_WIDGET (data));
}
...
action_unstretch_callback (GtkAction *action,
			   gpointer callback_data)
{
	anchor_canvas_container_unstretch (get_canvas_container (callback_data));
}
...
action_clean_up_callback (GtkAction *action,
			  gpointer callback_data)
{
	anchor_canvas_view_clean_up_by_name (NAUTILUS_CANVAS_VIEW (callback_data));
}

static gboolean
trash_link_is_selection (AnchorView *view)
{
	GList *selection;
	...
	if ((g_list_length (selection) == 1) &&
		...
		link = anchor_desktop_icon_file_get_link (NAUTILUS_DESKTOP_ICON_FILE (selection->data));
		/* link may be NULL if the link was recently removed (unmounted) */
		if (link != NULL &&
		    anchor_desktop_link_get_link_type (link) == NAUTILUS_DESKTOP_LINK_TRASH) {
			result = TRUE;
		}
		if (link) {
			g_object_unref (link);
...
{
	AnchorDesktopCanvasView *desktop_view;
        AnchorCanvasContainer *canvas_container;
	...
	NAUTILUS_VIEW_CLASS (anchor_desktop_canvas_view_parent_class)->update_menus (view);
	...
	/* Stretch */
        selection_count = anchor_view_get_selection_count (view);
	...
	gtk_action_set_sensitive (action,
				  ...
				  && !anchor_canvas_container_has_stretch_handles (canvas_container));
	...
	gtk_action_set_sensitive (action,
				  canvas_container != NULL
				  && anchor_canvas_container_is_stretched (canvas_container));
	gtk_action_set_visible (action, TRUE);
}
...
real_merge_menus (AnchorView *view)
{
	...
	NAUTILUS_VIEW_CLASS (anchor_desktop_canvas_view_parent_class)->merge_menus (view);
	...
	desktop_view->details->desktop_merge_id =
		gtk_ui_manager_add_ui_from_resource (ui_manager, "/org/gnome/anchor/anchor-desktop-canvas-view-ui.xml", NULL);
...
AnchorView *
anchor_desktop_canvas_view_new (AnchorWindowSlot *slot)
{
	return g_object_new (ANCHOR_TYPE_DESKTOP_CANVAS_VIEW,
			     "window-slot", slot,
