/*
 * anchor-window-slot-dnd.c - Handle DnD for widgets acting as
 * AnchorWindowSlot proxies
 *
...
} AnchorDragSlotProxyInfo;

static void
switch_tab (AnchorDragSlotProxyInfo *drag_info)
{
  ...
  notebook = gtk_widget_get_ancestor (GTK_WIDGET (drag_info->target_slot), ANCHOR_TYPE_NOTEBOOK);
...
static void
switch_location (AnchorDragSlotProxyInfo *drag_info)
{
  GFile *location;
  ...
  target_slot = anchor_window_get_active_slot (NAUTILUS_WINDOW (window));

  current_location = anchor_window_slot_get_location (target_slot);
  location = anchor_file_get_location (drag_info->target_file);
  if (! (current_location != NULL && g_file_equal (location, current_location))) {
	anchor_window_slot_open_location (target_slot, location, 0);
  }
...
{
  AnchorDragSlotProxyInfo *drag_info = user_data;
...
static void
slot_proxy_check_switch_location_timer (AnchorDragSlotProxyInfo *drag_info,
                                        GtkWidget *widget)
{
...
static void
slot_proxy_remove_switch_location_timer (AnchorDragSlotProxyInfo *drag_info)
{
...
slot_proxy_drag_motion (GtkWidget          *widget,
			...
			gpointer            user_data)
{
  AnchorDragSlotProxyInfo *drag_info;
  AnchorWindowSlot *target_slot;
  GtkWidget *window;
  GdkAtom target;
  int action;
  ...
  target_uri = NULL;
  if (drag_info->target_file != NULL) {
    target_uri = anchor_file_get_uri (drag_info->target_file);
  } else {
    if (drag_info->target_slot != NULL) {
    ...
    if (target_slot != NULL) {
      target_uri = anchor_window_slot_get_current_uri (target_slot);
    }
  }
  ...
  if (target_uri != NULL) {
    AnchorFile *file;
    gboolean can;
    file = anchor_file_get_existing_by_uri (target_uri);
    can = anchor_file_can_write (file);
    g_object_unref (file);
    if (!can) {
      action = 0;
      goto out;
  ...
  if (drag_info->have_data &&
    ...
    if (drag_info->info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
      anchor_drag_default_drop_action_for_icons (context, target_uri,
                                                   drag_info->data.selection_list,
                                                   &action);
    } else if (drag_info->info == NAUTILUS_ICON_DND_URI_LIST) {
      action = anchor_drag_default_drop_action_for_uri_list (context, target_uri);
...
drag_info_free (gpointer user_data)
{
  ...
  g_slice_free (AnchorDragSlotProxyInfo, drag_info);
}

static void
drag_info_clear (AnchorDragSlotProxyInfo *drag_info)
{
  slot_proxy_remove_switch_location_timer (drag_info);
  ...
  if (drag_info->info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
    anchor_drag_destroy_selection_list (drag_info->data.selection_list);
  } else if (drag_info->info == NAUTILUS_ICON_DND_URI_LIST) {
			...
			AnchorDragSlotProxyInfo *drag_info)
{
  ...
  target_view = NULL;
  if (target_slot != NULL) {
    target_view = anchor_window_slot_get_current_view (target_slot);
  }

  if (target_slot != NULL && target_view != NULL) {
    if (drag_info->info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
      uri_list = anchor_drag_uri_list_from_selection_list (drag_info->data.selection_list);
      g_assert (uri_list != NULL);

      anchor_view_drop_proxy_received_uris (target_view,
                                              uri_list,
                                              target_uri,
      ...
      anchor_view_handle_netscape_url_drop (target_view,
			       ...
			       gpointer            user_data)
{
  ...
  if (info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
    drag_info->data.selection_list = anchor_drag_build_selection_list (data);

    drag_info->have_valid_data = drag_info->data.selection_list != NULL;
  } else if (info == NAUTILUS_ICON_DND_URI_LIST) {
    uris = gtk_selection_data_get_uris (data);
    drag_info->data.uri_list = anchor_drag_uri_list_from_array ((const char **) uris);
    g_strfreev (uris);
...
void
anchor_drag_slot_proxy_init (GtkWidget *widget,
                               AnchorFile *target_file,
                               AnchorWindowSlot *target_slot)
{
  AnchorDragSlotProxyInfo *drag_info;
  ...
  drag_info = g_slice_new0 (AnchorDragSlotProxyInfo);

  g_object_set_data_full (G_OBJECT (widget), "drag-slot-proxy-data", drag_info,
                          drag_info_free);

  drag_info->is_notebook = (g_object_get_data (G_OBJECT (widget), "anchor-notebook-tab") != NULL);
