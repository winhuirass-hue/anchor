...
/* anchor-location-bar.c - Location bar for Anchor
 */
...
struct AnchorLocationEntryDetails {
...
G_DEFINE_TYPE (AnchorLocationEntry, anchor_location_entry, ANCHOR_TYPE_ENTRY);

void
anchor_location_entry_focus (AnchorLocationEntry *entry)
{
	/* Put the keyboard focus in the text field when switching to this mode,
	 * and select all text for easy overtyping
	 */
	gtk_widget_grab_focus (GTK_WIDGET (entry));
	anchor_entry_select_all (NAUTILUS_ENTRY (entry));
}

static GFile *
anchor_location_entry_get_location (AnchorLocationEntry *entry)
{
	char *user_location;
	GFile *location;
...
static void
emit_location_changed (AnchorLocationEntry *entry)
{
	GFile *location;

	location = anchor_location_entry_get_location (entry);
	g_signal_emit (entry, signals[LOCATION_CHANGED], 0, location);
	g_object_unref (location);
}

static void
anchor_location_entry_update_action (AnchorLocationEntry *entry)
{
	const char *current_text;
	GFile *location;

	if (entry->details->last_location == NULL){
		anchor_location_entry_set_secondary_action (entry,
...
static void
anchor_location_entry_update_current_uri (AnchorLocationEntry *entry,
					    const char *uri)
{
	g_free (entry->details->current_directory);
	entry->details->current_directory = g_strdup (uri);

	anchor_entry_set_text (NAUTILUS_ENTRY (entry), uri);
	set_position_and_selection_to_end (GTK_EDITABLE (entry));
}

void
anchor_location_entry_set_location (AnchorLocationEntry *entry,
				      GFile                 *location)
{
	...
	if (eel_uri_is_search (uri)) {
		anchor_location_entry_set_special_text (entry, "");
	} else {
		anchor_location_entry_update_current_uri (entry, formatted_uri);
	}
	...
	anchor_location_entry_update_action (entry);
			     ...
			     gpointer callback_data)
{
	...
	char *detail;
	GFile *location;
	AnchorLocationEntry *self = NAUTILUS_LOCATION_ENTRY (widget);
	...
	anchor_location_entry_set_location (self, location);
	...
	if (new_windows_for_extras) {
		...
		for (i = 1; names[i] != NULL; ++i) {
			new_window = anchor_application_create_window (application, screen);
			location = g_file_new_for_uri (names[i]);
			anchor_window_go_to (new_window, location);
			g_object_unref (location);
		}
	}
			...
			gpointer callback_data)
{
	...
	location = anchor_location_entry_get_location (self);
	uri = g_file_get_uri (location);
...
editable_event_after_callback (GtkEntry *entry,
			       GdkEvent *event,
			       AnchorLocationEntry *location_entry)
{
	GtkEditable *editable;
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_location_entry_parent_class)->finalize (object);
}
...
destroy (GtkWidget *object)
{
	...
	GTK_WIDGET_CLASS (anchor_location_entry_parent_class)->destroy (object);
}

static void
anchor_location_entry_text_changed (AnchorLocationEntry *entry,
				      GParamSpec            *pspec)
{
...
static void
anchor_location_entry_icon_release (GtkEntry *gentry,
...
static gboolean
anchor_location_entry_focus_in (GtkWidget     *widget,
				  GdkEventFocus *event)
{
	AnchorLocationEntry *entry = NAUTILUS_LOCATION_ENTRY (widget);
	...
	return GTK_WIDGET_CLASS (anchor_location_entry_parent_class)->focus_in_event (widget, event);
}

static void
anchor_location_entry_activate (GtkEntry *entry)
{
	AnchorLocationEntry *loc_entry;
	...
	GTK_ENTRY_CLASS (anchor_location_entry_parent_class)->activate (entry);
}

static void
anchor_location_entry_cancel (AnchorLocationEntry *entry)
{
	anchor_location_entry_set_location (entry, entry->details->last_location);
}

static void
anchor_location_entry_class_init (AnchorLocationEntryClass *class)
{
	...
	widget_class->focus_in_event = anchor_location_entry_focus_in;
	...
	entry_class->activate = anchor_location_entry_activate;

	class->cancel = anchor_location_entry_cancel;

	signals[CANCEL] = g_signal_new
		("cancel",
		 G_TYPE_FROM_CLASS (class),
		 G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		 G_STRUCT_OFFSET (AnchorLocationEntryClass,
				  cancel),
		 NULL, NULL,
	...
	g_type_class_add_private (class, sizeof (AnchorLocationEntryDetails));
}

void
anchor_location_entry_set_secondary_action (AnchorLocationEntry *entry,
					      AnchorLocationEntryAction secondary_action)
{
	gboolean rtl;
...
editable_activate_callback (GtkEntry *entry,
			    gpointer user_data)
{
	AnchorLocationEntry *self = user_data;
...
editable_changed_callback (GtkEntry *entry,
			   gpointer user_data)
{
	anchor_location_entry_update_action (NAUTILUS_LOCATION_ENTRY (entry));
}

static void
anchor_location_entry_init (AnchorLocationEntry *entry)
{
	GtkTargetList *targetlist;

	entry->details = G_TYPE_INSTANCE_GET_PRIVATE (entry, ANCHOR_TYPE_LOCATION_ENTRY,
						      AnchorLocationEntryDetails);
	...
	anchor_entry_set_special_tab_handling (NAUTILUS_ENTRY (entry), TRUE);

	g_signal_connect (entry, "event-after",
		          G_CALLBACK (editable_event_after_callback), entry);

	g_signal_connect (entry, "notify::text",
			  G_CALLBACK (anchor_location_entry_text_changed), NULL);

	g_signal_connect (entry, "icon-release",
			  G_CALLBACK (anchor_location_entry_icon_release), NULL);
...
GtkWidget *
anchor_location_entry_new (void)
{
	GtkWidget *entry;

	entry = gtk_widget_new (ANCHOR_TYPE_LOCATION_ENTRY, "max-width-chars", 350, NULL);

	return entry;
}

void
anchor_location_entry_set_special_text (AnchorLocationEntry *entry,
