...
/*
 * anchor-progress-ui-handler.c: file operation progress user interface.
 *
...
#include "anchor-progress-info-widget.h"

#include <glib/gi18n.h>

#include <libanchor-private/anchor-progress-info.h>
#include <libanchor-private/anchor-progress-info-manager.h>
...
G_DEFINE_TYPE (AnchorProgressUIHandler, anchor_progress_ui_handler, G_TYPE_OBJECT);
...
notification_show_details_cb (NotifyNotification *notification,
			      ...
			      gpointer user_data)
{
	AnchorProgressUIHandler *self = user_data;
...
static void
progress_ui_handler_ensure_notification (AnchorProgressUIHandler *self)
{
...
static void
progress_ui_handler_ensure_status_icon (AnchorProgressUIHandler *self)
{
	GIcon *icon;
	GtkStatusIcon *status_icon;
...
static void
progress_ui_handler_update_notification (AnchorProgressUIHandler *self)
{
	gchar *body;
...
static void
progress_ui_handler_update_status_icon (AnchorProgressUIHandler *self)
{
	gchar *tooltip;
...
static void
progress_ui_handler_ensure_window (AnchorProgressUIHandler *self)
{
...
static void
progress_ui_handler_update_notification_or_status (AnchorProgressUIHandler *self)
{
	if (server_has_persistence ()) {
		progress_ui_handler_update_notification (self);
	} else {
...
static void
progress_ui_handler_add_to_window (AnchorProgressUIHandler *self,
				   AnchorProgressInfo *info)
{
	GtkWidget *progress;

	progress = anchor_progress_info_widget_new (info);
...
static void
progress_ui_handler_show_complete_notification (AnchorProgressUIHandler *self)
{
...
static void
progress_ui_handler_hide_notification_or_status (AnchorProgressUIHandler *self)
{
	if (self->priv->status_icon != NULL) {
...
static void
progress_info_finished_cb (AnchorProgressInfo *info,
			   AnchorProgressUIHandler *self)
{
	self->priv->active_infos--;
...
static void
handle_new_progress_info (AnchorProgressUIHandler *self,
			  AnchorProgressInfo *info)
{
...
static TimeoutData *
timeout_data_new (AnchorProgressUIHandler *self,
		  AnchorProgressInfo *info)
{
	TimeoutData *retval;
...
{
	AnchorProgressInfo *info = data->info;
	AnchorProgressUIHandler *self = data->self;

	if (anchor_progress_info_get_is_paused (info)) {
		return TRUE;
	}

	if (!anchor_progress_info_get_is_finished (info)) {
...
static void
release_application (AnchorProgressInfo *info,
		     AnchorProgressUIHandler *self)
{
...
static void
progress_info_started_cb (AnchorProgressInfo *info,
			  AnchorProgressUIHandler *self)
{
	TimeoutData *data;
...
static void
new_progress_info_cb (AnchorProgressInfoManager *manager,
		      AnchorProgressInfo *info,
		      AnchorProgressUIHandler *self)
{
...
static void
anchor_progress_ui_handler_dispose (GObject *obj)
{
	AnchorProgressUIHandler *self = NAUTILUS_PROGRESS_UI_HANDLER (obj);

	g_clear_object (&self->priv->manager);

	G_OBJECT_CLASS (anchor_progress_ui_handler_parent_class)->dispose (obj);
}
...
static void
anchor_progress_ui_handler_init (AnchorProgressUIHandler *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ANCHOR_TYPE_PROGRESS_UI_HANDLER,
						  AnchorProgressUIHandlerPriv);

	self->priv->manager = anchor_progress_info_manager_new ();
	g_signal_connect (self->priv->manager, "new-progress-info",
			  G_CALLBACK (new_progress_info_cb), self);
}

static void
anchor_progress_ui_handler_class_init (AnchorProgressUIHandlerClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->dispose = anchor_progress_ui_handler_dispose;
	
	g_type_class_add_private (klass, sizeof (AnchorProgressUIHandlerPriv));
}

AnchorProgressUIHandler *
anchor_progress_ui_handler_new (void)
{
	return g_object_new (ANCHOR_TYPE_PROGRESS_UI_HANDLER, NULL);
}
