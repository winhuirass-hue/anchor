/*
 * anchor-shell-search-provider.c - Implementation of a GNOME Shell
 *   search provider
 *
...
#include "anchor-shell-search-provider-generated.h"
#include "anchor-shell-search-provider.h"

typedef struct {
  AnchorShellSearchProvider *self;
...
struct _NautilusShellSearchProvider {
  ...
  AnchorShellSearchProvider2 *skeleton;
...
G_DEFINE_TYPE (AnchorShellSearchProvider, anchor_shell_search_provider, G_TYPE_OBJECT)

static gchar *
get_display_name (AnchorShellSearchProvider *self,
                  AnchorFile                   *file)
{
  GFile *location;
  ...
  bookmark = anchor_bookmark_list_item_with_location (self->bookmarks, location, NULL);
  g_object_unref (location);

  if (bookmark)
    return g_strdup (anchor_bookmark_get_name (bookmark));
  else
    return anchor_file_get_display_name (file);
}

static GIcon *
get_gicon (AnchorShellSearchProvider *self,
           AnchorFile                   *file)
{
  GFile *location;
  AnchorBookmark *bookmark;
  ...
  if (bookmark)
    return anchor_bookmark_get_icon (bookmark);
  else
    return anchor_file_get_gicon (file, 0);
}
...
pending_search_finish (PendingSearch         *search,
                       ...
                       GVariant              *result)
{
  AnchorShellSearchProvider *self = search->self;
...
static void
cancel_current_search (AnchorShellSearchProvider *self)
{
  if (self->current_search != NULL)
    anchor_search_provider_stop (ANCHOR_SEARCH_PROVIDER (self->current_search->engine));
}

static void
search_hits_added_cb (AnchorSearchEngine *engine,
                      GList                *hits,
                      gpointer              user_data)

{
  ...
  for (l = hits; l != NULL; l = l->next) {
    hit = l->data;
    anchor_search_hit_compute_scores (hit, search->query);
    hit_uri = anchor_search_hit_get_uri (hit);
    g_debug ("    %s", hit_uri);
...
search_hit_compare_relevance (gconstpointer a,
                              gconstpointer b)
{
  AnchorSearchHit *hit_a, *hit_b;
  gdouble relevance_a, relevance_b;

  hit_a = NAUTILUS_SEARCH_HIT (a);
  hit_b = NAUTILUS_SEARCH_HIT (b);

  relevance_a = anchor_search_hit_get_relevance (hit_a);
  relevance_b = anchor_search_hit_get_relevance (hit_b);
...
static void
search_finished_cb (AnchorSearchEngine *engine,
                    gpointer              user_data)
{
  PendingSearch *search = user_data;
  GList *hits, *l;
  ...
  for (l = hits; l != NULL; l = l->next) {
    hit = l->data;
    g_variant_builder_add (&builder, "s", anchor_search_hit_get_uri (hit));
  }
...
static void
search_error_cb (AnchorSearchEngine *engine,
                 const gchar          *error_message,
                 gpointer              user_data)
{
  AnchorShellSearchProvider *self = user_data;
...
search_add_volumes_and_bookmarks (PendingSearch *search)
{
  ...
  /* first add bookmarks */
  length = anchor_bookmark_list_length (search->self->bookmarks);
  for (idx = 0; idx < length; idx++) {
    bookmark = anchor_bookmark_list_item_at (search->self->bookmarks, idx);
  ...
  for (l = candidates; l != NULL; l = l->next) {
    candidate = l->data;
    match = anchor_query_matches_string (search->query,
                                           candidate->string_for_compare);

    if (match > -1) {
      hit = anchor_search_hit_new (candidate->uri);
...
static void
execute_search (AnchorShellSearchProvider *self,
                GDBusMethodInvocation          *invocation,
                gchar                         **terms)
{
  ...
  home_uri = anchor_get_home_directory_uri ();

  query = anchor_query_new ();
  anchor_query_set_show_hidden_files (query, FALSE);
  anchor_query_set_text (query, terms_joined);
  anchor_query_set_location (query, home_uri);
  ...
  pending_search->engine = anchor_search_engine_new ();
  ...
  anchor_search_provider_set_query (ANCHOR_SEARCH_PROVIDER (pending_search->engine),
                                      query);
  anchor_search_provider_start (ANCHOR_SEARCH_PROVIDER (pending_search->engine));
...
static gboolean
handle_get_initial_result_set (AnchorShellSearchProvider2  *skeleton,
                               GDBusMethodInvocation         *invocation,
                               gchar                        **terms,
                               gpointer                       user_data)
{
...
static gboolean
handle_get_subsearch_result_set (AnchorShellSearchProvider2  *skeleton,
                                 GDBusMethodInvocation         *invocation,
                                 ...
                                 gpointer  user_data)
{
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    gicon = NULL;
    thumbnail_path = anchor_file_get_thumbnail_path (file);
    ...
    if (gicon == NULL) {
      gicon = G_ICON (anchor_file_get_icon_pixbuf (file, 128, TRUE,
						     icon_scale,
...
static gboolean
handle_get_result_metas (AnchorShellSearchProvider2  *skeleton,
                         GDBusMethodInvocation         *invocation,
                         gchar                        **results,
                         gpointer                       user_data)
{
  ...
  for (idx = 0; results[idx] != NULL; idx++) {
    ...
    if (!g_hash_table_lookup (self->metas_cache, uri)) {
      missing_files = g_list_prepend (missing_files, anchor_file_get_by_uri (uri));
    }
  }
  ...
  anchor_file_list_call_when_ready (missing_files,
                                      NAUTILUS_FILE_ATTRIBUTES_FOR_ICON,
                                      NULL,
                                      result_list_attributes_ready_cb,
                                      data);
  anchor_file_list_free (missing_files);
  return TRUE;
}

static gboolean
handle_activate_result (AnchorShellSearchProvider2 *skeleton,
                        GDBusMethodInvocation        *invocation,
                        gchar                        *result,
                        gchar                       **terms,
                        guint32                       timestamp,
                        gpointer                      user_data)
{
  ...
  anchor_shell_search_provider2_complete_activate_result (skeleton, invocation);
  return TRUE;
}

static gboolean
handle_launch_search (AnchorShellSearchProvider2 *skeleton,
                      GDBusMethodInvocation        *invocation,
                      gchar                       **terms,
                      guint32                       timestamp,
                      gpointer                      user_data)
{
  ...
  gchar *uri = anchor_get_home_directory_uri ();
  ...
  anchor_shell_search_provider2_complete_launch_search (skeleton, invocation);
  return TRUE;
}
                                 ...
                                 gpointer user_data)
{
  ...
  self->object_manager = g_dbus_object_manager_server_new ("/org/gnome/Anchor/SearchProvider");
  self->skeleton = anchor_shell_search_provider2_skeleton_new ();
  ...
  g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON (self->skeleton),
                                    connection,
                                    "/org/gnome/Anchor/SearchProvider", NULL);
...
{
  AnchorShellSearchProvider *self = NAUTILUS_SHELL_SEARCH_PROVIDER (obj);
  ...
  G_OBJECT_CLASS (anchor_shell_search_provider_parent_class)->dispose (obj);
}

static void
anchor_shell_search_provider_init (AnchorShellSearchProvider *self)
{
  self->metas_cache = g_hash_table_new_full (g_str_hash, g_str_equal,
                                             g_free, (GDestroyNotify) g_variant_unref);
  self->bookmarks = anchor_application_get_bookmarks (NAUTILUS_APPLICATION (g_application_get_default ()));
  self->volumes = g_volume_monitor_get ();

  g_application_hold (g_application_get_default ());
  self->name_owner_id = g_bus_own_name (G_BUS_TYPE_SESSION,
                                        "org.freedesktop.Anchor.SearchProvider",
...
static void
anchor_shell_search_provider_class_init (AnchorShellSearchProviderClass *klass)
{
...
AnchorShellSearchProvider *
anchor_shell_search_provider_new (void)
{
  return g_object_new (anchor_shell_search_provider_get_type (),
                       NULL);
}
