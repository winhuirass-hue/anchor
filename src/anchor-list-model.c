...
static int anchor_list_model_file_entry_compare_func (gconstpointer a,
							gconstpointer b,
							gpointer      user_data);
static void anchor_list_model_tree_model_init (GtkTreeModelIface *iface);
static void anchor_list_model_sortable_init (GtkTreeSortableIface *iface);

struct AnchorListModelDetails {
	GSequence *files;
...
G_DEFINE_TYPE_WITH_CODE (AnchorListModel, anchor_list_model, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_MODEL,
						anchor_list_model_tree_model_init)
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_SORTABLE,
						anchor_list_model_sortable_init));
...
{
	anchor_file_unref (file_entry->file);
	...
	if (file_entry->subdirectory != NULL) {
		anchor_directory_unref (file_entry->subdirectory);
	}
...
anchor_list_model_get_flags (GtkTreeModel *tree_model)
{
	return GTK_TREE_MODEL_ITERS_PERSIST;
}

static int
anchor_list_model_get_n_columns (GtkTreeModel *tree_model)
{
	return NAUTILUS_LIST_MODEL_NUM_COLUMNS + NAUTILUS_LIST_MODEL (tree_model)->details->columns->len;
}

static GType
anchor_list_model_get_column_type (GtkTreeModel *tree_model, int index)
{
	switch (index) {
	case NAUTILUS_LIST_MODEL_FILE_COLUMN:
		return ANCHOR_TYPE_FILE;
	case NAUTILUS_LIST_MODEL_SUBDIRECTORY_COLUMN:
		return ANCHOR_TYPE_DIRECTORY;
...
static void
anchor_list_model_ptr_to_iter (AnchorListModel *model, GSequenceIter *ptr, GtkTreeIter *iter)
{
...
static gboolean
anchor_list_model_get_iter (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreePath *path)
{
	AnchorListModel *model;
	GSequence *files;
	GSequenceIter *ptr;
	...
	model = (AnchorListModel *)tree_model;
	ptr = NULL;
	...
	anchor_list_model_ptr_to_iter (model, ptr, iter);
	
	return TRUE;
}

static GtkTreePath *
anchor_list_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	GtkTreePath *path;
	AnchorListModel *model;
	GSequenceIter *ptr;
...
static gint
anchor_list_model_get_icon_scale (AnchorListModel *model)
{
	gint retval = -1;
...
static void
anchor_list_model_get_value (GtkTreeModel *tree_model, GtkTreeIter *iter, int column, GValue *value)
{
	AnchorListModel *model;
	FileEntry *file_entry;
	AnchorFile *file;
	...
	switch (column) {
	case NAUTILUS_LIST_MODEL_FILE_COLUMN:
		g_value_init (value, ANCHOR_TYPE_FILE);

		g_value_set_object (value, file);
		break;
	case NAUTILUS_LIST_MODEL_SUBDIRECTORY_COLUMN:
		g_value_init (value, ANCHOR_TYPE_DIRECTORY);
	...
	case NAUTILUS_LIST_MODEL_LARGEST_ICON_COLUMN:
		...
		if (file != NULL) {
			zoom_level = anchor_list_model_get_zoom_level_from_column_id (column);
			icon_size = anchor_get_icon_size_for_zoom_level (zoom_level);
			icon_scale = anchor_list_model_get_icon_scale (model);
			...
			gicon = G_ICON (anchor_file_get_icon_pixbuf (file, icon_size, TRUE, icon_scale, flags));
			...
			for (l = emblem_icons; l != NULL; l = l->next) {
				emblem_icon = l->data;
				if (anchor_icon_theme_can_render (G_THEMED_ICON (emblem_icon))) {
			...
			icon_info = anchor_icon_info_lookup (gicon, icon_size, icon_scale);
			icon = anchor_icon_info_get_pixbuf_at_size (icon_info, icon_size);
			...
			if (model->details->highlight_files != NULL &&
			    g_list_find_custom (model->details->highlight_files,
			                        file, (GCompareFunc) anchor_file_compare_location))
			{
	...
	case NAUTILUS_LIST_MODEL_FILE_NAME_IS_EDITABLE_COLUMN:
		g_value_init (value, G_TYPE_BOOLEAN);
                ...
                g_value_set_boolean (value, file != NULL && anchor_file_can_rename (file));
                break;
 	default:
 		if (column >= NAUTILUS_LIST_MODEL_NUM_COLUMNS || column < NAUTILUS_LIST_MODEL_NUM_COLUMNS + model->details->columns->len) {
			...
			GQuark attribute;
			anchor_column = model->details->columns->pdata[column - NAUTILUS_LIST_MODEL_NUM_COLUMNS];
			...
			g_object_get (anchor_column, 
				      ...
				      NULL);
			if (file != NULL) {
				str = anchor_file_get_string_attribute_with_default_q (file, 
											 attribute);
...
static gboolean
anchor_list_model_iter_next (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	AnchorListModel *model;
...
static gboolean
anchor_list_model_iter_children (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent)
{
	AnchorListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
anchor_list_model_iter_has_child (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	FileEntry *file_entry;

	if (iter == NULL) {
		return !anchor_list_model_is_empty (NAUTILUS_LIST_MODEL (tree_model));
	}
...
static int
anchor_list_model_iter_n_children (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	AnchorListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
anchor_list_model_iter_nth_child (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent, int n)
{
	AnchorListModel *model;
	GSequenceIter *child;
	GSequence *files;
...
static gboolean
anchor_list_model_iter_parent (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child)
{
	AnchorListModel *model;
	FileEntry *file_entry;
...
lookup_file (AnchorListModel *model, AnchorFile *file,
	      ...
	      GHashTable *reverse_map)
{
	...
	if (ptr) {
		...
		anchor_list_model_ptr_to_iter (data->model, ptr, iter);
...
GList *
anchor_list_model_get_all_iters_for_file (AnchorListModel *model, AnchorFile *file)
{
	struct GetIters data;
...
gboolean
anchor_list_model_get_first_iter_for_file (AnchorListModel          *model,
					     AnchorFile         *file,
					     GtkTreeIter          *iter)
{
	GList *list;
	...
	list = anchor_list_model_get_all_iters_for_file (model, file);
	if (list != NULL) {
		res = TRUE;
...
gboolean
anchor_list_model_get_tree_iter_from_file (AnchorListModel *model, AnchorFile *file,
					     AnchorDirectory *directory,
					     GtkTreeIter *iter)
{
	GSequenceIter *ptr;
...
static int
anchor_list_model_file_entry_compare_func (gconstpointer a,
					     gconstpointer b,
					     gpointer      user_data)
{
	...
	model = (AnchorListModel *)user_data;

	file_entry1 = (FileEntry *)a;
	file_entry2 = (FileEntry *)b;
	
	if (file_entry1->file != NULL && file_entry2->file != NULL) {
		result = anchor_file_compare_for_sort_by_attribute_q (file_entry1->file, file_entry2->file,
...
int
anchor_list_model_compare_func (AnchorListModel *model,
				  AnchorFile *file1,
				  AnchorFile *file2)
{
	int result;

	result = anchor_file_compare_for_sort_by_attribute_q (file1, file2,
								model->details->sort_attribute,
								model->details->sort_directories_first,
								(model->details->order == GTK_SORT_DESCENDING));
...
static void
anchor_list_model_sort_file_entries (AnchorListModel *model, GSequence *files, GtkTreePath *path)
{
	...
	for (i = 0; i < length; ++i) {
		...
		if (file_entry->files != NULL) {
			...
			anchor_list_model_sort_file_entries (model, file_entry->files, path);
	...
	/* sort */
	g_sequence_sort (files, anchor_list_model_file_entry_compare_func, model);
...
static void
anchor_list_model_sort (AnchorListModel *model)
{
	GtkTreePath *path;

	path = gtk_tree_path_new ();

	anchor_list_model_sort_file_entries (model, model->details->files, path);

	gtk_tree_path_free (path);
}

static gboolean
anchor_list_model_get_sort_column_id (GtkTreeSortable *sortable,
					gint            *sort_column_id,
					GtkSortType     *order)
{
	AnchorListModel *model;
	...
	model = (AnchorListModel *)sortable;
	
	id = anchor_list_model_get_sort_column_id_from_attribute 
...
static void
anchor_list_model_set_sort_column_id (GtkTreeSortable *sortable, gint sort_column_id, GtkSortType order)
{
	AnchorListModel *model;

	model = (AnchorListModel *)sortable;

	model->details->sort_attribute = anchor_list_model_get_attribute_from_sort_column_id (model, sort_column_id);

	model->details->order = order;

	anchor_list_model_sort (model);
	gtk_tree_sortable_sort_column_changed (sortable);
}

static gboolean
anchor_list_model_has_default_sort_func (GtkTreeSortable *sortable)
{
	return FALSE;
}

static void
add_dummy_row (AnchorListModel *model, FileEntry *parent_entry)
{
	...
	dummy_file_entry->ptr = g_sequence_insert_sorted (parent_entry->files, dummy_file_entry,
							  anchor_list_model_file_entry_compare_func, model);
	iter.stamp = model->details->stamp;
...
gboolean
anchor_list_model_add_file (AnchorListModel *model, AnchorFile *file,
			      AnchorDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	...
	file_entry->file = anchor_file_ref (file);
...
void
anchor_list_model_file_changed (AnchorListModel *model, AnchorFile *file,
				  AnchorDirectory *directory)
{
	...
	g_sequence_sort_changed (ptr, anchor_list_model_file_entry_compare_func, model);
	...
	if (pos_before != pos_after) {
		...
		if (parent_file_entry == NULL) {
			...
			has_iter = TRUE;
			anchor_list_model_ptr_to_iter (model, parent_file_entry->ptr, &iter);
...
gboolean
anchor_list_model_is_empty (AnchorListModel *model)
{
	return (g_sequence_get_length (model->details->files) == 0);
}

guint
anchor_list_model_get_length (AnchorListModel *model)
{
	return g_sequence_get_length (model->details->files);
}

static void
anchor_list_model_remove (AnchorListModel *model, GtkTreeIter *iter)
{
	...
	if (file_entry->files != NULL) {
		while (g_sequence_get_length (file_entry->files) > 0) {
			...
			if (child_file_entry->file != NULL) {
				anchor_list_model_remove_file (model,
...
void
anchor_list_model_remove_file (AnchorListModel *model, AnchorFile *file,
			   AnchorDirectory *directory)
{
	GtkTreeIter iter;

	if (anchor_list_model_get_tree_iter_from_file (model, file, directory, &iter)) {
		anchor_list_model_remove (model, &iter);
	}
}

static void
anchor_list_model_clear_directory (AnchorListModel *model, GSequence *files)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	...
	while (g_sequence_get_length (files) > 0) {
		...
		file_entry = g_sequence_get (iter.user_data);
		if (file_entry->files != NULL) {
			anchor_list_model_clear_directory (model, file_entry->files);
		}
...
void
anchor_list_model_clear (AnchorListModel *model)
{
	g_return_if_fail (model != NULL);

	anchor_list_model_clear_directory (model, model->details->files);
}

AnchorFile *
anchor_list_model_file_for_path (AnchorListModel *model, GtkTreePath *path)
{
	AnchorFile *file;
	GtkTreeIter iter;
...
gboolean
anchor_list_model_load_subdirectory (AnchorListModel *model, GtkTreePath *path, AnchorDirectory **directory)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	AnchorDirectory *subdirectory;
	...
	subdirectory = anchor_directory_get_for_file (file_entry->file);

	if (g_hash_table_lookup (model->details->directory_reverse_map,
				 subdirectory) != NULL) {
		anchor_directory_unref (subdirectory);
	...
	/* Return a ref too */
	anchor_directory_ref (subdirectory);
...
void
anchor_list_model_unload_subdirectory (AnchorListModel *model, GtkTreeIter *iter)
{
	...
	while (g_sequence_get_length (file_entry->files) > 0) {
		...
		if (child_file_entry->file == NULL) {
			...
			anchor_list_model_ptr_to_iter (model, child_ptr, &child_iter);
			anchor_list_model_remove (model, &child_iter);
		}
	}
...
void
anchor_list_model_set_should_sort_directories_first (AnchorListModel *model, gboolean sort_directories_first)
{
...
int
anchor_list_model_get_sort_column_id_from_attribute (AnchorListModel *model,
						       GQuark attribute)
{
	guint i;
...
GQuark
anchor_list_model_get_attribute_from_sort_column_id (AnchorListModel *model,
						       int sort_column_id)
{
	AnchorColumn *column;
	int index;
...
AnchorZoomLevel
anchor_list_model_get_zoom_level_from_column_id (int column)
{
	switch (column) {
	case NAUTILUS_LIST_MODEL_SMALLEST_ICON_COLUMN:
...
int
anchor_list_model_get_column_id_from_zoom_level (AnchorZoomLevel zoom_level)
{
	switch (zoom_level) {
...
void
anchor_list_model_set_drag_view (AnchorListModel *model,
				   GtkTreeView *view,
				   int drag_begin_x,
				   int drag_begin_y)
{
...
GtkTreeView *
anchor_list_model_get_drag_view (AnchorListModel *model,
				   int *drag_begin_x,
				   int *drag_begin_y)
{
...
GtkTargetList *
anchor_list_model_get_drag_target_list ()
{
...
int               
anchor_list_model_add_column (AnchorListModel *model,
				AnchorColumn *column)
{
...
int
anchor_list_model_get_column_number (AnchorListModel *model,
...
static void
anchor_list_model_dispose (GObject *object)
{
	AnchorListModel *model;
	int i;
	...
	G_OBJECT_CLASS (anchor_list_model_parent_class)->dispose (object);
}

static void
anchor_list_model_finalize (GObject *object)
{
	AnchorListModel *model;
	...
	if (model->details->highlight_files != NULL) {
		anchor_file_list_free (model->details->highlight_files);
		model->details->highlight_files = NULL;
	}

	g_free (model->details);

	G_OBJECT_CLASS (anchor_list_model_parent_class)->finalize (object);
}

static void
anchor_list_model_init (AnchorListModel *model)
{
	model->details = g_new0 (AnchorListModelDetails, 1);
...
static void
anchor_list_model_class_init (AnchorListModelClass *klass)
{
	...
	object_class->finalize = anchor_list_model_finalize;
	object_class->dispose = anchor_list_model_dispose;

      list_model_signals[SUBDIRECTORY_UNLOADED] =
        g_signal_new ("subdirectory-unloaded",
                      ANCHOR_TYPE_LIST_MODEL,
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (AnchorListModelClass, subdirectory_unloaded),
                      NULL, NULL,
...
static void
anchor_list_model_tree_model_init (GtkTreeModelIface *iface)
{
	iface->get_flags = anchor_list_model_get_flags;
	iface->get_n_columns = anchor_list_model_get_n_columns;
	iface->get_column_type = anchor_list_model_get_column_type;
	iface->get_iter = anchor_list_model_get_iter;
	iface->get_path = anchor_list_model_get_path;
	iface->get_value = anchor_list_model_get_value;
	iface->iter_next = anchor_list_model_iter_next;
	iface->iter_children = anchor_list_model_iter_children;
	iface->iter_has_child = anchor_list_model_iter_has_child;
	iface->iter_n_children = anchor_list_model_iter_n_children;
	iface->iter_nth_child = anchor_list_model_iter_nth_child;
	iface->iter_parent = anchor_list_model_iter_parent;
}

static void
anchor_list_model_sortable_init (GtkTreeSortableIface *iface)
{
	iface->get_sort_column_id = anchor_list_model_get_sort_column_id;
	iface->set_sort_column_id = anchor_list_model_set_sort_column_id;
	iface->has_default_sort_func = anchor_list_model_has_default_sort_func;
}

void
anchor_list_model_subdirectory_done_loading (AnchorListModel *model, AnchorDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	 ...
	 */
	if (!anchor_directory_is_not_empty (directory) &&
             ...
             gpointer user_data)
{
	...
	iters = anchor_list_model_get_all_iters_for_file (model, file);
...
void
anchor_list_model_set_highlight_for_files (AnchorListModel *model,
					     GList *files)
{
	if (model->details->highlight_files != NULL) {
	...
	if (files != NULL) {
		model->details->highlight_files = anchor_file_list_copy (files);
		g_list_foreach (model->details->highlight_files,
		                refresh_row, model);
