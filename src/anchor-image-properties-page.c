...
struct AnchorImagePropertiesPageDetails {
...
typedef struct {
        GObject parent;
} AnchorImagePropertiesPageProvider;

typedef struct {
        GObjectClass parent;
} AnchorImagePropertiesPageProviderClass;


static GType anchor_image_properties_page_provider_get_type (void);
static void  property_page_provider_iface_init                (AnchorPropertyPageProviderIface *iface);


G_DEFINE_TYPE (AnchorImagePropertiesPage, anchor_image_properties_page, GTK_TYPE_BOX);

G_DEFINE_TYPE_WITH_CODE (AnchorImagePropertiesPageProvider, anchor_image_properties_page_provider, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (ANCHOR_TYPE_PROPERTY_PAGE_PROVIDER,
						property_page_provider_iface_init));

static void
anchor_image_properties_page_finalize (GObject *object)
{
	AnchorImagePropertiesPage *page;
	...
	G_OBJECT_CLASS (anchor_image_properties_page_parent_class)->finalize (object);
}
...
static void
append_item (AnchorImagePropertiesPage *page,
	     const char                  *name,
	     const char                  *value)
{
...
static gboolean
append_tag_value_pair (AnchorImagePropertiesPage *page,
		       ExifData *data,
		       ExifTag   tag,
		       char     *description) 
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (AnchorImagePropertiesPage *page,
		       XmpPtr      xmp,
		       const char *ns,
		       const char *propname,
		       char       *descr)
...
static gboolean
append_option_value_pair (AnchorImagePropertiesPage *page,
			  GdkPixbuf                   *pixbuf,
			  const char                  *key,
			  char                        *description)
{
...
static void
append_basic_info (AnchorImagePropertiesPage *page)
{
...
static void
append_options_info (AnchorImagePropertiesPage *page)
{
	GdkPixbuf *pixbuf;
...
static void
append_exif_info (AnchorImagePropertiesPage *page)
{
#ifdef HAVE_EXIF
	ExifData *exifdata;
...
static void
append_xmp_info (AnchorImagePropertiesPage *page)
{
#ifdef HAVE_EXEMPI
...
static void
load_finished (AnchorImagePropertiesPage *page)
{
	GtkWidget *label;
		    ...
		    gpointer      user_data)
{
	...
	AnchorImagePropertiesPage *page = data->page;
	GFile *file;
	...
	if (stream) {
		...
		mime_type = anchor_file_info_get_mime_type (data->info);
...
static void
load_location (AnchorImagePropertiesPage *page,
	       AnchorFileInfo            *info)
{
	GFile *file;
	char *uri;
	...
	uri = anchor_file_info_get_uri (info);
...
static void
anchor_image_properties_page_class_init (AnchorImagePropertiesPageClass *class)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (class);

	object_class->finalize = anchor_image_properties_page_finalize;

	g_type_class_add_private (object_class, sizeof(AnchorImagePropertiesPageDetails));
}

static void
anchor_image_properties_page_init (AnchorImagePropertiesPage *page)
{
	GtkWidget *sw;

	page->details = G_TYPE_INSTANCE_GET_PRIVATE (page,
						     ANCHOR_TYPE_IMAGE_PROPERTIES_PAGE,
						     AnchorImagePropertiesPageDetails);
...
static GList *
get_property_pages (AnchorPropertyPageProvider *provider,
                    GList *files)
{
	GList *pages;
	AnchorFileInfo *file;
	...
	pages = NULL;
	file = ANCHOR_FILE_INFO (files->data);

	mime_type = anchor_file_info_get_mime_type (file);
	if (mime_type != NULL
	    && is_mime_type_supported (mime_type)) {
		AnchorImagePropertiesPage *page;
		AnchorPropertyPage *real_page;

		page = g_object_new (anchor_image_properties_page_get_type (), NULL);
		load_location (page, file);

		real_page = anchor_property_page_new
			("AnchorImagePropertiesPage::property_page", 
...
static void 
property_page_provider_iface_init (AnchorPropertyPageProviderIface *iface)
{
...
static void
anchor_image_properties_page_provider_init (AnchorImagePropertiesPageProvider *sidebar)
{
}

static void
anchor_image_properties_page_provider_class_init (AnchorImagePropertiesPageProviderClass *class)
{
}

void
anchor_image_properties_page_register (void)
{
        anchor_module_add_type (anchor_image_properties_page_provider_get_type ());
}
