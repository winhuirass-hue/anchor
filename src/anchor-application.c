...
#include "anchor-freedesktop-dbus.h"
#include "anchor-image-properties-page.h"
#include "anchor-previewer.h"
#include "anchor-progress-ui-handler.h"
#include "anchor-self-check-functions.h"
#include "anchor-shell-search-provider.h"
#include "anchor-window.h"
#include "anchor-window-private.h"
#include "anchor-window-slot.h"

#include <libanchor-private/anchor-dbus-manager.h>
#include <libanchor-private/anchor-desktop-link-monitor.h>
#include <libanchor-private/anchor-directory-private.h>
...
#include <libanchor-private/anchor-lib-self-check-functions.h>
...
static GList *anchor_application_desktop_windows;

/* The saving of the accelerator map was requested  */
static gboolean save_of_accel_map_requested = FALSE;

static void     desktop_changed_callback          (gpointer                  user_data);
static void     anchor_application_open_desktop (AnchorApplication *application);

G_DEFINE_TYPE (AnchorApplication, anchor_application, GTK_TYPE_APPLICATION);

struct _NautilusApplicationPriv {
	AnchorProgressUIHandler *progress_handler;
	AnchorDBusManager *dbus_manager;
	AnchorFreedesktopDBus *fdb_manager;
	...
	AnchorBookmarkList *bookmark_list;

	GtkWidget *connect_server_window;

	AnchorShellSearchProvider *search_provider;
};
...
void
anchor_application_edit_bookmarks (AnchorApplication *application,
				     AnchorWindow      *window)
{
	GtkWindow *bookmarks_window;

	bookmarks_window = anchor_bookmarks_window_new (window);
	gtk_window_present (bookmarks_window);
}

void
anchor_application_notify_unmount_done (AnchorApplication *application,
					  const gchar *message)
{
	...
	if (message != NULL) {
		...
		notify_notification_set_hint (unplug,
					      "desktop-entry", g_variant_new_string ("anchor"));
...
void
anchor_application_notify_unmount_show (AnchorApplication *application,
					  const gchar *message)
{
	gchar **strings;
...
static gboolean
check_required_directories (AnchorApplication *application)
{
	char *user_directory;
	...
	user_directory = anchor_get_user_directory ();
	desktop_directory = anchor_get_desktop_directory ();
...
static void
menu_provider_items_updated_handler (AnchorMenuProvider *provider, GtkWidget* parent_window, gpointer data)
{

	g_signal_emit_by_name (anchor_signaller_get_current (),
			       "popup-menu-changed");
}
...
menu_provider_init_callback (void)
{
        ...
        for (l = providers; l != NULL; l = l->next) {
                AnchorMenuProvider *provider = ANCHOR_MENU_PROVIDER (l->data);
...
mark_desktop_files_trusted (void)
{
	...
	while ((info = g_file_enumerator_next_file (e, NULL, NULL)) != NULL) {
		...
		if (g_str_has_suffix (name, ".desktop") &&
			...
			anchor_file_mark_desktop_file_trusted (c,
								 NULL, FALSE,
								 NULL, NULL);
			g_object_unref (c);
		}
...
static void
do_upgrades_once (AnchorApplication *self)
{
	char *metafile_dir, *updated, *anchor_dir, *xdg_dir;
	const gchar *message;
	int fd, res;
	...
	anchor_dir = g_build_filename (g_get_home_dir (),
					 ".anchor", NULL);
	xdg_dir = anchor_get_user_directory ();
	if (g_file_test (anchor_dir, G_FILE_TEST_IS_DIR)) {
		/* test if we already attempted to migrate first */
		updated = g_build_filename (anchor_dir, "DEPRECATED-DIRECTORY", NULL);
		message = _("Anchor 3.0 deprecated this directory and tried migrating "
			    "this configuration to ~/.config/anchor");
		if (!g_file_test (updated, G_FILE_TEST_EXISTS)) {
			/* rename() works fine if the destination directory is
			 * empty.
			 */
			res = g_rename (anchor_dir, xdg_dir);
			  ...
			  AnchorDesktopWindow *window)
{
	...
	anchor_application_desktop_windows =
		g_list_remove (anchor_application_desktop_windows, window);

	return TRUE;
}

static void
anchor_application_create_desktop_windows (AnchorApplication *application)
{
	GdkScreen *screen;
	AnchorDesktopWindow *window;
	GtkWidget *selection_widget;
	...
	if (selection_widget != NULL) {
		window = anchor_desktop_window_new (GTK_APPLICATION (application), screen);
		...
		anchor_application_desktop_windows =
			g_list_prepend (anchor_application_desktop_windows, window);
	}
}

static gboolean
another_navigation_window_already_showing (AnchorApplication *application,
					   AnchorWindow *the_window)
{
	GList *l;
...
AnchorWindow *
anchor_application_create_window (AnchorApplication *application,
				    GdkScreen           *screen)
{
	AnchorWindow *window;
	...
	window = anchor_window_new (screen);
...
static AnchorWindowSlot *
get_window_slot_for_location (AnchorApplication *application, GFile *location)
{
	...
	for (l = gtk_application_get_windows (GTK_APPLICATION (application)); l; l = l->next) {
		...
		for (sl = anchor_window_get_slots (NAUTILUS_WINDOW (l->data)); sl; sl = sl->next) {
			AnchorWindowSlot *current = NAUTILUS_WINDOW_SLOT (sl->data);
			GFile *slot_location = anchor_window_slot_get_location (current);
...
static void
open_window (AnchorApplication *application,
	     GFile *location, GdkScreen *screen, const char *geometry)
{
	AnchorWindow *window;

	anchor_profile_start (NULL);
	window = anchor_application_create_window (application, screen);

	if (location != NULL) {
		anchor_window_go_to (window, location);
	} else {
		anchor_window_slot_go_home (anchor_window_get_active_slot (window), 0);
	}
...
static void
open_windows (AnchorApplication *application,
	      gboolean force_new,
	      GFile **files,
	      gint n_files,
	      GdkScreen *screen,
	...
	if (files == NULL || files[0] == NULL) {
		...
		for (i = 0; i < n_files; ++i) {
			...
			if (!slot) {
				...
				anchor_window_slot_open_location (slot, files[i], 0);

				AnchorWindow *window = anchor_window_slot_get_window (slot);
...
void
anchor_application_open_location (AnchorApplication *application,
				    GFile *location,
				    GFile *selection,
				    const char *startup_id)
{
	...
	if (!slot) {
		window = anchor_application_create_window (application, gdk_screen_get_default ());
	...
	if (selection != NULL) {
		sel_list = g_list_prepend (sel_list, anchor_file_get (selection));
	}

	gtk_window_set_startup_id (GTK_WINDOW (window), startup_id);
	anchor_window_slot_open_location_full (slot, location, 0, sel_list, NULL, NULL);

	if (sel_list != NULL) {
		anchor_file_list_free (sel_list);
	}
...
static void
anchor_application_open (GApplication *app,
			   GFile **files,
			   gint n_files,
			   const gchar *hint)
{
	AnchorApplication *self = NAUTILUS_APPLICATION (app);
		   ...
		   gpointer user_data)
{
	...
	window = anchor_application_create_window (NAUTILUS_APPLICATION (application),
						     cur_window ?
...
static gboolean
go_to_server_cb (AnchorWindow *window,
		 GFile          *location,
		 GError         *error,
		 gpointer        user_data)
{
	...
	if (error == NULL) {
		...
		datadir = g_build_filename (g_get_user_config_dir (), "anchor", NULL);
		...
		if (safe_to_save) {
			...
			title = anchor_file_get_display_name (file);
			    ...
			    GtkApplication *application)
{
	if (response == GTK_RESPONSE_OK) {
		GFile *location;
		AnchorWindow *window = NAUTILUS_WINDOW (gtk_application_get_active_window (application));

		location = anchor_connect_server_dialog_get_location (NAUTILUS_CONNECT_SERVER_DIALOG (dialog));
		if (location != NULL) {
...
GtkWidget *
anchor_application_connect_server (AnchorApplication *application,
				     AnchorWindow      *window)
{
	GtkWidget *dialog;
	...
	if (dialog == NULL) {
		dialog = anchor_connect_server_dialog_new (window);
			  ...
			  gpointer user_data)
{
	...
	anchor_application_connect_server (NAUTILUS_APPLICATION (application),
		       ...
		       gpointer user_data)
{
	...
	anchor_window_ensure_location_entry (window);
}
		  ...
		  gpointer user_data)
{
	...
	anchor_application_edit_bookmarks (NAUTILUS_APPLICATION (application),
					     NAUTILUS_WINDOW (gtk_application_get_active_window (application)));
}
...
{
	GtkApplication *application = user_data;
	anchor_file_management_properties_dialog_show (gtk_application_get_active_window (application));
}
	      ...
	      gpointer user_data)
{
	...
	anchor_window_show_about_dialog (NAUTILUS_WINDOW (gtk_application_get_active_window (application)));
}
...
action_force_desktop (GSimpleAction *action,
		      ...
		      gpointer user_data)
{
	AnchorApplication *self = user_data;

	anchor_application_open_desktop (self);
}
	     ...
	     gpointer user_data)
{
	...
	/* anchor_window_close() doesn't do anything for desktop windows */
	for (l = gtk_application_get_windows (GTK_APPLICATION (application)); l; l = l->next) {
		if (NAUTILUS_IS_WINDOW (l->data))
			anchor_window_close (NAUTILUS_WINDOW (l->data));
	}
}
	       ...
	       gpointer user_data)
{
	...
	anchor_query_set_location (query, uri);
	anchor_query_set_text (query, string);

	search_uri = anchor_search_directory_generate_new_uri ();
	...
	anchor_window_slot_open_location (anchor_window_get_active_slot (window), location, 0);
...
static void
anchor_application_init_actions (AnchorApplication *self)
{
	GtkBuilder *builder;
	GError *error = NULL;
	...
	gtk_builder_add_from_resource (builder, "/org/gnome/anchor/anchor-app-menu.ui", &error);
...
static void
anchor_application_init (AnchorApplication *application)
{
	application->priv =
		G_TYPE_INSTANCE_GET_PRIVATE (application, ANCHOR_TYPE_APPLICATION,
					     AnchorApplicationPriv);
}

static void
anchor_application_finalize (GObject *object)
{
	...
	notify_uninit ();
        ...
        G_OBJECT_CLASS (anchor_application_parent_class)->finalize (object);
}

static gboolean
do_cmdline_sanity_checks (AnchorApplication *self,
...
do_perform_self_checks (gint *exit_status)
{
	...
	anchor_profile_start (NULL);
	/* Run the checks (each twice) for anchor and libanchor-private. */

	anchor_run_self_checks ();
	anchor_run_lib_self_checks ();
...
static void
anchor_application_select (AnchorApplication *self,
			     GFile **files,
			     gint len)
{
...
static gboolean
anchor_application_local_command_line (GApplication *application,
					 gchar ***arguments,
					 gint *exit_status)
{
	...
	AnchorApplication *self = NAUTILUS_APPLICATION (application);
	...
	*exit_status = EXIT_SUCCESS;
	...
	if (version) {
		g_print ("GNOME anchor " PACKAGE_VERSION "\n");
		goto out;
	}
	...
	if (select_uris) {
		anchor_application_select (self, files, len);
	} else {
...
static void
anchor_application_open_desktop (AnchorApplication *application)
{
	/* Initialize the desktop link monitor singleton */
	anchor_desktop_link_monitor_get ();

	anchor_application_create_desktop_windows (application);
}

static void
anchor_application_close_desktop (void)
{
	g_list_foreach (anchor_application_desktop_windows,
			(GFunc) gtk_widget_destroy, NULL);
	g_list_free (anchor_application_desktop_windows);
	anchor_application_desktop_windows = NULL;

	anchor_desktop_link_monitor_shutdown ();
}
...
desktop_changed_callback (gpointer user_data)
{
	...
	if (g_settings_get_boolean (gnome_background_preferences, NAUTILUS_PREFERENCES_SHOW_DESKTOP)) {
		anchor_application_open_desktop (application);
	} else {
		anchor_application_close_desktop ();
	}
}

static void
init_desktop (AnchorApplication *self)
{
...
static gboolean 
anchor_application_save_accel_map (gpointer data)
{
	if (save_of_accel_map_requested) {
		char *accel_map_filename;
	 	accel_map_filename = anchor_get_accel_map_file ();
		...
		gpointer user_data)
{
	if (!save_of_accel_map_requested) {
		...
		g_timeout_add_seconds (NAUTILUS_ACCEL_MAP_SAVE_DELAY, 
				anchor_application_save_accel_map, NULL);
	}
}
		...
		GdkModifierType accel_mods, gboolean changed)
{
	...
	if (old_scripts_pt != NULL) {
		...
		tmp = anchor_get_scripts_directory_path ();
		tmp2 = anchor_escape_action_name (tmp, "");
		g_free (tmp);
...
init_gtk_accels (void)
{
	...
	if (performed_migration) {
		...
		old_scripts_location_esc = anchor_escape_action_name (old_scripts_directory_path, "");
...
static void
anchor_application_startup (GApplication *app)
{
	AnchorApplication *self = NAUTILUS_APPLICATION (app);
	 ...
	 * is called for us.
	 */
	G_APPLICATION_CLASS (anchor_application_parent_class)->startup (app);

	gtk_window_set_default_icon_name ("system-file-manager");

	/* initialize the previewer singleton */
	anchor_previewer_get_singleton ();

	/* create DBus manager */
	self->priv->dbus_manager = anchor_dbus_manager_new ();
	self->priv->fdb_manager = anchor_freedesktop_dbus_new ();

	/* initialize preferences and create the global GSettings objects */
	anchor_global_preferences_init ();

	/* register property pages */
	anchor_image_properties_page_register ();

	/* initialize theming */
	init_icons_and_styles ();
	init_gtk_accels ();
	
	/* initialize anchor modules */
	anchor_profile_start ("Modules");
	anchor_module_setup ();
	anchor_profile_end ("Modules");
	...
	self->priv->progress_handler = anchor_progress_ui_handler_new ();

	/* Bookmarks and search */
	self->priv->bookmark_list = anchor_bookmark_list_new ();
	self->priv->search_provider = anchor_shell_search_provider_new ();

	/* Check the user's .anchor directories and post warnings
	...
	anchor_application_init_actions (self);
	init_desktop (self);
...
static void
anchor_application_quit_mainloop (GApplication *app)
{
	...
	anchor_application_notify_unmount_done (NAUTILUS_APPLICATION (app), NULL);

	G_APPLICATION_CLASS (anchor_application_parent_class)->quit_mainloop (app);
}

static void
update_dbus_opened_locations (AnchorApplication *app)
{
	gint i;
	GList *l, *sl;
	...
	for (l = gtk_application_get_windows (GTK_APPLICATION (app)); l; l = l->next) {
		...
		for (sl = anchor_window_get_slots (NAUTILUS_WINDOW (l->data)); sl; sl = sl->next) {
			AnchorWindowSlot *slot = NAUTILUS_WINDOW_SLOT (sl->data);
			gchar *uri = anchor_window_slot_get_location_uri (slot);
	...
	anchor_freedesktop_dbus_set_open_locations (app->priv->fdb_manager,
...
on_slot_added (AnchorWindow      *window,
	       ...
	       AnchorApplication *application)
{
	if (anchor_window_slot_get_location (slot)) {
		update_dbus_opened_locations (application);
	}
...
static void
anchor_application_window_added (GtkApplication *app,
				   GtkWindow *window)
{
	/* chain to parent */
	GTK_APPLICATION_CLASS (anchor_application_parent_class)->window_added (app, window);
...
static void
anchor_application_window_removed (GtkApplication *app,
				     GtkWindow *window)
{
	...
	/* chain to parent */
	GTK_APPLICATION_CLASS (anchor_application_parent_class)->window_removed (app, window);
	...
	if (!l) {
		...
		anchor_previewer_call_close (previewer);
	}
...
static void
anchor_application_class_init (AnchorApplicationClass *class)
{
	...
	GtkApplicationClass *gtkapp_class;
        ...
        object_class->finalize = anchor_application_finalize;

	application_class = G_APPLICATION_CLASS (class);
	application_class->startup = anchor_application_startup;
	application_class->quit_mainloop = anchor_application_quit_mainloop;
	application_class->open = anchor_application_open;
	application_class->local_command_line = anchor_application_local_command_line;

	gtkapp_class = GTK_APPLICATION_CLASS (class);
	gtkapp_class->window_added = anchor_application_window_added;
	gtkapp_class->window_removed = anchor_application_window_removed;

	g_type_class_add_private (class, sizeof (AnchorApplicationPriv));
}

AnchorApplication *
anchor_application_new (void)
{
	return g_object_new (ANCHOR_TYPE_APPLICATION,
			     "application-id", "org.freedesktop.Anchor",
