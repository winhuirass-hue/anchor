/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-window-slot.c: Anchor window slot
...
#include <libanchor-extension/anchor-location-widget-provider.h>

G_DEFINE_TYPE (AnchorWindowSlot, anchor_window_slot, GTK_TYPE_BOX);
...
struct AnchorWindowSlotDetails {
	AnchorWindow *window;
	...
	/* Viewed file */
	AnchorView *content_view;
	AnchorView *new_content_view;
	AnchorFile *viewed_file;
	gboolean viewed_file_seen;
	gboolean viewed_file_in_trash;

	/* Information about bookmarks and history list */
	AnchorBookmark *current_location_bookmark;
	AnchorBookmark *last_location_bookmark;
	GList *back_list;
	GList *forward_list;

	/* Query editor */
	AnchorQueryEditor *query_editor;
	...
	AnchorLocationChangeType location_change_type;
	guint location_change_distance;
	char *pending_scroll_to;
	GList *pending_selection;
	gboolean pending_use_default_location;
	AnchorFile *determine_view_file;
	GCancellable *mount_cancellable;
	GError *mount_error;
	gboolean tried_mount;
	AnchorWindowGoToCallback open_callback;
...
static void anchor_window_slot_force_reload (AnchorWindowSlot *slot);
static void location_has_really_changed (AnchorWindowSlot *slot);
static void anchor_window_slot_connect_new_content_view (AnchorWindowSlot *slot);
static void anchor_window_slot_emit_location_change (AnchorWindowSlot *slot, GFile *from, GFile *to);

static void
anchor_window_slot_sync_search_widgets (AnchorWindowSlot *slot)
{
	AnchorDirectory *directory;
	gboolean toggle;

	if (slot != anchor_window_get_active_slot (slot->details->window)) {
		return;
	}
	...
	if (slot->details->content_view != NULL) {
		directory = anchor_view_get_model (slot->details->content_view);
		if (NAUTILUS_IS_SEARCH_DIRECTORY (directory)) {
			toggle = TRUE;
		}
	}

	anchor_window_slot_set_search_visible (slot, toggle);
}

static gboolean
anchor_window_slot_content_view_matches_iid (AnchorWindowSlot *slot,
					       const char *iid)
{
	if (slot->details->content_view == NULL) {
		return FALSE;
	}
	return g_strcmp0 (anchor_view_get_view_id (slot->details->content_view), iid) == 0;
}

static void
anchor_window_slot_sync_view_as_menus (AnchorWindowSlot *slot)
{
	...
	action_group = anchor_window_get_main_action_group (slot->details->window);

	if (anchor_window_slot_content_view_matches_iid (slot, NAUTILUS_LIST_VIEW_ID)) {
		action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_VIEW_LIST);
	} else if (anchor_window_slot_content_view_matches_iid (slot, NAUTILUS_CANVAS_VIEW_ID)) {
...
static void
sync_search_directory (AnchorWindowSlot *slot)
{
	AnchorDirectory *directory;
	AnchorQuery *query;
	gchar *text;
	...
	directory = anchor_directory_get_for_file (slot->details->viewed_file);
	g_assert (NAUTILUS_IS_SEARCH_DIRECTORY (directory));

	query = anchor_query_editor_get_query (slot->details->query_editor);
	text = anchor_query_get_text (query);

	if (!strlen (text)) {
		/* Prevent the location change from hiding the query editor in this case */
		slot->details->load_with_search = TRUE;
		location = anchor_query_editor_get_location (slot->details->query_editor);
		anchor_window_slot_open_location (slot, location, 0);
		g_object_unref (location);
	} else {
		anchor_search_directory_set_query (NAUTILUS_SEARCH_DIRECTORY (directory),
						     query);
		anchor_window_slot_force_reload (slot);
	}
...
static void
create_new_search (AnchorWindowSlot *slot)
{
	char *uri;
	AnchorDirectory *directory;
	GFile *location;
	...
	uri = anchor_search_directory_generate_new_uri ();
	...
	query = anchor_query_editor_get_query (slot->details->query_editor);
	anchor_search_directory_set_query (NAUTILUS_SEARCH_DIRECTORY (directory), query);
...
static void
query_editor_cancel_callback (AnchorQueryEditor *editor,
			      AnchorWindowSlot *slot)
{
	anchor_window_slot_set_search_visible (slot, FALSE);
}

static void
query_editor_activated_callback (AnchorQueryEditor *editor,
				 AnchorWindowSlot *slot)
{
	if (slot->details->content_view != NULL) {
		anchor_view_activate_selection (slot->details->content_view);
	}
}

static void
query_editor_changed_callback (AnchorQueryEditor *editor,
			       AnchorQuery *query,
			       gboolean reload,
			       AnchorWindowSlot *slot)
{
...
static void
hide_query_editor (AnchorWindowSlot *slot)
{
	...
	anchor_query_editor_set_query (slot->details->query_editor, NULL);
}

static GFile *
anchor_window_slot_get_current_location (AnchorWindowSlot *slot)
{
...
static void
show_query_editor (AnchorWindowSlot *slot)
{
	AnchorDirectory *directory;
	AnchorSearchDirectory *search_directory;
	GFile *location;

	location = anchor_window_slot_get_current_location (slot);
	directory = anchor_directory_get (location);

	if (NAUTILUS_IS_SEARCH_DIRECTORY (directory)) {
		AnchorQuery *query;
		search_directory = NAUTILUS_SEARCH_DIRECTORY (directory);
		query = anchor_search_directory_get_query (search_directory);
		if (query != NULL) {
			anchor_query_editor_set_query (slot->details->query_editor,
							 query);
			g_object_unref (query);
		}
	} else {
		anchor_query_editor_set_location (slot->details->query_editor, location);
	}
...
void
anchor_window_slot_set_search_visible (AnchorWindowSlot *slot,
					 gboolean            visible)
{
	GtkActionGroup *action_group;
	GtkAction *action;
	...
	active_slot = (slot == anchor_window_get_active_slot (slot->details->window));
	...
	if (visible) {
		...
		if (old_visible && active_slot) {
			...
			return_location = anchor_window_slot_get_query_editor_location (slot);
		...
		if (active_slot) {
			anchor_window_grab_focus (slot->details->window);
		}
	 ...
	 */
	if (return_location != NULL) {
		anchor_window_slot_open_location (slot, return_location, 0);
		g_object_unref (return_location);
	}
}

GFile *
anchor_window_slot_get_query_editor_location (AnchorWindowSlot *slot)
{
	return anchor_query_editor_get_location (slot->details->query_editor);
}

gboolean
anchor_window_slot_handle_event (AnchorWindowSlot *slot,
				   GdkEventKey        *event)
{
	AnchorWindow *window;
	gboolean retval;
	...
	retval = FALSE;
	window = anchor_window_slot_get_window (slot);
	if (!NAUTILUS_IS_DESKTOP_WINDOW (window)) {
		retval = anchor_query_editor_handle_event (slot->details->query_editor, event);
	}

	if (retval) {
		anchor_window_slot_set_search_visible (slot, TRUE);
	}
...
static void
real_active (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	int page_num;
	...
	anchor_window_sync_allow_stop (window, slot);
	anchor_window_sync_title (window, slot);
	anchor_window_sync_zoom_widgets (window);
	anchor_window_sync_location_widgets (window);
	anchor_window_slot_sync_search_widgets (slot);

	if (slot->details->viewed_file != NULL) {
		anchor_window_slot_sync_view_as_menus (slot);
		anchor_window_load_extension_menus (window);
	}
}

static void
real_inactive (AnchorWindowSlot *slot)
{
	AnchorWindow *window;

	window = anchor_window_slot_get_window (slot);
	g_assert (slot == anchor_window_get_active_slot (window));
}

static void
floating_bar_action_cb (AnchorFloatingBar *floating_bar,
			gint action,
			AnchorWindowSlot *slot)
{
...
remove_all_extra_location_widgets (GtkWidget *widget,
				   gpointer data)
{
	AnchorWindowSlot *slot = data;
	AnchorDirectory *directory;

	directory = anchor_directory_get (slot->details->location);
...
static void
anchor_window_slot_remove_extra_location_widgets (AnchorWindowSlot *slot)
{
...
static void
anchor_window_slot_add_extra_location_widget (AnchorWindowSlot *slot,
						GtkWidget *widget)
{
...
static void
anchor_window_slot_set_property (GObject *object,
				   guint property_id,
				   const GValue *value,
				   GParamSpec *pspec)
{
	AnchorWindowSlot *slot = NAUTILUS_WINDOW_SLOT (object);

	switch (property_id) {
	case PROP_WINDOW:
		anchor_window_slot_set_window (slot, g_value_get_object (value));
		break;
	default:
...
static void
anchor_window_slot_get_property (GObject *object,
				   guint property_id,
				   GValue *value,
				   GParamSpec *pspec)
{
...
static void
anchor_window_slot_constructed (GObject *object)
{
	AnchorWindowSlot *slot = NAUTILUS_WINDOW_SLOT (object);
	GtkWidget *extras_vbox;

	G_OBJECT_CLASS (anchor_window_slot_parent_class)->constructed (object);
	...
	slot->details->query_editor = NAUTILUS_QUERY_EDITOR (anchor_query_editor_new ());
	...
	anchor_window_slot_add_extra_location_widget (slot, slot->details->query_editor_revealer);
	...
	slot->details->floating_bar = anchor_floating_bar_new (NULL, NULL, FALSE);
...
static void
anchor_window_slot_init (AnchorWindowSlot *slot)
{
	slot->details = G_TYPE_INSTANCE_GET_PRIVATE
		(slot, ANCHOR_TYPE_WINDOW_SLOT, AnchorWindowSlotDetails);
}

static void
remove_loading_floating_bar (AnchorWindowSlot *slot)
{
	...
	anchor_floating_bar_cleanup_actions (NAUTILUS_FLOATING_BAR (slot->details->floating_bar));
}
...
static void begin_location_change                     (AnchorWindowSlot         *slot,
						       GFile                      *location,
						       GFile                      *previous_location,
						       GList                      *new_selection,
						       AnchorLocationChangeType  type,
						       guint                       distance,
						       const char                 *scroll_pos,
						       AnchorWindowGoToCallback  callback,
						       gpointer                    user_data);
static void free_location_change                      (AnchorWindowSlot         *slot);
static void end_location_change                       (AnchorWindowSlot         *slot);
static void cancel_location_change                    (AnchorWindowSlot         *slot);
static void got_file_info_for_view_selection_callback (AnchorFile               *file,
						       gpointer                    callback_data);
static gboolean create_content_view                   (AnchorWindowSlot         *slot,
						       const char                 *view_id,
						       GError                    **error);
static void display_view_selection_failure            (AnchorWindow             *window,
						       AnchorFile               *file,
						       GFile                      *location,
						       GError                     *error);
static void load_new_location                         (AnchorWindowSlot         *slot,
						       GFile                      *location,
						       GList                      *selection,
...
void
anchor_window_slot_open_location_full (AnchorWindowSlot *slot,
					 GFile *location,
...
{
	AnchorWindow *window;
        AnchorWindow *target_window;
        AnchorWindowSlot *target_slot;
	AnchorWindowOpenFlags slot_flags;
	GFile *old_location;
	GList *old_selection;
	...
	old_uri = anchor_window_slot_get_location_uri (slot);
	if (old_uri == NULL) {
	...
	anchor_profile_start ("Opening location, old: %s, new: %s", old_uri, new_uri);
	...
	if (is_desktop) {
		use_same = !anchor_desktop_window_loaded (NAUTILUS_DESKTOP_WINDOW (window));
	...
	old_location = anchor_window_slot_get_location (slot);

	/* now get/create the window */
	if (use_same) {
		target_window = window;
	} else {
		target_window = anchor_application_create_window
	...
	if ((flags & NAUTILUS_WINDOW_OPEN_FLAG_NEW_TAB) != 0) {
		...
		new_slot_position = g_settings_get_enum (anchor_preferences, NAUTILUS_PREFERENCES_NEW_TAB_POSITION);
		if (new_slot_position == NAUTILUS_NEW_TAB_POSITION_END) {
			slot_flags = NAUTILUS_WINDOW_OPEN_SLOT_APPEND;
		}

		target_slot = anchor_window_open_slot (window,
							 slot_flags);
	}
	...
	if (target_slot == NULL) {
		if (target_window == window) {
			...
			target_slot = anchor_window_get_active_slot (target_window);
		}
	}

	old_selection = NULL;
	if (slot->details->content_view != NULL) {
		old_selection = anchor_view_get_selection (slot->details->content_view);
	}

	if (target_window == window && target_slot == slot && !is_desktop &&
	    old_location && g_file_equal (old_location, location) &&
	    anchor_file_selection_equal (old_selection, new_selection)) {
	...
	anchor_file_list_free (old_selection);
	anchor_profile_end (NULL);
}

static gboolean
report_callback (AnchorWindowSlot *slot,
		 GError *error)
{
	if (slot->details->open_callback != NULL) {
		gboolean res;
		res = slot->details->open_callback (anchor_window_slot_get_window (slot),
 ...
 * @window: The AnchorWindow whose location should be changed.
 ...
 */
static void
begin_location_change (AnchorWindowSlot *slot,
                       GFile *location,
                       GFile *previous_location,
		       GList *new_selection,
                       AnchorLocationChangeType type,
		       ...
		       gpointer user_data)
{
	...
	if (new_selection == NULL && previous_location != NULL &&
		...
		if (parent != NULL) {
			new_selection = parent_selection =
				g_list_prepend (NULL, anchor_file_get (from_folder));
	...
	anchor_window_slot_set_allow_stop (slot, TRUE);
	anchor_window_slot_set_status (slot, NULL, NULL);
	...
	if (type == NAUTILUS_LOCATION_CHANGE_RELOAD) {
		force_reload = TRUE;
	} else if (!anchor_monitor_active ()) {
		force_reload = TRUE;
	} else {
		force_reload = !anchor_directory_is_local (directory);
	}

	if (force_reload) {
		anchor_directory_force_reload (directory);
		file = anchor_directory_get_corresponding_file (directory);
		anchor_file_invalidate_all_attributes (file);
	...
	if (parent_selection != NULL) {
        ...
        if (slot->details->current_location_bookmark != NULL &&
            slot->details->content_view != NULL) {
                current_pos = anchor_view_get_first_visible_file (slot->details->content_view);
                anchor_bookmark_set_scroll_pos (slot->details->current_location_bookmark, current_pos);
                g_free (current_pos);
        }

	/* Get the info needed for view selection */
	slot->details->determine_view_file = anchor_file_get (location);
	g_assert (slot->details->determine_view_file != NULL);

	anchor_file_call_when_ready (slot->details->determine_view_file,
...
static void
anchor_window_slot_set_location (AnchorWindowSlot *slot,
				   GFile *location)
{
	GFile *old_location;
	...
	if (slot == anchor_window_get_active_slot (slot->details->window)) {
		anchor_window_sync_location_widgets (slot->details->window);
	}

	anchor_window_slot_update_title (slot);
	anchor_window_slot_emit_location_change (slot, old_location, location);
...
static void
viewed_file_changed_callback (AnchorFile *file,
                              AnchorWindowSlot *slot)
{
        GFile *new_location;
	...
	g_assert (file == slot->details->viewed_file);
        ...
        if (!anchor_file_is_not_yet_confirmed (file)) {
                slot->details->viewed_file_seen = TRUE;
        }

	was_in_trash = slot->details->viewed_file_in_trash;

	slot->details->viewed_file_in_trash = is_in_trash = anchor_file_is_in_trash (file);

	if (anchor_file_is_gone (file) || (is_in_trash && !was_in_trash)) {
			...
			if (g_file_is_native (location)) {
				mount = anchor_get_mounted_mount_for_root (location);
			...
			if (parent != NULL) {
				...
				go_to_file = anchor_find_existing_uri_in_hierarchy (parent);
			} else {
				go_to_file = g_file_new_for_path (g_get_home_dir ());
			}

			anchor_window_slot_open_location (slot, go_to_file, 0);
			...
			g_object_unref (location);
                ...
                new_location = anchor_file_get_location (file);
		anchor_window_slot_set_location (slot, new_location);
		g_object_unref (new_location);
        }
}

static void
anchor_window_slot_set_viewed_file (AnchorWindowSlot *slot,
				      AnchorFile *file)
{
	...
	if (slot->details->viewed_file != NULL) {
		g_signal_handlers_disconnect_by_func (slot->details->viewed_file,
						      ...
						      slot);
		anchor_file_monitor_remove (slot->details->viewed_file,
					      slot);
	}
	...
	if (file != NULL) {
		...
		anchor_file_monitor_add (file, slot, attributes);

		g_signal_connect_object (file, "changed",
					 G_CALLBACK (viewed_file_changed_callback), slot, 0);
	}

	anchor_file_unref (slot->details->viewed_file);
			    ...
			    gpointer user_data)
{
	...
	slot->details->determine_view_file = anchor_file_get (slot->details->pending_location);
	...
	if (!g_file_mount_enclosing_volume_finish (G_FILE (source_object), res, &error)) {
		...
		anchor_file_invalidate_all_attributes (slot->details->determine_view_file);
		anchor_file_call_when_ready (slot->details->determine_view_file,
					       NAUTILUS_FILE_ATTRIBUTE_INFO |
					       NAUTILUS_FILE_ATTRIBUTE_MOUNT,
					       got_file_info_for_view_selection_callback,
...
static void
got_file_info_for_view_selection_callback (AnchorFile *file,
					   gpointer callback_data)
{
        GError *error = NULL;
	char *view_id;
	...
	AnchorWindow *window;
	AnchorWindowSlot *slot;
	AnchorFile *viewed_file, *parent_file;
	...
	} else if (anchor_file_get_file_info_error (file) != NULL) {
		error = g_error_copy (anchor_file_get_file_info_error (file));
	}
	...
	if (default_location != NULL &&
		...
		slot->details->determine_view_file = anchor_file_get (default_location);
	...
	if ((parent_file != NULL) &&
	    anchor_file_get_file_type (file) == G_FILE_TYPE_REGULAR) {
		if (slot->details->pending_selection != NULL) {
			g_list_free_full (slot->details->pending_selection, (GDestroyNotify) anchor_file_unref);
		}

		g_clear_object (&slot->details->pending_location);
		g_free (slot->details->pending_scroll_to);

		slot->details->pending_location = anchor_file_get_parent_location (file);
		slot->details->pending_selection = g_list_prepend (NULL, anchor_file_ref (file));
		slot->details->determine_view_file = parent_file;
		slot->details->pending_scroll_to = anchor_file_get_uri (file);
	...
	view_id = NULL;
		...
		mimetype = anchor_file_get_mime_type (file);

		/* Otherwise, use default */
		if (slot->details->content_view != NULL) {
			view_id = g_strdup (anchor_view_get_view_id (slot->details->content_view));
		}

		if (view_id == NULL) {
			view_id = anchor_global_preferences_get_default_folder_viewer_preference_as_iid ();
		}
	...
	if (view_id != NULL) {
		...
		if (!gtk_widget_get_visible (GTK_WIDGET (window))) {
			...
			if (g_list_length (gtk_application_get_windows (app)) == 1) {
				...
				if (!anchor_is_root_directory (location)) {
					if (!anchor_is_home_directory (location)) {
						anchor_window_slot_go_home (slot, FALSE);
					} else {
						GFile *root;
			...
			end_location_change (slot);
			slot_location = anchor_window_slot_get_location (slot);
			...
			if (slot_location == NULL) {
				...
				viewed_file = anchor_file_get (slot_location);
				anchor_window_slot_set_viewed_file (slot, viewed_file);
				anchor_file_unref (viewed_file);
 ...
 */
static gboolean
create_content_view (AnchorWindowSlot *slot,
		     const char *view_id,
		     GError **error_out)
{
	AnchorWindow *window;
	...
	if (NAUTILUS_IS_DESKTOP_WINDOW (window)) {
        ...
        if (anchor_window_slot_content_view_matches_iid (slot, view_id)) {
		...
		view = anchor_view_new (view_id, slot);

                slot->details->new_content_view = view;
		anchor_window_slot_connect_new_content_view (slot);
        }

	/* Forward search selection and state before loading the new model */
	old_location = anchor_window_slot_get_location (slot);
	old_directory = anchor_directory_get (old_location);
	new_directory = anchor_directory_get (slot->details->pending_location);

	if (NAUTILUS_IS_SEARCH_DIRECTORY (new_directory) &&
	    !NAUTILUS_IS_SEARCH_DIRECTORY (old_directory)) {
		anchor_search_directory_set_base_model (NAUTILUS_SEARCH_DIRECTORY (new_directory), old_directory);
	}
	...
	if (NAUTILUS_IS_SEARCH_DIRECTORY (old_directory) &&
		 ...
		 * before anchor_window_slot_sync_search_widgets() is called
		...
		if (slot->details->pending_selection == NULL) {
			slot->details->pending_selection = anchor_view_get_selection (slot->details->content_view);
		}
	}
	...
	/* Actually load the pending location and selection: */
		...
		selection = anchor_view_get_selection (slot->details->content_view);
		load_new_location (slot,
				   old_location,
				   selection,
				   FALSE,
...
static void
load_new_location (AnchorWindowSlot *slot,
		   GFile *location,
		   GList *selection,
		   gboolean tell_current_content_view,
		   gboolean tell_new_content_view)
{
	...
	/* Note, these may recurse into report_load_underway */
		...
		view = slot->details->content_view;
		anchor_view_load_location (slot->details->content_view, location);
        }
		...
		anchor_view_load_location (slot->details->new_content_view, location);
        }
	if (view != NULL) {
		/* new_content_view might have changed here if
		   report_load_underway was called from load_location */
		anchor_view_set_selection (view, selection_copy);
	}
...
static void
end_location_change (AnchorWindowSlot *slot)
{
	char *uri;
	...
	anchor_window_slot_set_allow_stop (slot, FALSE);
...
static void
free_location_change (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	...
	if (slot->details->mount_cancellable != NULL) {
		...
		anchor_file_cancel_call_when_ready
		...
		anchor_view_stop_loading (slot->details->new_content_view);
		slot->details->temporarily_ignore_view_signals = FALSE;

		anchor_window_disconnect_content_view (window, slot->details->new_content_view);
...
static void
cancel_location_change (AnchorWindowSlot *slot)
{
	GList *selection;
	GFile *location;

	location = anchor_window_slot_get_location (slot);
...
static void
display_view_selection_failure (AnchorWindow *window, AnchorFile *file,
...
void
anchor_window_slot_set_content_view (AnchorWindowSlot *slot,
				       const char *id)
{
	char *uri;
	...
	if (anchor_window_slot_content_view_matches_iid (slot, id)) {
		return;
        }
        ...
        if (anchor_view_get_selection_count (slot->details->content_view) == 0) {
                /* If there is no selection, queue a scroll to the same icon that
                 * is currently visible */
                slot->details->pending_scroll_to = anchor_view_get_first_visible_file (slot->details->content_view);
        }
...
void
anchor_window_back_or_forward (AnchorWindow *window,
				 gboolean back,
				 guint distance,
				 AnchorWindowOpenFlags flags)
{
	...
	location = anchor_bookmark_get_location (bookmark);

	if (flags != 0) {
		anchor_window_slot_open_location (slot, location, flags);
	} else {
		char *scroll_pos;

		old_location = anchor_window_slot_get_location (slot);
		scroll_pos = anchor_bookmark_get_scroll_pos (bookmark);
		begin_location_change
			(slot,
...
static void
anchor_window_slot_force_reload (AnchorWindowSlot *slot)
{
	GFile *location;
        char *current_pos;
	GList *selection;
...
void
anchor_window_slot_queue_reload (AnchorWindowSlot *slot)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));

	if (anchor_window_slot_get_location (slot) == NULL) {
		return;
	}

	if (slot->details->pending_location != NULL
	    || slot->details->content_view == NULL
	    || anchor_view_get_loading (slot->details->content_view)) {
...
static void
anchor_window_slot_clear_forward_list (AnchorWindowSlot *slot)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));
...
static void
anchor_window_slot_clear_back_list (AnchorWindowSlot *slot)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));
...
static void
anchor_window_slot_update_bookmark (AnchorWindowSlot *slot, AnchorFile *file)
{
        gboolean recreate;
	GFile *new_location;
	...
	if (slot->details->current_location_bookmark == NULL) {
		...
		bookmark_location = anchor_bookmark_get_location (slot->details->current_location_bookmark);
	...
	if (recreate) {
		...
		slot->details->current_location_bookmark = anchor_bookmark_new (new_location, display_name);
		g_free (display_name);
        }
...
static void
check_bookmark_location_matches (AnchorBookmark *bookmark, GFile *location)
{
        GFile *bookmark_location;
        char *bookmark_uri, *uri;

	bookmark_location = anchor_bookmark_get_location (bookmark);
 ...
 */
static void
check_last_bookmark_location_matches_slot (AnchorWindowSlot *slot)
{
	check_bookmark_location_matches (slot->details->last_location_bookmark,
					 anchor_window_slot_get_location (slot));
}

static void
handle_go_direction (AnchorWindowSlot *slot,
		     GFile              *location,
		     gboolean            forward)
{
	...
	check_bookmark_location_matches (g_list_nth_data (list, slot->details->location_change_distance),
					 location);
	g_assert (anchor_window_slot_get_location (slot) != NULL);
...
static void
handle_go_elsewhere (AnchorWindowSlot *slot,
		     GFile *location)
{
	GFile *slot_location;

	/* Clobber the entire forward list, and move displayed location to back list */
	anchor_window_slot_clear_forward_list (slot);
	slot_location = anchor_window_slot_get_location (slot);
...
static void
update_history (AnchorWindowSlot *slot,
                AnchorLocationChangeType type,
                GFile *new_location)
{
        switch (type) {
...
static void
anchor_window_slot_show_x_content_bar (AnchorWindowSlot *slot, GMount *mount, const char **x_content_types)
{
	GtkWidget *bar;

	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));

	bar = anchor_x_content_bar_new (mount, x_content_types);
	gtk_widget_show (bar);
	anchor_window_slot_add_extra_location_widget (slot, bar);
}
		       ...
		       gpointer user_data)
{
	...
	if (x_content_types != NULL && x_content_types[0] != NULL) {
		anchor_window_slot_show_x_content_bar (slot, data->mount, x_content_types);
	}
...
static void
anchor_window_slot_emit_location_change (AnchorWindowSlot *slot,
					   GFile *from,
					   GFile *to)
{
	char *from_uri = NULL;
...
static void
anchor_window_slot_show_trash_bar (AnchorWindowSlot *slot)
{
	GtkWidget *bar;
	AnchorView *view;

	view = anchor_window_slot_get_current_view (slot);
	bar = anchor_trash_bar_new (view);
	gtk_widget_show (bar);
...
static void
anchor_window_slot_show_special_location_bar (AnchorWindowSlot     *slot,
						AnchorSpecialLocation special_location)
{
	GtkWidget *bar;

	bar = anchor_special_location_bar_new (special_location);
	gtk_widget_show (bar);
...
static void
slot_add_extension_extra_widgets (AnchorWindowSlot *slot)
{
	GList *providers, *l;
	GtkWidget *widget;
	char *uri;
	...
	providers = anchor_module_get_extensions_for_type (ANCHOR_TYPE_LOCATION_WIDGET_PROVIDER);
	window = anchor_window_slot_get_window (slot);

	uri = anchor_window_slot_get_location_uri (slot);
	for (l = providers; l != NULL; l = l->next) {
		AnchorLocationWidgetProvider *provider;

		provider = NAUTILUS_LOCATION_WIDGET_PROVIDER (l->data);
		widget = anchor_location_widget_provider_get_widget (provider, uri, GTK_WIDGET (window));
		if (widget != NULL) {
			anchor_window_slot_add_extra_location_widget (slot, widget);
		}
	}
	g_free (uri);
...
static void
anchor_window_slot_update_for_new_location (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	...
	file = anchor_file_get (new_location);
	anchor_window_slot_update_bookmark (slot, file);
	...
	location_really_changed =
        ...
        /* Create a AnchorFile for this location, so we can catch it
         * if it goes away.
         */
	anchor_window_slot_set_viewed_file (slot, file);
	slot->details->viewed_file_seen = !anchor_file_is_not_yet_confirmed (file);
	slot->details->viewed_file_in_trash = anchor_file_is_in_trash (file);
        anchor_file_unref (file);

	anchor_window_slot_set_location (slot, new_location);

	if (slot == anchor_window_get_active_slot (window)) {
		/* Sync up and zoom action states */
		anchor_window_sync_up_button (window);
		anchor_window_sync_zoom_widgets (window);

		/* Sync the content view menu for this new location. */
		anchor_window_slot_sync_view_as_menus (slot);

		/* Load menus from anchor extensions for this location */
		anchor_window_load_extension_menus (window);
	}
...
static void
view_end_loading_cb (AnchorView       *view,
		     ...
		     AnchorWindowSlot *slot)
{
	...
	if (view == slot->details->content_view && all_files_seen) {
		if (slot->details->pending_scroll_to != NULL) {
			anchor_view_scroll_to_file (slot->details->content_view,
...
static void
real_setup_loading_floating_bar (AnchorWindowSlot *slot)
{
	gboolean disable_chrome;

	g_object_get (anchor_window_slot_get_window (slot),
	...
	anchor_floating_bar_cleanup_actions (NAUTILUS_FLOATING_BAR (slot->details->floating_bar));
	anchor_floating_bar_set_primary_label (NAUTILUS_FLOATING_BAR (slot->details->floating_bar),
						 NAUTILUS_IS_SEARCH_DIRECTORY (anchor_view_get_model (slot->details->content_view)) ?
						 _("Searching…") : _("Loading…"));
	anchor_floating_bar_set_details_label (NAUTILUS_FLOATING_BAR (slot->details->floating_bar), NULL);
	anchor_floating_bar_set_show_spinner (NAUTILUS_FLOATING_BAR (slot->details->floating_bar),
						TRUE);
	anchor_floating_bar_add_action (NAUTILUS_FLOATING_BAR (slot->details->floating_bar),
					  "gtk-stop",
...
{
	AnchorWindowSlot *slot = user_data;
...
static void
setup_loading_floating_bar (AnchorWindowSlot *slot)
{
...
static void
view_begin_loading_cb (AnchorView       *view,
		       AnchorWindowSlot *slot)
{
	if (slot->details->temporarily_ignore_view_signals) {
		return;
...
static void
anchor_window_slot_setup_extra_location_widgets (AnchorWindowSlot *slot)
{
	GFile *location;
	FindMountData *data;
	AnchorDirectory *directory;
	...
	if (anchor_directory_is_in_trash (directory)) {
		anchor_window_slot_show_trash_bar (slot);
	} else {
		AnchorFile *file;
		GFile *scripts_file;
		char *scripts_path = anchor_get_scripts_directory_path ();

		scripts_file = g_file_new_for_path (scripts_path);
		g_free (scripts_path);

		file = anchor_file_get (location);

		if (anchor_should_use_templates_directory () &&
		    anchor_file_is_user_special_directory (file, G_USER_DIRECTORY_TEMPLATES)) {
			anchor_window_slot_show_special_location_bar (slot, NAUTILUS_SPECIAL_LOCATION_TEMPLATES);
		} else if (g_file_equal (location, scripts_file)) {
			anchor_window_slot_show_special_location_bar (slot, NAUTILUS_SPECIAL_LOCATION_SCRIPTS);
		}
...
static void
anchor_window_slot_connect_new_content_view (AnchorWindowSlot *slot)
{
	if (slot->details->content_view != NULL) {
		/* disconnect old view */
...
static void
anchor_window_slot_switch_new_content_view (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	GtkWidget *widget;
	...
	if (slot->details->content_view != NULL) {
		anchor_window_disconnect_content_view (window, slot->details->content_view);
	...
	if (slot->details->new_content_view != NULL) {
		...
		/* connect new view */
		anchor_window_connect_content_view (slot->details->window, slot->details->content_view);

		if (!NAUTILUS_IS_SEARCH_DIRECTORY (anchor_view_get_model (slot->details->content_view)) &&
		    slot == anchor_window_get_active_slot (window)) {
			anchor_view_grab_focus (slot->details->content_view);
		}
	}
}
...
static void
location_has_really_changed (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	GFile *location;
	 ...
	 * anchor_window_slot_switch_new_content_view().
	 */
	anchor_window_slot_remove_extra_location_widgets (slot);
	anchor_window_slot_switch_new_content_view (slot);

	if (slot->details->pending_location != NULL) {
		/* Tell the window we are finished. */
		anchor_window_slot_update_for_new_location (slot);
	}

	/* Now that we finished switching to the new location,
	 * add back the extra location widgets.
	 */
	anchor_window_slot_setup_extra_location_widgets (slot);
...
static void
anchor_window_slot_dispose (GObject *object)
{
	AnchorWindowSlot *slot;
	GtkWidget *widget;
	...
	anchor_window_slot_clear_forward_list (slot);
	anchor_window_slot_clear_back_list (slot);

	anchor_window_slot_remove_extra_location_widgets (slot);

	if (slot->details->content_view) {
		anchor_window_disconnect_content_view (anchor_window_slot_get_window (slot),
	...
	anchor_window_slot_set_viewed_file (slot, NULL);
	...
	G_OBJECT_CLASS (anchor_window_slot_parent_class)->dispose (object);
}

static void
anchor_window_slot_class_init (AnchorWindowSlotClass *klass)
{
	...
	oclass->dispose = anchor_window_slot_dispose;
	oclass->constructed = anchor_window_slot_constructed;
	oclass->set_property = anchor_window_slot_set_property;
	oclass->get_property = anchor_window_slot_get_property;

	signals[ACTIVE] =
		g_signal_new ("active",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AnchorWindowSlotClass, active),
			      NULL, NULL,
	...
	signals[INACTIVE] =
		g_signal_new ("inactive",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AnchorWindowSlotClass, inactive),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE, 0);
	...
	properties[PROP_WINDOW] =
		g_param_spec_object ("window",
				     "The AnchorWindow",
				     "The AnchorWindow this slot is part of",
				     ANCHOR_TYPE_WINDOW,
				     G_PARAM_READWRITE | G_PARAM_CONSTRUCT);

	g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
	g_type_class_add_private (klass, sizeof (AnchorWindowSlotDetails));
}

GFile *
anchor_window_slot_get_location (AnchorWindowSlot *slot)
{
	g_assert (slot != NULL);
...
const gchar *
anchor_window_slot_get_title (AnchorWindowSlot *slot)
{
	return slot->details->title;
}

char *
anchor_window_slot_get_location_uri (AnchorWindowSlot *slot)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));
...
AnchorWindow *
anchor_window_slot_get_window (AnchorWindowSlot *slot)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));
...
void
anchor_window_slot_set_window (AnchorWindowSlot *slot,
				 AnchorWindow *window)
{
	g_assert (NAUTILUS_IS_WINDOW_SLOT (slot));
...
AnchorView *
anchor_window_slot_get_view (AnchorWindowSlot *slot)
{
	return slot->details->content_view;
}

/* anchor_window_slot_update_title:
 * 
 * Re-calculate the slot title.
 * Called when the location or view has changed.
 * @slot: The AnchorWindowSlot in question.
 * 
 */
void
anchor_window_slot_update_title (AnchorWindowSlot *slot)
{
	AnchorWindow *window;
	char *title;
	gboolean do_sync = FALSE;

	title = anchor_compute_title_for_location (slot->details->location);
	window = anchor_window_slot_get_window (slot);
...
gboolean
anchor_window_slot_get_allow_stop (AnchorWindowSlot *slot)
{
	return slot->details->allow_stop;
}

void
anchor_window_slot_set_allow_stop (AnchorWindowSlot *slot,
				     gboolean allow)
{
	AnchorWindow *window;
...
void
anchor_window_slot_stop_loading (AnchorWindowSlot *slot)
{
	anchor_view_stop_loading (slot->details->content_view);
...
static void
real_slot_set_short_status (AnchorWindowSlot *slot,
			    ...
			    const gchar *detail_status)
{
	...
	anchor_floating_bar_set_labels (NAUTILUS_FLOATING_BAR (slot->details->floating_bar),
...
static void
set_floating_bar_status (AnchorWindowSlot *slot,
			 const gchar *primary_status,
			 const gchar *detail_status)
{
...
void
anchor_window_slot_set_status (AnchorWindowSlot *slot,
...
char *
anchor_window_slot_get_current_uri (AnchorWindowSlot *slot)
{
	GFile *location;
...
AnchorView *
anchor_window_slot_get_current_view (AnchorWindowSlot *slot)
{
	if (slot->details->content_view != NULL) {
		return slot->details->content_view;
...
void
anchor_window_slot_go_home (AnchorWindowSlot *slot,
			      AnchorWindowOpenFlags flags)
{			      
	GFile *home;
	...
	anchor_window_slot_open_location (slot, home, flags);
	g_object_unref (home);
}

void
anchor_window_slot_go_up (AnchorWindowSlot *slot,
			    AnchorWindowOpenFlags flags)
{
	GFile *parent;
	...
	anchor_window_slot_open_location (slot, parent, flags);
	g_object_unref (parent);
}

AnchorFile *
anchor_window_slot_get_file (AnchorWindowSlot *slot)
{
	return slot->details->viewed_file;
}

AnchorBookmark *
anchor_window_slot_get_bookmark (AnchorWindowSlot *slot)
{
	return slot->details->current_location_bookmark;
}

GList *
anchor_window_slot_get_back_history (AnchorWindowSlot *slot)
{
	return slot->details->back_list;
}

GList *
anchor_window_slot_get_forward_history (AnchorWindowSlot *slot)
{
	return slot->details->forward_list;
}

AnchorWindowSlot *
anchor_window_slot_new (AnchorWindow *window)
{
	return g_object_new (ANCHOR_TYPE_WINDOW_SLOT,
			     "window", window,
			     NULL);
}
