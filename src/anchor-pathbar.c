...
#include "anchor-window-slot-dnd.h"
...
G_DEFINE_TYPE (AnchorPathBar, anchor_path_bar,
	       GTK_TYPE_CONTAINER);

static void     anchor_path_bar_scroll_up                (AnchorPathBar *path_bar);
static void     anchor_path_bar_scroll_down              (AnchorPathBar *path_bar);
static void     anchor_path_bar_stop_scrolling           (AnchorPathBar *path_bar);
static gboolean anchor_path_bar_slider_button_press      (GtkWidget       *widget,
							    GdkEventButton  *event,
							    AnchorPathBar *path_bar);
static gboolean anchor_path_bar_slider_button_release    (GtkWidget       *widget,
							    GdkEventButton  *event,
							    AnchorPathBar *path_bar);
static void     anchor_path_bar_check_icon_theme         (AnchorPathBar *path_bar);
static void     anchor_path_bar_update_button_appearance (ButtonData      *button_data);
static void     anchor_path_bar_update_button_state      (ButtonData      *button_data,
							    gboolean         current_dir);
static void     anchor_path_bar_update_path              (AnchorPathBar *path_bar,
							    GFile           *file_path);

static GtkWidget *
get_slider_button (AnchorPathBar  *path_bar,
...
slider_timeout (gpointer user_data)
{
	...
	if (gtk_widget_get_visible (GTK_WIDGET (path_bar))) {
		if (path_bar->priv->drag_slider_timeout_for_up_button) {
			anchor_path_bar_scroll_up (path_bar);
		} else {
			anchor_path_bar_scroll_down (path_bar);
		}
	}
...
static void
anchor_path_bar_slider_drag_motion (GtkWidget      *widget,
...
static void
anchor_path_bar_slider_drag_leave (GtkWidget      *widget,
				     GdkDragContext *context,
				     unsigned int    time,
				     gpointer        user_data)
{
...
static void
anchor_path_bar_init (AnchorPathBar *path_bar)
{
	path_bar->priv = G_TYPE_INSTANCE_GET_PRIVATE (path_bar, ANCHOR_TYPE_PATH_BAR, AnchorPathBarDetails);
	...
	gtk_widget_set_has_window (GTK_WIDGET (path_bar), FALSE);
        ...
        g_signal_connect_swapped (path_bar->priv->up_slider_button, "clicked", G_CALLBACK (anchor_path_bar_scroll_up), path_bar);
        g_signal_connect_swapped (path_bar->priv->down_slider_button, "clicked", G_CALLBACK (anchor_path_bar_scroll_down), path_bar);

        g_signal_connect (path_bar->priv->up_slider_button, "button-press-event", G_CALLBACK (anchor_path_bar_slider_button_press), path_bar);
        g_signal_connect (path_bar->priv->up_slider_button, "button-release-event", G_CALLBACK (anchor_path_bar_slider_button_release), path_bar);
        g_signal_connect (path_bar->priv->down_slider_button, "button-press-event", G_CALLBACK (anchor_path_bar_slider_button_press), path_bar);
        g_signal_connect (path_bar->priv->down_slider_button, "button-release-event", G_CALLBACK (anchor_path_bar_slider_button_release), path_bar);
	...
	g_signal_connect (path_bar->priv->up_slider_button,
			  "drag-motion",
			  G_CALLBACK (anchor_path_bar_slider_drag_motion),
			  path_bar);
	g_signal_connect (path_bar->priv->up_slider_button,
			  "drag-leave",
			  G_CALLBACK (anchor_path_bar_slider_drag_leave),
			  path_bar);
...
static void
anchor_path_bar_finalize (GObject *object)
{
        AnchorPathBar *path_bar;

        path_bar = NAUTILUS_PATH_BAR (object);

	anchor_path_bar_stop_scrolling (path_bar);
	...
	if (path_bar->priv->drag_slider_timeout != 0) {
        ...
        G_OBJECT_CLASS (anchor_path_bar_parent_class)->finalize (object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void
remove_settings_signal (AnchorPathBar *path_bar,
			GdkScreen  *screen)
{
...
static void
anchor_path_bar_dispose (GObject *object)
{
        remove_settings_signal (NAUTILUS_PATH_BAR (object), gtk_widget_get_screen (GTK_WIDGET (object)));

        G_OBJECT_CLASS (anchor_path_bar_parent_class)->dispose (object);
}
 ...
 */
static void
anchor_path_bar_get_preferred_width (GtkWidget *widget,
				       gint      *minimum,
				       gint      *natural)
{
...
static void
anchor_path_bar_get_preferred_height (GtkWidget *widget,
					gint      *minimum,
					gint      *natural)
{
	ButtonData *button_data;
...
static void
anchor_path_bar_update_slider_buttons (AnchorPathBar *path_bar)
{
...
static void
anchor_path_bar_unmap (GtkWidget *widget)
{
	anchor_path_bar_stop_scrolling (NAUTILUS_PATH_BAR (widget));
	gdk_window_hide (NAUTILUS_PATH_BAR (widget)->priv->event_window);

	GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->unmap (widget);
}

static void
anchor_path_bar_map (GtkWidget *widget)
{
	gdk_window_show (NAUTILUS_PATH_BAR (widget)->priv->event_window);

	GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->map (widget);
}


static void
child_ordering_changed (AnchorPathBar *path_bar)
{
	GList *l;
...
static void
anchor_path_bar_size_allocate (GtkWidget     *widget,
			    	 GtkAllocation *allocation)
{
	...
	if (need_sliders) {
		...
		needs_reorder |= gtk_widget_get_child_visible (path_bar->priv->down_slider_button) == FALSE;
      		...
      		anchor_path_bar_update_slider_buttons (path_bar);
    	} else {
...
static void
anchor_path_bar_style_updated (GtkWidget *widget)
{
	GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->style_updated (widget);

        anchor_path_bar_check_icon_theme (NAUTILUS_PATH_BAR (widget));
}

static void
anchor_path_bar_screen_changed (GtkWidget *widget,
			          GdkScreen *previous_screen)
{
        if (GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->screen_changed) {
                GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->screen_changed (widget, previous_screen);
	}
...
static gboolean
anchor_path_bar_scroll (GtkWidget      *widget,
...
static void
anchor_path_bar_realize (GtkWidget *widget)
{
	AnchorPathBar *path_bar;
	GtkAllocation allocation;
	GdkWindow *window;
...
static void
anchor_path_bar_unrealize (GtkWidget *widget)
{
	AnchorPathBar *path_bar;
	...
	GTK_WIDGET_CLASS (anchor_path_bar_parent_class)->unrealize (widget);
}

static void
anchor_path_bar_add (GtkContainer *container,
		       GtkWidget    *widget)
{
...
static void
anchor_path_bar_remove_1 (GtkContainer *container,
		       	    GtkWidget    *widget)
{
...
static void
anchor_path_bar_remove (GtkContainer *container,
		          GtkWidget    *widget)
{
        AnchorPathBar *path_bar;
        GList *children;
        ...
        if (widget == path_bar->priv->up_slider_button) {
                anchor_path_bar_remove_1 (container, widget);
...
static void
anchor_path_bar_forall (GtkContainer *container,
...
static void
anchor_path_bar_grab_notify (GtkWidget *widget,
...
static void
anchor_path_bar_state_changed (GtkWidget    *widget,
...
anchor_path_bar_get_path_for_child (GtkContainer *container,
				      GtkWidget    *child)
{
	AnchorPathBar *path_bar = NAUTILUS_PATH_BAR (container);
	GtkWidgetPath *path;
...
static void
anchor_path_bar_class_init (AnchorPathBarClass *path_bar_class)
{
        ...
        gobject_class->finalize = anchor_path_bar_finalize;
        gobject_class->dispose = anchor_path_bar_dispose;

	widget_class->get_preferred_height = anchor_path_bar_get_preferred_height;
	widget_class->get_preferred_width = anchor_path_bar_get_preferred_width;
	widget_class->realize = anchor_path_bar_realize;
	widget_class->unrealize = anchor_path_bar_unrealize;
	widget_class->unmap = anchor_path_bar_unmap;
	widget_class->map = anchor_path_bar_map;
        widget_class->size_allocate = anchor_path_bar_size_allocate;
        widget_class->style_updated = anchor_path_bar_style_updated;
        widget_class->screen_changed = anchor_path_bar_screen_changed;
        widget_class->grab_notify = anchor_path_bar_grab_notify;
        widget_class->state_changed = anchor_path_bar_state_changed;
	widget_class->scroll_event = anchor_path_bar_scroll;

        container_class->add = anchor_path_bar_add;
        container_class->forall = anchor_path_bar_forall;
        container_class->remove = anchor_path_bar_remove;
	container_class->get_path_for_child = anchor_path_bar_get_path_for_child;

        path_bar_signals [PATH_CLICKED] =
                g_signal_new ("path-clicked",
		  G_OBJECT_CLASS_TYPE (path_bar_class),
		  G_SIGNAL_RUN_FIRST,
		  G_STRUCT_OFFSET (AnchorPathBarClass, path_clicked),
		  NULL, NULL,
        ...
        path_bar_signals [PATH_EVENT] =
		  ...
		  G_STRUCT_OFFSET (AnchorPathBarClass, path_event),
		  NULL, NULL, NULL,
		  G_TYPE_BOOLEAN, 2,
		  G_TYPE_FILE,
		  GDK_TYPE_EVENT);
	 ...
	 g_type_class_add_private (path_bar_class, sizeof (AnchorPathBarDetails));
}

static void
anchor_path_bar_scroll_down (AnchorPathBar *path_bar)
{
        GList *list;
        GList *down_button;
        GList *up_button;
...
static void
anchor_path_bar_scroll_up (AnchorPathBar *path_bar)
{
        GList *list;
...
static gboolean
anchor_path_bar_scroll_timeout (AnchorPathBar *path_bar)
{
			...
			path_bar->priv->need_timer = FALSE;
	  		...
	  		path_bar->priv->timer = 
				g_timeout_add (SCROLL_TIMEOUT,
					       (GSourceFunc) anchor_path_bar_scroll_timeout,
					       path_bar);
...
static void 
anchor_path_bar_stop_scrolling (AnchorPathBar *path_bar)
{
        if (path_bar->priv->timer) {
...
static gboolean
anchor_path_bar_slider_button_press (GtkWidget       *widget, 
	   			       GdkEventButton  *event,
				       AnchorPathBar *path_bar)
{
...
static gboolean
anchor_path_bar_slider_button_release (GtkWidget      *widget, 
  				         GdkEventButton *event,
				         AnchorPathBar     *path_bar)
{
...
static void
reload_icons (AnchorPathBar *path_bar)
{
        GList *list;
		...
		if (button_data->type != NORMAL_BUTTON || button_data->is_root) {
                	anchor_path_bar_update_button_appearance (button_data);
		}
...
static void
anchor_path_bar_check_icon_theme (AnchorPathBar *path_bar)
{
        GtkSettings *settings;
...
static void
anchor_path_bar_clear_buttons (AnchorPathBar *path_bar)
{
...
get_gicon_for_mount (ButtonData *button_data)
{
	...
	icon = NULL;
	mount = anchor_get_mounted_mount_for_root (button_data->path);
...
button_data_free (ButtonData *button_data)
{
	...
	if (button_data->file != NULL) {
		...
		anchor_file_monitor_remove (button_data->file, button_data);
		anchor_file_unref (button_data->file);
	}
...
static void
anchor_path_bar_update_button_appearance (ButtonData *button_data)
{
...
static void
anchor_path_bar_update_button_state (ButtonData *button_data,
				       gboolean    current_dir)
{
	if (button_data->label != NULL) {
		   ...
		   GFile *location)
{
	...
	} else if (anchor_is_home_directory (location)) {
		button_data->type = HOME_BUTTON;
		button_data->is_root = TRUE;
	} else if ((mount = anchor_get_mounted_mount_for_root (location)) != NULL) {
...
static void
button_data_file_changed (AnchorFile *file,
			  ButtonData *button_data)
{
	...
	path_bar = (AnchorPathBar *) gtk_widget_get_ancestor (button_data->button,
								ANCHOR_TYPE_PATH_BAR);
	if (path_bar == NULL) {
		return;
	}
	...
	if (!g_file_equal (button_data->path, location)) {
		...
		if (renamed) {
			...
			if (child) {
				 ...
				 */
				current_location = anchor_file_get_location (current_button_data->file);
			}

        		anchor_path_bar_update_path (path_bar, location);
        		anchor_path_bar_set_path (path_bar, current_location);
			g_object_unref (location);
			g_object_unref (current_location);
			return;
		}
	} else if (anchor_file_is_gone (file)) {
		gint idx, position;
		...
		if (g_file_has_prefix (current_location, location) ||
			...
			anchor_path_bar_clear_buttons (path_bar);
...
static ButtonData *
make_button_data (AnchorPathBar  *path_bar,
		  AnchorFile     *file,
		  gboolean          current_dir)
{
	GFile *path;
	...
	path = anchor_file_get_location (file);
	child = NULL;
	...
	if (button_data->dir_name == NULL) {
		button_data->dir_name = anchor_file_get_display_name (file);
	}
	if (button_data->file == NULL) {
		button_data->file = anchor_file_ref (file);
		anchor_file_monitor_add (button_data->file, button_data,
        ...
        anchor_path_bar_update_button_state (button_data, current_dir);
	...
	anchor_drag_slot_proxy_init (button_data->button, button_data->file, NULL);
...
static gboolean
anchor_path_bar_check_parent_path (AnchorPathBar *path_bar,
				     GFile *location,
				     ButtonData **current_button_data)
{
        GList *list;
	...
	current_data = NULL;
		...
		anchor_path_bar_update_button_state (button_data, is_active);
        }
...
static void
anchor_path_bar_update_path (AnchorPathBar *path_bar,
			       GFile *file_path)
{
	AnchorFile *file;
	...
	file = anchor_file_get (file_path);
...
void
anchor_path_bar_set_path (AnchorPathBar *path_bar, 
			    GFile *file_path)
{
	ButtonData *button_data;
        ...
        if (!anchor_path_bar_check_parent_path (path_bar, file_path, &button_data)) {
		anchor_path_bar_update_path (path_bar, file_path);
