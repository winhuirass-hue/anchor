...
#include "anchor-desktop-item-properties.h"
...
#include <libanchor-extension/anchor-property-page-provider.h>
...
#include <libanchor-private/anchor-mime-application-chooser.h>
...
struct AnchorPropertiesWindowDetails {	
	...
	AnchorFile *group_change_file;
	char         *group_change_group;
	unsigned int  group_change_timeout;
	AnchorFile *owner_change_file;
...
static void schedule_directory_contents_update    (AnchorPropertiesWindow *window);
static void directory_contents_value_field_update (AnchorPropertiesWindow *window);
static void file_changed_callback                 (AnchorFile       *file,
						   gpointer            user_data);
static void permission_button_update              (AnchorPropertiesWindow *window,
						   GtkToggleButton    *button);
static void permission_combo_update               (AnchorPropertiesWindow *window,
						   GtkComboBox        *combo);
static void value_field_update                    (AnchorPropertiesWindow *window,
						   GtkLabel           *field);
static void properties_window_update              (AnchorPropertiesWindow *window,
						   GList              *files);
static void is_directory_ready_callback           (AnchorFile       *file,
						   gpointer            data);
static void cancel_group_change_callback          (AnchorPropertiesWindow *window);
static void cancel_owner_change_callback          (AnchorPropertiesWindow *window);
static void parent_widget_destroyed_callback      (GtkWidget          *widget,
						   gpointer            callback_data);
static void select_image_button_callback          (GtkWidget          *widget,
						   AnchorPropertiesWindow *properties_window);
...
static void append_extension_pages                (AnchorPropertiesWindow *window);

static gboolean name_field_focus_out              (AnchorEntry *name_field,
						   GdkEventFocus *event,
						   gpointer callback_data);
static void name_field_activate                   (AnchorEntry *name_field,
						   gpointer callback_data);
static GtkLabel *attach_ellipsizing_value_label   (GtkGrid *grid,
						   GtkWidget *sibling,
						   const char *initial_text);
						   
static GtkWidget* create_pie_widget 		  (AnchorPropertiesWindow *window);

G_DEFINE_TYPE (AnchorPropertiesWindow, anchor_properties_window, GTK_TYPE_DIALOG);

static gboolean
is_multi_file_window (AnchorPropertiesWindow *window)
{
	GList *l;
	int count;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		if (!anchor_file_is_gone (NAUTILUS_FILE (l->data))) {			
			count++;
			if (count > 1) {
				return TRUE;
			}	
...
static int
get_not_gone_original_file_count (AnchorPropertiesWindow *window)
{
	GList *l;
	int count;
...
static AnchorFile *
get_original_file (AnchorPropertiesWindow *window) 
{
...
static AnchorFile *
get_target_file_for_original_file (AnchorFile *file)
{
	...
	if (NAUTILUS_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			...
			if (location) {
				target_file = anchor_file_get (location);
		...
		uri_to_display = anchor_file_get_activation_uri (file);
		if (uri_to_display != NULL) {
			target_file = anchor_file_get_by_uri (uri_to_display);
...
static AnchorFile *
get_target_file (AnchorPropertiesWindow *window)
{
...
static void
get_image_for_properties_window (AnchorPropertiesWindow *window,
				 char **icon_name,
				 GdkPixbuf **icon_pixbuf)
{
	AnchorIconInfo *icon, *new_icon;
	GList *l;
	gint icon_scale;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		...
		if (!icon) {
			icon = anchor_file_get_icon (file, NAUTILUS_ICON_SIZE_STANDARD, icon_scale,
						       NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS |
						       NAUTILUS_FILE_ICON_FLAGS_IGNORE_VISITING);
		} else {
			new_icon = anchor_file_get_icon (file, NAUTILUS_ICON_SIZE_STANDARD, icon_scale,
							   NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS |
							   NAUTILUS_FILE_ICON_FLAGS_IGNORE_VISITING);
	...
	if (!icon) {
		icon = anchor_icon_info_lookup_from_name ("text-x-generic",
							    NAUTILUS_ICON_SIZE_STANDARD,
							    icon_scale);
	}

	if (icon_name != NULL) {
		*icon_name = g_strdup (anchor_icon_info_get_used_name (icon));
	}

	if (icon_pixbuf != NULL) {
		*icon_pixbuf = anchor_icon_info_get_pixbuf_at_size (icon, NAUTILUS_ICON_SIZE_STANDARD);
	}
...
static void
update_properties_window_icon (AnchorPropertiesWindow *window)
{
	GdkPixbuf *pixbuf;
...
static void
reset_icon (AnchorPropertiesWindow *properties_window)
{
	GList *l;
...
static void  
anchor_properties_window_drag_data_received (GtkWidget *widget, GdkDragContext *context,
					       int x, int y,
...
static GtkWidget *
create_image_widget (AnchorPropertiesWindow *window,
		     gboolean is_customizable)
{
	...
	if (is_customizable) {
		...
		g_signal_connect (image, "drag-data-received",
				  G_CALLBACK (anchor_properties_window_drag_data_received), NULL);
...
static void
set_name_field (AnchorPropertiesWindow *window,
		...
		const gchar *name)
{
	 ...
	 */
	use_label = is_multi_file_window (window) || !anchor_file_can_rename (get_original_file (window));
	...
	if (new_widget) {
		...
		if (use_label) {
			...
			window->details->name_field = anchor_entry_new ();
...
static void
update_name_field (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	...
	if (is_multi_file_window (window)) {
		...
		if (file == NULL || anchor_file_is_gone (file)) {
			...
			current_name = anchor_file_get_display_name (file);
		}
...
static void
name_field_restore_original_name (AnchorEntry *name_field)
{
	...
	anchor_entry_select_all (name_field);

	g_free (displayed_name);
}

static void
rename_callback (AnchorFile *file, GFile *res_loc, GError *error, gpointer callback_data)
{
	AnchorPropertiesWindow *window;

	window = NAUTILUS_PROPERTIES_WINDOW (callback_data);

	/* Complain to user if rename failed. */
	if (error != NULL) {
		anchor_report_error_renaming_file (file, 
...
static void
set_pending_name (AnchorPropertiesWindow *window, const char *name)
{
...
static void
name_field_done_editing (AnchorEntry *name_field, AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *new_name;
	const char *original_name;
	...
	if (strlen (new_name) == 0) {
		...
		if (strcmp (new_name, original_name) != 0) {		
			...
			g_object_ref (window);
			anchor_file_rename (file, new_name,
...
static gboolean
name_field_focus_out (AnchorEntry *name_field,
		      GdkEventFocus *event,
		      gpointer callback_data)
{
...
static void
name_field_activate (AnchorEntry *name_field, gpointer callback_data)
{
	...
	anchor_entry_select_all_at_idle (name_field);
}

static void
update_properties_window_title (AnchorPropertiesWindow *window)
{
	char *name, *title;
	AnchorFile *file;
...
static void
clear_extension_pages (AnchorPropertiesWindow *window)
{
	int i;
	int num_pages;
	GtkWidget *page;
...
static void
refresh_extension_pages (AnchorPropertiesWindow *window)
{
...
static void
remove_from_dialog (AnchorPropertiesWindow *window,
		    AnchorFile *file)
{
	int index;
	GList *original_link;
	...
	anchor_file_monitor_remove (original_file, &window->details->original_files);
	anchor_file_monitor_remove (target_file, &window->details->target_files);
...
static GList *
get_mime_list (AnchorPropertiesWindow *window)
{
	GList *ret;
	GList *l;
	...
	ret = NULL;
	for (l = window->details->target_files; l != NULL; l = l->next) {
		ret = g_list_append (ret, anchor_file_get_mime_type (NAUTILUS_FILE (l->data)));
	}
...
static gboolean
start_spinner_callback (AnchorPropertiesWindow *window)
{
...
static void
schedule_start_spinner (AnchorPropertiesWindow *window)
{
...
static void
stop_spinner (AnchorPropertiesWindow *window)
{
...
static void
stop_deep_count_for_file (AnchorPropertiesWindow *window,
			  AnchorFile             *file)
{
...
static void
start_deep_count_for_file (AnchorPropertiesWindow *window,
			   AnchorFile             *file)
{
	...
	if (!g_list_find (window->details->deep_count_files, file)) {
		...
		anchor_file_recompute_deep_counts (file);
...
static void
properties_window_update (AnchorPropertiesWindow *window, 
			  GList *files)
{
	GList *l;
	GList *mime_list;
	GList *tmp;
	AnchorFile *changed_file;
	...
	for (tmp = files; tmp != NULL; tmp = tmp->next) {
		...
		if (changed_file && anchor_file_is_gone (changed_file)) {
...
update_files_callback (gpointer data)
{
	...
	if (window->details->original_files == NULL) {
		...
		anchor_file_list_free (window->details->changed_files);
...
static void
schedule_files_update (AnchorPropertiesWindow *window)
 {
...
file_list_attributes_identical (GList *file_list, const char *attribute_name)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (first_attr == NULL) {
			first_attr = anchor_file_get_string_attribute_with_default (file, attribute_name);
		} else {
			char *attr;
			attr = anchor_file_get_string_attribute_with_default (file, attribute_name);
				...
				const char *inconsistent_value)
{
	if (file_list_attributes_identical (file_list, attribute_name)) {
		...
		for (l = file_list; l != NULL; l = l->next) {
			...
			file = NAUTILUS_FILE (l->data);
			if (!anchor_file_is_gone (file)) {
				return anchor_file_get_string_attribute_with_default
					(file, 
					 attribute_name);
			}
		}
...
file_list_all_directories (GList *file_list)
{
	GList *l;
	for (l = file_list; l != NULL; l = l->next) {
		if (!anchor_file_is_directory (NAUTILUS_FILE (l->data))) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
value_field_update (AnchorPropertiesWindow *window, GtkLabel *label)
{
...
static GtkWidget*
attach_value_field_internal (AnchorPropertiesWindow *window,
			     GtkGrid *grid,
			     GtkWidget *sibling,
...
static GtkWidget*
attach_value_field (AnchorPropertiesWindow *window,
		    GtkGrid *grid,
		    GtkWidget *sibling,
		    const char *file_attribute_name,
		    const char *inconsistent_string,
...
static GtkWidget*
attach_ellipsizing_value_field (AnchorPropertiesWindow *window,
				GtkGrid *grid,
				GtkWidget *sibling,
		    		const char *file_attribute_name,
				const char *inconsistent_string,
...
static void
group_change_callback (AnchorFile *file,
		       GFile *res_loc,
		       GError *error,
		       AnchorPropertiesWindow *window)
{
	...
	anchor_report_error_setting_group (file, error, GTK_WINDOW (window));
...
static void
cancel_group_change_callback (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *group;
	...
	anchor_file_cancel (file, (AnchorFileOperationCallback) group_change_callback, window);
...
static gboolean
schedule_group_change_timeout (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *group;
	...
	anchor_file_set_group
		(file,  group,
		 (AnchorFileOperationCallback) group_change_callback, window);
...
static void
schedule_group_change (AnchorPropertiesWindow *window,
		       AnchorFile       *file,
		       const char         *group)
{
	g_assert (NAUTILUS_IS_PROPERTIES_WINDOW (window));
	...
	window->details->group_change_file = anchor_file_ref (file);
...
static void
unschedule_or_cancel_group_change (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *group;
...
static void
changed_group_callback (GtkComboBox *combo_box, AnchorFile *file)
{
	AnchorPropertiesWindow *window;
	char *group;
	char *cur_group;
	...
	cur_group = anchor_file_get_group_name (file);
...
static void
synch_groups_combo_box (GtkComboBox *combo_box, AnchorFile *file)
{
	GList *groups;
	GList *node;
	GtkTreeModel *model;
	...
	groups = anchor_file_get_settable_group_names (file);
	...
	current_group_name = anchor_file_get_group_name (file);
		        ...
		        AnchorFile *file)
{
	...
	g_signal_connect_data (combo_box, "changed",
			       ...
			       anchor_file_ref (file),
			       (GClosureNotify)anchor_file_unref, 0);

	return combo_box;
}	

static void
owner_change_callback (AnchorFile *file,
		       ...
		       AnchorPropertiesWindow *window)
{
	...
	anchor_report_error_setting_owner (file, error, GTK_WINDOW (window));
...
static void
cancel_owner_change_callback (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *owner;
	...
	anchor_file_cancel (file, (AnchorFileOperationCallback) owner_change_callback, window);
...
static gboolean
schedule_owner_change_timeout (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *owner;
	...
	anchor_file_set_owner
		(file,  owner,
		 (AnchorFileOperationCallback) owner_change_callback, window);
...
static void
schedule_owner_change (AnchorPropertiesWindow *window,
		       AnchorFile       *file,
		       const char         *owner)
{
	g_assert (NAUTILUS_IS_PROPERTIES_WINDOW (window));
	...
	window->details->owner_change_file = anchor_file_ref (file);
...
static void
unschedule_or_cancel_owner_change (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *owner;
...
static void
changed_owner_callback (GtkComboBox *combo_box, AnchorFile* file)
{
	AnchorPropertiesWindow *window;
	char *owner_text;
	char **name_array;
	...
	g_free (owner_text);
	cur_owner = anchor_file_get_owner_name (file);
...
static void
synch_user_menu (GtkComboBox *combo_box, AnchorFile *file)
{
	GList *users;
	GList *node;
	GtkTreeModel *model;
	...
	owner_name = anchor_file_get_string_attribute (file, "owner");
...
static gboolean
file_has_prefix (AnchorFile *file,
		 GList *prefix_candidates)
{
	...
	for (p = prefix_candidates; p != NULL; p = p->next) {
		...
		candidate_location = anchor_file_get_location (NAUTILUS_FILE (p->data));
...
static void
directory_contents_value_field_update (AnchorPropertiesWindow *window)
{
	AnchorRequestStatus file_status;
	char *text, *temp;
	...
	for (l = window->details->target_files; l; l = l->next) {
		...
		if (anchor_file_is_directory (file)) {
			file_status = anchor_file_get_deep_counts (file,
								     &directory_count,
								     &file_count,
								     &file_unreadable,
								     &file_size,
			...
			++total_count;
			total_size += anchor_file_get_size (file);
		}
	}
...
static void
schedule_directory_contents_update (AnchorPropertiesWindow *window)
{
	g_assert (NAUTILUS_IS_PROPERTIES_WINDOW (window));
...
static GtkLabel *
attach_directory_contents_value_field (AnchorPropertiesWindow *window,
				       GtkGrid *grid,
				       GtkWidget *sibling)
{
	GtkLabel *value_field;
...
static void
append_title_value_pair (AnchorPropertiesWindow *window,
			 GtkGrid *grid,
			 const char *title, 
 			 const char *file_attribute_name,
...
static void
append_title_and_ellipsizing_value (AnchorPropertiesWindow *window,
				    GtkGrid *grid,
				    const char *title,
				    const char *file_attribute_name,
				    const char *inconsistent_state,
...
static void
append_directory_contents_fields (AnchorPropertiesWindow *window,
				  GtkGrid *grid)
{
...
static gboolean
is_merged_trash_directory (AnchorFile *file) 
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_computer_directory (AnchorFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_root_directory (AnchorFile *file)
{
	GFile *location;
	gboolean result;

	location = anchor_file_get_location (file);
	result = anchor_is_root_directory (location);
	g_object_unref (location);
...
static gboolean
is_network_directory (AnchorFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_burn_directory (AnchorFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_recent_directory (AnchorFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
should_show_custom_icon_buttons (AnchorPropertiesWindow *window) 
{
	if (is_multi_file_window (window)) {
		return FALSE;
	}
...
static gboolean
should_show_file_type (AnchorPropertiesWindow *window) 
{
	if (!is_multi_file_window (window) 
...
static gboolean
should_show_location_info (AnchorPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (is_merged_trash_directory (get_target_file (window)) ||
...
static gboolean
should_show_accessed_date (AnchorPropertiesWindow *window) 
{
...
static gboolean
should_show_link_target (AnchorPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && anchor_file_is_symbolic_link (get_target_file (window))) {
		return TRUE;
	}
...
static gboolean
location_show_original (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	...
	return (file != NULL && !anchor_file_is_in_recent (file));
}

static gboolean
should_show_free_space (AnchorPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && (is_merged_trash_directory (get_target_file (window)) ||
		is_computer_directory (get_target_file (window)) ||
...
static gboolean
should_show_volume_info (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
...
static gboolean
should_show_volume_usage (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
...
static GtkWidget* 
create_pie_widget (AnchorPropertiesWindow *window)
{
	AnchorFile		*file;
	GtkGrid                 *grid;
...
static GtkWidget*
create_volume_usage_widget (AnchorPropertiesWindow *window)
{
...
static void
create_basic_page (AnchorPropertiesWindow *window)
{
	GtkGrid *grid;
	...
	hbox = create_page_with_hbox (window->details->notebook, _("Basic"),
				      "help:gnome-help/anchor-file-properties-basic");
	...
	if (NAUTILUS_IS_ENTRY (window->details->name_field)) {
		anchor_entry_select_all (NAUTILUS_ENTRY (window->details->name_field));
		gtk_widget_grab_focus (GTK_WIDGET (window->details->name_field));
	}

	if (anchor_desktop_item_properties_should_show (window->details->target_files)) {
		...
		gtk_size_group_add_widget (label_size_group,
					   GTK_WIDGET (window->details->name_label));
		box = anchor_desktop_item_properties_make_box (label_size_group,
								 window->details->target_files);
	...
	if (is_multi_file_window (window) ||
	    anchor_file_is_directory (get_target_file (window))) {
...
static gboolean 
files_has_directory (AnchorPropertiesWindow *window)
{
	GList *l;
...
static gboolean
files_has_changable_permissions_directory (AnchorPropertiesWindow *window)
{
	GList *l;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (anchor_file_is_directory (file) &&
		    ...
		    anchor_file_can_set_permissions (file)) {
			changable = TRUE;
		} else {
			changable = FALSE;
			break;
...
static gboolean
files_has_file (AnchorPropertiesWindow *window)
{
	GList *l;
...
static void
start_long_operation (AnchorPropertiesWindow *window)
{
...
static void
end_long_operation (AnchorPropertiesWindow *window)
{
...
static void
permission_change_callback (AnchorFile *file,
			    GFile *res_loc,
			    GError *error,
			    gpointer callback_data)
{
	...
	/* Report the error if it's an error. */
	anchor_report_error_setting_permissions (file, error, NULL);

	g_object_unref (window);
}

static void
update_permissions (AnchorPropertiesWindow *window,
		    ...
		    gboolean use_original)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (!apply_to_both_folder_and_dir &&
		    ((anchor_file_is_directory (file) && !is_folder) ||
		     (!anchor_file_is_directory (file) && is_folder))) {
			continue;
		}

		permissions = anchor_file_get_permissions (file);
		if (use_original) {
			gpointer ptr;
...
static gboolean
initial_permission_state_consistent (AnchorPropertiesWindow *window,
				     guint32 mask,
				     gboolean is_folder,
...
static void
permission_button_update (AnchorPropertiesWindow *window,
			  GtkToggleButton *button)
{
	GList *l;
	gboolean all_set;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		file_permissions = anchor_file_get_permissions (file);
...
static void
set_up_permissions_checkbox (AnchorPropertiesWindow *window,
...
static GtkWidget *
add_execute_checkbox_with_label (AnchorPropertiesWindow *window,
				 GtkGrid *grid,
				 GtkWidget *sibling,
				 const char *label,
...
static void
permission_combo_changed (GtkWidget *combo, AnchorPropertiesWindow *window)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
permission_combo_update (AnchorPropertiesWindow *window,
			 GtkComboBox *combo)
{
	PermissionType type;
...
static void
add_permissions_combo_box (AnchorPropertiesWindow *window,
			   GtkGrid *grid,
			   PermissionType type,
			   gboolean is_folder,
...
static void
create_simple_permissions (AnchorPropertiesWindow *window, GtkGrid *page_grid)
{
	...
	if (!is_multi_file_window (window) && anchor_file_can_set_owner (get_target_file (window))) {
	...
	if (!is_multi_file_window (window) && anchor_file_can_set_group (get_target_file (window))) {
			       ...
			       AnchorPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (anchor_file_is_directory (file) &&
			...
			start_long_operation (window);
			g_object_ref (window);
			anchor_file_set_permissions_recursive (uri,
								 file_permission,
...
static void
create_permissions_page (AnchorPropertiesWindow *window)
{
	...
	vbox = create_page_with_vbox (window->details->notebook,
				      _("Permissions"),
				      "help:gnome-help/anchor-file-properties-permissions");
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (!is_multi_file_window (window)) {
			file_name = anchor_file_get_display_name (get_target_file (window));
...
static void
append_extension_pages (AnchorPropertiesWindow *window)
{
	GList *providers;
	GList *p;
	
 	providers = anchor_module_get_extensions_for_type (ANCHOR_TYPE_PROPERTY_PAGE_PROVIDER);
	
	for (p = providers; p != NULL; p = p->next) {
		AnchorPropertyPageProvider *provider;
		GList *pages;
		GList *l;

		provider = NAUTILUS_PROPERTY_PAGE_PROVIDER (p->data);
		
		pages = anchor_property_page_provider_get_pages 
			(provider, window->details->original_files);
		
		for (l = pages; l != NULL; l = l->next) {
			AnchorPropertyPage *page;
...
static gboolean
should_show_permissions (AnchorPropertiesWindow *window) 
{
	AnchorFile *file;
...
get_pending_key (GList *file_list)
{
	...
	uris = NULL;
	for (l = file_list; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, anchor_file_get_uri (NAUTILUS_FILE (l->data)));
	}
		  ...
		  const char *startup_id)
{
	...
	data->original_files = anchor_file_list_copy (original_files);
	data->target_files = anchor_file_list_copy (target_files);
...
{
	anchor_file_list_free (data->original_files);
	anchor_file_list_free (data->target_files);
...
static void
file_changed_callback (AnchorFile *file, gpointer user_data)
{
	AnchorPropertiesWindow *window = NAUTILUS_PROPERTIES_WINDOW (user_data);
...
static gboolean
is_a_special_file (AnchorFile *file)
{
	if (file == NULL ||
...
static gboolean
should_show_open_with (AnchorPropertiesWindow *window)
{
	AnchorFile *file;
	char *mime_type;
	char *extension;
	...
	if (is_multi_file_window (window)) {
		...
		for (l = window->details->target_files; l; l = l->next) {
			file = NAUTILUS_FILE (l->data);
			if (anchor_file_is_directory (file) || is_a_special_file (file)) {
				return FALSE;
			}
		}
	...
	extension = anchor_file_get_extension (file);
...
static void
create_open_with_page (AnchorPropertiesWindow *window)
{
	GtkWidget *vbox;
	char *mime_type;
	GList *files = NULL;
	...
	mime_type = anchor_file_get_mime_type (target_file);
	...
	vbox = anchor_mime_application_chooser_new (files, mime_type);
...
static AnchorPropertiesWindow *
create_properties_window (StartupData *startup_data)
{
	AnchorPropertiesWindow *window;
	GList *l;

	window = NAUTILUS_PROPERTIES_WINDOW (gtk_widget_new (ANCHOR_TYPE_PROPERTIES_WINDOW,
							     "use-header-bar", TRUE,
							     NULL));

	window->details->original_files = anchor_file_list_copy (startup_data->original_files);
	
	window->details->target_files = anchor_file_list_copy (startup_data->target_files);

	gtk_window_set_wmclass (GTK_WINDOW (window), "file_properties", "Anchor");
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		anchor_file_monitor_add (file, &window->details->target_files, attributes);
	}	
...
static void
add_window (AnchorPropertiesWindow *window)
{
	if (!is_multi_file_window (window)) {
...
static void
remove_window (AnchorPropertiesWindow *window)
{
	gpointer key;
...
static void
is_directory_ready_callback (AnchorFile *file,
			     gpointer data)
{
	...
	if (g_hash_table_size (startup_data->pending_files) == 0) {
		AnchorPropertiesWindow *new_window;
...
void
anchor_properties_window_present (GList       *original_files,
				    GtkWidget   *parent_widget,
				    const gchar *startup_id) 
{
	...
	anchor_file_list_free (target_files);
	g_free(pending_key);
...
real_destroy (GtkWidget *object)
{
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		anchor_file_monitor_remove (NAUTILUS_FILE (l->data), &window->details->original_files);
	}
	anchor_file_list_free (window->details->original_files);
	window->details->original_files = NULL;
	
	for (l = window->details->target_files; l != NULL; l = l->next) {
		anchor_file_monitor_remove (NAUTILUS_FILE (l->data), &window->details->target_files);
	}
	anchor_file_list_free (window->details->target_files);
	...
	GTK_WIDGET_CLASS (anchor_properties_window_parent_class)->destroy (object);
}
...
real_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_properties_window_parent_class)->finalize (object);
}
...
static void
set_icon (const char* icon_uri, AnchorPropertiesWindow *properties_window)
{
	AnchorFile *file;
	char *file_uri;
	char *icon_path;
	...
	if (icon_path != NULL) {
		...
		for (l = properties_window->details->original_files; l != NULL; l = l->next) {
			...
			if (anchor_file_is_mime_type (file, "application/x-desktop")) {
				if (anchor_link_local_set_icon (file_uri, icon_path)) {
				...
				anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_CUSTOM_ICON, NULL, real_icon_uri);
				anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_ICON_SCALE, NULL, NULL);
			      ...
			      AnchorPropertiesWindow *window)
{
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		file = NAUTILUS_FILE (l->data);
		image_path = anchor_file_get_metadata (file, NAUTILUS_METADATA_KEY_CUSTOM_ICON, NULL);
...
static void
anchor_properties_window_class_init (AnchorPropertiesWindowClass *class)
{
	...
	g_type_class_add_private (class, sizeof (AnchorPropertiesWindowDetails));
}

static void
anchor_properties_window_init (AnchorPropertiesWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, ANCHOR_TYPE_PROPERTIES_WINDOW,
						       AnchorPropertiesWindowDetails);
}
