...
static int  anchor_notebook_insert_page	 (GtkNotebook *notebook,
					  GtkWidget *child,
					  GtkWidget *tab_label,
					  GtkWidget *menu_label,
					  int position);
static void anchor_notebook_remove	 (GtkContainer *container,
...
G_DEFINE_TYPE (AnchorNotebook, anchor_notebook, GTK_TYPE_NOTEBOOK);

static void
anchor_notebook_class_init (AnchorNotebookClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);
	GtkNotebookClass *notebook_class = GTK_NOTEBOOK_CLASS (klass);

	container_class->remove = anchor_notebook_remove;

	notebook_class->insert_page = anchor_notebook_insert_page;

	signals[TAB_CLOSE_REQUEST] =
		g_signal_new ("tab-close-request",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AnchorNotebookClass, tab_close_request),
			      NULL, NULL,
...
find_notebook_at_pointer (gint abs_x, gint abs_y)
{
	...
	/* toplevel should be an AnchorWindow */
...
static gboolean
is_in_notebook_window (AnchorNotebook *notebook,
		       gint abs_x, gint abs_y)
{
	AnchorNotebook *nb_at_pointer;
...
static gint
find_tab_num_at_pos (AnchorNotebook *notebook, gint abs_x, gint abs_y)
{
...
static gboolean
button_press_cb (AnchorNotebook *notebook,
...
static void
anchor_notebook_init (AnchorNotebook *notebook)
{
...
void
anchor_notebook_sync_loading (AnchorNotebook *notebook,
				AnchorWindowSlot *slot)
{
	...
	allow_stop = anchor_window_slot_get_allow_stop (slot);
...
void
anchor_notebook_sync_tab_label (AnchorNotebook *notebook,
				  AnchorWindowSlot *slot)
{
	...
	gtk_label_set_text (GTK_LABEL (label), anchor_window_slot_get_title (slot));
	...
	if (location != NULL) {
		...
		title_name = anchor_window_slot_get_title (slot);
			 ...
			 AnchorWindowSlot *slot)
{
	...
	notebook = gtk_widget_get_ancestor (GTK_WIDGET (slot), ANCHOR_TYPE_NOTEBOOK);
	if (notebook != NULL) {
...
static GtkWidget *
build_tab_label (AnchorNotebook *nb, AnchorWindowSlot *slot)
{
	...
	gtk_widget_set_name (close_button, "anchor-tab-close-button");
	...
	g_object_set_data (G_OBJECT (box), "anchor-notebook-tab", GINT_TO_POINTER (1));
	anchor_drag_slot_proxy_init (box, NULL, slot);
...
static int
anchor_notebook_insert_page (GtkNotebook *gnotebook,
			       GtkWidget *tab_widget,
			       GtkWidget *tab_label,
			       GtkWidget *menu_label,
			       int position)
{
	...
	position = GTK_NOTEBOOK_CLASS (anchor_notebook_parent_class)->insert_page (gnotebook,
										     tab_widget,
										     tab_label,
										     menu_label,
										     position);
...
int
anchor_notebook_add_tab (AnchorNotebook *notebook,
			   AnchorWindowSlot *slot,
			   int position,
			   gboolean jump_to)
{
	...
	anchor_notebook_sync_tab_label (notebook, slot);
	anchor_notebook_sync_loading (notebook, slot);
...
static void
anchor_notebook_remove (GtkContainer *container,
			  GtkWidget *tab_widget)
{
	GtkNotebook *gnotebook = GTK_NOTEBOOK (container);
	GTK_CONTAINER_CLASS (anchor_notebook_parent_class)->remove (container, tab_widget);
...
void
anchor_notebook_reorder_current_child_relative (AnchorNotebook *notebook,
						  int offset)
{
	...
	if (!anchor_notebook_can_reorder_current_child_relative (notebook, offset)) {
		return;
	}
...
static gboolean
anchor_notebook_is_valid_relative_position (AnchorNotebook *notebook,
					      int offset)
{
	GtkNotebook *gnotebook;
	int page;
...
gboolean
anchor_notebook_can_reorder_current_child_relative (AnchorNotebook *notebook,
						      int offset)
{
	g_return_val_if_fail (NAUTILUS_IS_NOTEBOOK (notebook), FALSE);

	return anchor_notebook_is_valid_relative_position (notebook, offset);
}

void
anchor_notebook_next_page (AnchorNotebook *notebook)
{
...
void
anchor_notebook_prev_page (AnchorNotebook *notebook)
{
	gint current_page;
