...
#define NAUTILUS_X_CONTENT_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), ANCHOR_TYPE_X_CONTENT_BAR, AnchorXContentBarPrivate))

struct AnchorXContentBarPrivate
{
	GtkWidget *label;
...
G_DEFINE_TYPE (AnchorXContentBar, anchor_x_content_bar, GTK_TYPE_INFO_BAR)
			 ...
			 gpointer user_data)
{
	...
	AnchorXContentBar *bar = user_data;
	...
	if (default_app != NULL) {
		anchor_launch_application_for_mount (default_app, bar->priv->mount,
...
static void
anchor_x_content_bar_set_x_content_types (AnchorXContentBar *bar, const char **x_content_types)
{
	char *message = NULL;
	guint num_types;
	guint n;
...
static void
anchor_x_content_bar_set_mount (AnchorXContentBar *bar, GMount *mount)
{
...
static void
anchor_x_content_bar_set_property (GObject      *object,
				     guint         prop_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
	AnchorXContentBar *bar;

	bar = NAUTILUS_X_CONTENT_BAR (object);

	switch (prop_id) {
	case PROP_MOUNT:
		anchor_x_content_bar_set_mount (bar, G_MOUNT (g_value_get_object (value)));
		break;
	case PROP_X_CONTENT_TYPES:
		anchor_x_content_bar_set_x_content_types (bar, g_value_get_boxed (value));
		break;
	default:
...
static void
anchor_x_content_bar_get_property (GObject    *object,
				     guint       prop_id,
				     GValue     *value,
				     GParamSpec *pspec)
{
...
static void
anchor_x_content_bar_finalize (GObject *object)
{
	AnchorXContentBar *bar = NAUTILUS_X_CONTENT_BAR (object);

	g_strfreev (bar->priv->x_content_types);
	if (bar->priv->mount != NULL)
		g_object_unref (bar->priv->mount);

        G_OBJECT_CLASS (anchor_x_content_bar_parent_class)->finalize (object);
}

static void
anchor_x_content_bar_class_init (AnchorXContentBarClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);
	object_class->get_property = anchor_x_content_bar_get_property;
	object_class->set_property = anchor_x_content_bar_set_property;
	object_class->finalize = anchor_x_content_bar_finalize;

	g_type_class_add_private (klass, sizeof (AnchorXContentBarPrivate));
...
static void
anchor_x_content_bar_init (AnchorXContentBar *bar)
{
...
GtkWidget *
anchor_x_content_bar_new (GMount *mount,
			    const char **x_content_types)
{
	return g_object_new (ANCHOR_TYPE_X_CONTENT_BAR,
