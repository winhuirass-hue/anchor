...
} AnchorQueryEditorRowType;

typedef struct {
	AnchorQueryEditorRowType type;
	AnchorQueryEditor *editor;
	GtkWidget *toolbar;
	GtkWidget *hbox;
	GtkWidget *combo;

	GtkWidget *type_widget;
} AnchorQueryEditorRow;


typedef struct {
	const char *name;
	GtkWidget * (*create_widgets)      (AnchorQueryEditorRow *row);
	void        (*add_to_query)        (AnchorQueryEditorRow *row,
					    AnchorQuery          *query);
	void        (*add_rows_from_query) (AnchorQueryEditor *editor,
					    AnchorQuery *query);
} AnchorQueryEditorRowOps;

struct AnchorQueryEditorDetails {
	GtkWidget *entry;
...
static void entry_activate_cb (GtkWidget *entry, AnchorQueryEditor *editor);
static void entry_changed_cb  (GtkWidget *entry, AnchorQueryEditor *editor);
static void anchor_query_editor_changed_force (AnchorQueryEditor *editor,
						 gboolean             force);
static void anchor_query_editor_changed (AnchorQueryEditor *editor);
static AnchorQueryEditorRow * anchor_query_editor_add_row (AnchorQueryEditor *editor,
							       AnchorQueryEditorRowType type);

static GtkWidget *type_row_create_widgets      (AnchorQueryEditorRow *row);
static void       type_row_add_to_query        (AnchorQueryEditorRow *row,
					        AnchorQuery          *query);
static void       type_add_rows_from_query     (AnchorQueryEditor    *editor,
					        AnchorQuery          *query);



static AnchorQueryEditorRowOps row_type[] = {
	{ N_("File Type"),
...
G_DEFINE_TYPE (AnchorQueryEditor, anchor_query_editor, GTK_TYPE_BOX);

gboolean
anchor_query_editor_handle_event (AnchorQueryEditor *editor,
				    GdkEventKey         *event)
{
	GtkWidget *toplevel;
	GtkWidget *old_focus;
...
static void
row_destroy (AnchorQueryEditorRow *row)
{
...
static void
anchor_query_editor_dispose (GObject *object)
{
	AnchorQueryEditor *editor;
	...
	G_OBJECT_CLASS (anchor_query_editor_parent_class)->dispose (object);
}

static void
anchor_query_editor_grab_focus (GtkWidget *widget)
{
	AnchorQueryEditor *editor = NAUTILUS_QUERY_EDITOR (widget);
...
static void
anchor_query_editor_class_init (AnchorQueryEditorClass *class)
{
	...
	gobject_class = G_OBJECT_CLASS (class);
        gobject_class->dispose = anchor_query_editor_dispose;

	widget_class = GTK_WIDGET_CLASS (class);
	widget_class->grab_focus = anchor_query_editor_grab_focus;

	signals[CHANGED] =
		g_signal_new ("changed",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorQueryEditorClass, changed),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ANCHOR_TYPE_QUERY, G_TYPE_BOOLEAN);

	signals[CANCEL] =
		g_signal_new ("cancel",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		              G_STRUCT_OFFSET (AnchorQueryEditorClass, cancel),
		              NULL, NULL,
	...
	signals[ACTIVATED] =
		g_signal_new ("activated",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		              G_STRUCT_OFFSET (AnchorQueryEditorClass, activated),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	...
	g_type_class_add_private (class, sizeof (AnchorQueryEditorDetails));
}

GFile *
anchor_query_editor_get_location (AnchorQueryEditor *editor)
{
	GFile *file = NULL;
...
static void
entry_activate_cb (GtkWidget *entry, AnchorQueryEditor *editor)
{
...
typing_timeout_cb (gpointer user_data)
{
	...
	anchor_query_editor_changed (editor);
...
static void
entry_changed_cb (GtkWidget *entry, AnchorQueryEditor *editor)
{
...
static void
type_add_custom_type (AnchorQueryEditorRow *row,
...
static void
type_combo_changed (GtkComboBox *combo_box, AnchorQueryEditorRow *row)
{
	GtkTreeIter iter;
	gboolean other;
	GtkTreeModel *model;
	...
	anchor_query_editor_changed (row->editor);
}

static GtkWidget *
type_row_create_widgets (AnchorQueryEditorRow *row)
{
	GtkWidget *combo;
...
static void
type_row_add_to_query (AnchorQueryEditorRow *row,
		       AnchorQuery          *query)
{
	GtkTreeIter iter;
	char **mimetypes;
	...
	if (mimetypes != NULL) {
		while (*mimetypes != NULL) {
			anchor_query_add_mime_type (query, *mimetypes);
			mimetypes++;
		}
	}
	if (mimetype) {
		anchor_query_add_mime_type (query, mimetype);
		g_free (mimetype);
	}
}
...
static void
type_add_rows_from_query (AnchorQueryEditor    *editor,
			  AnchorQuery          *query)
{
	GList *mime_types;
	char *mime_type;
	const char *desc;
	AnchorQueryEditorRow *row;
	GtkTreeIter iter;
	int i;
	GtkTreeModel *model;
	GList *l;

	mime_types = anchor_query_get_mime_types (query);
	...
	for (i = 0; i < G_N_ELEMENTS (mime_type_groups); i++) {
		if (all_group_types_in_list (mime_type_groups[i].mimetypes,
			...
			row = anchor_query_editor_add_row (editor,
							     NAUTILUS_QUERY_EDITOR_ROW_TYPE);
...
static AnchorQueryEditorRowType
get_next_free_type (AnchorQueryEditor *editor)
{
	AnchorQueryEditorRow *row;
	AnchorQueryEditorRowType type;
	gboolean found;
...
static void
remove_row_cb (GtkButton *clicked_button, AnchorQueryEditorRow *row)
{
	AnchorQueryEditor *editor;
...
static void
create_type_widgets (AnchorQueryEditorRow *row)
{
	row->type_widget = row_type[row->type].create_widgets (row);
}

static void
row_type_combo_changed_cb (GtkComboBox *combo_box, AnchorQueryEditorRow *row)
{
	AnchorQueryEditorRowType type;
...
static AnchorQueryEditorRow *
anchor_query_editor_add_row (AnchorQueryEditor *editor,
			       AnchorQueryEditorRowType type)
{
	...
	row = g_new0 (AnchorQueryEditorRow, 1);
	row->editor = editor;
	row->type = type;
...
static void
add_new_row_cb (GtkButton *clicked_button, AnchorQueryEditor *editor)
{
	anchor_query_editor_add_row (editor, get_next_free_type (editor));
	anchor_query_editor_changed (editor);
}

static void
anchor_query_editor_init (AnchorQueryEditor *editor)
{
	editor->details = G_TYPE_INSTANCE_GET_PRIVATE (editor, ANCHOR_TYPE_QUERY_EDITOR,
						       AnchorQueryEditorDetails);
			  ...
			  AnchorQueryEditor *editor)
{
	if (event->keyval == GDK_KEY_Down) {
		anchor_window_grab_focus (NAUTILUS_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (widget))));
	}
	return FALSE;
}

static void
setup_widgets (AnchorQueryEditor *editor)
{
	GtkToolItem *item;
...
static void
anchor_query_editor_changed_force (AnchorQueryEditor *editor, gboolean force_reload)
{
	AnchorQuery *query;
	...
	query = anchor_query_editor_get_query (editor);
...
static void
anchor_query_editor_changed (AnchorQueryEditor *editor)
{
	anchor_query_editor_changed_force (editor, TRUE);
}

static void
add_location_to_query (AnchorQueryEditor *editor,
		       AnchorQuery       *query)
{
	char *uri;

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (editor->details->search_all_button))) {
		uri = anchor_get_home_directory_uri ();
	} else {
...
AnchorQuery *
anchor_query_editor_get_query (AnchorQueryEditor *editor)
{
	...
	anchor_query_set_text (query, query_text);
...
GtkWidget *
anchor_query_editor_new (void)
{
	GtkWidget *editor;

	editor = g_object_new (ANCHOR_TYPE_QUERY_EDITOR, NULL);
...
static void
update_location (AnchorQueryEditor *editor)
{
	AnchorFile *file;
	GtkWidget *label;

	file = anchor_file_get_by_uri (editor->details->current_uri);

	if (file != NULL) {
		char *name;
		if (anchor_file_is_home (file)) {
...
void
anchor_query_editor_set_location (AnchorQueryEditor *editor,
				    GFile               *location)
{
...
static void
update_rows (AnchorQueryEditor *editor,
	     AnchorQuery       *query)
{
	AnchorQueryEditorRowType type;
...
void
anchor_query_editor_set_query (AnchorQueryEditor	*editor,
				 AnchorQuery		*query)
{
	char *text = NULL;
	...
	if (query != NULL) {
		...
		editor->details->current_uri = anchor_query_get_location (query);
		update_location (editor);
	}
