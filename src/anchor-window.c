...
/* anchor-window.c: Implementation of the main window object */
...
#include "anchor-bookmarks-window.h"
...
G_DEFINE_TYPE (AnchorWindow, anchor_window, GTK_TYPE_APPLICATION_WINDOW);
...
void
anchor_window_go_to (AnchorWindow *window, GFile *location)
{
	g_return_if_fail (NAUTILUS_IS_WINDOW (window));

	anchor_window_slot_open_location (anchor_window_get_active_slot (window),
					    location, 0);
}

void
anchor_window_go_to_full (AnchorWindow *window,
			    GFile          *location,
			    ...
			    gpointer        user_data)
{
	...
	anchor_window_slot_open_location_full (anchor_window_get_active_slot (window),
...
bookmark_list_get_uri_index (GList *list, GFile *location)
{
	...
	for (i = 0, l = list; l != NULL; i++, l = l->next) {
		...
		tmp = anchor_bookmark_get_location (bookmark);
...
static void
unset_focus_widget (AnchorWindow *window)
{
...
static void
remember_focus_widget (AnchorWindow *window)
{
...
void
anchor_window_grab_focus (AnchorWindow *window)
{
	AnchorWindowSlot *slot;
	AnchorView *view;

	slot = anchor_window_get_active_slot (window);
	view = anchor_window_slot_get_view (slot);

	if (view) {
		anchor_view_grab_focus (view);
	}
}

static void
restore_focus_widget (AnchorWindow *window)
{
	if (window->details->last_focus_widget != NULL) {
		if (NAUTILUS_IS_VIEW (window->details->last_focus_widget)) {
			anchor_view_grab_focus (NAUTILUS_VIEW (window->details->last_focus_widget));
		} else {
...
location_entry_cancel_callback (GtkWidget      *widget,
				AnchorWindow *window)
{
	anchor_toolbar_set_show_location_entry (NAUTILUS_TOOLBAR (window->details->toolbar), FALSE);
					  ...
					  AnchorWindow *window)
{
	...
	anchor_window_slot_open_location (window->details->active_slot, location, 0);
}
			 ...
			 AnchorWindow *window)
{
	...
	anchor_window_set_active_slot (anchor_window_slot_get_window (slot),
					 slot);
}

static void
close_slot (AnchorWindow     *window,
	    AnchorWindowSlot *slot,
...
AnchorWindowSlot *
anchor_window_open_slot (AnchorWindow             *window,
			   AnchorWindowOpenSlotFlags flags)
{
	...
	slot = anchor_window_slot_new (window);

	g_signal_handlers_block_by_func (window->details->notebook,
					 G_CALLBACK (notebook_switch_page_cb),
					 window);
	anchor_notebook_add_tab (NAUTILUS_NOTEBOOK (window->details->notebook),
				   slot,
...
void
anchor_window_new_tab (AnchorWindow *window)
{
	AnchorWindowSlot *current_slot;
	AnchorWindowSlot *new_slot;
	AnchorWindowOpenFlags flags;
	GFile *location;
	int new_slot_position;
	char *scheme;

	current_slot = anchor_window_get_active_slot (window);
	location = anchor_window_slot_get_location (current_slot);
	...
	if (location != NULL) {
		...
		if (strcmp (scheme, "x-anchor-search") == 0) {
		...
		new_slot = anchor_window_open_slot (window, flags);
		anchor_window_set_active_slot (window, new_slot);
		anchor_window_slot_open_location (new_slot, location, 0);
		g_object_unref (location);
	}
}

static void
update_cursor (AnchorWindow *window)
{
	AnchorWindowSlot *slot;
	GdkCursor *cursor;
	...
	if (anchor_window_slot_get_allow_stop (slot)) {
...
void
anchor_window_sync_allow_stop (AnchorWindow *window,
				 AnchorWindowSlot *slot)
{
	...
	stop_action = gtk_action_group_get_action (anchor_window_get_main_action_group (window),
						   NAUTILUS_ACTION_STOP);
	reload_action = gtk_action_group_get_action (anchor_window_get_main_action_group (window),
						     NAUTILUS_ACTION_RELOAD);
	allow_stop = gtk_action_get_sensitive (stop_action);

	slot_allow_stop = anchor_window_slot_get_allow_stop (slot);
	slot_is_active = (slot == anchor_window_get_active_slot (window));
	...
	if (!slot_is_active ||
		...
		anchor_notebook_sync_loading (NAUTILUS_NOTEBOOK (window->details->notebook), slot);
	}
}

void
anchor_window_prompt_for_location (AnchorWindow *window,
				     GFile          *location)
{
	GtkWidget *entry;
	...
	anchor_location_entry_set_location (NAUTILUS_LOCATION_ENTRY (entry), location);
}
...
/* This must be called when construction of AnchorWindow is finished,
 * since it depends on the type of the argument, which isn't decided at
 * construction time.
 */
static void
anchor_window_set_initial_window_geometry (AnchorWindow *window)
{
	GdkScreen *screen;
	...
	show_sidebar = g_settings_get_boolean (anchor_window_state, NAUTILUS_WINDOW_STATE_START_WITH_SIDEBAR);
...
save_sidebar_width_cb (gpointer user_data)
{
	...
	g_settings_set_int (anchor_window_state,
...
static void
setup_side_pane_width (AnchorWindow *window)
{
	g_return_if_fail (window->details->sidebar != NULL);

	window->details->side_pane_width =
		g_settings_get_int (anchor_window_state,
				    NAUTILUS_WINDOW_STATE_SIDEBAR_WIDTH);
...
places_sidebar_open_location_cb (GtkPlacesSidebar	*sidebar,
				 ...
				 gpointer		 user_data)
{
	AnchorWindow *window = NAUTILUS_WINDOW (user_data);
	AnchorWindowOpenFlags flags;
	...
	anchor_window_slot_open_location (window->details->active_slot, location, flags);
}
...
{
	AnchorWindow *window = NAUTILUS_WINDOW (user_data);
	AnchorApplication *application = NAUTILUS_APPLICATION (g_application_get_default ());
	GtkWidget *dialog;
 
	dialog = anchor_application_connect_server (application, window);
...
build_selection_list_from_gfile_list (GList *gfile_list)
{
	...
	for (l = gfile_list; l; l = l->next) {
		GFile *file;
		AnchorDragSelectionItem *item;

		file = l->data;

		item = anchor_drag_selection_item_new ();
		item->uri = g_file_get_uri (file);
		item->file = anchor_file_get_existing (file);
					 ...
					 gpointer          user_data)
{
	...
	anchor_drag_default_drop_action_for_icons (context, uri, items, &action);

	anchor_drag_destroy_selection_list (items);
	g_free (uri);
...
places_sidebar_drag_action_ask_cb (GtkPlacesSidebar *sidebar,
				   ...
				   gpointer          user_data)
{
	return anchor_drag_drop_action_ask (GTK_WIDGET (sidebar), actions);
}
				     ...
				     gpointer          user_data)
{
	...
	anchor_file_operations_copy_move (source_uri_list, NULL, dest_uri, action, GTK_WIDGET (sidebar), NULL, NULL);
...
static void
window_loading_uri_cb (AnchorWindow *window,
		       char           *location,
		       gpointer        user_data)
{
		...
		gpointer     user_data)
{
	...
	anchor_file_operations_empty_trash (GTK_WIDGET (window));
}
	       ...
	       gpointer     user_data)
{
	...
	file = anchor_file_get (selected);
	g_object_unref (selected);

	list = g_list_append (NULL, file);
	anchor_properties_window_present (list, GTK_WIDGET (window), NULL);
	   ...
	   gpointer     user_data)
{
	...
	volume = g_object_get_data (G_OBJECT (item), "anchor-volume");
				  ...
				  gpointer          user_data)
{
	...
	if (selected_item) {
		...
		if (g_file_equal (trash, selected_item)) {
			...
			if (anchor_trash_monitor_is_empty ())
	...
	if (selected_volume) {
		if (should_show_format_command (selected_volume)) {
			...
			g_object_set_data_full (G_OBJECT (item), "anchor-volume",
...
static void
anchor_window_set_up_sidebar (AnchorWindow *window)
{
...
static void
anchor_window_tear_down_sidebar (AnchorWindow *window)
{
...
void
anchor_window_hide_sidebar (AnchorWindow *window)
{
	...
	anchor_window_tear_down_sidebar (window);
}

void
anchor_window_show_sidebar (AnchorWindow *window)
{
	...
	anchor_window_set_up_sidebar (window);
}

gboolean
anchor_window_disable_chrome_mapping (GValue *value,
					GVariant *variant,
					gpointer user_data)
{
	AnchorWindow *window = user_data;
...
static inline AnchorWindowSlot *
get_first_inactive_slot (AnchorWindow *window)
{
	GList *l;
	AnchorWindowSlot *slot;
...
void
anchor_window_slot_close (AnchorWindow     *window,
			    AnchorWindowSlot *slot)
{
	AnchorWindowSlot *next_slot;
	...
	if (window->details->active_slot == slot) {
		...
		anchor_window_set_active_slot (window, next_slot);
	}
...
static void
anchor_window_sync_bookmarks (AnchorWindow *window)
{
	gboolean can_bookmark = FALSE;
	AnchorWindowSlot *slot;
	AnchorBookmarkList *bookmarks;
	GtkAction *action;
	GFile *location;
	...
	if (location != NULL) {
		bookmarks = anchor_application_get_bookmarks
			(NAUTILUS_APPLICATION (gtk_window_get_application (GTK_WINDOW (window))));
		can_bookmark = anchor_bookmark_list_can_bookmark_location (bookmarks, location);
	}

	action = gtk_action_group_get_action (anchor_window_get_main_action_group (window),
					      NAUTILUS_ACTION_ADD_BOOKMARK);
	gtk_action_set_sensitive (action, can_bookmark);
}

void
anchor_window_sync_location_widgets (AnchorWindow *window)
{
	AnchorWindowSlot *slot, *active_slot;
	GFile *location;
	...
	if (location != NULL) {
		...
		location_entry = anchor_toolbar_get_location_entry (NAUTILUS_TOOLBAR (window->details->toolbar));
		if (location_entry != NULL && GTK_IS_WIDGET (location_entry)) {
			anchor_location_entry_set_location (NAUTILUS_LOCATION_ENTRY (location_entry), location);
		}

		path_bar = anchor_toolbar_get_path_bar (NAUTILUS_TOOLBAR (window->details->toolbar));
		anchor_path_bar_set_path (NAUTILUS_PATH_BAR (path_bar), location);
	}

	anchor_window_sync_up_button (window);

	/* Check if the back and forward buttons need enabling or disabling. */
	active_slot = anchor_window_get_active_slot (window);
	action_group = anchor_window_get_main_action_group (window);

	action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_BACK);
	gtk_action_set_sensitive (action, anchor_window_slot_get_back_history (active_slot) != NULL);

	action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_FORWARD);
	gtk_action_set_sensitive (action, anchor_window_slot_get_forward_history (active_slot) != NULL);

	anchor_window_sync_bookmarks (window);
}

GtkWidget *
anchor_window_ensure_location_entry (AnchorWindow *window)
{
	GtkWidget *location_entry;

	remember_focus_widget (window);

	anchor_toolbar_set_show_location_entry (NAUTILUS_TOOLBAR (window->details->toolbar), TRUE);

	location_entry = anchor_toolbar_get_location_entry (NAUTILUS_TOOLBAR (window->details->toolbar));
	anchor_location_entry_focus (NAUTILUS_LOCATION_ENTRY (location_entry));
				    ...
				    AnchorWindow *window)
{
	...
	i = bookmark_list_get_uri_index (anchor_window_slot_get_back_history (slot), location);
	if (i >= 0) {
		anchor_window_back_or_forward (window, TRUE, i, 0);
	} else {
...
static gboolean
path_bar_path_event_callback (AnchorPathBar *path_bar,
			      GFile           *location,
			      ...
			      AnchorWindow  *window)
{
	...
	if (event->button == 3) {
		...
		if (view != NULL) {
			...
			anchor_view_pop_up_location_context_menu (view, event, uri);
			g_free (uri);
		}
...
static void
notebook_popup_menu_show (AnchorWindow *window,
			  GdkEventButton *event)
{
	GtkWidget *popup;
	GtkWidget *item;
	...
	AnchorNotebook *notebook;

	notebook = NAUTILUS_NOTEBOOK (window->details->notebook);

	can_move_left = anchor_notebook_can_reorder_current_child_relative (notebook, -1);
	can_move_right = anchor_notebook_can_reorder_current_child_relative (notebook, 1);
...
static void
notebook_tab_close_requested (AnchorNotebook   *notebook,
			      AnchorWindowSlot *slot,
			      AnchorWindow     *window)
{
	anchor_window_slot_close (window, slot);
...
static GtkWidget *
create_toolbar (AnchorWindow *window)
{
	...
	/* build the toolbar */
	toolbar = anchor_toolbar_new (NAUTILUS_WINDOW (window));
	...
	path_bar = anchor_toolbar_get_path_bar (NAUTILUS_TOOLBAR (toolbar));
	...
	location_entry = anchor_toolbar_get_location_entry (NAUTILUS_TOOLBAR (toolbar));
	gtk_size_group_add_widget (header_size_group, location_entry);

	anchor_clipboard_set_up_editable (GTK_EDITABLE (location_entry),
					    anchor_window_get_ui_manager (NAUTILUS_WINDOW (window)),
					    TRUE);
...
static void
notebook_num_pages_changed (AnchorWindow *window)
{
	AnchorView *view;
	...
	view = anchor_window_slot_get_current_view (active_slot);
	if (view == NULL) {
		return;
	}
...
{
	AnchorWindow *window = user_data;
	AnchorWindowSlot *slot = NAUTILUS_WINDOW_SLOT (page), *next_slot;
	gboolean dnd_slot;
...
{
	AnchorWindow *window = user_data;
	AnchorWindowSlot *slot = NAUTILUS_WINDOW_SLOT (page);
	AnchorWindowSlot *dummy_slot;
	gboolean dnd_slot;

	/* It's too early here to call notebook_num_pages_changed(),
	 * since the view might not be associagted to the slot yet.
	 * Defer the call to anchor_window_connect_content_view().
	 */
	...
	anchor_window_slot_set_window (slot, window);
	window->details->slots = g_list_append (window->details->slots, slot);
	g_signal_emit (window, signals[SLOT_ADDED], 0, slot);

	anchor_window_set_active_slot (window, slot);
			   ...
			   gpointer user_data)
{
	...
	new_window = anchor_application_create_window
...
static GtkWidget *
create_notebook (AnchorWindow *window)
{
	GtkWidget *notebook;

	notebook = g_object_new (ANCHOR_TYPE_NOTEBOOK, NULL);
...
static void
anchor_window_constructed (GObject *self)
{
	AnchorWindow *window;
	GtkWidget *grid;
	AnchorWindowSlot *slot;
	...
	G_OBJECT_CLASS (anchor_window_parent_class)->constructed (self);
	...
	anchor_window_initialize_menus (window);
	anchor_window_initialize_actions (window);

	/* Register to menu provider extension signal managing menu updates */
	g_signal_connect_object (anchor_signaller_get_current (), "popup-menu-changed",
			 G_CALLBACK (anchor_window_load_extension_menus), window, G_CONNECT_SWAPPED);
	...
	anchor_window_set_initial_window_geometry (window);

	slot = anchor_window_open_slot (window, 0);
	anchor_window_set_active_slot (window, slot);

	window->details->bookmarks_id =
		g_signal_connect_swapped (anchor_application_get_bookmarks (application), "changed",
					  G_CALLBACK (anchor_window_sync_bookmarks), window);

	anchor_profile_end (NULL);
}

static void
anchor_window_set_property (GObject *object,
			      guint arg_id,
			      const GValue *value,
			      GParamSpec *pspec)
{
...
static void
anchor_window_get_property (GObject *object,
			      guint arg_id,
			      GValue *value,
			      GParamSpec *pspec)
{
...
static void
anchor_window_destroy (GtkWidget *object)
{
	AnchorWindow *window;
	AnchorApplication *application;
	GList *slots_copy;
	...
	if (window->details->bookmarks_id != 0) {
		...
		g_signal_handler_disconnect (anchor_application_get_bookmarks (application),
					     window->details->bookmarks_id);
		window->details->bookmarks_id = 0;
	}

	GTK_WIDGET_CLASS (anchor_window_parent_class)->destroy (object);
}

static void
anchor_window_finalize (GObject *object)
{
	AnchorWindow *window;
	...
	anchor_window_finalize_menus (window);

	g_clear_object (&window->details->ui_manager);

	/* anchor_window_close() should have run */
	g_assert (window->details->slots == NULL);

	G_OBJECT_CLASS (anchor_window_parent_class)->finalize (object);
}

void
anchor_window_view_visible (AnchorWindow *window,
			      AnchorView   *view)
{
	AnchorWindowSlot *slot;
	GList *l;
	 ...
	 */
	slot = anchor_view_get_anchor_window_slot (view);
	if (g_object_get_data (G_OBJECT (slot), "anchor-window-view-visible") != NULL) {
		return;
	}

	g_object_set_data (G_OBJECT (slot), "anchor-window-view-visible", GINT_TO_POINTER (1));
	...
	anchor_window_grab_focus (window);
...
static void
anchor_window_save_geometry (AnchorWindow *window)
{
	...
	if (gtk_widget_get_window (GTK_WIDGET (window))) {
		...
		if (!is_maximized) {
			g_settings_set_string
				(anchor_window_state, NAUTILUS_WINDOW_STATE_GEOMETRY,
				 geometry_string);
		}
		g_free (geometry_string);

		g_settings_set_boolean
			(anchor_window_state, NAUTILUS_WINDOW_STATE_MAXIMIZED,
			 is_maximized);
	}
}

void
anchor_window_close (AnchorWindow *window)
{
 ...
 */
static void
anchor_window_report_location_change (AnchorWindow *window)
{
	AnchorWindowSlot *slot;
	gchar *uri;

	slot = anchor_window_get_active_slot (window);
	uri = anchor_window_slot_get_current_uri (slot);
...
void
anchor_window_set_active_slot (AnchorWindow *window, AnchorWindowSlot *new_slot)
{
	AnchorWindowSlot *old_slot;
	AnchorView *view;
	...
	if (new_slot) {
		g_assert ((window == anchor_window_slot_get_window (new_slot)));
	}

	old_slot = anchor_window_get_active_slot (window);
	...
	if (old_slot != NULL) {
		view = anchor_window_slot_get_view (old_slot);
		if (view != NULL) {
			/* inform window */
			anchor_window_disconnect_content_view (window, view);
		}
	...
	if (new_slot) {
		view = anchor_window_slot_get_view (new_slot);
		if (view != NULL) {
                        /* inform window */
                        anchor_window_connect_content_view (window, view);
                }

		/* inform sidebar panels */
                anchor_window_report_location_change (window);
...
static void
anchor_window_realize (GtkWidget *widget)
{
	GTK_WIDGET_CLASS (anchor_window_parent_class)->realize (widget);
	update_cursor (NAUTILUS_WINDOW (widget));
}

static gboolean
anchor_window_key_press_event (GtkWidget *widget,
				 GdkEventKey *event)
{
	AnchorWindow *window;
	AnchorWindowSlot *active_slot;
	...
	active_slot = anchor_window_get_active_slot (window);
	view =  anchor_window_slot_get_view (active_slot);

	if (view != NULL && anchor_view_get_is_renaming (view)) {
	...
	if (GTK_WIDGET_CLASS (anchor_window_parent_class)->key_press_event (widget, event)) {
		return TRUE;
	}

	if (anchor_window_slot_handle_event (window->details->active_slot, event)) {
		return TRUE;
	}
...
void
anchor_window_sync_up_button (AnchorWindow *window)
{
	GtkAction *action;
	GtkActionGroup *action_group;
	AnchorWindowSlot *slot;
...
void
anchor_window_sync_title (AnchorWindow *window,
			    AnchorWindowSlot *slot)
{
	...
	if (slot == anchor_window_get_active_slot (window)) {
		gtk_window_set_title (GTK_WINDOW (window), anchor_window_slot_get_title (slot));
	}

	anchor_notebook_sync_tab_label (NAUTILUS_NOTEBOOK (window->details->notebook), slot);
}

void
anchor_window_sync_zoom_widgets (AnchorWindow *window)
{
	AnchorWindowSlot *slot;
	AnchorView *view;
	GtkActionGroup *action_group;
	...
	if (view != NULL) {
		supports_zooming = anchor_view_supports_zooming (view);
		zoom_level = anchor_view_get_zoom_level (view);
		can_zoom = supports_zooming &&
			   zoom_level >= NAUTILUS_ZOOM_LEVEL_SMALLEST &&
			   zoom_level <= NAUTILUS_ZOOM_LEVEL_LARGEST;
		can_zoom_in = can_zoom && anchor_view_can_zoom_in (view);
		can_zoom_out = can_zoom && anchor_view_can_zoom_out (view);
	} else {
...
static void
zoom_level_changed_callback (AnchorView *view,
                             AnchorWindow *window)
{
	g_assert (NAUTILUS_IS_WINDOW (window));
...
*/
void
anchor_window_connect_content_view (AnchorWindow *window,
				      AnchorView *view)
{
	AnchorWindowSlot *slot;
...
void
anchor_window_disconnect_content_view (AnchorWindow *window,
					 AnchorView *view)
{
	AnchorWindowSlot *slot;
 ...
 */
static void
anchor_window_show (GtkWidget *widget)
{	
	AnchorWindow *window;

	window = NAUTILUS_WINDOW (widget);

	GTK_WIDGET_CLASS (anchor_window_parent_class)->show (widget);	

	gtk_ui_manager_ensure_update (window->details->ui_manager);
}

GtkUIManager *
anchor_window_get_ui_manager (AnchorWindow *window)
{
...
GtkActionGroup *
anchor_window_get_main_action_group (AnchorWindow *window)
{
	g_return_val_if_fail (NAUTILUS_IS_WINDOW (window), NULL);
...
AnchorWindowSlot *
anchor_window_get_active_slot (AnchorWindow *window)
{
	g_assert (NAUTILUS_IS_WINDOW (window));
...
GList *
anchor_window_get_slots (AnchorWindow *window)
{
	g_assert (NAUTILUS_IS_WINDOW (window));
...
static gboolean
anchor_window_state_event (GtkWidget *widget,
			     GdkEventWindowState *event)
{
	if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED) {
		g_settings_set_boolean (anchor_window_state, NAUTILUS_WINDOW_STATE_MAXIMIZED,
					event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED);
	}

	if (GTK_WIDGET_CLASS (anchor_window_parent_class)->window_state_event != NULL) {
		return GTK_WIDGET_CLASS (anchor_window_parent_class)->window_state_event (widget, event);
	}
...
static gboolean
anchor_window_delete_event (GtkWidget *widget,
			      GdkEventAny *event)
{
	anchor_window_close (NAUTILUS_WINDOW (widget));
	return FALSE;
}

static gboolean
anchor_window_button_press_event (GtkWidget *widget,
				    GdkEventButton *event)
{
	AnchorWindow *window;
	gboolean handled;
	...
	if (mouse_extra_buttons && (event->button == mouse_back_button)) {
		anchor_window_back_or_forward (window, TRUE, 0, 0);
		handled = TRUE; 
	} else if (mouse_extra_buttons && (event->button == mouse_forward_button)) {
		anchor_window_back_or_forward (window, FALSE, 0, 0);
		handled = TRUE;
	} else if (GTK_WIDGET_CLASS (anchor_window_parent_class)->button_press_event) {
		handled = GTK_WIDGET_CLASS (anchor_window_parent_class)->button_press_event (widget, event);
	} else {
		handled = FALSE;
	}
	return handled;
...
mouse_back_button_changed (gpointer callback_data)
{
	...
	new_back_button = g_settings_get_int (anchor_preferences, NAUTILUS_PREFERENCES_MOUSE_BACK_BUTTON);
...
mouse_forward_button_changed (gpointer callback_data)
{
	...
	new_forward_button = g_settings_get_int (anchor_preferences, NAUTILUS_PREFERENCES_MOUSE_FORWARD_BUTTON);
...
{
	mouse_extra_buttons = g_settings_get_boolean (anchor_preferences, NAUTILUS_PREFERENCES_MOUSE_USE_EXTRA_BUTTONS);
}
...
static void
anchor_window_init (AnchorWindow *window)
{
	GtkWindowGroup *window_group;

	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, ANCHOR_TYPE_WINDOW, AnchorWindowDetails);
...
real_window_close (AnchorWindow *window)
{
	...
	anchor_window_save_geometry (window);

	gtk_widget_destroy (GTK_WIDGET (window));
}

static void
anchor_window_class_init (AnchorWindowClass *class)
{
	GObjectClass *oclass = G_OBJECT_CLASS (class);
	GtkWidgetClass *wclass = GTK_WIDGET_CLASS (class);

	oclass->finalize = anchor_window_finalize;
	oclass->constructed = anchor_window_constructed;
	oclass->get_property = anchor_window_get_property;
	oclass->set_property = anchor_window_set_property;

	wclass->destroy = anchor_window_destroy;
	wclass->show = anchor_window_show;
	wclass->realize = anchor_window_realize;
	wclass->key_press_event = anchor_window_key_press_event;
	wclass->window_state_event = anchor_window_state_event;
	wclass->button_press_event = anchor_window_button_press_event;
	wclass->delete_event = anchor_window_delete_event;
	...
	signals[SLOT_ADDED] =
		g_signal_new ("slot-added",
			      ...
			      G_TYPE_NONE, 1, ANCHOR_TYPE_WINDOW_SLOT);
	...
	g_type_class_add_private (oclass, sizeof (AnchorWindowDetails));
}

AnchorWindow *
anchor_window_new (GdkScreen *screen)
{
	return g_object_new (ANCHOR_TYPE_WINDOW,
			     "screen", screen,
			     NULL);
}

AnchorWindowOpenFlags
anchor_event_get_window_open_flags (void)
{
	AnchorWindowOpenFlags flags = 0;
	GdkEvent *event;
...
void
anchor_window_show_about_dialog (AnchorWindow *window)
{
	const gchar *authors[] = {
		"Alexander Larsson",
		"Ali Abdin",
