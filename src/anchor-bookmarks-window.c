...
/* anchor-bookmarks-window.c - implementation of bookmark-editing window.
 */
...
/* We store a pointer to the bookmark in a column so when an item is moved
   ...
   anchor_bookmark_list_insert_item() makes a copy of the bookmark, so we
...
G_DEFINE_TYPE (AnchorBookmarksWindow, anchor_bookmarks_window, GTK_TYPE_WINDOW)
...
struct AnchorBookmarksWindowPrivate {
...
static gboolean
get_selection_exists (AnchorBookmarksWindow *self)
{
	return gtk_tree_selection_get_selected (self->priv->selection, NULL, NULL);
}

static guint
get_selected_row (AnchorBookmarksWindow *self)
{
	GtkTreeIter       iter;
	GtkTreePath      *path;
	GtkTreeModel     *model;
...
static AnchorBookmark *
get_selected_bookmark (AnchorBookmarksWindow *self)
{
	if (!get_selection_exists (self)) {
		return NULL;
	}

	if (anchor_bookmark_list_length (self->priv->bookmarks) < 1) {
		return NULL;
	}

	return anchor_bookmark_list_item_at (self->priv->bookmarks,
					       get_selected_row (self));
}

static int
anchor_bookmarks_window_key_press_event_cb (GtkWindow *window, 
					      GdkEventKey *event, 
					      gpointer user_data)
{
...
static void
setup_empty_list (AnchorBookmarksWindow *self)
{
...
static void
update_widgets_sensitivity (AnchorBookmarksWindow *self)
{
	AnchorBookmark *selected;
	int n_active;
	int index = -1;
	gboolean builtin;
	...
	if (selected != NULL) {
		...
		builtin = anchor_bookmark_get_is_builtin (selected);
	} else {
		builtin = FALSE;
	}
...
on_selection_changed (GtkTreeSelection *treeselection,
		      gpointer user_data)
{
	AnchorBookmarksWindow *self = user_data;
	AnchorBookmark *selected;
	...
	if (selected) {
		name = anchor_bookmark_get_name (selected);
		location = anchor_bookmark_get_location (selected);
	...
	anchor_entry_set_text (NAUTILUS_ENTRY (self->priv->name_field),
				 name ? name : "");
	...
	anchor_entry_set_text (NAUTILUS_ENTRY (self->priv->uri_field),
...
static void
bookmarks_set_empty (AnchorBookmarksWindow *self,
		     gboolean empty)
{
	GtkTreeIter iter;
	...
	if (empty) {
		...
		if (anchor_bookmark_list_length (self->priv->bookmarks) > 0 &&
...
static void
repopulate (AnchorBookmarksWindow *self)
{
	AnchorBookmark *selected;
	GtkListStore *store;
	GtkTreePath *path;
	...
	for (index = 0; index < anchor_bookmark_list_length (self->priv->bookmarks); ++index) {
		AnchorBookmark *bookmark;
		const char       *bookmark_name;
		GIcon            *bookmark_icon;
		GtkTreeIter       iter;

		bookmark = anchor_bookmark_list_item_at (self->priv->bookmarks, index);
		bookmark_name = anchor_bookmark_get_name (bookmark);
		bookmark_icon = anchor_bookmark_get_symbolic_icon (bookmark);
...
static void
on_bookmark_list_changed (AnchorBookmarkList *bookmarks,
			  gpointer user_data)
{
	AnchorBookmarksWindow *self = user_data;
...
static void
bookmarks_delete_bookmark (AnchorBookmarksWindow *self)
{
	GtkTreeIter iter;
	GtkTreePath *path;
		      ...
		      gpointer   user_data)
{
	...
	anchor_bookmark_list_move_item (self->priv->bookmarks, row, row - 1);
	gtk_tree_model_iter_nth_child (GTK_TREE_MODEL (self->priv->model),
				       &iter, NULL, row - 1);
	gtk_tree_selection_select_iter (self->priv->selection, &iter);
}
...
{
	AnchorBookmarksWindow *self = user_data;
	AnchorBookmark *bookmark = NULL, *bookmark_in_list;
	gint *indices, row;
	...
	if (row < (gint) anchor_bookmark_list_length (self->priv->bookmarks)) {
		bookmark_in_list = anchor_bookmark_list_item_at (self->priv->bookmarks, row);
	...
	if (insert_bookmark) {
		...
		anchor_bookmark_list_insert_item (self->priv->bookmarks,
						    bookmark, row);
		...
		bookmark = anchor_bookmark_list_item_at (self->priv->bookmarks, row);
...
static void
update_bookmark_from_text (AnchorBookmarksWindow *self)
{
	AnchorBookmark *bookmark, *bookmark_in_list;
	const char *name;
	GIcon *icon;
	guint selected_row;
	GtkTreeIter iter;
	...
	bookmark = anchor_bookmark_new (location,
					  gtk_entry_get_text (GTK_ENTRY (self->priv->name_field)));
	...
	anchor_bookmark_list_delete_item_at (self->priv->bookmarks, selected_row);
	anchor_bookmark_list_insert_item (self->priv->bookmarks, bookmark, selected_row);
	...
	bookmark_in_list = anchor_bookmark_list_item_at (self->priv->bookmarks, selected_row);
	name = anchor_bookmark_get_name (bookmark_in_list);
	icon = anchor_bookmark_get_symbolic_icon (bookmark_in_list);
                  ...
                  gpointer           user_data)
{
	...
	anchor_window_go_to (self->priv->parent_window, location);
	g_object_unref (location);
}
		...
		gpointer user_data)
{
	...
	g_signal_handler_block (self->priv->bookmarks, self->priv->bookmarks_changed_id);
	anchor_bookmark_list_delete_item_at (self->priv->bookmarks, row);
	g_signal_handler_unblock (self->priv->bookmarks, self->priv->bookmarks_changed_id);
}
...
static void
name_or_uri_field_activate (AnchorEntry *entry,
			    gpointer user_data)
{
	AnchorBookmarksWindow *self = user_data;

	update_bookmark_from_text (self);
	anchor_entry_select_all_at_idle (entry);
}
...
static void
anchor_bookmarks_window_dispose (GObject *object)
{
	AnchorBookmarksWindow *self = NAUTILUS_BOOKMARKS_WINDOW (object);
	...
	G_OBJECT_CLASS (anchor_bookmarks_window_parent_class)->dispose (object);
}

static void
anchor_bookmarks_window_constructed (GObject *object)
{
	AnchorBookmarksWindow *self = NAUTILUS_BOOKMARKS_WINDOW (object);
	GtkBuilder *builder;
	GError *error = NULL;
	...
	G_OBJECT_CLASS (anchor_bookmarks_window_parent_class)->constructed (object);

	builder = gtk_builder_new ();
	if (!gtk_builder_add_from_resource (builder,
					    "/org/gnome/anchor/anchor-bookmarks-window.ui",
					    &error)) {
	...
	g_signal_connect (window, "key-press-event",
			  G_CALLBACK (anchor_bookmarks_window_key_press_event_cb), NULL);
	...
	/* name entry */
	self->priv->name_field = anchor_entry_new ();
	...
	/* URI entry */
	self->priv->uri_field = anchor_entry_new ();
	...
	self->priv->bookmarks = anchor_application_get_bookmarks
...
static void
anchor_bookmarks_window_set_property (GObject *object,
					guint arg_id,
					const GValue *value,
					GParamSpec *pspec)
{
...
static void
anchor_bookmarks_window_init (AnchorBookmarksWindow *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ANCHOR_TYPE_BOOKMARKS_WINDOW,
						  AnchorBookmarksWindowPrivate);
}

static void
anchor_bookmarks_window_class_init (AnchorBookmarksWindowClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->set_property = anchor_bookmarks_window_set_property;
	oclass->dispose = anchor_bookmarks_window_dispose;
	oclass->constructed = anchor_bookmarks_window_constructed;

	properties[PROP_PARENT_WINDOW] =
		g_param_spec_object ("parent-window",
				     "The AnchorWindow",
				     "The parent AnchorWindow",
				     ANCHOR_TYPE_WINDOW,
				     G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY);

	g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
	g_type_class_add_private (klass, sizeof (AnchorBookmarksWindowPrivate));
}

/**
 * anchor_bookmarks_window_new:
 * 
 * Create a new bookmark-editing window. 
 * @parent_window: The parent AnchorWindow.
 *
 * Return value: A pointer to the new window.
 **/
GtkWindow *
anchor_bookmarks_window_new (AnchorWindow *parent_window)
{
	return g_object_new (ANCHOR_TYPE_BOOKMARKS_WINDOW,
