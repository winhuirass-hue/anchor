...
#include <libanchor-extension/anchor-column-provider.h>
#include <libanchor-private/anchor-clipboard-monitor.h>
#include <libanchor-private/anchor-column-chooser.h>
#include <libanchor-private/anchor-column-utilities.h>
...
#include <libanchor-private/anchor-tree-view-drag-dest.h>
...
struct AnchorListViewDetails {
	GtkTreeView *tree_view;
	AnchorListModel *model;
	...
	AnchorTreeViewDragDest *drag_dest;
...
static GList *anchor_list_view_get_selection                   (AnchorView   *view);
static GList *anchor_list_view_get_selection_for_file_transfer (AnchorView   *view);
static void   anchor_list_view_set_zoom_level                  (AnchorListView        *view,
								  AnchorZoomLevel  new_level,
								  gboolean           always_set_level);
static void   anchor_list_view_scroll_to_file                  (AnchorListView        *view,
								  AnchorFile      *file);
static void   anchor_list_view_rename_callback                 (AnchorFile      *file,
								  GFile             *result_location,
								  GError            *error,
								  gpointer           callback_data);

static void   apply_columns_settings                             (AnchorListView *list_view,
                                                                  char **column_order,
                                                                  char **visible_columns);
static char **get_visible_columns                                (AnchorListView *list_view);
static char **get_default_visible_columns                        (AnchorListView *list_view);
static char **get_column_order                                   (AnchorListView *list_view);
static char **get_default_column_order                           (AnchorListView *list_view);


G_DEFINE_TYPE (AnchorListView, anchor_list_view, ANCHOR_TYPE_VIEW);
...
{
	AnchorFileSortType default_sort_order;
	...
	retval = anchor_file_get_default_sort_attribute (file, reversed);

	if (retval == NULL) {
		default_sort_order = g_settings_get_enum (anchor_preferences,
							  NAUTILUS_PREFERENCES_DEFAULT_SORT_ORDER);
		default_sort_reversed = g_settings_get_boolean (anchor_preferences,
...
static void
preview_selected_items (AnchorListView *view)
{
	GList *file_list;
	
	file_list = anchor_list_view_get_selection (NAUTILUS_VIEW (view));

	if (file_list != NULL) {
		anchor_view_preview_files (NAUTILUS_VIEW (view),
					     file_list, NULL);
...
static void
activate_selected_items (AnchorListView *view)
{
	GList *file_list;
	...
	anchor_view_activate_files (NAUTILUS_VIEW (view),
				      file_list,
				      0, TRUE);
	anchor_file_list_free (file_list);
...
static void
activate_selected_items_alternate (AnchorListView *view,
				   AnchorFile *file,
...
{
	return g_settings_get_enum (anchor_preferences,
				    NAUTILUS_PREFERENCES_CLICK_POLICY);
}

static void
anchor_list_view_did_not_drag (AnchorListView *view,
			...
			guint time)
{
	...
	anchor_drag_drag_data_get_from_cache (selection_cache, context, selection_data, info, time);
}

static void
stop_drag_check (AnchorListView *view)
{		
	view->details->drag_button = 0;
}

static cairo_surface_t *
get_drag_surface (AnchorListView *view)
{
	GtkTreeModel *model;
	GtkTreePath *path;
	GtkTreeIter iter;
	...
	if (gtk_tree_view_get_path_at_pos (view->details->tree_view, 
		...
		gtk_tree_model_get (model, &iter,
				    anchor_list_model_get_column_id_from_zoom_level (view->details->zoom_level),
				    &ret,
				    -1);
...
typedef struct {
	AnchorListView *view;
	AnchorDragEachSelectedItemDataGet iteratee;
		      ...
		      gpointer data)
{
	...
	treeview = anchor_list_model_get_drag_view (context->view->details->model,
						      NULL,
						      &drag_begin_y);
	column = gtk_tree_view_get_column (treeview, 0);

	file = anchor_list_model_file_for_path (NAUTILUS_LIST_MODEL (model), path);
	if (file == NULL) {
		return;
	}
	...
	uri = anchor_file_get_activation_uri (file);
...
static void
each_item_get_data_binder (AnchorDragEachSelectedItemDataGet iteratee,
			   gpointer iterator_context,
			   gpointer data)
{
	AnchorListView *view = NAUTILUS_LIST_VIEW (iterator_context);
		     ...
		     AnchorListView *view)
{
	...
	selection_cache = anchor_drag_create_selection_cache (view,
								each_item_get_data_binder);

	g_object_set_data_full (G_OBJECT (context),
				"drag-info",
				selection_cache,
				(GDestroyNotify)anchor_drag_destroy_selection_list);
}
			...
			gpointer callback_data)
{
	...
	if (view->details->drag_button != 0) {
		if (!source_target_list) {
			source_target_list = anchor_list_model_get_drag_target_list ();
		}
...
static void
do_popup_menu (GtkWidget *widget, AnchorListView *view, GdkEventButton *event)
{
 	if (tree_view_has_selection (GTK_TREE_VIEW (widget))) {
		anchor_view_pop_up_selection_context_menu (NAUTILUS_VIEW (view), event);
	} else {
                anchor_view_pop_up_background_context_menu (NAUTILUS_VIEW (view), event);
	}
}

static void
row_activated_callback (GtkTreeView *treeview, GtkTreePath *path, 
			GtkTreeViewColumn *column, AnchorListView *view)
{
	activate_selected_items (view);
}
...
button_press_callback (GtkWidget *widget, GdkEventButton *event, gpointer callback_data)
{
	...
	anchor_list_model_set_drag_view
	...
	show_expanders = g_settings_get_boolean (anchor_list_view_preferences,
	...
	if (event->type == GDK_2BUTTON_PRESS) {
		...
		if (!on_expander &&
			...
			if ((event->button == 1) && button_event_modifies_selection (event)) {
				file = anchor_list_model_file_for_path (view->details->model, path);
				if (file != NULL) {
			 ...
			 gpointer callback_data)
{
	...
	if (event->button == view->details->drag_button) {
		...
		if (!view->details->drag_started &&
			...
			anchor_list_view_did_not_drag (view, event);
		}
	}
	return FALSE;
}
...
static void
subdirectory_done_loading_callback (AnchorDirectory *directory, AnchorListView *view)
{
	anchor_list_model_subdirectory_done_loading (view->details->model, directory);
}
		       ...
		       gpointer callback_data)
{
	...
	if (!anchor_list_model_load_subdirectory (view->details->model, path, &directory)) {
		return;
	}
	...
	anchor_view_add_subdirectory (NAUTILUS_VIEW (view), directory);

	if (anchor_directory_are_all_files_seen (directory)) {
		anchor_list_model_subdirectory_done_loading (view->details->model,
							       directory);
	} else {
...
unload_delay_data_free (UnloadDelayData *unload_data)
{
	...
	anchor_directory_unref (unload_data->directory);
	anchor_file_unref (unload_data->file);

	g_slice_free (UnloadDelayData, unload_data);
}

static UnloadDelayData *
unload_delay_data_new (AnchorFile *file,
		       AnchorDirectory *parent_directory,
		       AnchorListView *view)
{
	...
	unload_data->file = anchor_file_ref (file);
	unload_data->directory = anchor_directory_ref (parent_directory);
...
unload_file_timeout (gpointer data)
{
	...
	if (anchor_list_model_get_tree_iter_from_file (model,
							 unload_data->file,
		...
		if (!gtk_tree_view_row_expanded (unload_data->view->details->tree_view,
						 path)) {
			anchor_list_model_unload_subdirectory (model, &iter);
		}
		gtk_tree_path_free (path);
	}
...
static void
subdirectory_unloaded_callback (AnchorListModel *model,
				AnchorDirectory *directory,
				gpointer callback_data)
{
	AnchorListView *view;
	...
	g_signal_handlers_disconnect_by_func (directory,
					      G_CALLBACK (subdirectory_done_loading_callback),
					      view);
	anchor_view_remove_subdirectory (NAUTILUS_VIEW (view), directory);
}
...
key_press_callback (GtkWidget *widget, GdkEventKey *event, gpointer callback_data)
{
	...
	case GDK_KEY_F10:
		if (event->state & GDK_CONTROL_MASK) {
			anchor_view_pop_up_background_context_menu (view, &button_event);
			handled = TRUE;
		}
		break;
	case GDK_KEY_Right:
...
test_expand_row_callback (GtkTreeView *tree_view,
			  ...
			  gboolean user_data)
{
	return !g_settings_get_boolean (anchor_list_view_preferences,
					NAUTILUS_PREFERENCES_LIST_VIEW_USE_TREE);
}

static void
anchor_list_view_reveal_selection (AnchorView *view)
{
	GList *selection;
	...
	if (selection != NULL) {
		...
		file = selection->data;
		if (anchor_list_model_get_first_iter_for_file (list_view->details->model, file, &iter)) {
			      ...
			      AnchorListView *view)
{
	...
	sort_attr = anchor_list_model_get_attribute_from_sort_column_id (view->details->model, sort_column_id);

	default_sort_column_id = anchor_list_model_get_sort_column_id_from_attribute (view->details->model,
										  g_quark_from_string (get_default_sort_order (file, &default_sort_reversed)));
	default_sort_attr = anchor_list_model_get_attribute_from_sort_column_id (view->details->model, default_sort_column_id);
	anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
	...
	if (view->details->last_sort_attr != sort_attr &&
		...
		if (sort_attr == default_sort_attr) {
			...
			reversed = g_settings_get_boolean (anchor_preferences,
							   NAUTILUS_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER);
		} else {
			reversed = anchor_file_is_date_sort_attribute_q (sort_attr);
		}
	...
	anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
				    default_reversed_attr, reversed_attr);

	/* Make sure selected item(s) is visible after sort */
	anchor_list_view_reveal_selection (NAUTILUS_VIEW (view));
...
editable_focus_out_cb (GtkWidget *widget,
		       ...
		       gpointer user_data)
{
	AnchorListView *view = user_data;

	view->details->editable_widget = NULL;

	anchor_view_set_is_renaming (NAUTILUS_VIEW (view), FALSE);
	anchor_view_unfreeze_updates (NAUTILUS_VIEW (view));
}
				  ...
				  AnchorListView *list_view)
{
	...
	anchor_clipboard_set_up_editable
		(GTK_EDITABLE (entry),
		 anchor_view_get_ui_manager (NAUTILUS_VIEW (list_view)),
		 FALSE);
}
		      ...
		      AnchorListView    *view)
{
	...
	if (strcmp (new_text, view->details->original_name) != 0) {
		view->details->renaming_file = anchor_file_ref (file);
		view->details->rename_done = FALSE;
		anchor_rename_file (file, new_text, anchor_list_view_rename_callback, g_object_ref (view));
...
static char *
get_root_uri_callback (AnchorTreeViewDragDest *dest,
		       gpointer user_data)
{
	AnchorListView *view;
	...
	return anchor_view_get_uri (NAUTILUS_VIEW (view));
}

static AnchorFile *
get_file_for_path_callback (AnchorTreeViewDragDest *dest,
			    GtkTreePath *path,
			    gpointer user_data)
{
	AnchorListView *view;
	...
	return anchor_list_model_file_for_path (view->details->model, path);
}

/* Handles an URL received from Mozilla */
static void
list_view_handle_netscape_url (AnchorTreeViewDragDest *dest, const char *encoded_url,
			       const char *target_uri, GdkDragAction action, int x, int y, AnchorListView *view)
{
...
static void
list_view_handle_uri_list (AnchorTreeViewDragDest *dest, const char *item_uris,
			   const char *target_uri,
			   GdkDragAction action, int x, int y, AnchorListView *view)
{
...
static void
list_view_handle_text (AnchorTreeViewDragDest *dest, const char *text,
		       const char *target_uri,
		       GdkDragAction action, int x, int y, AnchorListView *view)
{
...
static void
list_view_handle_raw (AnchorTreeViewDragDest *dest, const char *raw_data,
...
static void
list_view_handle_hover (AnchorTreeViewDragDest *dest,
			const char *target_uri,
			AnchorListView *view)
{
...
static void
move_copy_items_callback (AnchorTreeViewDragDest *dest,
                            ...
                            AnchorListView *list_view)
{
	...
	file = anchor_view_get_directory_as_file (NAUTILUS_VIEW (list_view));
	...
	anchor_file_set_metadata_list (file,
                                ...
                                AnchorListView *list_view)
{
	...
	anchor_file_set_metadata_list (file, NAUTILUS_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
	anchor_file_set_metadata_list (file, NAUTILUS_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
                       ...
                       AnchorListView *list_view)
{
	...
	all_columns = anchor_get_columns_for_file (file);
	all_columns = anchor_sort_columns (all_columns, column_order);
	...
	anchor_column_list_free (all_columns);
	g_strfreev (column_order);
	g_strfreev (visible_columns);
...
static void
apply_columns_settings (AnchorListView *list_view,
			char **column_order,
			char **visible_columns)
{
	GList *all_columns;
                         ...
                         gboolean           show_trash_orig)
{
	...
	if (show_trash_orig && anchor_file_is_in_trash (file)) {
		AnchorFile *orig_file;
	...
	if (!anchor_file_is_in_recent (file)) {
		dir_location = anchor_file_get_parent_location (file);
	} else {
		GFile *activation_location;

		activation_location = anchor_file_get_activation_location (file);
	...
	if (!NAUTILUS_IS_SEARCH_DIRECTORY (directory)) {
		...
		query = anchor_search_directory_get_query (NAUTILUS_SEARCH_DIRECTORY (directory));
		base_uri = anchor_query_get_location (query);
		base = anchor_file_get_by_uri (base_uri);

		if (!anchor_file_is_in_recent (base)) {
			base_location = anchor_file_get_location (base);
		} else {
			base_location = g_object_ref (home_location);
		}

		anchor_file_unref (base);
		g_free (base_uri);
...
static void
set_up_pixbuf_size (AnchorListView *view)
{
	int icon_size;

	/* Make all rows the same size. */
	icon_size = anchor_get_icon_size_for_zoom_level (view->details->zoom_level);
...
static gint
get_icon_scale_callback (AnchorListModel *model,
			 AnchorListView  *view)
{
...
static void
create_and_set_up_tree_view (AnchorListView *view)
{
	GtkCellRenderer *cell;
	GtkTreeViewColumn *column;
	AtkObject *atk_obj;
	GList *anchor_columns;
	GList *l;
	...
	view->details->drag_dest = 
		anchor_tree_view_drag_dest_new (view->details->tree_view);
	...
	view->details->model = g_object_new (ANCHOR_TYPE_LIST_MODEL, NULL);
	gtk_tree_view_set_model (view->details->tree_view, GTK_TREE_MODEL (view->details->model));
	/* Need the model for the dnd drop icon "accept" change */
	anchor_list_model_set_drag_view (NAUTILUS_LIST_MODEL (view->details->model),
	...
	g_settings_bind (anchor_list_view_preferences, NAUTILUS_PREFERENCES_LIST_VIEW_USE_TREE,
			 view->details->tree_view, "show-expanders",
			 G_SETTINGS_BIND_DEFAULT);

	anchor_columns = anchor_get_all_columns ();

	for (l = anchor_columns; l != NULL; l = l->next) {
		...
		anchor_column = NAUTILUS_COLUMN (l->data);

		g_object_get (anchor_column, 
			      "name", &name, 
			      "label", &label,
			      "xalign", &xalign,
		...
		column_num = anchor_list_model_add_column (view->details->model,
						       anchor_column);
		...
		if (!strcmp (name, "name")) {
			...
			gtk_tree_view_column_set_attributes (view->details->file_name_column,
							     cell,
							     "surface", anchor_list_model_get_column_id_from_zoom_level (view->details->zoom_level),
							     NULL);
	...
	anchor_column_list_free (anchor_columns);

	default_visible_columns = g_settings_get_strv (anchor_list_view_preferences,
						       NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
	default_column_order = g_settings_get_strv (anchor_list_view_preferences,
...
static void
anchor_list_view_add_file (AnchorView *view, AnchorFile *file, AnchorDirectory *directory)
{
	AnchorListModel *model;

	model = NAUTILUS_LIST_VIEW (view)->details->model;
	anchor_list_model_add_file (model, file, directory);
}

static char **
get_default_visible_columns (AnchorListView *list_view)
{
	AnchorFile *file;
	AnchorDirectory *directory;
	...
	directory = anchor_view_get_model (NAUTILUS_VIEW (list_view));
	if (NAUTILUS_IS_SEARCH_DIRECTORY (directory)) {
		return g_strdupv ((gchar **) default_search_visible_columns);
	}

	return g_settings_get_strv (anchor_list_view_preferences,
	                            NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
}

static char **
get_visible_columns (AnchorListView *list_view)
{
	AnchorFile *file;
	...
	visible_columns = anchor_file_get_metadata_list
		(file,
		 NAUTILUS_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS);
...
static char **
get_default_column_order (AnchorListView *list_view)
{
	AnchorFile *file;
	AnchorDirectory *directory;
...
static char **
get_column_order (AnchorListView *list_view)
{
	AnchorFile *file;
	GList *column_order;
	...
	column_order = anchor_file_get_metadata_list
		(file,
...
static void
set_columns_settings_from_metadata_and_preferences (AnchorListView *list_view)
{
	char **column_order;
	char **visible_columns;
...
static void
set_sort_order_from_metadata_and_preferences (AnchorListView *list_view)
{
	char *sort_attribute;
	int sort_column_id;
	AnchorFile *file;
	...
	file = anchor_view_get_directory_as_file (NAUTILUS_VIEW (list_view));
	sort_attribute = anchor_file_get_metadata (file,
						     NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_COLUMN,
						     NULL);
	sort_column_id = anchor_list_model_get_sort_column_id_from_attribute (list_view->details->model,
	...
	if (sort_column_id == -1) {
		sort_column_id =
			anchor_list_model_get_sort_column_id_from_attribute (list_view->details->model,
									 g_quark_from_string (default_sort_order));
	}

	sort_reversed = anchor_file_get_boolean_metadata (file,
...
get_default_zoom_level (void) {
	...
	default_zoom_level = g_settings_get_enum (anchor_list_view_preferences,
...
static void
anchor_list_view_begin_loading (AnchorView *view)
{
	AnchorListView *list_view;
...
static void
stop_cell_editing (AnchorListView *list_view)
{
	GtkTreeViewColumn *column;
...
static void
anchor_list_view_clear (AnchorView *view)
{
	AnchorListView *list_view;
	...
	if (list_view->details->model != NULL) {
		...
		anchor_list_model_clear (list_view->details->model);
	}
}

static void
anchor_list_view_rename_callback (AnchorFile *file,
				    GFile *result_location,
				    GError *error,
				    gpointer callback_data)
{
	...
	if (view->details->renaming_file) {
		...
		if (error != NULL) {
			 ...
			 */
			anchor_file_unref (view->details->renaming_file);
...
static void
anchor_list_view_file_changed (AnchorView *view, AnchorFile *file, AnchorDirectory *directory)
{
	AnchorListView *listview;
	GtkTreeIter iter;
	GtkTreePath *file_path;

	listview = NAUTILUS_LIST_VIEW (view);
	
	anchor_list_model_file_changed (listview->details->model, file, directory);
	...
	if (listview->details->renaming_file != NULL &&
		 ...
		 */
		if (anchor_list_model_get_tree_iter_from_file (listview->details->model, file, directory, &iter)) {
		...
		anchor_file_unref (listview->details->renaming_file);
...
static char *
anchor_list_view_get_backing_uri (AnchorView *view)
{
	AnchorListView *list_view;
	AnchorListModel *list_model;
	AnchorFile *file;
	GtkTreeView *tree_view;
	GtkTreeSelection *selection;
	GtkTreePath *path;
	...
	if (length == 1) {
		...
		file = anchor_list_model_file_for_path (list_model, path);
		if (file == NULL) {
	...
	if (uri == NULL && length > 0) {
		 ...
		 * is handled by AnchorView::get_backing_directory.) */
		...
		if (is_common && !is_root) {
			...
			file = anchor_list_model_file_for_path (list_model, path);
			g_assert (file != NULL);
			uri = anchor_file_get_parent_uri (file);
			anchor_file_unref (file);
	...
	return NAUTILUS_VIEW_CLASS (anchor_list_view_parent_class)->get_backing_uri (view);
}

static void
anchor_list_view_get_selection_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	GList **list;
	AnchorFile *file;
...
static GList *
anchor_list_view_get_selection (AnchorView *view)
{
	GList *list;
	...
	gtk_tree_selection_selected_foreach (gtk_tree_view_get_selection (NAUTILUS_LIST_VIEW (view)->details->tree_view),
					     anchor_list_view_get_selection_foreach_func, &list);

	return g_list_reverse (list);
}

static void
anchor_list_view_get_selection_for_file_transfer_foreach_func (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
	AnchorFile *file;
...
static GList *
anchor_list_view_get_selection_for_file_transfer (AnchorView *view)
{
	struct SelectionForeachData selection_data;
	...
	gtk_tree_selection_selected_foreach (selection_data.selection,
					     anchor_list_view_get_selection_for_file_transfer_foreach_func, &selection_data);

	return g_list_reverse (selection_data.list);
}

static gboolean
anchor_list_view_is_empty (AnchorView *view)
{
	return anchor_list_model_is_empty (NAUTILUS_LIST_VIEW (view)->details->model);
}

static void
anchor_list_view_end_file_changes (AnchorView *view)
{
	AnchorListView *list_view;
...
static void
anchor_list_view_remove_file (AnchorView *view, AnchorFile *file, AnchorDirectory *directory)
{
	GtkTreePath *path;
	GtkTreePath *file_path;
	GtkTreeIter iter;
	...
	if (anchor_list_model_get_tree_iter_from_file (list_view->details->model, file, directory, &iter)) {
		...
		anchor_list_model_remove_file (list_view->details->model, file, directory);
...
static void
anchor_list_view_set_selection (AnchorView *view, GList *selection)
{
	AnchorListView *list_view;
	...
	for (node = selection; node != NULL; node = node->next) {
		file = node->data;
		iters = anchor_list_model_get_all_iters_for_file (list_view->details->model, file);
...
static void
anchor_list_view_invert_selection (AnchorView *view)
{
	AnchorListView *list_view;
	GtkTreeSelection *tree_selection;
	GList *node;
	...
	gtk_tree_selection_selected_foreach (tree_selection,
					     anchor_list_view_get_selection_foreach_func, &selection);
...
static void
column_chooser_changed_callback (AnchorColumnChooser *chooser,
				 AnchorListView *view)
{
	AnchorFile *file;
	char **visible_columns;
	...
	anchor_column_chooser_get_settings (chooser,
					      &visible_columns,
					      &column_order);
...
static void
column_chooser_set_from_arrays (AnchorColumnChooser *chooser,
                                AnchorListView *view,
                                char **visible_columns,
                                char **column_order)
{
	...
	anchor_column_chooser_set_settings (chooser,
					      visible_columns, 
					      column_order);
...
static void
column_chooser_set_from_settings (AnchorColumnChooser *chooser,
				  AnchorListView *view)
{
	char **visible_columns;
	char **column_order;
...
static void
column_chooser_use_default_callback (AnchorColumnChooser *chooser,
				     AnchorListView *view)
{
	AnchorFile *file;
	char **default_columns;
	...
	file = anchor_view_get_directory_as_file 
		(NAUTILUS_VIEW (view));
...
static GtkWidget *
create_column_editor (AnchorListView *view)
{
	GtkWidget *window;
	GtkWidget *label;
	GtkWidget *box;
	...
	column_chooser = anchor_column_chooser_new (file);
...
static void
anchor_list_view_merge_menus (AnchorView *view)
{
	AnchorListView *list_view;
	...
	NAUTILUS_VIEW_CLASS (anchor_list_view_parent_class)->merge_menus (view);
	...
	list_view->details->list_merge_id =
		gtk_ui_manager_add_ui_from_resource (ui_manager, "/org/gnome/anchor/anchor-list-view-ui.xml", NULL);

	list_view->details->menus_ready = TRUE;
}

static void
anchor_list_view_unmerge_menus (AnchorView *view)
{
	AnchorListView *list_view;
	GtkUIManager *ui_manager;
	...
	NAUTILUS_VIEW_CLASS (anchor_list_view_parent_class)->unmerge_menus (view);
...
static void
anchor_list_view_update_menus (AnchorView *view)
{
	AnchorListView *list_view;
	...
	NAUTILUS_VIEW_CLASS (anchor_list_view_parent_class)->update_menus (view);
}

/* Reset sort criteria and zoom level to match defaults */
static void
anchor_list_view_reset_to_defaults (AnchorView *view)
{
	AnchorFile *file;
	const gchar *default_sort_order;
	gboolean default_sort_reversed;
	...
	anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL, NULL);
	anchor_file_set_metadata (file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_REVERSED, NULL, NULL);
	anchor_file_set_metadata_list (file, NAUTILUS_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
	anchor_file_set_metadata_list (file, NAUTILUS_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
	...
	gtk_tree_sortable_set_sort_column_id
		(GTK_TREE_SORTABLE (NAUTILUS_LIST_VIEW (view)->details->model),
		 anchor_list_model_get_sort_column_id_from_attribute (NAUTILUS_LIST_VIEW (view)->details->model,
								  g_quark_from_string (default_sort_order)),
		 default_sort_reversed ? GTK_SORT_DESCENDING : GTK_SORT_ASCENDING);

	anchor_list_view_set_zoom_level (NAUTILUS_LIST_VIEW (view), get_default_zoom_level (), FALSE);
...
static void
anchor_list_view_set_zoom_level (AnchorListView *view,
				   AnchorZoomLevel new_level,
				   gboolean always_emit)
{
	...
	column = anchor_list_model_get_column_id_from_zoom_level (new_level);
	gtk_tree_view_column_set_attributes (view->details->file_name_column,
...
static void
anchor_list_view_bump_zoom_level (AnchorView *view, int zoom_increment)
{
	AnchorListView *list_view;
	gint new_level;
	...
	if (new_level >= NAUTILUS_ZOOM_LEVEL_SMALLEST &&
	    new_level <= NAUTILUS_ZOOM_LEVEL_LARGEST) {
		anchor_list_view_set_zoom_level (list_view, new_level, FALSE);
	}
}

static AnchorZoomLevel
anchor_list_view_get_zoom_level (AnchorView *view)
{
	AnchorListView *list_view;
...
static void
anchor_list_view_zoom_to_level (AnchorView *view,
				  AnchorZoomLevel zoom_level)
{
	AnchorListView *list_view;
	...
	anchor_list_view_set_zoom_level (list_view, zoom_level, FALSE);
}

static void
anchor_list_view_restore_default_zoom_level (AnchorView *view)
{
	AnchorListView *list_view;
	...
	anchor_list_view_set_zoom_level (list_view, get_default_zoom_level (), FALSE);
}

static gboolean 
anchor_list_view_can_zoom_in (AnchorView *view) 
{
...
static gboolean 
anchor_list_view_can_zoom_out (AnchorView *view) 
{
	g_return_val_if_fail (NAUTILUS_IS_LIST_VIEW (view), FALSE);
...
static void
anchor_list_view_start_renaming_file (AnchorView *view,
					AnchorFile *file,
					gboolean select_all)
{
	AnchorListView *list_view;
	...
	NAUTILUS_VIEW_CLASS (anchor_list_view_parent_class)->start_renaming_file (view, file, select_all);

	/* Freeze updates to the view to prevent losing rename focus when the tree view updates */
	anchor_view_freeze_updates (NAUTILUS_VIEW (view));
...
static void
anchor_list_view_click_policy_changed (AnchorView *directory_view)
{
	GdkWindow *win;
	GdkDisplay *display;
	AnchorListView *view;
...
static void
anchor_list_view_sort_directories_first_changed (AnchorView *view)
{
	AnchorListView *list_view;

	list_view = NAUTILUS_LIST_VIEW (view);

	anchor_list_model_set_should_sort_directories_first (list_view->details->model,
							 anchor_view_should_sort_directories_first (view));
}

static int
anchor_list_view_compare_files (AnchorView *view, AnchorFile *file1, AnchorFile *file2)
{
	AnchorListView *list_view;

	list_view = NAUTILUS_LIST_VIEW (view);
	return anchor_list_model_compare_func (list_view->details->model, file1, file2);
}

static gboolean
anchor_list_view_using_manual_layout (AnchorView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_LIST_VIEW (view), FALSE);
...
static void
anchor_list_view_dispose (GObject *object)
{
	AnchorListView *list_view;
	...
	G_OBJECT_CLASS (anchor_list_view_parent_class)->dispose (object);
}

static void
anchor_list_view_finalize (GObject *object)
{
	AnchorListView *list_view;
	...
	g_signal_handlers_disconnect_by_func (anchor_preferences,
					      ...
					      list_view);
	g_signal_handlers_disconnect_by_func (anchor_list_view_preferences,
	...
	G_OBJECT_CLASS (anchor_list_view_parent_class)->finalize (object);
}

static char *
anchor_list_view_get_first_visible_file (AnchorView *view)
{
	AnchorFile *file;
	GtkTreePath *path;
	GtkTreeIter iter;
...
static void
anchor_list_view_scroll_to_file (AnchorListView *view,
				   AnchorFile *file)
{
	GtkTreePath *path;
	GtkTreeIter iter;
	
	if (!anchor_list_model_get_first_iter_for_file (view->details->model, file, &iter)) {
		return;
	}
...
static void
list_view_scroll_to_file (AnchorView *view,
			  const char *uri)
{
	AnchorFile *file;
	...
	if (uri != NULL) {
		...
		if (file != NULL) {
			anchor_list_view_scroll_to_file (NAUTILUS_LIST_VIEW (view), file);
			anchor_file_unref (file);
		}
	}
}
...
static void
list_view_notify_clipboard_info (AnchorClipboardMonitor *monitor,
                                 ...
                                 AnchorListView *view)
{
	...
	if (info != NULL && info->cut) {
		anchor_list_model_set_highlight_for_files (view->details->model, info->files);
	} else {
		anchor_list_model_set_highlight_for_files (view->details->model, NULL);
	}
}

static void
anchor_list_view_end_loading (AnchorView *view,
...
static const char *
anchor_list_view_get_id (AnchorView *view)
{
	return NAUTILUS_LIST_VIEW_ID;
}

static void
anchor_list_view_class_init (AnchorListViewClass *class)
{
	AnchorViewClass *anchor_view_class;

	anchor_view_class = NAUTILUS_VIEW_CLASS (class);

	G_OBJECT_CLASS (class)->dispose = anchor_list_view_dispose;
	G_OBJECT_CLASS (class)->finalize = anchor_list_view_finalize;

	anchor_view_class->add_file = anchor_list_view_add_file;
	anchor_view_class->begin_loading = anchor_list_view_begin_loading;
	anchor_view_class->end_loading = anchor_list_view_end_loading;
	anchor_view_class->bump_zoom_level = anchor_list_view_bump_zoom_level;
	anchor_view_class->can_zoom_in = anchor_list_view_can_zoom_in;
	anchor_view_class->can_zoom_out = anchor_list_view_can_zoom_out;
        anchor_view_class->click_policy_changed = anchor_list_view_click_policy_changed;
	anchor_view_class->clear = anchor_list_view_clear;
	anchor_view_class->file_changed = anchor_list_view_file_changed;
	anchor_view_class->get_backing_uri = anchor_list_view_get_backing_uri;
	anchor_view_class->get_selection = anchor_list_view_get_selection;
	anchor_view_class->get_selection_for_file_transfer = anchor_list_view_get_selection_for_file_transfer;
	anchor_view_class->is_empty = anchor_list_view_is_empty;
	anchor_view_class->remove_file = anchor_list_view_remove_file;
	anchor_view_class->merge_menus = anchor_list_view_merge_menus;
	anchor_view_class->unmerge_menus = anchor_list_view_unmerge_menus;
	anchor_view_class->update_menus = anchor_list_view_update_menus;
	anchor_view_class->reset_to_defaults = anchor_list_view_reset_to_defaults;
	anchor_view_class->restore_default_zoom_level = anchor_list_view_restore_default_zoom_level;
	anchor_view_class->reveal_selection = anchor_list_view_reveal_selection;
	anchor_view_class->select_all = anchor_list_view_select_all;
	anchor_view_class->select_first = anchor_list_view_select_first;
	anchor_view_class->set_selection = anchor_list_view_set_selection;
	anchor_view_class->invert_selection = anchor_list_view_invert_selection;
	anchor_view_class->compare_files = anchor_list_view_compare_files;
	anchor_view_class->sort_directories_first_changed = anchor_list_view_sort_directories_first_changed;
	anchor_view_class->start_renaming_file = anchor_list_view_start_renaming_file;
	anchor_view_class->get_zoom_level = anchor_list_view_get_zoom_level;
	anchor_view_class->zoom_to_level = anchor_list_view_zoom_to_level;
	anchor_view_class->end_file_changes = anchor_list_view_end_file_changes;
	anchor_view_class->using_manual_layout = anchor_list_view_using_manual_layout;
	anchor_view_class->get_view_id = anchor_list_view_get_id;
	anchor_view_class->get_first_visible_file = anchor_list_view_get_first_visible_file;
	anchor_view_class->scroll_to_file = list_view_scroll_to_file;
}

static void
anchor_list_view_init (AnchorListView *list_view)
{
	list_view->details = g_new0 (AnchorListViewDetails, 1);
	...
	g_signal_connect_swapped (anchor_preferences,
				  ...
				  G_CALLBACK (default_sort_order_changed_callback),
				  list_view);
	g_signal_connect_swapped (anchor_list_view_preferences,
	...
	anchor_list_view_click_policy_changed (NAUTILUS_VIEW (list_view));

	anchor_list_view_sort_directories_first_changed (NAUTILUS_VIEW (list_view));
	anchor_list_view_set_zoom_level (list_view, get_default_zoom_level (), TRUE);
...
AnchorView *
anchor_list_view_new (AnchorWindowSlot *slot)
{
	return g_object_new (ANCHOR_TYPE_LIST_VIEW,
			     "window-slot", slot,
			     NULL);
}

GtkTreeView*
anchor_list_view_get_tree_view (AnchorListView *list_view)
{
