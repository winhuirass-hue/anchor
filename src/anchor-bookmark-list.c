...
/* anchor-bookmark-list.c - implementation of centralized list of bookmarks.
 */
...
static void        anchor_bookmark_list_load_file     (AnchorBookmarkList *bookmarks);
static void        anchor_bookmark_list_save_file     (AnchorBookmarkList *bookmarks);

G_DEFINE_TYPE(AnchorBookmarkList, anchor_bookmark_list, G_TYPE_OBJECT)
...
new_bookmark_from_uri (const char *uri, const char *label)
{
	...
	if (location) {
		new_bookmark = anchor_bookmark_new (location, label);
...
static GFile *
anchor_bookmark_list_get_legacy_file (void)
{
	char *filename;
	GFile *file;
...
static GFile *
anchor_bookmark_list_get_file (void)
{
	char *filename;
	GFile *file;
...
static void
bookmark_in_list_changed_callback (AnchorBookmark     *bookmark,
				   AnchorBookmarkList *bookmarks)
{
	...
	anchor_bookmark_list_save_file (bookmarks);
}
...
static void
stop_monitoring_bookmark (AnchorBookmarkList *bookmarks,
...
static void
clear (AnchorBookmarkList *bookmarks)
{
...
do_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_bookmark_list_parent_class)->finalize (object);
}

static void
anchor_bookmark_list_class_init (AnchorBookmarkListClass *class)
{
	...
	signals[CHANGED] =
		g_signal_new ("changed",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorBookmarkListClass, 
					       changed),
		              NULL, NULL,
			     ...
			     gpointer           user_data)
{
	if (eflags == G_FILE_MONITOR_EVENT_CHANGED ||
		...
		anchor_bookmark_list_load_file (NAUTILUS_BOOKMARK_LIST (user_data));
	}
}

static void
anchor_bookmark_list_init (AnchorBookmarkList *bookmarks)
{
	GFile *file;

	bookmarks->pending_ops = g_queue_new ();

	anchor_bookmark_list_load_file (bookmarks);

	file = anchor_bookmark_list_get_file ();
...
static void
insert_bookmark_internal (AnchorBookmarkList *bookmarks,
			  AnchorBookmark     *bookmark,
			  int                   index)
{
...
/**
 * anchor_bookmark_list_append:
 *
 * Append a bookmark to a bookmark list.
 * @bookmarks: AnchorBookmarkList to append to.
 * @bookmark: Bookmark to append a copy of.
 **/
void
anchor_bookmark_list_append (AnchorBookmarkList *bookmarks, 
			       AnchorBookmark     *bookmark)
{
	...
	if (g_list_find_custom (bookmarks->list, bookmark,
				anchor_bookmark_compare_with) != NULL) {
		return;
	}
...
/**
 * anchor_bookmark_list_delete_item_at:
 * 
 ...
 **/
void
anchor_bookmark_list_delete_item_at (AnchorBookmarkList *bookmarks, 
				       guint                 index)
{
	GList *doomed;
...
/**
 * anchor_bookmark_list_move_item:
 *
 ...
 **/
void
anchor_bookmark_list_move_item (AnchorBookmarkList *bookmarks,
				  guint index,
				  guint destination)
{
	GList *bookmark_item;
...
/**
 * anchor_bookmark_list_insert_item:
 * 
 ...
 **/
void
anchor_bookmark_list_insert_item (AnchorBookmarkList *bookmarks,
				    AnchorBookmark     *new_bookmark,
				    guint                 index)
{
	g_return_if_fail (NAUTILUS_IS_BOOKMARK_LIST (bookmarks));
 ...
 **/
AnchorBookmark *
anchor_bookmark_list_item_at (AnchorBookmarkList *bookmarks, guint index)
{
...
/**
 * anchor_bookmark_list_item_with_location:
 *
 ...
 **/
AnchorBookmark *
anchor_bookmark_list_item_with_location (AnchorBookmarkList *bookmarks,
					   GFile                *location,
					   guint                *index)
{
	GList *node;
 ...
 **/
guint
anchor_bookmark_list_length (AnchorBookmarkList *bookmarks)
{
...
static void
process_next_op (AnchorBookmarkList *bookmarks);

static void
op_processed_cb (AnchorBookmarkList *self)
{
...
load_callback (GObject *source,
	       ...
	       gpointer user_data)
{
	AnchorBookmarkList *self = NAUTILUS_BOOKMARK_LIST (source);
	gchar *contents;
	char **lines;
	int i;
		...
		GCancellable *cancellable)
{
	...
	if (!g_file_query_exists (file, NULL)) {
		g_object_unref (file);
		file = anchor_bookmark_list_get_legacy_file ();
	}
...
static void
load_file_async (AnchorBookmarkList *self)
{
...
static void
save_file_async (AnchorBookmarkList *self)
{
	GSimpleAsyncResult *result;
	...
	for (l = self->list; l; l = l->next) {
		...
		if (anchor_bookmark_get_has_custom_name (bookmark)) {
			const char *label;
			char *uri;
			label = anchor_bookmark_get_name (bookmark);
...
/**
 * anchor_bookmark_list_save_file:
 * 
 * Save bookmarks to disk.
 * @bookmarks: the list of bookmarks to save.
 **/
static void
anchor_bookmark_list_save_file (AnchorBookmarkList *bookmarks)
{
	g_signal_emit (bookmarks, signals[CHANGED], 0);
...
gboolean
anchor_bookmark_list_can_bookmark_location (AnchorBookmarkList *list,
					      GFile                *location)
{
	AnchorBookmark *bookmark;
	gboolean is_builtin;

	if (anchor_bookmark_list_item_with_location (list, location, NULL)) {
		return FALSE;
	}
	...
	bookmark = anchor_bookmark_new (location, NULL);
	is_builtin = anchor_bookmark_get_is_builtin (bookmark);
 ...
 **/
AnchorBookmarkList *
anchor_bookmark_list_new (void)
{
	AnchorBookmarkList *list;

	list = NAUTILUS_BOOKMARK_LIST (g_object_new (ANCHOR_TYPE_BOOKMARK_LIST, NULL));
