...
/* anchor-mime-actions.c - uri-specific versions of mime action functions
...
typedef struct {
	...
	AnchorFileListHandle *files_handle;
...
{
	anchor_file_unref (location->file);
	g_free (location->uri);
	g_free (location);
}
...
launch_location_from_file (AnchorFile *file)
{
	LaunchLocation *location;
	location = g_new (LaunchLocation, 1);
	location->file = anchor_file_ref (file);
	location->uri = anchor_file_get_uri (file);
				 ...
				 const char *uri)
{
	anchor_file_unref (location->file);
	g_free (location->uri);
	location->file = anchor_file_get_by_uri (uri);
...
static GList*
filter_anchor_handler (GList *apps)
{
	GList *l, *next;
	GAppInfo *application;
	const char *id;
...
static gboolean
anchor_mime_actions_check_if_required_attributes_ready (AnchorFile *file)
{
	AnchorFileAttributes attributes;
	gboolean ready;

	attributes = anchor_mime_actions_get_required_file_attributes ();
	ready = anchor_file_check_if_ready (file, attributes);

	return ready;
}

AnchorFileAttributes 
anchor_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (AnchorFile *file)
{
	GFile *location;
	char *path;
	gboolean res;
...
GAppInfo *
anchor_mime_get_default_application_for_file (AnchorFile *file)
{
	GAppInfo *app;
	char *mime_type;
	char *uri_scheme;

	if (!anchor_mime_actions_check_if_required_attributes_ready (file)) {
		return NULL;
	}
	...
	if (app == NULL) {
		uri_scheme = anchor_file_get_uri_scheme (file);
		if (uri_scheme != NULL) {
...
static int
file_compare_by_mime_type (AnchorFile *file_a,
			   AnchorFile *file_b)
{
	char *mime_type_a, *mime_type_b;
	int ret;
	
	mime_type_a = anchor_file_get_mime_type (file_a);
	mime_type_b = anchor_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (AnchorFile *file_a,
			    AnchorFile *file_b) {
	char *parent_uri_a, *parent_uri_b;
	int ret;

	parent_uri_a = anchor_file_get_parent_uri (file_a);
	parent_uri_b = anchor_file_get_parent_uri (file_b);
...
GList *
anchor_mime_get_applications_for_file (AnchorFile *file)
{
	char *mime_type;
	char *uri_scheme;
	GList *result;
	...
	return filter_anchor_handler (result);
}

GAppInfo *
anchor_mime_get_default_application_for_files (GList *files)
{
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_app = anchor_mime_get_default_application_for_file (file);
...
GList *
anchor_mime_get_applications_for_files (GList *files)
{
	GList *l, *sorted_files;
	AnchorFile *file;
	GList *one_ret, *ret;
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_ret = anchor_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, AnchorFile *file)
{
	char *target_path;
	char *display_name;
	char *prompt;
	...
	can_trash = anchor_file_can_trash (file) && !anchor_file_is_in_trash (file);
	...
	target_path = anchor_file_get_symbolic_link_target_path (file);
	if (target_path == NULL) {
...
get_executable_text_file_action (GtkWindow *parent_window, AnchorFile *file)
{
	GtkDialog *dialog;
	char *file_name;
	char *prompt;
	...
	g_assert (anchor_file_contains_text (file));

	preferences_value = g_settings_get_enum	(anchor_preferences,
	...
	file_name = anchor_file_get_display_name (file);
...
gboolean
anchor_mime_file_opens_in_view (AnchorFile *file)
{
  return (anchor_file_is_directory (file) ||
	  NAUTILUS_IS_DESKTOP_ICON_FILE (file));
}

static ActivationAction
get_activation_action (AnchorFile *file)
{
	...
	if (activation_uri == NULL) {
		activation_uri = anchor_file_get_uri (file);
	}
	...
	if (anchor_file_is_launchable (file)) {
		...
		if (!executable_path) {
			action = ACTIVATION_ACTION_DO_NOTHING;
		} else if (anchor_file_contains_text (file)) {
	...
	if (action == ACTIVATION_ACTION_DO_NOTHING) {
		if (anchor_mime_file_opens_in_view (file)) {
...
gboolean
anchor_mime_file_launches (AnchorFile *file)
{
...
gboolean
anchor_mime_file_opens_in_external_app (AnchorFile *file)
{
  ActivationAction activation_action;
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of AnchorFiles,
			    ...
			    GList **unhandled_uris)
{
	...
	*unhandled_uris = NULL;
	...
	for (l = uris; l != NULL; l = l->next) {
		...
		app = anchor_mime_get_default_application_for_file (file);
		if (app != NULL) {
			app_uris = NULL;
...
static gboolean
file_was_cancelled (AnchorFile *file)
{
	GError *error;
	
	error = anchor_file_get_file_info_error (file);
	return
		error != NULL &&
...
static gboolean
file_was_not_mounted (AnchorFile *file)
{
	GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
	...
	anchor_file_list_free (parameters->mountables);
	anchor_file_list_free (parameters->start_mountables);
	anchor_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
	...
	if (parameters->files_handle) {
		anchor_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_install_free (ActivateParametersInstall *parameters_install)
{
	...
	anchor_file_unref (parameters_install->file);
	anchor_file_list_free (parameters_install->files);
...
static char *
get_application_no_mime_type_handler_message (AnchorFile *file, char *uri)
{
...
choose_program (GtkDialog *message_dialog, int response, gpointer callback_data)
{
	...
	g_object_set_data_full (G_OBJECT (dialog), 
				"mime-action:file",
				anchor_file_ref (file),
				(GDestroyNotify)anchor_file_unref);
...
show_unhandled_type_error (ActivateParametersInstall *parameters)
{
	...
	char *mime_type = anchor_file_get_mime_type (parameters->file);
	...
	g_object_set_data_full (G_OBJECT (dialog), 
				"mime-action:file",
				anchor_file_ref (parameters->file),
				(GDestroyNotify)anchor_file_unref);
					    ...
					    gpointer      user_data)
{
	...
	anchor_mime_activate_files (parameters_install->parent_window,
                                    ...
                                    ActivateParametersInstall *parameters_install)
{
	...
	if (response_id == GTK_RESPONSE_YES) {
		mime_type = anchor_file_get_mime_type (parameters_install->file);
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
	...
	show_install_mime = g_settings_get_boolean (anchor_preferences, NAUTILUS_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
...
activate_parameters_desktop_free (ActivateParametersDesktop *parameters_desktop)
{
	...
	anchor_file_unref (parameters_desktop->file);
				      ...
				      ActivateParametersDesktop *parameters)
{
	...
	case RESPONSE_RUN:
		...
		uri = anchor_file_get_uri (parameters->file);
		DEBUG ("Launching untrusted launcher %s", uri);
		anchor_launch_desktop_file (screen, uri, NULL,
					      parameters->parent_window);
		g_free (uri);
		break;
	case RESPONSE_MARK_TRUSTED:
		file = anchor_file_get_location (parameters->file);
		anchor_file_mark_desktop_file_trusted (file,
							 parameters->parent_window,
							 TRUE, 
							 NULL, NULL);
		g_object_unref (file);
		       ...
		       AnchorFile *file)
{
	...
	if (!anchor_file_is_trusted_link (file)) {
		...
		parameters_desktop->file = anchor_file_ref (file);
...
activate_files (ActivateParameters *parameters)
{
	...
	for (l = launch_files; l != NULL; l = l->next) {
		...
		anchor_launch_application_from_command (screen, quoted_path, FALSE, NULL);
		g_free (quoted_path);
		g_free (executable_path);
		g_free (uri);
	...
	for (l = launch_in_terminal_files; l != NULL; l = l->next) {
		...
		anchor_launch_application_from_command (screen, quoted_path, TRUE, NULL);
	...
	if (parameters->slot != NULL &&
		...
		if ((flags & NAUTILUS_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
		    g_settings_get_enum (anchor_preferences, NAUTILUS_PREFERENCES_NEW_TAB_POSITION) ==
		...
		for (l = open_in_view_files; l != NULL; l = l->next) {
			...
			anchor_window_slot_open_location (parameters->slot, f, flags);
			g_object_unref (f);
			g_free (uri);
		}
	}
	...
	if (open_files) {
		for (l = open_in_app_parameters; l != NULL; l = l->next) {
			...
			anchor_launch_application_by_uri (one_parameters->application,
							    one_parameters->uris,
							    parameters->parent_window);
	...
	window = NULL;
	if (parameters->slot != NULL) {
		window = anchor_window_slot_get_window (parameters->slot);
	}
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		loc = l->data;
		next = l->next;
		anchor_file_invalidate_all_attributes (loc->file);
	}
	
	files = get_file_list_for_launch_locations (parameters->locations);
	anchor_file_list_call_when_ready
		(files,
		 anchor_mime_actions_get_required_file_attributes (),
					 ...
					 gpointer callback_data)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		...
		if (anchor_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
		    !anchor_file_has_activation_uri (file)) {
	...
	for (l = parameters->locations; l != NULL; l = l->next) {
		 ...
		 */
		uri = anchor_file_get_activation_uri (location->file);
		if (uri != NULL) {
...
activation_get_activation_uris (ActivateParameters *parameters)
{
	...
	files = get_file_list_for_launch_locations (parameters->locations);
	anchor_file_list_call_when_ready
		(files, anchor_mime_actions_get_required_file_attributes (),
		 &parameters->files_handle,
...
static void
activation_mountable_mounted (AnchorFile  *file,
			      ...
			      gpointer       callback_data)
{
	...
	if (error == NULL) {
		...
		target_file = anchor_file_get (result_location);
...
static void
activation_mountable_started (AnchorFile  *file,
			      GFile         *gfile_of_file,
			      GError        *error,
			      gpointer       callback_data)
{
 ...
 * @files: A GList of AnchorFiles to activate.
 * 
 **/
void
anchor_mime_activate_files (GtkWindow *parent_window,
			      AnchorWindowSlot *slot,
			      GList *files,
			      ...
			      gboolean user_confirmation)
{
	...
	if (file_count == 1) {
		file_name = anchor_file_get_display_name (files->data);
 ...
 * @view: FMDirectoryView in question.
 * @file: A AnchorFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 * 
 **/

void
anchor_mime_activate_file (GtkWindow *parent_window,
			     AnchorWindowSlot *slot,
			     AnchorFile *file,
			     const char *launch_directory,
			     AnchorWindowOpenFlags flags)
{
	...
	anchor_mime_activate_files (parent_window, slot, files, launch_directory, flags, FALSE);
	g_list_free (files);
}
