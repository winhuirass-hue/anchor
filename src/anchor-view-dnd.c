...
/*
 * anchor-view-dnd.c: DnD helpers for AnchorView
 *
...
static inline void
view_widget_to_file_operation_position (AnchorView *view,
                                        GdkPoint *position)
{
	AnchorViewClass *class = NAUTILUS_VIEW_GET_CLASS (view);
...
static void
view_widget_to_file_operation_position_xy (AnchorView *view,
                                           int *x, int *y)
{
	GdkPoint position;
				  ...
				  gpointer user_data)
{
	...
	anchor_link_local_create (data->target_uri,
				    link_name,
				    link_display_name,
				    icon_name,
				    data->url,
...
void
anchor_view_handle_netscape_url_drop (AnchorView  *view,
                                        ...
                                        int            y)
{
	...
	if (action == GDK_ACTION_LINK) {
		...
		anchor_view_move_copy_items (view, uri_list, points,
                                               target_uri,
                                               action, x, y);
...
void
anchor_view_handle_uri_list_drop (AnchorView  *view,
                                    ...
                                    int            y)
{
	...
	container_uri = NULL;
	if (target_uri == NULL) {
		container_uri = anchor_view_get_backing_uri (view);
		g_assert (container_uri != NULL);
	}

	if (action == GDK_ACTION_ASK) {
		action = anchor_drag_drop_action_ask
			(GTK_WIDGET (view),
	...
	anchor_view_move_copy_items (view, real_uri_list, points,
...
void
anchor_view_handle_text_drop (AnchorView  *view,
                                const char    *text,
                                const char    *target_uri,
                                GdkDragAction  action,
                                int            x,
                                int            y)
{
	...
	anchor_view_new_file_with_initial_contents (view,
						      target_uri != NULL ? target_uri : container_uri,
						      filename,
						      text,
						      length,
...
void
anchor_view_handle_raw_drop (AnchorView *view,
                               const char   *raw_data,
                               int           length,
                               const char   *target_uri,
                               ...
                               int           y)
{
	...
	anchor_view_new_file_with_initial_contents (
...
void 
anchor_view_drop_proxy_received_uris (AnchorView *view,
					...
					GdkDragAction action)
{
	...
	anchor_view_move_copy_items (view, source_uri_list, NULL,
				       target_uri != NULL ? target_uri : container_uri,
				       action, 0, 0);
...
void
anchor_view_handle_hover (AnchorView *view,
			    const char   *target_uri)
{
	...
	current_location = anchor_window_slot_get_location (slot);
