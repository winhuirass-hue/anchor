...
/* anchor-convert-metadata.c - Convert old metadata format to gvfs metadata.
 *
...
static char *
get_uri_from_anchor_metafile_name (const char *filename)
{
	GString *s;
	char c;
	char *base_name, *p;
...
convert_anchor_file (char *file)
{
	...
	uri = get_uri_from_anchor_metafile_name (file);
	if (uri == NULL) {
		g_free (contents);
		return;
	}
...
main (int argc, char *argv[])
{
	...
	context = g_option_context_new ("<anchor metadata files> - convert anchor metadata");
