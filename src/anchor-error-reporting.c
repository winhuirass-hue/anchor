...
/* anchor-error-reporting.h - implementation of file manager functions that report
 	                        errors to the user.
...
#define NEW_NAME_TAG "Anchor: new name"
#define MAXIMUM_DISPLAYED_FILE_NAME_LENGTH	50

static void finish_rename (AnchorFile *file, gboolean stop_timer, GError *error);

void
anchor_report_error_loading_directory (AnchorFile *file,
					 GError *error,
...
void
anchor_report_error_setting_group (AnchorFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
	...
	if (message == NULL) {
		...
		g_warning ("Hit unhandled case %s:%d in anchor_report_error_setting_group", 
...
void
anchor_report_error_setting_owner (AnchorFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
...
void
anchor_report_error_setting_permissions (AnchorFile *file,
					   GError *error,
					   GtkWindow *parent_window)
{
	char *file_name;
...
void
anchor_report_error_renaming_file (AnchorFile *file,
				     const char *new_name,
				     GError *error,
				     GtkWindow *parent_window)
{
	 ...
	 */
	original_name = anchor_file_get_display_name (file);
	...
	if (message == NULL) {
		/* We should invent decent error messages for every case we actually experience. */
		g_warning ("Hit unhandled case %s:%d in anchor_report_error_renaming_file", 
			   g_quark_to_string (error->domain), error->code);
		/* fall through */
...
static void
anchor_rename_data_free (AnchorRenameData *data)
{
	g_free (data->name);
	g_free (data);
}
...
static void
rename_callback (AnchorFile *file, GFile *result_location,
		 GError *error, gpointer callback_data)
{
	AnchorRenameData *data;
	...
	if (error) {
	  if (!(error->domain == G_IO_ERROR && error->code == G_IO_ERROR_CANCELLED)) {
		...
		anchor_report_error_renaming_file (file, data->name, error, NULL);
	  } else {
		cancelled = TRUE;
	  }
	}
...
{
	anchor_file_cancel (NAUTILUS_FILE (callback_data), rename_callback, NULL);
}

static void
finish_rename (AnchorFile *file, gboolean stop_timer, GError *error)
{
	AnchorRenameData *data;
	...
	anchor_file_cancel (file, rename_callback, NULL);
	if (stop_timer) {
...
void
anchor_rename_file (AnchorFile *file,
		      const char *new_name,
		      AnchorFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	data = g_new0 (AnchorRenameData, 1);
	...
	g_object_set_data_full (G_OBJECT (file),
				NEW_NAME_TAG,
				data, (GDestroyNotify)anchor_rename_data_free);
