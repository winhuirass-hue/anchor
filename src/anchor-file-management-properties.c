...
/* anchor-file-management-properties.c - Functions to create and show the anchor preference dialog.
...
static void
anchor_file_management_properties_size_group_create (GtkBuilder *builder,
						       char *prefix,
						       int items)
{
...
static void
columns_changed_callback (AnchorColumnChooser *chooser,
			  gpointer callback_data)
{
	char **visible_columns;
	char **column_order;

	anchor_column_chooser_get_settings (NAUTILUS_COLUMN_CHOOSER (chooser),
					      &visible_columns,
					      &column_order);

	g_settings_set_strv (anchor_list_view_preferences,
				...
				gpointer user_data)
{
	...
	g_settings_set_strv (anchor_icon_view_preferences,
...
update_icon_captions_from_settings (GtkBuilder *builder)
{
	...
	captions = g_settings_get_strv (anchor_icon_view_preferences, NAUTILUS_PREFERENCES_ICON_VIEW_CAPTIONS);
	if (captions == NULL)
		return;
...
static void
anchor_file_management_properties_dialog_setup_icon_caption_page (GtkBuilder *builder)
{
	GList *columns;
	int i;
	gboolean writable;

	writable = g_settings_is_writable (anchor_icon_view_preferences,
					   NAUTILUS_PREFERENCES_ICON_VIEW_CAPTIONS);

	columns = anchor_get_common_columns ();
	...
	anchor_column_list_free (columns);

	update_icon_captions_from_settings (builder);
}

static void
set_columns_from_settings (AnchorColumnChooser *chooser)
{
	char **visible_columns;
	char **column_order;

	visible_columns = g_settings_get_strv (anchor_list_view_preferences,
					       NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
	column_order = g_settings_get_strv (anchor_list_view_preferences,
					    NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER);

	anchor_column_chooser_set_settings (NAUTILUS_COLUMN_CHOOSER (chooser),
					      visible_columns,
					      column_order);
...
static void
use_default_callback (AnchorColumnChooser *chooser,
		      gpointer user_data)
{
	g_settings_reset (anchor_list_view_preferences,
			  NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
	g_settings_reset (anchor_list_view_preferences,
			  NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER);
...
static void
anchor_file_management_properties_dialog_setup_list_column_page (GtkBuilder *builder)
{
	GtkWidget *chooser;
	GtkWidget *box;

	chooser = anchor_column_chooser_new (NULL);
...
static  void
anchor_file_management_properties_dialog_setup (GtkBuilder *builder, GtkWindow *window)
{
	GtkWidget *dialog;

	/* setup UI */
	anchor_file_management_properties_size_group_create (builder,
							       "views_label",
							       4);
	anchor_file_management_properties_size_group_create (builder,
							       "captions_label",
	...
	/* setup preferences */
	bind_builder_bool (builder, anchor_preferences,
	...
	bind_builder_radio (builder, anchor_preferences,
	...
	bind_builder_uint_enum (builder, anchor_preferences,
	...
	anchor_file_management_properties_dialog_setup_icon_caption_page (builder);
	anchor_file_management_properties_dialog_setup_list_column_page (builder);
...
void
anchor_file_management_properties_dialog_show (GtkWindow *window)
{
	GtkBuilder *builder;
	...
	gtk_builder_add_from_resource (builder,
				       "/org/gnome/anchor/anchor-file-management-properties.ui",
				       NULL);

	anchor_file_management_properties_dialog_setup (builder, window);
