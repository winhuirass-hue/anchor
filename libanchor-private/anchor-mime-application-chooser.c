...
/*
 *  anchor-mime-application-chooser.c: an mime-application chooser
 *
...
#include <config.h>
#include "anchor-mime-application-chooser.h"
...
G_DEFINE_TYPE (AnchorMimeApplicationChooser, anchor_mime_application_chooser, GTK_TYPE_BOX);

static void
add_clicked_cb (GtkButton *button,
		gpointer user_data)
{
	AnchorMimeApplicationChooser *chooser = user_data;
	GAppInfo *info;
	gchar *message;
	GError *error = NULL;
...
reset_clicked_cb (GtkButton *button,
                  gpointer   user_data)
{
	AnchorMimeApplicationChooser *chooser;
...
static void
anchor_mime_application_chooser_apply_labels (AnchorMimeApplicationChooser *chooser)
{
...
static void
anchor_mime_application_chooser_build_ui (AnchorMimeApplicationChooser *chooser)
{
...
static void
anchor_mime_application_chooser_init (AnchorMimeApplicationChooser *chooser)
{
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE (chooser, ANCHOR_TYPE_MIME_APPLICATION_CHOOSER,
							AnchorMimeApplicationChooserDetails);
...
static void
anchor_mime_application_chooser_constructed (GObject *object)
{
	AnchorMimeApplicationChooser *chooser = NAUTILUS_MIME_APPLICATION_CHOOSER (object);

	if (G_OBJECT_CLASS (anchor_mime_application_chooser_parent_class)->constructed != NULL)
		G_OBJECT_CLASS (anchor_mime_application_chooser_parent_class)->constructed (object);

	anchor_mime_application_chooser_build_ui (chooser);
	anchor_mime_application_chooser_apply_labels (chooser);
}

static void
anchor_mime_application_chooser_finalize (GObject *object)
{
	AnchorMimeApplicationChooser *chooser;
	...
	anchor_file_list_free (chooser->details->files);

	G_OBJECT_CLASS (anchor_mime_application_chooser_parent_class)->finalize (object);
}

static void
anchor_mime_application_chooser_get_property (GObject *object,
						guint property_id,
						GValue *value,
						GParamSpec *pspec)
{
...
static void
anchor_mime_application_chooser_set_property (GObject *object,
						guint property_id,
						const GValue *value,
						GParamSpec *pspec)
{
	...
	case PROP_CONTENT_TYPE:
		...
		break;
	case PROP_FILES:
		chooser->details->files = anchor_file_list_copy (g_value_get_pointer (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
...
static void
anchor_mime_application_chooser_class_init (AnchorMimeApplicationChooserClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	gobject_class->set_property = anchor_mime_application_chooser_set_property;
	gobject_class->get_property = anchor_mime_application_chooser_get_property;
	gobject_class->finalize = anchor_mime_application_chooser_finalize;
	gobject_class->constructed = anchor_mime_application_chooser_constructed;
	...
	g_type_class_add_private (class, sizeof (AnchorMimeApplicationChooserDetails));
}

GtkWidget *
anchor_mime_application_chooser_new (GList *files,
				       const char *mime_type)
{
	GtkWidget *chooser;

	chooser = g_object_new (ANCHOR_TYPE_MIME_APPLICATION_CHOOSER,
				"files", files,
