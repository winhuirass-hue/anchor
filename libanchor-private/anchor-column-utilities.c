...
get_builtin_columns (void)
{
	...
	columns = g_list_append (NULL,
				 g_object_new (ANCHOR_TYPE_COLUMN,
					       "name", "name",
					       "attribute", "name",
					       "label", _("Name"),
...
get_extension_columns (void)
{
	...
	providers = anchor_module_get_extensions_for_type (ANCHOR_TYPE_COLUMN_PROVIDER);
	
	columns = NULL;
	
	for (l = providers; l != NULL; l = l->next) {
		AnchorColumnProvider *provider;
		GList *provider_columns;
		
		provider = NAUTILUS_COLUMN_PROVIDER (l->data);
		provider_columns = anchor_column_provider_get_columns (provider);
...
get_trash_columns (void)
{
	...
	return anchor_column_list_copy (columns);
}
...
GList *
anchor_get_common_columns (void)
{
	static GList *columns = NULL;
...
GList *
anchor_get_all_columns (void)
{
	GList *columns = NULL;

	columns = g_list_concat (anchor_get_common_columns (),
	                         get_trash_columns ());
...
GList *
anchor_get_columns_for_file (AnchorFile *file)
{
	GList *columns;
...
GList *
anchor_column_list_copy (GList *columns) 
{
	GList *ret;
	GList *l;
...
void
anchor_column_list_free (GList *columns)
{
	GList *l;
...
static int
column_compare (AnchorColumn *a, AnchorColumn *b, char **column_order)
{
	int index_a;
	int index_b;
	char *name_a;
...
GList *
anchor_sort_columns (GList  *columns, 
		       char  **column_order)
{
