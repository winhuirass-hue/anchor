/*
 * anchor-debug: debug loggers for anchor
 *
...
static void
anchor_debug_set_flags_from_env ()
{
  guint nkeys;
  ...
  if (flags_string)
    anchor_debug_set_flags (g_parse_debug_string (flags_string, keys, nkeys));

  initialized = TRUE;
}

void
anchor_debug_set_flags (DebugFlags new_flags)
{
  flags |= new_flags;
  initialized = TRUE;
}
...
gboolean
anchor_debug_flag_is_set (DebugFlags flag)
{
  return flag & flags;
}

void
anchor_debug (DebugFlags flag,
                const gchar *format,
                ...)
{
  va_list args;
  va_start (args, format);
  anchor_debug_valist (flag, format, args);
  va_end (args);
}

void
anchor_debug_valist (DebugFlags flag,
                       const gchar *format,
                       va_list args)
{
...
static void
anchor_debug_files_valist (DebugFlags flag,
                             GList *files,
                             const gchar *format,
                             va_list args)
{
...
void
anchor_debug_files (DebugFlags flag,
                      GList *files,
                      const gchar *format,
                      ...)
{
  ...
  va_start (args, format);
  anchor_debug_files_valist (flag, files, format, args);
  va_end (args);
}
