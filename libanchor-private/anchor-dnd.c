...
/* anchor-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
#include <config.h>
#include "anchor-dnd.h"

#include "anchor-program-choosing.h"
...
void
anchor_drag_init (AnchorDragInfo     *drag_info,
...
void
anchor_drag_finalize (AnchorDragInfo *drag_info)
{
	gtk_target_list_unref (drag_info->target_list);
	anchor_drag_destroy_selection_list (drag_info->selection_list);
	anchor_drag_destroy_selection_list (drag_info->selection_cache);

	g_free (drag_info);
}


/* Functions to deal with AnchorDragSelectionItems.  */

AnchorDragSelectionItem *
anchor_drag_selection_item_new (void)
{
	return g_new0 (AnchorDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (AnchorDragSelectionItem *item)
{
...
void
anchor_drag_destroy_selection_list (GList *list)
{
	GList *p;
...
char **
anchor_drag_uri_array_from_selection_list (const GList *selection_list)
{
	GList *uri_list;
	char **uris;

	uri_list = anchor_drag_uri_list_from_selection_list (selection_list);
	uris = anchor_drag_uri_array_from_list (uri_list);
...
GList *
anchor_drag_uri_list_from_selection_list (const GList *selection_list)
{
	AnchorDragSelectionItem *selection_item;
	GList *uri_list;
	const GList *l;

	uri_list = NULL;
	for (l = selection_list; l != NULL; l = l->next) {
		selection_item = (AnchorDragSelectionItem *) l->data;
...
char **
anchor_drag_uri_array_from_list (const GList *uri_list)
{
	const GList *l;
	char **uris;
	int i;
...
GList *
anchor_drag_uri_list_from_array (const char **uris)
{
	GList *uri_list;
	int i;
...
GList *
anchor_drag_build_selection_list (GtkSelectionData *data)
{
	GList *result;
	const guchar *p, *oldp;
	int size;
	...
	while (size > 0) {
		...
		item->uri[len] = 0;
		item->file = anchor_file_get_existing_by_uri (item->uri);
...
static gboolean
anchor_drag_file_local_internal (const char *target_uri_string,
...
gboolean
anchor_drag_uris_local (const char *target_uri,
			  const GList *source_uri_list)
{
	/* must have at least one item */
	g_assert (source_uri_list);
	
	return anchor_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
anchor_drag_items_local (const char *target_uri_string,
			   const GList *selection_list)
{
	/* must have at least one item */
	g_assert (selection_list);

	return anchor_drag_file_local_internal (target_uri_string,
						  ((AnchorDragSelectionItem *)selection_list->data)->uri);
}

gboolean
anchor_drag_items_in_trash (const GList *selection_list)
{
	 ...
	 * and currently dropped items can only be from the same container
	 */
	return eel_uri_is_trash (((AnchorDragSelectionItem *)selection_list->data)->uri);
}

gboolean
anchor_drag_items_on_desktop (const GList *selection_list)
{
	char *uri;
	 ...
	 * and currently dropped items can only be from the same container
	 */
	uri = ((AnchorDragSelectionItem *)selection_list->data)->uri;
	if (eel_uri_is_desktop (uri)) {
		return TRUE;
	}
	
	desktop = anchor_get_desktop_location ();
...
GdkDragAction
anchor_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
...
static gboolean
check_same_fs (AnchorFile *file1,
	       AnchorFile *file2)
{
	char *id1, *id2;
	gboolean result;
	...
	if (file1 != NULL && file2 != NULL) {
		id1 = anchor_file_get_filesystem_id (file1);
		id2 = anchor_file_get_filesystem_id (file2);
...
{
	AnchorFile *naut_file;
	gboolean ret;

	/* if there's no a cached AnchorFile, it returns NULL */
	naut_file = anchor_file_get_existing (file);
	if (naut_file == NULL) {
		return FALSE;
	}
	
	ret = anchor_file_can_delete (naut_file);
	anchor_file_unref (naut_file);

	return ret;
}

void
anchor_drag_default_drop_action_for_icons (GdkDragContext *context,
					     ...
					     int *action)
{
	...
	AnchorFile *dropped_file, *target_file;
	...
	dropped_uri = ((AnchorDragSelectionItem *)items->data)->uri;
	dropped_file = ((AnchorDragSelectionItem *)items->data)->file;
	target_file = anchor_file_get_existing_by_uri (target_uri_string);
	...
	if (eel_uri_is_trash (target_uri_string)) {
	...
	} else if (dropped_file != NULL && anchor_file_is_launcher (dropped_file)) {
		if (actions & GDK_ACTION_MOVE) {
			*action = GDK_ACTION_MOVE;
		}
		anchor_file_unref (target_file);
		...
		target = anchor_get_desktop_location ();

		anchor_file_unref (target_file);
		target_file = anchor_file_get (target);
	...
	} else if (target_file != NULL && anchor_file_is_archive (target_file)) {
...
GdkDragAction
anchor_drag_default_drop_action_for_uri_list (GdkDragContext *context,
		...
		gpointer data)
{
	...
	item->file = anchor_file_get_existing_by_uri (uri);
	item->icon_x = x;
	item->icon_y = y;
	item->icon_width = w;
	item->icon_height = h;
...
GList *
anchor_drag_create_selection_cache (gpointer container_context,
...
gboolean
anchor_drag_drag_data_get_from_cache (GList *cache,
					...
					guint32 time)
{
	GList *l;
	GString *result;
	AnchorDragEachSelectedItemDataGet func;
	...
	for (l = cache; l != NULL; l = l->next) {
		AnchorDragSelectionItem *item = l->data;
...
gboolean
anchor_drag_drag_data_get (GtkWidget *widget,
			GdkDragContext *context,
			GtkSelectionData *selection_data,
			guint info,
			guint32 time,
...
GdkDragAction
anchor_drag_drop_action_ask (GtkWidget *widget,
			       GdkDragAction actions)
{
	GtkWidget *menu;
	GtkWidget *menu_item;
...
gboolean
anchor_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
anchor_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
anchor_drag_autoscroll_start (AnchorDragInfo *drag_info,
				GtkWidget        *widget,
				GSourceFunc       callback,
				gpointer          user_data)
{
	if (anchor_drag_autoscroll_in_scroll_region (widget)) {
...
void
anchor_drag_autoscroll_stop (AnchorDragInfo *drag_info)
{
	if (drag_info->auto_scroll_timeout_id != 0) {
		g_source_remove (drag_info->auto_scroll_timeout_id);
		drag_info->auto_scroll_timeout_id = 0;
...
gboolean
anchor_drag_selection_includes_special_link (GList *selection_list)
{
	GList *node;
	char *uri;

	for (node = selection_list; node != NULL; node = node->next) {
		uri = ((AnchorDragSelectionItem *) node->data)->uri;
