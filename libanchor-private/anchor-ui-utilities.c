...
/* anchor-ui-utilities.c - helper functions for GtkUIManager stuff
...
void
anchor_ui_unmerge_ui (GtkUIManager *ui_manager,
			guint *merge_id,
...
void
anchor_ui_prepare_merge_ui (GtkUIManager *ui_manager,
			      const char *name,
			      guint *merge_id,
			      GtkActionGroup **action_group)
{
...
{
	anchor_menu_item_activate (ANCHOR_MENU_ITEM (callback_data));
}

GtkAction *
anchor_action_from_menu_item (AnchorMenuItem *item,
				GtkWidget        *parent_widget)
{
	...
	if (icon_name != NULL) {
		pixbuf = anchor_ui_get_menu_icon (icon_name, parent_widget);
		if (pixbuf != NULL) {
...
GdkPixbuf *
anchor_ui_get_menu_icon (const char *icon_name,
			   GtkWidget  *parent_widget)
{
	AnchorIconInfo *info;
	GdkPixbuf *pixbuf;
	...
	if (g_path_is_absolute (icon_name)) {
		info = anchor_icon_info_lookup_from_path (icon_name, size, scale);
	} else {
		info = anchor_icon_info_lookup_from_name (icon_name, size, scale);
	}
...
char *
anchor_escape_action_name (const char *action_name,
			     const char *prefix)
{
	GString *s;
...
static GdkPixbuf *
anchor_get_thumbnail_frame (void)
{
	static GdkPixbuf *thumbnail_frame = NULL;

	if (thumbnail_frame == NULL) {
		thumbnail_frame = gdk_pixbuf_new_from_resource ("/org/gnome/anchor/icons/thumbnail_frame.png", NULL);
	}
...
void
anchor_ui_frame_image (GdkPixbuf **pixbuf)
{
	GdkPixbuf *pixbuf_with_frame, *frame;
	int left_offset, top_offset, right_offset, bottom_offset;

	frame = anchor_get_thumbnail_frame ();
	if (frame == NULL) {
		return;
	}
...
ensure_filmholes (void)
{
	if (filmholes_left == NULL) {
		filmholes_left = gdk_pixbuf_new_from_resource ("/org/gnome/anchor/icons/filmholes.png", NULL);
	}
...
void
anchor_ui_frame_video (GdkPixbuf **pixbuf)
{
	int width, height;
