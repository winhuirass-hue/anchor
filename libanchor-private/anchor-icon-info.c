...
#include "anchor-default-file-icon.h"
#include <gtk/gtk.h>
#include <gio/gio.h>
...
G_DEFINE_TYPE (AnchorIconInfo,
	       anchor_icon_info,
	       G_TYPE_OBJECT);

static void
anchor_icon_info_init (AnchorIconInfo *icon)
{
...
gboolean
anchor_icon_info_is_fallback (AnchorIconInfo  *icon)
{
...
static void
anchor_icon_info_finalize (GObject *object)
{
        AnchorIconInfo *icon;
	...
	g_free (icon->icon_name);
        ...
        G_OBJECT_CLASS (anchor_icon_info_parent_class)->finalize (object);
}

static void
anchor_icon_info_class_init (AnchorIconInfoClass *icon_info_class)
{
        GObjectClass *gobject_class;

        gobject_class = (GObjectClass *) icon_info_class;

        gobject_class->finalize = anchor_icon_info_finalize;

}

AnchorIconInfo *
anchor_icon_info_new_for_pixbuf (GdkPixbuf *pixbuf,
				   gint       scale)
{
	AnchorIconInfo *icon;

	icon = g_object_new (ANCHOR_TYPE_ICON_INFO, NULL);
...
anchor_icon_info_new_for_icon_info (GtkIconInfo *icon_info,
				      gint         scale)
{
	AnchorIconInfo *icon;
	GdkPoint *points;
...
void
anchor_icon_info_clear_caches (void)
{
	if (loadable_icon_cache) {
...
AnchorIconInfo *
anchor_icon_info_lookup (GIcon *icon,
			   int size,
			   int scale)
{
	...
	if (G_IS_LOADABLE_ICON (icon)) {
		...
		icon_info = anchor_icon_info_new_for_pixbuf (pixbuf, scale);
		...
		if (gtkicon_info == NULL) {
			return anchor_icon_info_new_for_pixbuf (NULL, scale);
		}
		...
		icon_info = anchor_icon_info_new_for_icon_info (gtkicon_info, scale);
...
AnchorIconInfo *
anchor_icon_info_lookup_from_name (const char *name,
				     int size,
				     int scale)
{
	GIcon *icon;
	...
	info = anchor_icon_info_lookup (icon, size, scale);
	g_object_unref (icon);
	return info;
}

AnchorIconInfo *
anchor_icon_info_lookup_from_path (const char *path,
				     int size,
				     int scale)
{
	GFile *icon_file;
...
GdkPixbuf *
anchor_icon_info_get_pixbuf_nodefault (AnchorIconInfo  *icon)
{
	GdkPixbuf *res;
...
GdkPixbuf *
anchor_icon_info_get_pixbuf (AnchorIconInfo *icon)
{
	GdkPixbuf *res;

	res = anchor_icon_info_get_pixbuf_nodefault (icon);
	if (res == NULL) {
		res = gdk_pixbuf_new_from_data (anchor_default_file_icon,
						GDK_COLORSPACE_RGB,
						TRUE,
						8,
						anchor_default_file_icon_width,
						anchor_default_file_icon_height,
						anchor_default_file_icon_width * 4, /* stride */
...
GdkPixbuf *
anchor_icon_info_get_pixbuf_nodefault_at_size (AnchorIconInfo  *icon,
						 gsize              forced_size)
{
	...
	pixbuf = anchor_icon_info_get_pixbuf_nodefault (icon);
...
GdkPixbuf *
anchor_icon_info_get_pixbuf_at_size (AnchorIconInfo  *icon,
				       gsize              forced_size)
{
	GdkPixbuf *pixbuf, *scaled_pixbuf;
	int w, h, s;
	...
	pixbuf = anchor_icon_info_get_pixbuf (icon);
...
gboolean
anchor_icon_info_get_embedded_rect (AnchorIconInfo  *icon,
...
gboolean
anchor_icon_info_get_attach_points (AnchorIconInfo  *icon,
				      GdkPoint         **points,
				      gint              *n_points)
{
...
const char *
anchor_icon_info_get_display_name   (AnchorIconInfo  *icon)
{
	return icon->display_name;
}

const char *
anchor_icon_info_get_used_name (AnchorIconInfo  *icon)
{
 ...
 */
guint
anchor_get_icon_size_for_zoom_level (AnchorZoomLevel zoom_level)
{
	switch (zoom_level) {
...
gint
anchor_get_icon_size_for_stock_size (GtkIconSize size)
{
  gint w, h;
...
guint
anchor_icon_get_emblem_size_for_icon_size (guint size)
{
	if (size >= 96)
		return 48;
	if (size >= 64)
...
gboolean
anchor_icon_theme_can_render (GThemedIcon *icon)
{
	GtkIconTheme *icon_theme;
