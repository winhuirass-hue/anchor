/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-monitor.c: file and directory change monitoring for anchor
	     ...
	     gpointer user_data)
{
	...
	case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT:
		anchor_file_changes_queue_file_changed (child);
		break;
	case G_FILE_MONITOR_EVENT_UNMOUNTED:
	case G_FILE_MONITOR_EVENT_DELETED:
		anchor_file_changes_queue_file_removed (child);
		break;
	case G_FILE_MONITOR_EVENT_CREATED:
		anchor_file_changes_queue_file_added (child);
		break;
	}
...
AnchorMonitor *
anchor_monitor_directory (GFile *location)
{
	GFileMonitor *dir_monitor;
	AnchorMonitor *ret;

	ret = g_slice_new0 (AnchorMonitor);
...
void 
anchor_monitor_cancel (AnchorMonitor *monitor)
{
	...
	g_slice_free (AnchorMonitor, monitor);
}
