...
static void  anchor_query_class_init       (AnchorQueryClass *class);
static void  anchor_query_init             (AnchorQuery      *query);

G_DEFINE_TYPE (AnchorQuery, anchor_query, G_TYPE_OBJECT);
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_query_parent_class)->finalize (object);
}

static void
anchor_query_class_init (AnchorQueryClass *class)
{
	...
	g_type_class_add_private (class, sizeof (AnchorQueryDetails));
}

static void
anchor_query_init (AnchorQuery *query)
{
	query->details = G_TYPE_INSTANCE_GET_PRIVATE (query, ANCHOR_TYPE_QUERY,
						      AnchorQueryDetails);
	query->details->show_hidden = TRUE;
	query->details->location_uri = anchor_get_home_directory_uri ();
}
...
gdouble
anchor_query_matches_string (AnchorQuery *query,
			       const gchar *string)
{
...
AnchorQuery *
anchor_query_new (void)
{
	return g_object_new (ANCHOR_TYPE_QUERY,  NULL);
}


char *
anchor_query_get_text (AnchorQuery *query)
{
	return g_strdup (query->details->text);
}

void 
anchor_query_set_text (AnchorQuery *query, const char *text)
{
	g_free (query->details->text);
...
char *
anchor_query_get_location (AnchorQuery *query)
{
	return g_strdup (query->details->location_uri);
}
	
void
anchor_query_set_location (AnchorQuery *query, const char *uri)
{
	g_free (query->details->location_uri);
...
GList *
anchor_query_get_mime_types (AnchorQuery *query)
{
	return g_list_copy_deep (query->details->mime_types, (GCopyFunc) g_strdup, NULL);
}

void
anchor_query_set_mime_types (AnchorQuery *query, GList *mime_types)
{
...
void
anchor_query_add_mime_type (AnchorQuery *query, const char *mime_type)
{
...
gboolean
anchor_query_get_show_hidden_files (AnchorQuery *query)
{
	return query->details->show_hidden;
}

void
anchor_query_set_show_hidden_files (AnchorQuery *query, gboolean show_hidden)
{
	query->details->show_hidden = show_hidden;
}

char *
anchor_query_to_readable_string (AnchorQuery *query)
{
	 ...
	 GError **err)
{
	...
	if (info->in_text) {
		anchor_query_set_text (info->query, t);
	} else if (info->in_location) {
		uri = decode_home_uri (t);
		anchor_query_set_location (info->query, uri);
		g_free (uri);
	} else if (info->in_mimetypes && info->in_mimetype) {
		anchor_query_add_mime_type (info->query, t);
	}
...
static AnchorQuery *
anchor_query_parse_xml (char *xml, gsize xml_len)
{
...
anchor_query_load (char *file)
{
	...
	query = anchor_query_parse_xml (xml, xml_len);
	g_free (xml);
...
static char *
anchor_query_to_xml (AnchorQuery *query)
{
	GString *xml;
	char *text;
	char *uri;
...
gboolean
anchor_query_save (AnchorQuery *query, char *file)
{
	char *xml;
	GError *err = NULL;
	gboolean res;
