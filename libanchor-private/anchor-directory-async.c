/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-directory-async.c: Anchor directory model state machine.
...
struct DirectoryLoadState {
	...
	AnchorFile *load_directory_file;
	int load_file_count;
};

struct MimeListState {
	AnchorDirectory *directory;
	AnchorFile *mime_list_file;
	GCancellable *cancellable;
	GFileEnumerator *enumerator;
...
typedef struct {
	AnchorFile *file; /* Which file, NULL means all. */
	union {
		AnchorDirectoryCallback directory;
		AnchorFileCallback file;
	} callback;
...
typedef gboolean (* FileCheck) (AnchorFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
							       GFile                  *location);
static gboolean request_is_satisfied                          (AnchorDirectory      *directory,
							       AnchorFile           *file,
							       Request                 request);
static void     cancel_loading_attributes                     (AnchorDirectory      *directory,
							       AnchorFileAttributes  file_attributes);
static void     add_all_files_to_work_queue                   (AnchorDirectory      *directory);
static void     link_info_done                                (AnchorDirectory      *directory,
							       AnchorFile           *file,
							       const char             *uri,
							       const char             *name,
							       GIcon                  *icon,
							       ...
							       gboolean                is_foreign);
static void     move_file_to_low_priority_queue               (AnchorDirectory      *directory,
							       AnchorFile           *file);
static void     move_file_to_extension_queue                  (AnchorDirectory      *directory,
							       AnchorFile           *file);
static void     anchor_directory_invalidate_file_attributes (AnchorDirectory      *directory,
							       AnchorFileAttributes  file_attributes);

/* Some helpers for case-insensitive strings.
 * Move to anchor-glib-extensions?
 */
...
#if 0
static void
anchor_directory_verify_request_counts (AnchorDirectory *directory)
{
	GList *l;
 ...
 */
static gboolean
async_job_start (AnchorDirectory *directory,
		 const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
/* End a job. */
static void
async_job_end (AnchorDirectory *directory,
	       const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
async_job_wake_up (void)
{
	...
	while (async_job_count < MAX_ASYNC_JOBS) {
		...
		anchor_directory_async_state_changed
...
static void
directory_count_cancel (AnchorDirectory *directory)
{
...
static void
deep_count_cancel (AnchorDirectory *directory)
{
...
static void
mime_list_cancel (AnchorDirectory *directory)
{
...
static void
top_left_cancel (AnchorDirectory *directory)
{
...
static void
link_info_cancel (AnchorDirectory *directory)
{
...
static void
thumbnail_cancel (AnchorDirectory *directory)
{
...
static void
mount_cancel (AnchorDirectory *directory)
{
...
static void
file_info_cancel (AnchorDirectory *directory)
{
...
static void
new_files_cancel (AnchorDirectory *directory)
{
	GList *l;
	NewFilesState *state;
...
static GList *
find_monitor (AnchorDirectory *directory,
	      AnchorFile *file,
	      gconstpointer client)
{
	Monitor monitor;
...
static void
remove_monitor_link (AnchorDirectory *directory,
		     GList *link)
{
	Monitor *monitor;
...
static void
remove_monitor (AnchorDirectory *directory,
		AnchorFile *file,
		gconstpointer client)
{
...
Request
anchor_directory_set_up_request (AnchorFileAttributes file_attributes)
{
	Request request;
...
static void
mime_db_changed_callback (GObject *ignore, AnchorDirectory *dir)
{
	AnchorFileAttributes attrs;
	...
	anchor_directory_force_reload_internal (dir, attrs);
}

void
anchor_directory_monitor_add_internal (AnchorDirectory *directory,
					 AnchorFile *file,
					 gconstpointer client,
					 ...
					 gpointer callback_data)
{
	...
	_anchor_profile_log ("uri %s file-uri %s client %p", dir_uri, file_uri, client);
	g_free (dir_uri);
	g_free (file_uri);
	...
	monitor->client = client;
	monitor->request = anchor_directory_set_up_request (file_attributes);
	 ...
	 * anchor almost always shows the whole directory anyway, and
	 * it allows us to avoid one file monitor per file in a directory.
	 */
	if (directory->details->monitor == NULL) {
		directory->details->monitor = anchor_monitor_directory (directory->details->location);
	}
	...
	_anchor_profile_log (NULL);
}

static void
set_file_unconfirmed (AnchorFile *file, gboolean unconfirmed)
{
	AnchorDirectory *directory;
...
static gboolean
should_skip_file (AnchorDirectory *directory, GFileInfo *info)
{
...
dequeue_pending_idle_callback (gpointer callback_data)
{
	...
	_anchor_profile_log ("nitems %d", g_list_length (directory->details->pending_file_info));
	...
	/* Build a list of AnchorFile objects. */
	for (node = pending_file_info; node != NULL; node = node->next) {
		...
		file = anchor_directory_find_file_by_name (directory, name);
		if (file != NULL) {
			...
			if (!file->details->is_added) {
				 ...
				 * This can happen if someone called anchor_file_get_by_uri()
			...
			} else if (anchor_file_update_info (file, file_info)) {
				/* File changed, notify about the change. */
				anchor_file_ref (file);
				changed_files = g_list_prepend (changed_files, file);
			}
		} else {
			/* new file, create a anchor file object and add it to the list */
			file = anchor_file_new_from_info (directory, file_info);
	...
	anchor_directory_emit_change_signals (directory, changed_files);
	anchor_file_list_free (changed_files);
	anchor_directory_emit_files_added (directory, added_files);
	anchor_file_list_free (added_files);

	if (directory->details->directory_loaded &&
	    !directory->details->directory_loaded_sent_notification) {
		/* Send the done_loading signal. */
		anchor_directory_emit_done_loading (directory);
...
void
anchor_directory_schedule_dequeue_pending (AnchorDirectory *directory)
{
...
static void
directory_load_one (AnchorDirectory *directory,
		    GFileInfo *info)
{
	if (info == NULL) {
		return;
	...
	anchor_directory_schedule_dequeue_pending (directory);
}

static void
directory_load_cancel (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
file_list_cancel (AnchorDirectory *directory)
{
...
static void
directory_load_done (AnchorDirectory *directory,
		     GError *error)
{
	GList *node;
	...
	if (error != NULL) {
		...
		anchor_directory_emit_load_error (directory, error);
	}
...
void
anchor_directory_monitor_remove_internal (AnchorDirectory *directory,
					    AnchorFile *file,
					    gconstpointer client)
{
	g_assert (NAUTILUS_IS_DIRECTORY (directory));
...
FileMonitors *
anchor_directory_remove_file_monitors (AnchorDirectory *directory,
					 AnchorFile *file)
{
...
void
anchor_directory_add_file_monitors (AnchorDirectory *directory,
				      AnchorFile *file,
...
static void
ready_callback_call (AnchorDirectory *directory,
...
void
anchor_directory_call_when_ready_internal (AnchorDirectory *directory,
					     AnchorFile *file,
					     AnchorFileAttributes file_attributes,
					     ...
					     gpointer callback_data)
{
	...
	callback.request = anchor_directory_set_up_request (file_attributes);
...
gboolean      
anchor_directory_check_if_ready_internal (AnchorDirectory *directory,
					    AnchorFile *file,
					    AnchorFileAttributes file_attributes)
{
	Request request;
...
static void
remove_callback_link_keep_data (AnchorDirectory *directory,
				GList *link)
{
	ReadyCallback *callback;
...
static void
remove_callback_link (AnchorDirectory *directory,
		      GList *link)
{
	ReadyCallback *callback;
...
void
anchor_directory_cancel_callback_internal (AnchorDirectory *directory,
					     AnchorFile *file,
					     AnchorDirectoryCallback directory_callback,
					     AnchorFileCallback file_callback,
					     gpointer callback_data)
		    ...
		    gpointer user_data)
{
	...
	directory = anchor_directory_ref (state->directory);
...
void
anchor_directory_get_info_for_new_files (AnchorDirectory *directory,
					   GList *location_list)
{
	NewFilesState *state;
	GFile *location;
...
void
anchor_async_destroying_file (AnchorFile *file)
{
	AnchorDirectory *directory;
	gboolean changed;
	GList *node, *next;
...
static gboolean
lacks_directory_count (AnchorFile *file)
{
	return !file->details->directory_count_is_up_to_date
		&& anchor_file_should_show_directory_item_count (file);
}

static gboolean
should_get_directory_count_now (AnchorFile *file)
{
...
static gboolean
lacks_top_left (AnchorFile *file)
{
	return file->details->file_info_is_up_to_date &&
		!file->details->top_left_text_is_up_to_date 
		&& anchor_file_should_get_top_left_text (file);
}

static gboolean
lacks_large_top_left (AnchorFile *file)
{
	return file->details->file_info_is_up_to_date &&
		(!file->details->top_left_text_is_up_to_date ||
...
static gboolean
lacks_link_info (AnchorFile *file)
{
...
static gboolean
lacks_extension_info (AnchorFile *file)
{
	return file->details->pending_info_providers != NULL;
}

static gboolean
lacks_thumbnail (AnchorFile *file)
{
	return anchor_file_should_show_thumbnail (file) &&
		file->details->thumbnail_path != NULL &&
		!file->details->thumbnail_is_up_to_date;
}

static gboolean
lacks_mount (AnchorFile *file)
{
	return (!file->details->mount_is_up_to_date &&
		(
		 ...
		 (file->details->type == G_FILE_TYPE_DIRECTORY &&
		  anchor_file_is_self_owned (file)) ||
...
static gboolean
has_problem (AnchorDirectory *directory, AnchorFile *file, FileCheck problem)
{
	GList *node;
...
static gboolean
request_is_satisfied (AnchorDirectory *directory,
		      AnchorFile *file,
		      Request request)
{
...
static void
schedule_call_ready_callbacks (AnchorDirectory *directory)
{
 ...
 * before then */
static gboolean
call_ready_callbacks (AnchorDirectory *directory)
{
	gboolean found_any;
	GList *node, *next;
	ReadyCallback *callback;
...
gboolean
anchor_directory_has_active_request_for_file (AnchorDirectory *directory,
						AnchorFile *file)
{
	GList *node;
	ReadyCallback *callback;
...
gboolean
anchor_directory_is_anyone_monitoring_file_list (AnchorDirectory *directory)
{
...
gboolean
anchor_directory_is_file_list_monitored (AnchorDirectory *directory) 
{
	return directory->details->file_list_monitored;
}

static void
mark_all_files_unconfirmed (AnchorDirectory *directory)
{
	GList *node;
	AnchorFile *file;
...
directory_load_state_free (DirectoryLoadState *state)
{
	...
	anchor_file_unref (state->load_directory_file);
	g_object_unref (state->cancellable);
	g_free (state);
}
...
static void
start_monitoring_file_list (AnchorDirectory *directory)
{
	DirectoryLoadState *state;
	...
	if (!directory->details->file_list_monitored) {
		...
		anchor_file_list_ref (directory->details->file_list);
	}
...
void
anchor_directory_stop_monitoring_file_list (AnchorDirectory *directory)
{
	if (!directory->details->file_list_monitored) {
		g_assert (directory->details->directory_load_in_progress == NULL);
		return;
	...
	anchor_file_list_unref (directory->details->file_list);
	directory->details->directory_loaded = FALSE;
}

static void
file_list_start_or_stop (AnchorDirectory *directory)
{
	if (anchor_directory_is_anyone_monitoring_file_list (directory)) {
		start_monitoring_file_list (directory);
	} else {
		anchor_directory_stop_monitoring_file_list (directory);
	}
}

void
anchor_file_invalidate_count_and_mime_list (AnchorFile *file)
{
	...
	anchor_file_invalidate_attributes (file, attributes);
}
 ...
 */
void
anchor_directory_invalidate_count_and_mime_list (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
anchor_directory_invalidate_file_attributes (AnchorDirectory      *directory,
					       AnchorFileAttributes  file_attributes)
{
	GList *node;
	...
	for (node = directory->details->file_list; node != NULL; node = node->next) {
		anchor_file_invalidate_attributes_internal (NAUTILUS_FILE (node->data),
							      file_attributes);
	}

	if (directory->details->as_file != NULL) {
		anchor_file_invalidate_attributes_internal (directory->details->as_file,
							      file_attributes);
	}
}

void
anchor_directory_force_reload_internal (AnchorDirectory     *directory,
					  AnchorFileAttributes file_attributes)
{
	_anchor_profile_log (NULL);

	/* invalidate attributes that are getting reloaded for all files */
	anchor_directory_invalidate_file_attributes (directory, file_attributes);
...
static void
directory_count_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
count_children_done (AnchorDirectory *directory,
		     AnchorFile *count_file,
		     gboolean succeeded,
		     int count)
{
	 ...
	 */
	anchor_file_changed (count_file);
...
directory_count_state_free (DirectoryCountState *state)
{
	...
	g_object_unref (state->cancellable);
	anchor_directory_unref (state->directory);
	g_free (state);
}
...
static void
directory_count_start (AnchorDirectory *directory,
		       AnchorFile *file,
		       gboolean *doing_io)
{
	DirectoryCountState *state;
	...
	*doing_io = TRUE;
	...
	state->directory = anchor_directory_ref (directory);
	state->cancellable = g_cancellable_new ();
...
deep_count_next_dir (DeepCountState *state)
{
	...
	anchor_file_updated_deep_count_in_progress (file);
...
static void
deep_count_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
deep_count_start (AnchorDirectory *directory,
		  AnchorFile *file,
		  gboolean *doing_io)
{
	GFile *location;
...
static void
mime_list_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
mime_list_start (AnchorDirectory *directory,
		 AnchorFile *file,
		 gboolean *doing_io)
{
	MimeListState *state;
...
static void
top_left_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
			...
			gpointer callback_data)
{
	...
	AnchorDirectory *directory;
	AnchorFileDetails *file_details;
	gsize file_size;
	char *file_contents;
	...
	if (g_file_load_partial_contents_finish (G_FILE (source_object),
						 ...
						 NULL, NULL)) {
		file_details->top_left_text = anchor_extract_top_left_text (file_contents, state->large, file_size);
	...
	anchor_file_changed (state->file);
...
static void
top_left_start (AnchorDirectory *directory,
		AnchorFile *file,
		gboolean *doing_io)
{
	GFile *location;
		     ...
		     gpointer user_data)
{
	...
	if (info == NULL) {
		if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_FOUND) {
			/* mark file as gone */
			anchor_file_mark_gone (get_info_file);
		}
		get_info_file->details->file_info_is_up_to_date = TRUE;
		anchor_file_clear_info (get_info_file);
		get_info_file->details->get_info_failed = TRUE;
		get_info_file->details->get_info_error = error;
	} else {
		anchor_file_update_info (get_info_file, info);
		g_object_unref (info);
	}

	anchor_file_changed (get_info_file);
	anchor_file_unref (get_info_file);
...
static void
file_info_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
file_info_start (AnchorDirectory *directory,
		 AnchorFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static gboolean
is_link_trusted (AnchorFile *file,
		 gboolean is_launcher)
{
	GFile *location;
	gboolean res;
	...
	if (anchor_file_can_execute (file)) {
		return TRUE;
	}
	...
	if (anchor_file_is_local (file)) {
		location = anchor_file_get_location (file);
		res = anchor_is_in_system_dir (location);
		g_object_unref (location);
	}
...
static void
link_info_done (AnchorDirectory *directory,
		AnchorFile *file,
		const char *uri,
		const char *name, 
		GIcon *icon,
		...
		gboolean is_foreign)
{
	...
	if (is_trusted) {
		anchor_file_set_display_name (file, name, name, TRUE);
	} else {
		anchor_file_set_display_name (file, NULL, NULL, TRUE);
	}
...
static void
link_info_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
link_info_got_data (AnchorDirectory *directory,
		    AnchorFile *file,
		    gboolean result,
		    goffset bytes_read,
		    char *file_contents)
{
	...
	/* Handle the case where we read the Anchor link. */
	if (result) {
		link_uri = anchor_file_get_uri (file);
		anchor_link_get_link_info_given_file_contents (file_contents, bytes_read, link_uri,
...
static void
link_info_anchor_link_read_callback (GObject *source_object,
				       GAsyncResult *res,
				       gpointer user_data)
{
...
static void
link_info_start (AnchorDirectory *directory,
		 AnchorFile *file,
		 gboolean *doing_io)
{
	GFile *location;
	gboolean anchor_style_link;
	LinkInfoReadState *state;
	...
	*doing_io = TRUE;
	...
	anchor_style_link = anchor_file_is_anchor_link (file);
	location = anchor_file_get_location (file);
	
	/* If it's not a link we are done. If it is, we need to read it. */
	if (!anchor_style_link) {
		...
		g_file_load_contents_async (location,
					    state->cancellable,
					    link_info_anchor_link_read_callback,
					    state);
	}
	g_object_unref (location);
}
...
static void
thumbnail_done (AnchorDirectory *directory,
		AnchorFile *file,
		GdkPixbuf *pixbuf,
		gboolean tried_original)
{
...
static void
thumbnail_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
thumbnail_got_pixbuf (AnchorDirectory *directory,
		      AnchorFile *file,
		      GdkPixbuf *pixbuf,
		      gboolean tried_original)
{
...
static void
thumbnail_start (AnchorDirectory *directory,
		 AnchorFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static void
mount_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
got_mount (MountState *state, GMount *mount)
{
	...
	file = anchor_file_ref (state->file);

	file->details->mount_is_up_to_date = TRUE;
	anchor_file_set_mount (file, mount);
			       ...
			       gpointer user_data)
{
	...
	if (mount) {
		...
		location = anchor_file_get_location (state->file);
...
static void
mount_start (AnchorDirectory *directory,
	     AnchorFile *file,
	     gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	if (file->details->type == G_FILE_TYPE_MOUNTABLE) {
		...
		mount = NULL;
		target = anchor_file_get_activation_location (file);
		if (target != NULL) {
...
static void
filesystem_info_cancel (AnchorDirectory *directory)
{
	if (directory->details->filesystem_info_state != NULL) {
...
static void
filesystem_info_stop (AnchorDirectory *directory)
{
	AnchorFile *file;
...
static void
filesystem_info_start (AnchorDirectory *directory,
		       AnchorFile *file,
		       gboolean *doing_io)
{
	GFile *location;
...
static void
extension_info_cancel (AnchorDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		if (directory->details->extension_info_idle) {
			...
			anchor_info_provider_cancel_update 
...
static void
extension_info_stop (AnchorDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		AnchorFile *file;
...
static void
finish_info_provider (AnchorDirectory *directory,
		      AnchorFile *file,
		      AnchorInfoProvider *provider)
{
	...
	if (file->details->pending_info_providers == NULL) {
		anchor_file_info_providers_done (file);
	}
}
...
info_provider_idle_callback (gpointer user_data)
{
	...
	if (response->handle != directory->details->extension_info_in_progress
		...
		g_warning ("Unexpected plugin response.  This probably indicates a bug in a Anchor extension: handle=%p", response->handle);
	} else {
		AnchorFile *file;
		async_job_end (directory, "extension info");
...
static void
info_provider_callback (AnchorInfoProvider *provider,
			AnchorOperationHandle *handle,
			AnchorOperationResult result,
			gpointer user_data)
{
...
static void
extension_info_start (AnchorDirectory *directory,
		      AnchorFile *file,
		      gboolean *doing_io)
{
	AnchorInfoProvider *provider;
	...
	*doing_io = TRUE;
	...
	result = anchor_info_provider_update_file_info
		(provider, 
		 ANCHOR_FILE_INFO (file), 
		 update_complete, 
		 &handle);
...
static void
start_or_stop_io (AnchorDirectory *directory)
{
	AnchorFile *file;
	gboolean doing_io;
	...
	while (!anchor_file_queue_is_empty (directory->details->high_priority_queue)) {
		file = anchor_file_queue_head (directory->details->high_priority_queue);
	...
	while (!anchor_file_queue_is_empty (directory->details->low_priority_queue)) {
		file = anchor_file_queue_head (directory->details->low_priority_queue);
	...
	while (!anchor_file_queue_is_empty (directory->details->extension_queue)) {
		file = anchor_file_queue_head (directory->details->extension_queue);
 ...
 */
void
anchor_directory_async_state_changed (AnchorDirectory *directory)
{
...
void
anchor_directory_cancel (AnchorDirectory *directory)
{
...
static void
cancel_directory_count_for_file (AnchorDirectory *directory,
				 AnchorFile      *file)
{
	if (directory->details->count_in_progress != NULL &&
	    directory->details->count_in_progress->count_file == file) {
...
static void
cancel_deep_counts_for_file (AnchorDirectory *directory,
			     AnchorFile      *file)
{
	if (directory->details->deep_count_file == file) {
		deep_count_cancel (directory);
...
static void
cancel_mime_list_for_file (AnchorDirectory *directory,
			   AnchorFile      *file)
{
	if (directory->details->mime_list_in_progress != NULL &&
	    directory->details->mime_list_in_progress->mime_list_file == file) {
...
static void
cancel_top_left_text_for_file (AnchorDirectory *directory,
			       AnchorFile      *file)
{
	if (directory->details->top_left_read_state != NULL &&
	    directory->details->top_left_read_state->file == file) {
...
static void
cancel_file_info_for_file (AnchorDirectory *directory,
			   AnchorFile      *file)
{
	if (directory->details->get_info_file == file) {
		file_info_cancel (directory);
...
static void
cancel_thumbnail_for_file (AnchorDirectory *directory,
			   AnchorFile      *file)
{
	if (directory->details->thumbnail_state != NULL &&
	    directory->details->thumbnail_state->file == file) {
...
static void
cancel_mount_for_file (AnchorDirectory *directory,
			   AnchorFile      *file)
{
	if (directory->details->mount_state != NULL &&
	    directory->details->mount_state->file == file) {
...
static void
cancel_filesystem_info_for_file (AnchorDirectory *directory,
				 AnchorFile      *file)
{
	if (directory->details->filesystem_info_state != NULL &&
	    directory->details->filesystem_info_state->file == file) {
...
static void
cancel_link_info_for_file (AnchorDirectory *directory,
			   AnchorFile      *file)
{
	if (directory->details->link_info_read_state != NULL &&
	    directory->details->link_info_read_state->file == file) {
...
static void
cancel_loading_attributes (AnchorDirectory *directory,
			   AnchorFileAttributes file_attributes)
{
	Request request;
...
void
anchor_directory_cancel_loading_file_attributes (AnchorDirectory      *directory,
						   AnchorFile           *file,
						   AnchorFileAttributes  file_attributes)
{
	Request request;
...
void
anchor_directory_add_file_to_work_queue (AnchorDirectory *directory,
					   AnchorFile *file)
{
	g_return_if_fail (file->details->directory == directory);

	anchor_file_queue_enqueue (directory->details->high_priority_queue,
				     file);
}


static void
add_all_files_to_work_queue (AnchorDirectory *directory)
{
	GList *node;
	AnchorFile *file;
...
void
anchor_directory_remove_file_from_work_queue (AnchorDirectory *directory,
						AnchorFile *file)
{
	anchor_file_queue_remove (directory->details->high_priority_queue,
				    file);
	anchor_file_queue_remove (directory->details->low_priority_queue,
				    file);
	anchor_file_queue_remove (directory->details->extension_queue,
				    file);
}


static void
move_file_to_low_priority_queue (AnchorDirectory *directory,
				 AnchorFile *file)
{
	/* Must add before removing to avoid ref underflow */
	anchor_file_queue_enqueue (directory->details->low_priority_queue,
				     file);
	anchor_file_queue_remove (directory->details->high_priority_queue,
				    file);
}
...
static void
move_file_to_extension_queue (AnchorDirectory *directory,
			      AnchorFile *file)
{
	/* Must add before removing to avoid ref underflow */
	anchor_file_queue_enqueue (directory->details->extension_queue,
				     file);
	anchor_file_queue_remove (directory->details->low_priority_queue,
				    file);
}
