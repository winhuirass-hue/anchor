/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-thumbnails.h: Thumbnail code for icon factory.
...
/* The list of AnchorThumbnailInfo structs containing information about the
...
static AnchorThumbnailInfo *currently_thumbnailing = NULL;
...
static void
free_thumbnail_info (AnchorThumbnailInfo *info)
{
...
void
anchor_thumbnail_remove_from_queue (const char *file_uri)
{
	GList *node;
...
void
anchor_thumbnail_prioritize (const char *file_uri)
{
	GList *node;
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   We do this in an idle callback as I don't think anchor_file_changed() is
   thread-safe. */
static gboolean
thumbnail_thread_notify_file_changed (gpointer image_uri)
{
	...
	file = anchor_file_get_by_uri ((char *) image_uri);
#ifdef DEBUG_THUMBNAILS
	g_message ("(Thumbnail Thread) Notifying file changed file:%p uri: %s\n", file, (char*) image_uri);
#endif

	if (file != NULL) {
		anchor_file_set_is_thumbnailing (file, FALSE);
...
gboolean
anchor_can_thumbnail_internally (AnchorFile *file)
{
	char *mime_type;
	gboolean res;
...
gboolean
anchor_thumbnail_is_mimetype_limited_by_size (const char *mime_type)
{
	return pixbuf_can_load_type (mime_type);
}

gboolean
anchor_can_thumbnail (AnchorFile *file)
{
	...
	mime_type = anchor_file_get_mime_type (file);
	mtime = anchor_file_get_mtime (file);
...
void
anchor_create_thumbnail (AnchorFile *file)
{
	time_t file_mtime = 0;
	AnchorThumbnailInfo *info;
	AnchorThumbnailInfo *existing_info;
	GList *existing, *node;

	anchor_file_set_is_thumbnailing (file, TRUE);

	info = g_new0 (AnchorThumbnailInfo, 1);
	info->image_uri = anchor_file_get_uri (file);
	info->mime_type = anchor_file_get_mime_type (file);
	
	/* Hopefully the AnchorFile will already have the image file mtime,
...
thumbnail_thread_start (gpointer data)
{
	...
	for (;;) {
		...
		*/
		...
		/* We need to call anchor_file_changed(), but I don't think that is
