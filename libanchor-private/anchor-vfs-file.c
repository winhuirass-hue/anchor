/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-vfs-file.c: Subclass of AnchorFile to help implement the
...
G_DEFINE_TYPE (AnchorVFSFile, anchor_vfs_file, ANCHOR_TYPE_FILE);

static void             
vfs_file_monitor_add (AnchorFile *file,
...
static void
vfs_file_monitor_remove (AnchorFile *file,
			 gconstpointer client)
{
...
static void
vfs_file_call_when_ready (AnchorFile *file,
...
static void
vfs_file_cancel_call_when_ready (AnchorFile *file,
				 AnchorFileCallback callback,
				 gpointer callback_data)
{
...
static gboolean
vfs_file_check_if_ready (AnchorFile *file,
			 AnchorFileAttributes file_attributes)
{
				...
				gpointer callback_data)
{
	...
	if (new_info != NULL) {
		if (anchor_file_update_info (file, new_info)) {
...
static void
vfs_file_set_metadata (AnchorFile           *file,
		       const char             *key,
		       const char             *value)
{
	GFileInfo *info;
...
static void
vfs_file_set_metadata_as_list (AnchorFile           *file,
			       const char             *key,
			       char                  **value)
{
	GFile *location;
...
static gboolean
vfs_file_get_item_count (AnchorFile *file, 
			 guint *count,
...
vfs_file_get_deep_counts (AnchorFile *file,
			  ...
			  goffset *total_size)
{
	...
	type = anchor_file_get_file_type (file);
...
static gboolean
vfs_file_get_date (AnchorFile *file,
...
static char *
vfs_file_get_where_string (AnchorFile *file)
{
	...
	if (!anchor_file_is_in_recent (file)) {
		location = anchor_file_ref (file);
	} else {
		activation_location = anchor_file_get_activation_location (file);
		location = anchor_file_get (activation_location);
		g_object_unref (activation_location);
	}

	where_string = anchor_file_get_parent_uri_for_display (location);
			 ...
			 gpointer callback_data)
{
	...
	mounted_on = g_file_mount_mountable_finish (G_FILE (source_object),
						    res, &error);
	anchor_file_operation_complete (op, mounted_on, error);
	if (mounted_on) {
...
static void
vfs_file_mount (AnchorFile                   *file,
			 ...
			 gpointer callback_data)
{
    ...
    anchor_file_operation_complete (op, G_FILE (source_object), error);
	if (error) {
		g_error_free (error);
	}
}
...
static void
vfs_file_unmount (AnchorFile                   *file,
		  GMountOperation                *mount_op,
		  GCancellable                   *cancellable,
		  AnchorFileOperationCallback   callback,
		  gpointer                        callback_data)
...
static void
vfs_file_eject (AnchorFile                   *file,
		GMountOperation                *mount_op,
		GCancellable                   *cancellable,
		AnchorFileOperationCallback   callback,
		gpointer                        callback_data)
...
static void
vfs_file_start (AnchorFile                   *file,
		GMountOperation                *mount_op,
		GCancellable                   *cancellable,
		AnchorFileOperationCallback   callback,
		gpointer                        callback_data)
...
static void
vfs_file_poll_for_media (AnchorFile *file)
{
	AnchorFileOperation *op;
	GFile *location;

	op = anchor_file_operation_new (file, NULL, NULL);
...
static void
anchor_vfs_file_init (AnchorVFSFile *file)
{
}

static void
anchor_vfs_file_class_init (AnchorVFSFileClass *klass)
{
	AnchorFileClass *file_class = NAUTILUS_FILE_CLASS (klass);
