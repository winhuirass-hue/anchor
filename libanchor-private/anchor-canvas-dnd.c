...
/* anchor-canvas-dnd.c - Drag & drop handling for the canvas container widget.
...
#include <config.h>
#include <math.h>
#include "anchor-canvas-dnd.h"

#include "anchor-file-dnd.h"
...
static char * anchor_canvas_container_find_drop_target (AnchorCanvasContainer *container,
							GdkDragContext *context,
							int x, int y, gboolean *icon_hit,
							gboolean rewrite_desktop);

static EelCanvasItem *
create_selection_shadow (AnchorCanvasContainer *container,
			 GList *list)
{
...
static gboolean
icon_get_data_binder (AnchorCanvasIcon *icon, gpointer data)
{
	...
	uri = anchor_canvas_container_get_icon_uri (container, icon);
	if (!eel_uri_is_desktop (uri)) {
		g_free (uri);
		uri = anchor_canvas_container_get_icon_activation_uri (container, icon);
	}
...
/* Iterate over each selected icon in a AnchorCanvasContainer,
 * calling each_function on each.
 */
static void
anchor_canvas_container_each_selected_icon (AnchorCanvasContainer *container,
	gboolean (*each_function) (AnchorCanvasIcon *, gpointer), gpointer data)
{
	GList *p;
...
/* Adaptor function used with anchor_canvas_container_each_selected_icon
 * to help iterate over all selected items, passing uris, x, y, w and h
 * values to the iteratee
 */
static void
each_icon_get_data_binder (AnchorDragEachSelectedItemDataGet iteratee, 
	gpointer iterator_context, gpointer data)
{
	...
	anchor_canvas_container_each_selected_icon (container, icon_get_data_binder, &context);
}
...
drag_data_get_callback (GtkWidget *widget,
			...
			gpointer data)
{
	AnchorDragInfo *drag_info;

	g_assert (widget != NULL);
	g_assert (NAUTILUS_IS_CANVAS_CONTAINER (widget));
	g_return_if_fail (context != NULL);

	/* Call common function from anchor-drag that set's up
	...
	anchor_drag_drag_data_get_from_cache (drag_info->selection_cache, context, selection_data, info, time);
}
...
static void
anchor_canvas_container_position_shadow (AnchorCanvasContainer *container,
					 int x, int y)
{
...
static void
anchor_canvas_container_dropped_canvas_feedback (GtkWidget *widget,
					       GtkSelectionData *data,
					       int x, int y)
{
	AnchorCanvasContainer *container;
	AnchorCanvasDndInfo *dnd_info;

	container = NAUTILUS_CANVAS_CONTAINER (widget);
	dnd_info = container->details->dnd_info;
	
	/* Delete old selection list. */
	anchor_drag_destroy_selection_list (dnd_info->drag_info.selection_list);
	...
	dnd_info->drag_info.selection_list = anchor_drag_build_selection_list (data);
	dnd_info->shadow = create_selection_shadow (container, dnd_info->drag_info.selection_list);
	anchor_canvas_container_position_shadow (container, x, y);
}
...
static void
set_direct_save_uri (GtkWidget *widget, GdkDragContext *context, AnchorDragInfo *drag_info, int x, int y)
{
	GFile *base, *child;
	...
	drop_target = anchor_canvas_container_find_drop_target (NAUTILUS_CANVAS_CONTAINER (widget), 
...
get_data_on_first_target_we_support (GtkWidget *widget, GdkDragContext *context, guint32 time, int x, int y)
{
	...
	if (anchor_canvas_container_get_is_desktop (NAUTILUS_CANVAS_CONTAINER (widget))) {
...
static void
anchor_canvas_container_ensure_drag_data (AnchorCanvasContainer *container,
					  GdkDragContext *context,
					  guint32 time)
{
	AnchorCanvasDndInfo *dnd_info;
...
anchor_canvas_container_item_at (AnchorCanvasContainer *container,
				 int x, int y)
{
	GList *p;
	int size;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		eel_canvas_w2c (EEL_CANVAS (container),
				...
				&canvas_point.x1,
				&canvas_point.y1);
		if (anchor_canvas_item_hit_test_rectangle (icon->item, canvas_point)) {
			return icon;
		}
	}
...
static char *
get_container_uri (AnchorCanvasContainer *container)
{
	char *uri;
...
static gboolean
anchor_canvas_container_selection_items_local (AnchorCanvasContainer *container,
					       GList *items)
{
	...
	if (eel_uri_is_desktop (container_uri_string)) {
		result = anchor_drag_items_on_desktop (items);
	} else {
		result = anchor_drag_items_local (container_uri_string, items);
	}
...
/* handle dropped url */
static void
receive_dropped_netscape_url (AnchorCanvasContainer *container, const char *encoded_url, GdkDragContext *context, int x, int y)
{
	char *drop_target;
	...
	drop_target = anchor_canvas_container_find_drop_target (container, context, x, y, NULL, TRUE);
...
static void
receive_dropped_uri_list (AnchorCanvasContainer *container, const char *uri_list, GdkDragContext *context, int x, int y)
{	
	char *drop_target;
...
/* handle dropped text */
static void
receive_dropped_text (AnchorCanvasContainer *container, const char *text, GdkDragContext *context, int x, int y)
{	
	char *drop_target;
...
static void
receive_dropped_raw (AnchorCanvasContainer *container, const char *raw_data, int length, const char *direct_save_uri, GdkDragContext *context, int x, int y)
{
	char *drop_target;
...
auto_scroll_timeout_callback (gpointer data)
{
	...
	anchor_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
	...
	if (!anchor_canvas_container_scroll (container, (int)x_scroll_delta, (int)y_scroll_delta)) {
...
static void
set_up_auto_scroll_if_needed (AnchorCanvasContainer *container)
{
	anchor_drag_autoscroll_start (&container->details->dnd_info->drag_info,
...
static void
stop_auto_scroll (AnchorCanvasContainer *container)
{
	anchor_drag_autoscroll_stop (&container->details->dnd_info->drag_info);
}

static void
handle_local_move (AnchorCanvasContainer *container,
		   double world_x, double world_y)
{
	GList *moved_icons, *p;
	AnchorDragSelectionItem *item;
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
		...
		icon = anchor_canvas_container_get_icon_by_uri
			(container, item->uri);
		...
		if (icon == NULL) {
			...
			file = anchor_file_get_by_uri (item->uri);
			...
			anchor_canvas_container_add (container, NAUTILUS_CANVAS_ICON_DATA (file));
		...
		if (item->got_icon_position) {
			anchor_canvas_container_move_icon
				(container, icon,
...
static void
handle_nonlocal_move (AnchorCanvasContainer *container,
		      ...
		      gboolean icon_hit)
{
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
		...
		source_uris = g_list_prepend (source_uris, ((AnchorDragSelectionItem *)p->data)->uri);
	}
	...
	if (!icon_hit) {
		...
		for (index = 0, p = container->details->dnd_info->drag_info.selection_list;
			p != NULL; index++, p = p->next) {
		    	item_x = ((AnchorDragSelectionItem *)p->data)->icon_x;
			if (is_rtl)
				item_x = -item_x - ((AnchorDragSelectionItem *)p->data)->icon_width;
		     	g_array_index (source_item_locations, GdkPoint, index).x = item_x;
		     	g_array_index (source_item_locations, GdkPoint, index).y =
				((AnchorDragSelectionItem *)p->data)->icon_y;
		}
	}
	...
	if (eel_uri_is_desktop (target_uri)) {
		target_uri = anchor_get_desktop_directory_uri ();
...
static char *
anchor_canvas_container_find_drop_target (AnchorCanvasContainer *container,
					  GdkDragContext *context,
					  int x, int y,
					  gboolean *icon_hit,
					  gboolean rewrite_desktop)
{
	AnchorCanvasIcon *drop_target_icon;
	double world_x, world_y;
	AnchorFile *file;
	char *icon_uri;
	char *container_uri;
	...
	drop_target_icon = anchor_canvas_container_item_at (container, world_x, world_y);
	if (drop_target_icon != NULL) {
		icon_uri = anchor_canvas_container_get_icon_uri (container, drop_target_icon);
		if (icon_uri != NULL) {
			file = anchor_file_get_by_uri (icon_uri);

			if (!anchor_drag_can_accept_info (file,
	...
	if (drop_target_icon == NULL) {
		...
		if (container_uri != NULL) {
			if (rewrite_desktop && eel_uri_is_desktop (container_uri)) {
				...
				container_uri = anchor_get_desktop_directory_uri ();
			} else {
				gboolean can;
				file = anchor_file_get_by_uri (container_uri);
				can = anchor_drag_can_accept_info (file,
								     container->details->dnd_info->drag_info.data_type,
								     container->details->dnd_info->drag_info.selection_list);
				g_object_unref (file);
				if (!can) {
	...
	return anchor_canvas_container_get_icon_drop_target_uri (container, drop_target_icon);
}

static void
anchor_canvas_container_receive_dropped_icons (AnchorCanvasContainer *container,
					       GdkDragContext *context,
					       int x, int y)
{
	char *drop_target;
	...
	if (real_action == GDK_ACTION_ASK) {
		...
		if (anchor_drag_selection_includes_special_link (container->details->dnd_info->drag_info.selection_list)) {
			/* We only want to move the trash */
			action = GDK_ACTION_MOVE;
		} else {
			action = GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK;
		}
		real_action = anchor_drag_drop_action_ask
	...
	if (real_action > 0) {
		...
		drop_target = anchor_canvas_container_find_drop_target (container, 
		...
		if (!icon_hit && real_action == GDK_ACTION_MOVE) {
			 ...
			 */
			local_move_only = anchor_canvas_container_selection_items_local
	...
	g_free (drop_target);
	anchor_drag_destroy_selection_list (container->details->dnd_info->drag_info.selection_list);
	container->details->dnd_info->drag_info.selection_list = NULL;
}

static void
anchor_canvas_container_get_drop_action (AnchorCanvasContainer *container,
					 GdkDragContext *context,
					 int x, int y,
					 int *action)
{
	...
	*action = 0;
	...
	case NAUTILUS_ICON_DND_GNOME_ICON_LIST:
		if (container->details->dnd_info->drag_info.selection_list != NULL) {
			anchor_drag_default_drop_action_for_icons (context, drop_target, 
								     container->details->dnd_info->drag_info.selection_list, 
								     action);
		}
		break;
	case NAUTILUS_ICON_DND_URI_LIST:
		*action = anchor_drag_default_drop_action_for_uri_list (context, drop_target);
		break;

	case NAUTILUS_ICON_DND_NETSCAPE_URL:
		*action = anchor_drag_default_drop_action_for_netscape_url (context);
		break;
...
static void
set_drop_target (AnchorCanvasContainer *container,
		 AnchorCanvasIcon *icon)
{
	...
	anchor_canvas_container_update_icon (container, old_icon);
	anchor_canvas_container_update_icon (container, icon);
}

static void
anchor_canvas_dnd_update_drop_target (AnchorCanvasContainer *container,
				      GdkDragContext *context,
				      int x, int y)
{
	AnchorCanvasIcon *icon;
	...
	/* Find the item we hit with our drop, if any. */
	icon = anchor_canvas_container_item_at (container, world_x, world_y);
...
static void
remove_hover_timer (AnchorCanvasDndInfo *dnd_info)
{
...
static void
anchor_canvas_container_free_drag_data (AnchorCanvasContainer *container)
{
	AnchorCanvasDndInfo *dnd_info;
		     ...
		     gpointer data)
{
	...
	anchor_canvas_container_free_drag_data(NAUTILUS_CANVAS_CONTAINER (widget));
}
		     ...
		     gpointer        data)
{
	...
	start_y = container->details->dnd_info->drag_info.start_y +
        ...
        surface = anchor_canvas_item_get_drag_surface (container->details->drag_icon->item);
	...
	drag_info->selection_cache = anchor_drag_create_selection_cache (widget,
									   each_icon_get_data_binder);

}

void
anchor_canvas_dnd_begin_drag (AnchorCanvasContainer *container,
...
{
	AnchorCanvasContainer *container = user_data;
	AnchorCanvasDndInfo *dnd_info;
...
static void
check_hover_timer (AnchorCanvasContainer *container,
		   const char              *uri)
{
	AnchorCanvasDndInfo *dnd_info;
		      ...
		      guint32 time)
{
	...
	anchor_canvas_container_ensure_drag_data (NAUTILUS_CANVAS_CONTAINER (widget), context, time);
	anchor_canvas_container_position_shadow (NAUTILUS_CANVAS_CONTAINER (widget), x, y);
	anchor_canvas_dnd_update_drop_target (NAUTILUS_CANVAS_CONTAINER (widget), context, x, y);
	 ...
	 */
	action = 0;
	anchor_canvas_container_get_drop_action (NAUTILUS_CANVAS_CONTAINER (widget), context, x, y,
						 &action);
	if (action != 0) {
		char *uri;
		uri = anchor_canvas_container_find_drop_target (NAUTILUS_CANVAS_CONTAINER (widget),
								  context, x, y, NULL, TRUE);
...
void
anchor_canvas_dnd_end_drag (AnchorCanvasContainer *container)
{
	AnchorCanvasDndInfo *dnd_info;
			     ...
			     gpointer user_data)
{
	...
	switch (info) {
	case NAUTILUS_ICON_DND_GNOME_ICON_LIST:
		anchor_canvas_container_dropped_canvas_feedback (widget, data, x, y);
		break;
	case NAUTILUS_ICON_DND_URI_LIST:
	case NAUTILUS_ICON_DND_TEXT:
	case NAUTILUS_ICON_DND_XDNDDIRECTSAVE:
	...
	if (drag_info->drop_occured) {
		...
		switch (info) {
		case NAUTILUS_ICON_DND_GNOME_ICON_LIST:
			anchor_canvas_container_receive_dropped_icons
		...
		case NAUTILUS_ICON_DND_XDNDDIRECTSAVE:
		{
			...
			if (selection_format == 8 &&
				...
				p.x = x; p.y = y;
				anchor_file_changes_queue_schedule_position_set (
				                 location,
				                 p,
...
void
anchor_canvas_dnd_init (AnchorCanvasContainer *container)
{
	...
	container->details->dnd_info = g_new0 (AnchorCanvasDndInfo, 1);
	anchor_drag_init (&container->details->dnd_info->drag_info,
...
void
anchor_canvas_dnd_fini (AnchorCanvasContainer *container)
{
	g_return_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container));
	...
	if (container->details->dnd_info != NULL) {
		...
		anchor_drag_finalize (&container->details->dnd_info->drag_info);
