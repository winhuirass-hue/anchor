...
/* Private part of the AnchorCanvasItem structure. */
struct AnchorCanvasItemDetails {
	/* The image, text, font. */
	double x, y;
	GdkPixbuf *pixbuf;
...
static void anchor_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType anchor_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (AnchorCanvasItem, anchor_canvas_item, EEL_TYPE_CANVAS_ITEM,
			 G_IMPLEMENT_INTERFACE (EEL_TYPE_ACCESSIBLE_TEXT,
						anchor_canvas_item_text_interface_init));

/* private */
static void     get_icon_rectangle            (AnchorCanvasItem        *item,
						 EelIRect                      *rect);
...
static PangoLayout *get_label_layout                 (PangoLayout                  **layout,
						      AnchorCanvasItem        *item,
						      const char                    *text);
static gboolean hit_test_stretch_handle              (AnchorCanvasItem        *item,
						      EelIRect                       icon_rect,
						      GtkCornerType *corner);
;

static void       anchor_canvas_item_ensure_bounds_up_to_date (AnchorCanvasItem *canvas_item);

/* Object initialization function for the canvas item. */
static void
anchor_canvas_item_init (AnchorCanvasItem *canvas_item)
{
	canvas_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((canvas_item), ANCHOR_TYPE_CANVAS_ITEM, AnchorCanvasItemDetails);
	anchor_canvas_item_invalidate_label_size (canvas_item);
}

static void
anchor_canvas_item_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_canvas_item_parent_class)->finalize (object);
}
...
static void
anchor_canvas_item_invalidate_bounds_cache (AnchorCanvasItem *item)
{
...
void
anchor_canvas_item_invalidate_label_size (AnchorCanvasItem *item)
{
	...
	anchor_canvas_item_invalidate_bounds_cache (item);
...
static void
anchor_canvas_item_set_property (GObject        *object,
				   guint           property_id,
				   const GValue   *value,
				   GParamSpec     *pspec)
{
	...
	case PROP_EDITABLE_TEXT:
		...
		anchor_canvas_item_invalidate_label_size (item);
	...
	default:
		g_warning ("anchor_canvas_item_set_property on unknown argument");
		return;
	}
...
static void
anchor_canvas_item_get_property (GObject        *object,
				   guint           property_id,
				   GValue         *value,
				   GParamSpec     *pspec)
{
...
static void
get_scaled_icon_size (AnchorCanvasItem *item,
		      gint *width,
		      gint *height)
{
	EelCanvas *canvas;
...
void
anchor_canvas_item_set_image (AnchorCanvasItem *item,
				GdkPixbuf *image)
{
	AnchorCanvasItemDetails *details;	
...
void 
anchor_canvas_item_set_attach_points (AnchorCanvasItem *item,
					GdkPoint *attach_points,
					int n_attach_points)
{
...
void
anchor_canvas_item_set_embedded_text_rect (AnchorCanvasItem       *item,
...
void
anchor_canvas_item_set_embedded_text (AnchorCanvasItem       *item,
					const char                   *text)
{
 ...
 */
static void
recompute_bounding_box (AnchorCanvasItem *canvas_item,
...
static EelIRect
compute_text_rectangle (const AnchorCanvasItem *item,
...
void
anchor_canvas_item_update_bounds (AnchorCanvasItem *item,
				    double i2w_dx, double i2w_dy)
{
...
static void
anchor_canvas_item_update (EelCanvasItem *item,
			     double i2w_dx, double i2w_dy,
			     gint flags)
{
	anchor_canvas_item_update_bounds (NAUTILUS_CANVAS_ITEM (item), i2w_dx, i2w_dy);

	eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

	EEL_CANVAS_ITEM_CLASS (anchor_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
in_single_click_mode (void)
{
	...
	click_policy = g_settings_get_enum (anchor_preferences,
...
static void
prepare_pango_layout_width (AnchorCanvasItem *item,
			    PangoLayout *layout)
{
	if (anchor_canvas_item_get_max_text_width (item) < 0) {
		pango_layout_set_width (layout, -1);
	} else {
		pango_layout_set_width (layout, floor (anchor_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
		pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
	}
}

static void
prepare_pango_layout_for_measure_entire_text (AnchorCanvasItem *item,
					      PangoLayout *layout)
{
...
static void
prepare_pango_layout_for_draw (AnchorCanvasItem *item,
			       PangoLayout *layout)
{
	AnchorCanvasItemDetails *details;
	...
	if (needs_highlight ||
		...
		pango_layout_set_height (layout,
					 anchor_canvas_container_get_max_layout_lines_for_pango (container));
	}
}

static void
measure_label_text (AnchorCanvasItem *item)
{
	AnchorCanvasItemDetails *details;
	AnchorCanvasContainer *container;
	...
	if (have_editable) {
		...
		layout_get_size_for_layout (editable_layout,
					    anchor_canvas_container_get_max_layout_lines (container),
					    editable_height_for_entire_text,
...
static void
draw_label_text (AnchorCanvasItem *item,
                 cairo_t *cr,
		 EelIRect icon_rect)
{
	AnchorCanvasItemDetails *details;
	...
	max_text_width = floor (anchor_canvas_item_get_max_text_width (item));
...
void
anchor_canvas_item_set_is_visible (AnchorCanvasItem       *item,
				     gboolean                      visible)
{
	...
	if (!visible) {
		anchor_canvas_item_invalidate_label (item);
	}
}

void
anchor_canvas_item_invalidate_label (AnchorCanvasItem     *item)
{
	anchor_canvas_item_invalidate_label_size (item);
...
get_knob_pixbuf (void)
{
	...
	knob_pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
						    ...
						    8, 0, NULL);
	if (!knob_pixbuf) {
		GInputStream *stream = g_resources_open_stream ("/org/gnome/anchor/icons/knob.png", 0, NULL);
		if (stream != NULL) {
...
static void
draw_stretch_handles (AnchorCanvasItem *item,
                      cairo_t *cr,
		      const EelIRect *rect)
{
	GtkWidget *widget;
...
real_map_surface (AnchorCanvasItem *canvas_item)
{
	EelCanvas *canvas;
...
static cairo_surface_t *
map_surface (AnchorCanvasItem *canvas_item)
{
...
static void
draw_embedded_text (AnchorCanvasItem *item,
                    cairo_t *cr,
		    int x, int y)
{
	PangoLayout *layout;
...
cairo_surface_t *
anchor_canvas_item_get_drag_surface (AnchorCanvasItem *item)
{
	cairo_surface_t *surface;
	EelCanvas *canvas;
	int width, height;
	...
	gtk_style_context_save (context);
	gtk_style_context_add_class (context, "anchor-canvas-item");
...
static void
anchor_canvas_item_draw (EelCanvasItem *item,
			   cairo_t *cr,
			   cairo_region_t *region)
{
	AnchorCanvasContainer *container;
	AnchorCanvasItem *canvas_item;
	AnchorCanvasItemDetails *details;
	EelIRect icon_rect;
...
static PangoLayout *
create_label_layout (AnchorCanvasItem *item,
		     const char *text)
{
	PangoLayout *layout;
	PangoContext *context;
...
static int
anchor_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	AnchorCanvasItem *canvas_item;
	GdkCursor *cursor;
	GdkWindow *cursor_window;
...
static gboolean
hit_test (AnchorCanvasItem *canvas_item, EelIRect icon_rect)
{
	AnchorCanvasItemDetails *details;
...
static double
anchor_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
...
static void
anchor_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
	AnchorCanvasItem *canvas_item;
	AnchorCanvasItemDetails *details;
...
void
anchor_canvas_item_get_bounds_for_layout (AnchorCanvasItem *canvas_item,
					    double *x1, double *y1, double *x2, double *y2)
{
	...
	anchor_canvas_item_ensure_bounds_up_to_date (canvas_item);
...
void
anchor_canvas_item_get_bounds_for_entire_item (AnchorCanvasItem *canvas_item,
						 double *x1, double *y1, double *x2, double *y2)
{
	AnchorCanvasItemDetails *details;
	EelIRect *total_rect;
...
static void
anchor_canvas_item_bounds (EelCanvasItem *item,
			     double *x1, double *y1, double *x2, double *y2)
{
	AnchorCanvasItem *canvas_item;
	AnchorCanvasItemDetails *details;
...
static void
anchor_canvas_item_ensure_bounds_up_to_date (AnchorCanvasItem *canvas_item)
{
	AnchorCanvasItemDetails *details;
...
EelDRect
anchor_canvas_item_get_icon_rectangle (const AnchorCanvasItem *item)
{
	EelDRect rectangle;
	double pixels_per_unit;
	gint width, height;
...
EelDRect
anchor_canvas_item_get_text_rectangle (AnchorCanvasItem *item,
					 gboolean for_layout)
{
	/* FIXME */
	EelIRect icon_rectangle;
...
static void
get_icon_rectangle (AnchorCanvasItem *item,
		    EelIRect *rect)
{
	gint width, height;
...
void
anchor_canvas_item_set_show_stretch_handles (AnchorCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (AnchorCanvasItem *item,
...
gboolean
anchor_canvas_item_hit_test_stretch_handles (AnchorCanvasItem *item,
					       gdouble world_x,
					       gdouble world_y,
					       GtkCornerType *corner)
{
...
/* anchor_canvas_item_hit_test_rectangle
 *
 * Check and see if there is an intersection between the item and the
 * canvas rect.
 */
gboolean
anchor_canvas_item_hit_test_rectangle (AnchorCanvasItem *item, EelIRect icon_rect)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_ITEM (item), FALSE);
...
const char *
anchor_canvas_item_get_editable_text (AnchorCanvasItem *canvas_item)
{
...
void
anchor_canvas_item_set_renaming (AnchorCanvasItem *item, gboolean state)
{
	g_return_if_fail (NAUTILUS_IS_CANVAS_ITEM (item));
...
double
anchor_canvas_item_get_max_text_width (AnchorCanvasItem *item)
{
	EelCanvasItem *canvas_item;
...
void
anchor_canvas_item_set_entire_text (AnchorCanvasItem       *item,
...
static void
anchor_canvas_item_class_init (AnchorCanvasItemClass *class)
{
	...
	object_class->finalize = anchor_canvas_item_finalize;
	object_class->set_property = anchor_canvas_item_set_property;
	object_class->get_property = anchor_canvas_item_get_property;
	...
	item_class->update = anchor_canvas_item_update;
	item_class->draw = anchor_canvas_item_draw;
	item_class->point = anchor_canvas_item_point;
	item_class->translate = anchor_canvas_item_translate;
	item_class->bounds = anchor_canvas_item_bounds;
	item_class->event = anchor_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       ANCHOR_TYPE_CANVAS_ITEM,
				       anchor_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (AnchorCanvasItemDetails));
}

static GailTextUtil *
anchor_canvas_item_get_text (GObject *text)
{
	return NAUTILUS_CANVAS_ITEM (text)->details->text_util;
}

static void
anchor_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = anchor_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *anchor_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *anchor_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} AnchorCanvasItemAccessiblePrivate;

typedef struct {
	AnchorCanvasItem *item;
	gint action_number;
} AnchorCanvasItemAccessibleActionContext;

typedef struct {
	EelCanvasItemAccessible parent;
	AnchorCanvasItemAccessiblePrivate *priv;
} AnchorCanvasItemAccessible;

typedef struct {
	EelCanvasItemAccessibleClass parent_class;
} AnchorCanvasItemAccessibleClass;

#define GET_ACCESSIBLE_PRIV(o) ((AnchorCanvasItemAccessible *) o)->priv;

/* accessible AtkAction interface */
static gboolean
anchor_canvas_item_accessible_idle_do_action (gpointer data)
{
	AnchorCanvasItem *item;
	AnchorCanvasItemAccessibleActionContext *ctx;
...
static gboolean
anchor_canvas_item_accessible_do_action (AtkAction *accessible,
					   int i)
{
	AnchorCanvasItem *item;
	AnchorCanvasItemAccessibleActionContext *ctx;
	...
	case ACTION_MENU:
		...
		ctx = g_new (AnchorCanvasItemAccessibleActionContext, 1);
		ctx->action_number = i;
		ctx->item = item;
		g_queue_push_head (container->details->a11y_item_action_queue, ctx);
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler = g_idle_add (anchor_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
                g_warning ("Invalid action passed to AnchorCanvasItemAccessible::do_action");
                return FALSE;
        }
...
static int
anchor_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
anchor_canvas_item_accessible_action_get_description (AtkAction *accessible,
							int i)
{
	AnchorCanvasItemAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return anchor_canvas_item_accessible_action_descriptions[i];
	}
}

static const char *
anchor_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return anchor_canvas_item_accessible_action_names[i];
}

static const char *
anchor_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
						       int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
anchor_canvas_item_accessible_action_set_description (AtkAction *accessible,
							int i,
...
static void
anchor_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = anchor_canvas_item_accessible_do_action;
	iface->get_n_actions = anchor_canvas_item_accessible_get_n_actions;
	iface->get_description = anchor_canvas_item_accessible_action_get_description;
	iface->get_keybinding = anchor_canvas_item_accessible_action_get_keybinding;
	iface->get_name = anchor_canvas_item_accessible_action_get_name;
	iface->set_description = anchor_canvas_item_accessible_action_set_description;
}

static const gchar *
anchor_canvas_item_accessible_get_name (AtkObject *accessible)
{
	AnchorCanvasItem *item;
...
static const gchar*
anchor_canvas_item_accessible_get_description (AtkObject *accessible)
{
	AnchorCanvasItem *item;
...
static AtkObject *
anchor_canvas_item_accessible_get_parent (AtkObject *accessible)
{
	AnchorCanvasItem *item;
...
static int
anchor_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
	AnchorCanvasItem *item;
	AnchorCanvasContainer *container;
	GList *l;
...
static const gchar *
anchor_canvas_item_accessible_get_image_description (AtkImage *image)
{
	AnchorCanvasItemAccessiblePrivate *priv;
	AnchorCanvasItem *item;
	AnchorCanvasIcon *icon;
	...
	if (priv->image_description) {
		...
		icon = item->user_data;
		container = NAUTILUS_CANVAS_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
		description = anchor_canvas_container_get_icon_description (container, icon->data);
...
static void
anchor_canvas_item_accessible_get_image_size
(AtkImage *image, 
 gint     *width,
 gint     *height)
{
...
static void
anchor_canvas_item_accessible_get_image_position
(AtkImage		 *image,
 gint                    *x,
 gint	                 *y,
...
static gboolean
anchor_canvas_item_accessible_set_image_description (AtkImage    *image,
...
static void
anchor_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
	iface->get_image_description = anchor_canvas_item_accessible_get_image_description;
	iface->set_image_description = anchor_canvas_item_accessible_set_image_description;
	iface->get_image_size        = anchor_canvas_item_accessible_get_image_size;
	iface->get_image_position    = anchor_canvas_item_accessible_get_image_position;
}

/* accessible text interface */
static gint
anchor_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
						     gint           x,
						     gint           y,
						     AtkCoordType coords)
{
...
static void
anchor_canvas_item_accessible_get_character_extents (AtkText	   *text,
						       gint	   offset,
						       gint	   *x,
						       gint	   *y,
						       gint	   *width,
...
static void
anchor_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
	...
	iface->get_character_extents   = anchor_canvas_item_accessible_get_character_extents;
	iface->get_offset_at_point     = anchor_canvas_item_accessible_get_offset_at_point;
}

static GType anchor_canvas_item_accessible_get_type (void);

G_DEFINE_TYPE_WITH_CODE (AnchorCanvasItemAccessible,
			 anchor_canvas_item_accessible,
			 eel_canvas_item_accessible_get_type (),
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						anchor_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						anchor_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						anchor_canvas_item_accessible_action_interface_init));

static AtkStateSet*
anchor_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
	...
	state_set = ATK_OBJECT_CLASS (anchor_canvas_item_accessible_parent_class)->ref_state_set (accessible);
...
static void
anchor_canvas_item_accessible_finalize (GObject *object)
{
	AnchorCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (anchor_canvas_item_accessible_parent_class)->finalize (object);
}

static void
anchor_canvas_item_accessible_initialize (AtkObject *accessible,
					    gpointer widget)
{
	ATK_OBJECT_CLASS (anchor_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_CANVAS);
}

static void
anchor_canvas_item_accessible_class_init (AnchorCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = anchor_canvas_item_accessible_finalize;

	aclass->initialize = anchor_canvas_item_accessible_initialize;

	aclass->get_name = anchor_canvas_item_accessible_get_name;
	aclass->get_description = anchor_canvas_item_accessible_get_description;
	aclass->get_parent = anchor_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = anchor_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = anchor_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (AnchorCanvasItemAccessiblePrivate));
}

static void
anchor_canvas_item_accessible_init (AnchorCanvasItemAccessible *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_canvas_item_accessible_get_type (),
						  AnchorCanvasItemAccessiblePrivate);
}

/* dummy typedef */
typedef AtkObjectFactory      AnchorCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass AnchorCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (AnchorCanvasItemAccessibleFactory, anchor_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);

static AtkObject *
anchor_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
	AtkObject *accessible;
	AnchorCanvasItem *item;
	GString *item_text;
	...
	accessible = g_object_new (anchor_canvas_item_accessible_get_type (), NULL);
...
static GType
anchor_canvas_item_accessible_factory_get_accessible_type (void)
{
	return anchor_canvas_item_accessible_get_type ();
}

static void
anchor_canvas_item_accessible_factory_init (AnchorCanvasItemAccessibleFactory *self)
{
}

static void
anchor_canvas_item_accessible_factory_class_init (AnchorCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = anchor_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = anchor_canvas_item_accessible_factory_get_accessible_type;
}
