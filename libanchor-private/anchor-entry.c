...
static void anchor_entry_editable_init (GtkEditableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (AnchorEntry, anchor_entry, GTK_TYPE_ENTRY,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_EDITABLE,
						anchor_entry_editable_init));

static GtkEditableInterface *parent_editable_interface = NULL;

static void
anchor_entry_init (AnchorEntry *entry)
{
	entry->details = g_new0 (AnchorEntryDetails, 1);
}

GtkWidget *
anchor_entry_new (void)
{
	return gtk_widget_new (ANCHOR_TYPE_ENTRY, NULL);
}

GtkWidget *
anchor_entry_new_with_max_length (guint16 max)
{
	GtkWidget *widget;

	widget = gtk_widget_new (ANCHOR_TYPE_ENTRY, NULL);
...
static void
anchor_entry_finalize (GObject *object)
{
	AnchorEntry *entry;
	...
	G_OBJECT_CLASS (anchor_entry_parent_class)->finalize (object);
}

static gboolean
anchor_entry_key_press (GtkWidget *widget, GdkEventKey *event)
{
	AnchorEntry *entry;
	...
	result = GTK_WIDGET_CLASS (anchor_entry_parent_class)->key_press_event (widget, event);
...
static gboolean
anchor_entry_motion_notify (GtkWidget *widget, GdkEventMotion *event)
{
	int result;
	...
	result = GTK_WIDGET_CLASS (anchor_entry_parent_class)->motion_notify_event (widget, event);
 ...
 **/
void
anchor_entry_select_all (AnchorEntry *entry)
{
...
/**
 * anchor_entry_select_all_at_idle
 *
 ...
 **/
void
anchor_entry_select_all_at_idle (AnchorEntry *entry)
{
	g_return_if_fail (NAUTILUS_IS_ENTRY (entry));
...
void
anchor_entry_set_text (AnchorEntry *entry, const gchar *text)
{
	g_return_if_fail (NAUTILUS_IS_ENTRY (entry));
...
static void
anchor_entry_set_selection_bounds (GtkEditable *editable,
				     int start_pos,
				     int end_pos)
{
...
static gboolean
anchor_entry_button_press (GtkWidget *widget,
			     GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (anchor_entry_parent_class)->button_press_event (widget, event);
...
static gboolean
anchor_entry_button_release (GtkWidget *widget,
			       GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (anchor_entry_parent_class)->button_release_event (widget, event);
...
static void
anchor_entry_insert_text (GtkEditable *editable, const gchar *text,
...
static void 
anchor_entry_delete_text (GtkEditable *editable, int start_pos, int end_pos)
{
 ...
 * this with type-completion behavior in AnchorLocationBar (see bug 5313).
 * There's a FIXME comment that seems to be about this same issue in
 * gtk+/gtkselection.c, gtk_selection_clear.
 */
static gboolean
anchor_entry_selection_clear (GtkWidget *widget,
			        GdkEventSelection *event)
{
	...
	return GTK_WIDGET_CLASS (anchor_entry_parent_class)->selection_clear_event (widget, event);
}

static void
anchor_entry_editable_init (GtkEditableInterface *iface)
{
	parent_editable_interface = g_type_interface_peek_parent (iface);

	iface->insert_text = anchor_entry_insert_text;
	iface->delete_text = anchor_entry_delete_text;
	iface->set_selection_bounds = anchor_entry_set_selection_bounds;
...
static void
anchor_entry_class_init (AnchorEntryClass *class)
{
	...
	widget_class->button_press_event = anchor_entry_button_press;
	widget_class->button_release_event = anchor_entry_button_release;
	widget_class->key_press_event = anchor_entry_key_press;
	widget_class->motion_notify_event = anchor_entry_motion_notify;
	widget_class->selection_clear_event = anchor_entry_selection_clear;
	
	gobject_class->finalize = anchor_entry_finalize;
	...
	signals[SELECTION_CHANGED] = g_signal_new
		("selection-changed",
		 ...
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AnchorEntryClass, selection_changed),
		 NULL, NULL,
...
void
anchor_entry_set_special_tab_handling (AnchorEntry *entry,
					 gboolean special_tab_handling)
{
	g_return_if_fail (NAUTILUS_IS_ENTRY (entry));
