...
void anchor_debug_set_flags (DebugFlags flags);
gboolean anchor_debug_flag_is_set (DebugFlags flag);

void anchor_debug_valist (DebugFlags flag,
                            const gchar *format, va_list args);

void anchor_debug (DebugFlags flag, const gchar *format, ...)
  G_GNUC_PRINTF (2, 3);

void anchor_debug_files (DebugFlags flag, GList *files,
                           const gchar *format, ...) G_GNUC_PRINTF (3, 4);

#ifdef DEBUG_FLAG

#define DEBUG(format, ...) \
  anchor_debug (DEBUG_FLAG, "%s: %s: " format, G_STRFUNC, G_STRLOC, \
                  ##__VA_ARGS__)

#define DEBUG_FILES(files, format, ...) \
  anchor_debug_files (DEBUG_FLAG, files, "%s:" format, G_STRFUNC, \
                        ##__VA_ARGS__)

#define DEBUGGING anchor_debug_flag_is_set(DEBUG_FLAG)
