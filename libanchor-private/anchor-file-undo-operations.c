...
/* anchor-file-undo-operations.c - Manages undo/redo of file operations
 *
...
G_DEFINE_TYPE (AnchorFileUndoInfo, anchor_file_undo_info, G_TYPE_OBJECT)
...
/* description helpers */
static void
anchor_file_undo_info_init (AnchorFileUndoInfo *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ANCHOR_TYPE_FILE_UNDO_INFO,
						  AnchorFileUndoInfoDetails);
	self->priv->apply_async_result = NULL;
}

static void
anchor_file_undo_info_get_property (GObject *object,
				      guint property_id,
				      GValue *value,
				      GParamSpec *pspec)
{
	AnchorFileUndoInfo *self = NAUTILUS_FILE_UNDO_INFO (object);
...
static void
anchor_file_undo_info_set_property (GObject *object,
				      guint property_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
...
static void
anchor_file_redo_info_warn_redo (AnchorFileUndoInfo *self,
...
static void
anchor_file_undo_info_warn_undo (AnchorFileUndoInfo *self,
				   GtkWindow *parent_window)
{
...
static void
anchor_file_undo_info_strings_func (AnchorFileUndoInfo *self,
				      gchar **undo_label,
				      gchar **undo_description,
				      gchar **redo_label,
				      gchar **redo_description)
...
static void
anchor_file_undo_info_finalize (GObject *obj)
{
	AnchorFileUndoInfo *self = NAUTILUS_FILE_UNDO_INFO (obj);

	g_clear_object (&self->priv->apply_async_result);

	G_OBJECT_CLASS (anchor_file_undo_info_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_class_init (AnchorFileUndoInfoClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_finalize;
	oclass->get_property = anchor_file_undo_info_get_property;
	oclass->set_property = anchor_file_undo_info_set_property;

	klass->undo_func = anchor_file_undo_info_warn_undo;
	klass->redo_func = anchor_file_redo_info_warn_redo;
	klass->strings_func = anchor_file_undo_info_strings_func;
	...
	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoDetails));
	g_object_class_install_properties (oclass, N_PROPERTIES, properties);
}

static AnchorFileUndoOp
anchor_file_undo_info_get_op_type (AnchorFileUndoInfo *self)
{
	return self->priv->op_type;
}

static gint
anchor_file_undo_info_get_item_count (AnchorFileUndoInfo *self)
{
	return self->priv->count;
}

void
anchor_file_undo_info_apply_async (AnchorFileUndoInfo *self,
				     gboolean undo,
				     GtkWindow *parent_window,
				     ...
				     gpointer user_data)
{
	...
	self->priv->apply_async_result = 
		g_simple_async_result_new (G_OBJECT (self),
					   callback, user_data,
					   anchor_file_undo_info_apply_async);
...
gboolean
anchor_file_undo_info_apply_finish (AnchorFileUndoInfo *self,
				      GAsyncResult *res,
				      gboolean *user_cancel,
				      GError **error)
{
...
void
anchor_file_undo_info_get_strings (AnchorFileUndoInfo *self,
				     gchar **undo_label,
				     gchar **undo_description,
				     gchar **redo_label,
				     gchar **redo_description)
...
static void
file_undo_info_complete_apply (AnchorFileUndoInfo *self,
			       gboolean success,
			       gboolean user_cancel)
{
...
file_undo_info_transfer_callback (GHashTable * debuting_uris,
				  gboolean success,
                                  gpointer user_data)
{
	AnchorFileUndoInfo *self = user_data;
...
static void
file_undo_info_operation_callback (AnchorFile * file,
...
G_DEFINE_TYPE (AnchorFileUndoInfoExt, anchor_file_undo_info_ext, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static char *
ext_get_first_target_short_name (AnchorFileUndoInfoExt *self)
{
	GList *targets_first;
	char *file_name = NULL;
...
static void
ext_strings_func (AnchorFileUndoInfo *info,
		  gchar **undo_label,
		  gchar **undo_description,
		  gchar **redo_label,
		  gchar **redo_description)
{
	AnchorFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
	gint count = anchor_file_undo_info_get_item_count (info);
...
static void
ext_create_link_redo_func (AnchorFileUndoInfoExt *self,
			   GtkWindow *parent_window)
{
	anchor_file_operations_link (self->priv->sources, NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_duplicate_redo_func (AnchorFileUndoInfoExt *self,
			 GtkWindow *parent_window)
{
	anchor_file_operations_duplicate (self->priv->sources, NULL, parent_window,
					    file_undo_info_transfer_callback, self);
}

static void
ext_copy_redo_func (AnchorFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	anchor_file_operations_copy (self->priv->sources, NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_move_restore_redo_func (AnchorFileUndoInfoExt *self,
			    GtkWindow *parent_window)
{
	anchor_file_operations_move (self->priv->sources, NULL,
				       self->priv->dest_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_redo_func (AnchorFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	AnchorFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
...
static void
ext_restore_undo_func (AnchorFileUndoInfoExt *self,
		       GtkWindow *parent_window)
{
	anchor_file_operations_trash_or_delete (self->priv->destinations, parent_window,
						  file_undo_info_delete_callback, self);
}


static void
ext_move_undo_func (AnchorFileUndoInfoExt *self,
		    GtkWindow *parent_window)
{
	anchor_file_operations_move (self->priv->destinations, NULL,
				       self->priv->src_dir, parent_window,
				       file_undo_info_transfer_callback, self);
}

static void
ext_copy_duplicate_undo_func (AnchorFileUndoInfoExt *self,
			      GtkWindow *parent_window)
{
	GList *files;
	...
	anchor_file_operations_delete (files, parent_window,
					 file_undo_info_delete_callback, self);
...
static void
ext_undo_func (AnchorFileUndoInfo *info,
	       GtkWindow *parent_window)
{
	AnchorFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
...
static void
anchor_file_undo_info_ext_init (AnchorFileUndoInfoExt *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_ext_get_type (),
						  AnchorFileUndoInfoExtDetails);
}

static void
anchor_file_undo_info_ext_finalize (GObject *obj)
{
	AnchorFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT (obj);
	...
	G_OBJECT_CLASS (anchor_file_undo_info_ext_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_ext_class_init (AnchorFileUndoInfoExtClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_ext_finalize;

	iclass->undo_func = ext_undo_func;
	iclass->redo_func = ext_redo_func;
	iclass->strings_func = ext_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoExtDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_ext_new (AnchorFileUndoOp op_type,
				 gint item_count,
				 GFile *src_dir,
				 GFile *target_dir)
{
	AnchorFileUndoInfoExt *retval;

	retval = g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_EXT,
			       "op-type", op_type,
...
void
anchor_file_undo_info_ext_add_origin_target_pair (AnchorFileUndoInfoExt *self,
						    GFile                   *origin,
						    GFile                   *target)
{
	self->priv->sources =
...
G_DEFINE_TYPE (AnchorFileUndoInfoCreate, anchor_file_undo_info_create, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
create_strings_func (AnchorFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
...
{
	AnchorFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
	char *name;
...
static void
create_from_template_redo_func (AnchorFileUndoInfoCreate *self,
				GtkWindow *parent_window)
{
	GFile *parent;
	...
	anchor_file_operations_new_file_from_template (NULL, NULL,
							 parent_uri, new_name,
							 self->priv->template,
...
static void
create_folder_redo_func (AnchorFileUndoInfoCreate *self,
			 GtkWindow *parent_window)
{
	GFile *parent;
	gchar *parent_uri;
	...
	parent = g_file_get_parent (self->priv->target_file);
	parent_uri = g_file_get_uri (parent);
	anchor_file_operations_new_folder (NULL, NULL, parent_uri,
					     create_callback, self);
...
static void
create_empty_redo_func (AnchorFileUndoInfoCreate *self,
			GtkWindow *parent_window)

{
	...
	parent_uri = g_file_get_uri (parent);
	new_name = g_file_get_parse_name (self->priv->target_file);
	anchor_file_operations_new_file (NULL, NULL, parent_uri,
					   new_name,
					   self->priv->template,
					   self->priv->length,
					   create_callback, self);
...
static void
create_redo_func (AnchorFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	AnchorFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
...
static void
create_undo_func (AnchorFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	AnchorFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE (info);
	GList *files = NULL;
...
static void
anchor_file_undo_info_create_init (AnchorFileUndoInfoCreate *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_create_get_type (),
						  AnchorFileUndoInfoCreateDetails);
}

static void
anchor_file_undo_info_create_finalize (GObject *obj)
{
	AnchorFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE (obj);
	g_clear_object (&self->priv->target_file);
	g_free (self->priv->template);	

	G_OBJECT_CLASS (anchor_file_undo_info_create_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_create_class_init (AnchorFileUndoInfoCreateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_create_finalize;

	iclass->undo_func = create_undo_func;
	iclass->redo_func = create_redo_func;
	iclass->strings_func = create_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoCreateDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_create_new (AnchorFileUndoOp op_type)
{
	return g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_CREATE,
			     "op-type", op_type,
			     "item-count", 1,
			     NULL);
}
...
void
anchor_file_undo_info_create_set_data (AnchorFileUndoInfoCreate *self,
                                         GFile                      *file,
                                         const char                 *template,
					 gint                        length)
{
...
/* rename */
G_DEFINE_TYPE (AnchorFileUndoInfoRename, anchor_file_undo_info_rename, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
rename_strings_func (AnchorFileUndoInfo *info,
		     gchar **undo_label,
		     gchar **undo_description,
		     gchar **redo_label,
		     gchar **redo_description)
{
	AnchorFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME (info);
...
static void
rename_redo_func (AnchorFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	AnchorFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME (info);
	gchar *new_name;
	...
	file = anchor_file_get (self->priv->old_file);
...
static void
rename_undo_func (AnchorFileUndoInfo *info,
		  GtkWindow *parent_window)
{
	AnchorFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME (info);
	gchar *new_name;
	...
	file = anchor_file_get (self->priv->new_file);
	anchor_file_rename (file, new_name,
			      file_undo_info_operation_callback, self);
...
static void
anchor_file_undo_info_rename_init (AnchorFileUndoInfoRename *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_rename_get_type (),
						  AnchorFileUndoInfoRenameDetails);
}

static void
anchor_file_undo_info_rename_finalize (GObject *obj)
{
	AnchorFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME (obj);
	g_clear_object (&self->priv->old_file);
	g_clear_object (&self->priv->new_file);

	G_OBJECT_CLASS (anchor_file_undo_info_rename_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_rename_class_init (AnchorFileUndoInfoRenameClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_rename_finalize;

	iclass->undo_func = rename_undo_func;
	iclass->redo_func = rename_redo_func;
	iclass->strings_func = rename_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoRenameDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_rename_new (void)
{
	return g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_RENAME,
...
void
anchor_file_undo_info_rename_set_data (AnchorFileUndoInfoRename *self,
					 GFile                      *old_file,
					 GFile                      *new_file)
{
...
/* trash */
G_DEFINE_TYPE (AnchorFileUndoInfoTrash, anchor_file_undo_info_trash, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
trash_strings_func (AnchorFileUndoInfo *info,
		    gchar **undo_label,
		    gchar **undo_description,
		    gchar **redo_label,
		    gchar **redo_description)
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (info);
...
trash_redo_func_callback (GHashTable *debuting_uris,
			  ...
			  gpointer user_data)
{
	AnchorFileUndoInfoTrash *self = user_data;
...
static void
trash_redo_func (AnchorFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (info);
	...
	if (g_hash_table_size (self->priv->trashed) > 0) {
		...
		anchor_file_operations_trash_or_delete (locations, parent_window,
...
static GHashTable *
trash_retrieve_files_to_restore_finish (AnchorFileUndoInfoTrash *self,
					GAsyncResult *res,
					GError **error)
{
...
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (object);
...
static void
trash_retrieve_files_to_restore_async (AnchorFileUndoInfoTrash *self,
				 GAsyncReadyCallback callback,
				 gpointer user_data)
{
...
trash_retrieve_files_ready (GObject *source,
			    ...
			    gpointer user_data)
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (source);
...
static void
trash_undo_func (AnchorFileUndoInfo *info,
		 GtkWindow *parent_window)
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (info);
...
static void
anchor_file_undo_info_trash_init (AnchorFileUndoInfoTrash *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_trash_get_type (),
						  AnchorFileUndoInfoTrashDetails);
	self->priv->trashed =
		g_hash_table_new_full (g_file_hash, (GEqualFunc) g_file_equal, 
				       g_object_unref, NULL);
}
...
static void
anchor_file_undo_info_trash_finalize (GObject *obj)
{
	AnchorFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH (obj);
	g_hash_table_destroy (self->priv->trashed);

	G_OBJECT_CLASS (anchor_file_undo_info_trash_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_trash_class_init (AnchorFileUndoInfoTrashClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_trash_finalize;

	iclass->undo_func = trash_undo_func;
	iclass->redo_func = trash_redo_func;
	iclass->strings_func = trash_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoTrashDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_trash_new (gint item_count)
{
	return g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_TRASH,
...
void
anchor_file_undo_info_trash_add_file (AnchorFileUndoInfoTrash *self,
					GFile                     *file)
{
	GTimeVal current_time;
...
G_DEFINE_TYPE (AnchorFileUndoInfoRecPermissions, anchor_file_undo_info_rec_permissions, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
rec_permissions_strings_func (AnchorFileUndoInfo *info,
			      gchar **undo_label,
			      gchar **undo_description,
			      gchar **redo_label,
			      gchar **redo_description)
{
	AnchorFileUndoInfoRecPermissions *self = NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	char *name;
...
static void
rec_permissions_redo_func (AnchorFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	AnchorFileUndoInfoRecPermissions *self = NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS (info);
	gchar *parent_uri;
	...
	anchor_file_set_permissions_recursive (parent_uri,
...
static void
rec_permissions_undo_func (AnchorFileUndoInfo *info,
			   GtkWindow *parent_window)
{
	AnchorFileUndoInfoRecPermissions *self = NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS (info);
...
static void
anchor_file_undo_info_rec_permissions_init (AnchorFileUndoInfoRecPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_rec_permissions_get_type (),
						  AnchorFileUndoInfoRecPermissionsDetails);
...
static void
anchor_file_undo_info_rec_permissions_finalize (GObject *obj)
{
	AnchorFileUndoInfoRecPermissions *self = NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS (obj);

	g_hash_table_destroy (self->priv->original_permissions);
	g_clear_object (&self->priv->dest_dir);

	G_OBJECT_CLASS (anchor_file_undo_info_rec_permissions_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_rec_permissions_class_init (AnchorFileUndoInfoRecPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_rec_permissions_finalize;
	...
	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoRecPermissionsDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_rec_permissions_new (GFile   *dest,
					     guint32 file_permissions,
					     guint32 file_mask,
					     guint32 dir_permissions,
					     guint32 dir_mask)
{
	AnchorFileUndoInfoRecPermissions *retval;

	retval = g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_REC_PERMISSIONS,
...
void
anchor_file_undo_info_rec_permissions_add_file (AnchorFileUndoInfoRecPermissions *self,
						  GFile                              *file,
						  guint32                             permission)
{
...
G_DEFINE_TYPE (AnchorFileUndoInfoPermissions, anchor_file_undo_info_permissions, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
permissions_strings_func (AnchorFileUndoInfo *info,
			  gchar **undo_label,
			  gchar **undo_description,
			  gchar **redo_label,
			  gchar **redo_description)
{
	AnchorFileUndoInfoPermissions *self = NAUTILUS_FILE_UNDO_INFO_PERMISSIONS (info);
	gchar *name;
...
static void
permissions_real_func (AnchorFileUndoInfoPermissions *self,
		       guint32 permissions)
{
	AnchorFile *file;

	file = anchor_file_get (self->priv->target_file);
	anchor_file_set_permissions (file, permissions,
				       file_undo_info_operation_callback, self);
...
static void
permissions_redo_func (AnchorFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	AnchorFileUndoInfoPermissions *self = NAUTILUS_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
permissions_undo_func (AnchorFileUndoInfo *info, 
		       GtkWindow *parent_window)
{
	AnchorFileUndoInfoPermissions *self = NAUTILUS_FILE_UNDO_INFO_PERMISSIONS (info);
...
static void
anchor_file_undo_info_permissions_init (AnchorFileUndoInfoPermissions *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_permissions_get_type (),
						  AnchorFileUndoInfoPermissionsDetails);
}

static void
anchor_file_undo_info_permissions_finalize (GObject *obj)
{
	AnchorFileUndoInfoPermissions *self = NAUTILUS_FILE_UNDO_INFO_PERMISSIONS (obj);
	g_clear_object (&self->priv->target_file);

	G_OBJECT_CLASS (anchor_file_undo_info_permissions_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_permissions_class_init (AnchorFileUndoInfoPermissionsClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_permissions_finalize;

	iclass->undo_func = permissions_undo_func;
	iclass->redo_func = permissions_redo_func;
	iclass->strings_func = permissions_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoPermissionsDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_permissions_new (GFile   *file,
					 guint32  current_permissions,
					 guint32  new_permissions)
{
	AnchorFileUndoInfoPermissions *retval;

	retval = g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_PERMISSIONS,
...
G_DEFINE_TYPE (AnchorFileUndoInfoOwnership, anchor_file_undo_info_ownership, ANCHOR_TYPE_FILE_UNDO_INFO)
...
static void
ownership_strings_func (AnchorFileUndoInfo *info,
			gchar **undo_label,
			gchar **undo_description,
			gchar **redo_label,
			gchar **redo_description)
{
	AnchorFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP (info);
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (info);
	gchar *name;
...
static void
ownership_real_func (AnchorFileUndoInfoOwnership *self,
		     const gchar *ownership)
{
	AnchorFileUndoOp op_type = anchor_file_undo_info_get_op_type (NAUTILUS_FILE_UNDO_INFO (self));
	AnchorFile *file;

	file = anchor_file_get (self->priv->target_file);

	if (op_type == NAUTILUS_FILE_UNDO_OP_CHANGE_OWNER) {
		anchor_file_set_owner (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	} else if (op_type == NAUTILUS_FILE_UNDO_OP_CHANGE_GROUP) {
		anchor_file_set_group (file,
					 ownership,
					 file_undo_info_operation_callback, self);
	}
...
static void
ownership_redo_func (AnchorFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	AnchorFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
ownership_undo_func (AnchorFileUndoInfo *info,
		     GtkWindow *parent_window)
{
	AnchorFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP (info);
...
static void
anchor_file_undo_info_ownership_init (AnchorFileUndoInfoOwnership *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_file_undo_info_ownership_get_type (),
						  AnchorFileUndoInfoOwnershipDetails);
}

static void
anchor_file_undo_info_ownership_finalize (GObject *obj)
{
	AnchorFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP (obj);

	g_clear_object (&self->priv->target_file);
	g_free (self->priv->original_ownership);
	g_free (self->priv->new_ownership);

	G_OBJECT_CLASS (anchor_file_undo_info_ownership_parent_class)->finalize (obj);
}

static void
anchor_file_undo_info_ownership_class_init (AnchorFileUndoInfoOwnershipClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);
	AnchorFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS (klass);

	oclass->finalize = anchor_file_undo_info_ownership_finalize;

	iclass->undo_func = ownership_undo_func;
	iclass->redo_func = ownership_redo_func;
	iclass->strings_func = ownership_strings_func;

	g_type_class_add_private (klass, sizeof (AnchorFileUndoInfoOwnershipDetails));
}

AnchorFileUndoInfo *
anchor_file_undo_info_ownership_new (AnchorFileUndoOp  op_type,
				       GFile              *file,
				       const char         *current_data,
				       const char         *new_data)
{
	AnchorFileUndoInfoOwnership *retval;

	retval = g_object_new (ANCHOR_TYPE_FILE_UNDO_INFO_OWNERSHIP,
			       "item-count", 1,
			       "op-type", op_type,
			       NULL);
