...
struct AnchorSearchEngineTrackerDetails {
...
G_DEFINE_TYPE_WITH_CODE (AnchorSearchEngineTracker,
			 anchor_search_engine_tracker,
			 G_TYPE_OBJECT,
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_search_engine_tracker_parent_class)->finalize (object);
}

#define BATCH_SIZE 100

static void
check_pending_hits (AnchorSearchEngineTracker *tracker,
		    gboolean force_send)
{
	GList *hits = NULL;
	AnchorSearchHit *hit;
	...
	anchor_search_provider_hits_added (ANCHOR_SEARCH_PROVIDER (tracker), hits);
	g_list_free_full (hits, g_object_unref);
}

static void
search_finished (AnchorSearchEngineTracker *tracker,
		 GError *error)
{
	if (error == NULL) {
	...
	if (error && !g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
		anchor_search_provider_error (ANCHOR_SEARCH_PROVIDER (tracker), error->message);
	} else {
		anchor_search_provider_finished (ANCHOR_SEARCH_PROVIDER (tracker));
	}
...
static void
cursor_next (AnchorSearchEngineTracker *tracker,
                 ...
                 gpointer      user_data)
{
	...
	match = anchor_query_matches_string (tracker->details->query, basename);
	anchor_search_hit_set_fts_rank (hit, rank + match);
	g_free (basename);

	if (g_time_val_from_iso8601 (mtime_str, &tv)) {
		GDateTime *dt;
		dt = g_date_time_new_from_timeval_local (&tv);
		anchor_search_hit_set_modification_time (hit, dt);
	...
	if (g_time_val_from_iso8601 (atime_str, &tv)) {
		GDateTime *dt;
		dt = g_date_time_new_from_timeval_local (&tv);
		anchor_search_hit_set_access_time (hit, dt);
		g_date_time_unref (dt);
	} else {
...
{
	AnchorSearchEngineTracker *tracker = user_data;
...
static void
anchor_search_engine_tracker_start (AnchorSearchProvider *provider)
{
	AnchorSearchEngineTracker *tracker;
	...
	query_text = anchor_query_get_text (tracker->details->query);
	downcase = g_utf8_strdown (query_text, -1);
	search_text = tracker_sparql_escape_string (downcase);
	g_free (query_text);
	g_free (downcase);

	location_uri = anchor_query_get_location (tracker->details->query);
	mimetypes = anchor_query_get_mime_types (tracker->details->query);
...
static void
anchor_search_engine_tracker_stop (AnchorSearchProvider *provider)
{
	AnchorSearchEngineTracker *tracker;
...
static void
anchor_search_engine_tracker_set_query (AnchorSearchProvider *provider,
					  AnchorQuery *query)
{
	AnchorSearchEngineTracker *tracker;
...
{
	iface->set_query = anchor_search_engine_tracker_set_query;
	iface->start = anchor_search_engine_tracker_start;
	iface->stop = anchor_search_engine_tracker_stop;
}

static void
anchor_search_engine_tracker_class_init (AnchorSearchEngineTrackerClass *class)
{
	...
	g_type_class_add_private (class, sizeof (AnchorSearchEngineTrackerDetails));
}

static void
anchor_search_engine_tracker_init (AnchorSearchEngineTracker *engine)
{
	GError *error = NULL;

	engine->details = G_TYPE_INSTANCE_GET_PRIVATE (engine, ANCHOR_TYPE_SEARCH_ENGINE_TRACKER,
						       AnchorSearchEngineTrackerDetails);
...
anchor_search_engine_tracker_new (void)
{
	return g_object_new (ANCHOR_TYPE_SEARCH_ENGINE_TRACKER, NULL);
}
