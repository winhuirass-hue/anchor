...
/* 
   anchor-trash-monitor.c: Anchor trash state watcher.
...
struct AnchorTrashMonitorDetails {
	gboolean empty;
	GIcon *icon;
...
static AnchorTrashMonitor *anchor_trash_monitor = NULL;

G_DEFINE_TYPE(AnchorTrashMonitor, anchor_trash_monitor, G_TYPE_OBJECT)

static void
anchor_trash_monitor_finalize (GObject *object)
{
	AnchorTrashMonitor *trash_monitor;
	...
	G_OBJECT_CLASS (anchor_trash_monitor_parent_class)->finalize (object);
}

static void
anchor_trash_monitor_class_init (AnchorTrashMonitorClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = anchor_trash_monitor_finalize;

	signals[TRASH_STATE_CHANGED] = g_signal_new
		("trash-state-changed",
		 G_TYPE_FROM_CLASS (object_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AnchorTrashMonitorClass, trash_state_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__BOOLEAN,
		 G_TYPE_NONE, 1,
		 G_TYPE_BOOLEAN);

	g_type_class_add_private (object_class, sizeof(AnchorTrashMonitorDetails));
}

static void
update_icon (AnchorTrashMonitor *trash_monitor)
{
...
static void
update_empty_info (AnchorTrashMonitor *trash_monitor,
		   gboolean is_empty)
{
...
enumerate_next_files_cb (GObject *source,
			 ...
			 gpointer user_data)
{
	AnchorTrashMonitor *trash_monitor = user_data;
	GList *infos;
...
static void
schedule_update_info (AnchorTrashMonitor *trash_monitor)
{
	GFile *location;
...
static void
anchor_trash_monitor_init (AnchorTrashMonitor *trash_monitor)
{
	GFile *location;

	trash_monitor->details = G_TYPE_INSTANCE_GET_PRIVATE (trash_monitor,
							      ANCHOR_TYPE_TRASH_MONITOR,
							      AnchorTrashMonitorDetails);
...
{
	g_object_unref (anchor_trash_monitor);
}

AnchorTrashMonitor *
anchor_trash_monitor_get (void)
{
	if (anchor_trash_monitor == NULL) {
		/* not running yet, start it up */

		anchor_trash_monitor = NAUTILUS_TRASH_MONITOR
			(g_object_new (ANCHOR_TYPE_TRASH_MONITOR, NULL));
		eel_debug_call_at_shutdown (unref_trash_monitor);
	}

	return anchor_trash_monitor;
}

gboolean
anchor_trash_monitor_is_empty (void)
{
	AnchorTrashMonitor *monitor;

	monitor = anchor_trash_monitor_get ();
	return monitor->details->empty;
}

GIcon *
anchor_trash_monitor_get_icon (void)
{
	AnchorTrashMonitor *monitor;
