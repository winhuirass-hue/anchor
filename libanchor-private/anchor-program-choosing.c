...
/* anchor-program-choosing.c - functions for selecting and activating
...
void
anchor_launch_application_for_mount (GAppInfo *app_info,
				       GMount *mount,
				       GtkWindow *parent_window)
{
	...
	file = anchor_file_get (root);
	g_object_unref (root);

	files = g_list_append (NULL, file);
	anchor_launch_application (app_info,
				     files,
				     parent_window);

	g_list_free_full (files, (GDestroyNotify) anchor_file_unref);
}
 ...
 */
void
anchor_launch_application (GAppInfo *application, 
			     GList *files,
			     GtkWindow *parent_window)
{
	GList *uris, *l;
	...
	for (l = files; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, anchor_file_get_activation_uri (l->data));
	}
	uris = g_list_reverse (uris);
	anchor_launch_application_by_uri (application, uris,
					    parent_window);
	g_list_free_full (uris, g_free);
}

void
anchor_launch_application_by_uri (GAppInfo *application, 
				    GList *uris,
				    GtkWindow *parent_window)
{
	char *uri;
	...
	file = anchor_file_get_by_uri (uris->data);
	icon = anchor_file_get_icon (file,
	...
	if (icon) {
		gdk_app_launch_context_set_icon_name (launch_context,
							anchor_icon_info_get_used_name (icon));
		g_object_unref (icon);
	}
	...
	if (result) {
		for (l = uris; l != NULL; l = l->next) {
			file = anchor_file_get_by_uri (l->data);
			anchor_recent_add_file (file, application);
			anchor_file_unref (file);
		}
	}
...
/**
 * anchor_launch_application_from_command:
 * 
 ...
 */
void
anchor_launch_application_from_command (GdkScreen  *screen,
 ...
 */
void
anchor_launch_application_from_command_array (GdkScreen  *screen,
						const char *command_string,
						gboolean use_terminal,
...
void
anchor_launch_desktop_file (GdkScreen   *screen,
