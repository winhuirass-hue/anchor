/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-desktop-directory.c: Subclass of AnchorDirectory to implement
...
struct AnchorDesktopDirectoryDetails {
	AnchorDirectory *real_directory;
	GHashTable *callbacks;
	GHashTable *monitors;
};

typedef struct {
	AnchorDesktopDirectory *desktop_dir;
...
G_DEFINE_TYPE (AnchorDesktopDirectory, anchor_desktop_directory,
	       ANCHOR_TYPE_DIRECTORY);

static gboolean
desktop_contains_file (AnchorDirectory *directory,
		       AnchorFile *file)
{
	AnchorDesktopDirectory *desktop;

	desktop = NAUTILUS_DESKTOP_DIRECTORY (directory);

	if (anchor_directory_contains_file (desktop->details->real_directory, file)) {
		return TRUE;
	}
...
merged_callback_destroy (MergedCallback *merged_callback)
{
	...
	anchor_file_list_free (merged_callback->merged_file_list);
...
static void
directory_ready_callback (AnchorDirectory *directory,
			  GList *files,
			  gpointer callback_data)
{
...
static void
desktop_call_when_ready (AnchorDirectory *directory,
...
static void
desktop_cancel_callback (AnchorDirectory *directory,
			AnchorDirectoryCallback callback,
			gpointer callback_data)
{
	AnchorDesktopDirectory *desktop;
	...
	for (node = merged_callback->non_ready_directories; node != NULL; node = node->next) {
		anchor_directory_cancel_callback
			(node->data,
			 directory_ready_callback, merged_callback);
	}
	merged_callback_destroy (merged_callback);
...
merged_monitor_destroy (MergedMonitor *monitor)
{
	...
	anchor_directory_file_monitor_remove (desktop->details->real_directory, monitor);

	anchor_directory_monitor_remove_internal (NAUTILUS_DIRECTORY (desktop), NULL, monitor);
	
	g_free (monitor);
}

static void
build_merged_callback_list (AnchorDirectory *directory,
			    GList *file_list,
			    gpointer callback_data)
{
	GList **merged_list;
	...
	*merged_list = g_list_concat (*merged_list,
				      anchor_file_list_copy (file_list));
}

static void
desktop_monitor_add (AnchorDirectory *directory,
		    ...
		    gpointer callback_data)
{
	...
	anchor_file_list_free (merged_callback_list);
}

static void
desktop_monitor_remove (AnchorDirectory *directory,
		       gconstpointer client)
{
	AnchorDesktopDirectory *desktop;
	MergedMonitor *monitor;
...
static void
desktop_force_reload (AnchorDirectory *directory)
{
	AnchorDesktopDirectory *desktop;

	desktop = NAUTILUS_DESKTOP_DIRECTORY (directory);

	anchor_directory_force_reload (desktop->details->real_directory);
...
static gboolean
desktop_are_all_files_seen (AnchorDirectory *directory)
{
	AnchorDesktopDirectory *desktop;

	desktop = NAUTILUS_DESKTOP_DIRECTORY (directory);

	if (!anchor_directory_are_all_files_seen (desktop->details->real_directory)) {
		return FALSE;
	}
...
static gboolean
desktop_is_not_empty (AnchorDirectory *directory)
{
	AnchorDesktopDirectory *desktop;

	desktop = NAUTILUS_DESKTOP_DIRECTORY (directory);

	if (anchor_directory_is_not_empty (desktop->details->real_directory)) {
		return TRUE;
	}
...
static GList *
desktop_get_file_list (AnchorDirectory *directory)
{
	GList *real_dir_file_list, *desktop_dir_file_list = NULL;
	
	real_dir_file_list = anchor_directory_get_file_list
				(NAUTILUS_DESKTOP_DIRECTORY (directory)->details->real_directory);
	desktop_dir_file_list = NAUTILUS_DIRECTORY_CLASS (anchor_desktop_directory_parent_class)->get_file_list (directory);

	return g_list_concat (real_dir_file_list, desktop_dir_file_list);
}

AnchorDirectory *
anchor_desktop_directory_get_real_directory (AnchorDesktopDirectory *desktop)
{
	anchor_directory_ref (desktop->details->real_directory);
...
desktop_finalize (GObject *object)
{
	...
	anchor_directory_unref (desktop->details->real_directory);
	...
	G_OBJECT_CLASS (anchor_desktop_directory_parent_class)->finalize (object);
}

static void
done_loading_callback (AnchorDirectory *real_directory,
		       AnchorDesktopDirectory *desktop)
{
     anchor_directory_emit_done_loading (NAUTILUS_DIRECTORY (desktop));
}


static void
forward_files_added_cover (AnchorDirectory *real_directory,
			   GList *files,
			   gpointer callback_data)
{
	anchor_directory_emit_files_added (NAUTILUS_DIRECTORY (callback_data), files);
}

static void
forward_files_changed_cover (AnchorDirectory *real_directory,
			     GList *files,
			     gpointer callback_data)
{
	anchor_directory_emit_files_changed (NAUTILUS_DIRECTORY (callback_data), files);
}

static void
update_desktop_directory (AnchorDesktopDirectory *desktop)
{
	char *desktop_path;
	char *desktop_uri;
	AnchorDirectory *real_directory;
...
desktop_directory_changed_callback (gpointer data)
{
	...
	anchor_directory_force_reload (NAUTILUS_DIRECTORY (data));
}

static void
anchor_desktop_directory_init (AnchorDesktopDirectory *desktop)
{
	desktop->details = g_new0 (AnchorDesktopDirectoryDetails, 1);
...
static void
anchor_desktop_directory_class_init (AnchorDesktopDirectoryClass *class)
{
	AnchorDirectoryClass *directory_class;
	 ...
	 * in AnchorDesktopDirectory->details->real_directory,
