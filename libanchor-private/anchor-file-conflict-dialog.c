...
G_DEFINE_TYPE (AnchorFileConflictDialog,
	       anchor_file_conflict_dialog,
	       GTK_TYPE_DIALOG);

#define NAUTILUS_FILE_CONFLICT_DIALOG_GET_PRIVATE(object)		\
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), ANCHOR_TYPE_FILE_CONFLICT_DIALOG, \
				      AnchorFileConflictDialogDetails))

static void
file_icons_changed (AnchorFile *file,
		    AnchorFileConflictDialog *fcd)
{
	GdkPixbuf *pixbuf;

	pixbuf = anchor_file_get_icon_pixbuf (fcd->details->destination,
	...
	pixbuf = anchor_file_get_icon_pixbuf (fcd->details->source,
						NAUTILUS_ICON_SIZE_LARGE,
						TRUE,
...
file_list_ready_cb (GList *files,
		    gpointer user_data)
{
	AnchorFileConflictDialog *fcd = user_data;
	AnchorFile *src, *dest, *dest_dir;
	...
	src_mtime = anchor_file_get_mtime (src);
	dest_mtime = anchor_file_get_mtime (dest);

	dest_name = anchor_file_get_display_name (dest);
	dest_dir_name = anchor_file_get_display_name (dest_dir);

	source_is_dir = anchor_file_is_directory (src);
	dest_is_dir = anchor_file_is_directory (dest);

	type = anchor_file_get_mime_type (dest);
	should_show_type = !anchor_file_is_mime_type (src, type);
	...
	/* Set up file icons */
	pixbuf = anchor_file_get_icon_pixbuf (dest,
						NAUTILUS_ICON_SIZE_LARGE,
						TRUE,
	...
	pixbuf = anchor_file_get_icon_pixbuf (src,
						NAUTILUS_ICON_SIZE_LARGE,
						TRUE,
						gtk_widget_get_scale_factor (fcd->details->titles_vbox),
						NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS);
	...
	date = anchor_file_get_string_attribute (dest,
						   "date_modified");
	size = anchor_file_get_string_attribute (dest, "size");

	if (should_show_type) {
		type = anchor_file_get_string_attribute (dest, "type");
	}
	...
	/* Second label */
	label = gtk_label_new (NULL);
	date = anchor_file_get_string_attribute (src,
						   "date_modified");
	size = anchor_file_get_string_attribute (src, "size");

	if (should_show_type) {
		type = anchor_file_get_string_attribute (src, "type");
	}
	...
	/* Populate the entry */
	edit_name = anchor_file_get_edit_name (dest);
	...
	anchor_file_monitor_add (src, fcd, NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);
	anchor_file_monitor_add (dest, fcd, NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);
...
static void
build_dialog_appearance (AnchorFileConflictDialog *fcd)
{
	GList *files = NULL;
	AnchorFileConflictDialogDetails *details = fcd->details;
	...
	anchor_file_list_call_when_ready (files,
...
set_source_and_destination (GtkWidget *w,
			    ...
			    GFile *dest_dir)
{
	AnchorFileConflictDialog *dialog;
	AnchorFileConflictDialogDetails *details;

	dialog = NAUTILUS_FILE_CONFLICT_DIALOG (w);
	details = dialog->details;

	details->source = anchor_file_get (source);
	details->destination = anchor_file_get (destination);
	details->dest_dir = anchor_file_get (dest_dir);
...
static void
anchor_file_conflict_dialog_init (AnchorFileConflictDialog *fcd)
{
...
do_finalize (GObject *self)
{
	...
	if (details->handle != NULL) {
		anchor_file_list_cancel_call_when_ready (details->handle);
	}

	if (details->src_handler_id) {
		g_signal_handler_disconnect (details->source, details->src_handler_id);
		anchor_file_monitor_remove (details->source, self);
	}

	if (details->dest_handler_id) {
		g_signal_handler_disconnect (details->destination, details->dest_handler_id);
		anchor_file_monitor_remove (details->destination, self);
	}

	anchor_file_unref (details->source);
	anchor_file_unref (details->destination);
	anchor_file_unref (details->dest_dir);

	G_OBJECT_CLASS (anchor_file_conflict_dialog_parent_class)->finalize (self);
}

static void
anchor_file_conflict_dialog_class_init (AnchorFileConflictDialogClass *klass)
{
	G_OBJECT_CLASS (klass)->finalize = do_finalize;

	g_type_class_add_private (klass, sizeof (AnchorFileConflictDialogDetails));
}

char *
anchor_file_conflict_dialog_get_new_name (AnchorFileConflictDialog *dialog)
{
...
gboolean
anchor_file_conflict_dialog_get_apply_to_all (AnchorFileConflictDialog *dialog)
{
...
GtkWidget *
anchor_file_conflict_dialog_new (GtkWindow *parent,
				   GFile *source,
				   GFile *destination,
				   GFile *dest_dir)
{
	...
	dialog = GTK_WIDGET (g_object_new (ANCHOR_TYPE_FILE_CONFLICT_DIALOG,
					   "use-header-bar", TRUE,
					   "modal", TRUE,
