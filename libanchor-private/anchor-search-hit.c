...
G_DEFINE_TYPE (AnchorSearchHit, anchor_search_hit, G_TYPE_OBJECT)

void
anchor_search_hit_compute_scores (AnchorSearchHit *hit,
				    AnchorQuery     *query)
{
	GDateTime *now;
	char *query_uri;
	...
	query_uri = anchor_query_get_location (query);
...
const char *
anchor_search_hit_get_uri (AnchorSearchHit *hit)
{
	return hit->details->uri;
}

gdouble
anchor_search_hit_get_relevance (AnchorSearchHit *hit)
{
	return hit->details->relevance;
}

static void
anchor_search_hit_set_uri (AnchorSearchHit *hit,
			     const char        *uri)
{
...
void
anchor_search_hit_set_fts_rank (AnchorSearchHit *hit,
				  gdouble            rank)
{
...
void
anchor_search_hit_set_modification_time (AnchorSearchHit *hit,
					   GDateTime         *date)
{
	if (hit->details->modification_time != NULL)
...
void
anchor_search_hit_set_access_time (AnchorSearchHit *hit,
				     GDateTime         *date)
{
	if (hit->details->access_time != NULL)
...
static void
anchor_search_hit_set_property (GObject *object,
				  guint arg_id,
				  const GValue *value,
				  GParamSpec *pspec)
{
	...
	case PROP_RELEVANCE:
		...
		break;
	case PROP_FTS_RANK:
		anchor_search_hit_set_fts_rank (hit, g_value_get_double (value));
		break;
	case PROP_URI:
		anchor_search_hit_set_uri (hit, g_value_get_string (value));
		break;
	case PROP_MODIFICATION_TIME:
		anchor_search_hit_set_modification_time (hit, g_value_get_boxed (value));
		break;
	case PROP_ACCESS_TIME:
		anchor_search_hit_set_access_time (hit, g_value_get_boxed (value));
		break;
	default:
...
static void
anchor_search_hit_get_property (GObject *object,
				  guint arg_id,
				  GValue *value,
				  GParamSpec *pspec)
{
...
static void
anchor_search_hit_finalize (GObject *object)
{
	AnchorSearchHit *hit = NAUTILUS_SEARCH_HIT (object);
	...
	G_OBJECT_CLASS (anchor_search_hit_parent_class)->finalize (object);
}

static void
anchor_search_hit_class_init (AnchorSearchHitClass *class)
{
	GObjectClass *object_class;

	object_class = (GObjectClass *) class;

	object_class->finalize = anchor_search_hit_finalize;
	object_class->get_property = anchor_search_hit_get_property;
	object_class->set_property = anchor_search_hit_set_property;
	...
	g_type_class_add_private (class, sizeof (AnchorSearchHitDetails));
}

static void
anchor_search_hit_init (AnchorSearchHit *hit)
{
	hit->details = G_TYPE_INSTANCE_GET_PRIVATE (hit,
						    ANCHOR_TYPE_SEARCH_HIT,
						    AnchorSearchHitDetails);
}

AnchorSearchHit *
anchor_search_hit_new (const char *uri)
{
	AnchorSearchHit *hit;

	hit = g_object_new (ANCHOR_TYPE_SEARCH_HIT,
			    "uri", uri,
			    NULL);
