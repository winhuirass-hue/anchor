...
/* anchor-canvas-container.c - Canvas container widget.
...
#include <X11/Xatom.h>
#include <math.h>
#include "anchor-canvas-container.h"

#include "anchor-global-preferences.h"
#include "anchor-canvas-private.h"
#include "anchor-lib-self-check-functions.h"
#include "anchor-selection-canvas-item.h"
...
} AnchorCanvasContainerAccessiblePrivate;

static GType         anchor_canvas_container_accessible_get_type  (void);
static void          preview_selected_items                         (AnchorCanvasContainer *container);
static void          activate_selected_items                        (AnchorCanvasContainer *container);
static void          activate_selected_items_alternate              (AnchorCanvasContainer *container,
								     AnchorCanvasIcon          *icon);
static void          compute_stretch                                (StretchState          *start,
								     StretchState          *current);
static AnchorCanvasIcon *get_first_selected_icon                        (AnchorCanvasContainer *container);
static AnchorCanvasIcon *get_nth_selected_icon                          (AnchorCanvasContainer *container,
									 int                    index);
static gboolean      has_multiple_selection                         (AnchorCanvasContainer *container);
static gboolean      all_selected                                   (AnchorCanvasContainer *container);
static gboolean      has_selection                                  (AnchorCanvasContainer *container);
static void          icon_destroy                                   (AnchorCanvasContainer *container,
								       AnchorCanvasIcon          *icon);
static void          end_renaming_mode                              (AnchorCanvasContainer *container,
								     gboolean               commit);
static AnchorCanvasIcon *get_icon_being_renamed                         (AnchorCanvasContainer *container);
static void          finish_adding_new_icons                        (AnchorCanvasContainer *container);
static inline void   icon_get_bounding_box                          (AnchorCanvasIcon          *icon,
								       int                   *x1_return,
								       int                   *y1_return,
								       int                   *x2_return,
								       int                   *y2_return,
								       AnchorCanvasItemBoundsUsage usage);
static gboolean      is_renaming                                    (AnchorCanvasContainer *container);
static gboolean      is_renaming_pending                            (AnchorCanvasContainer *container);
static void          process_pending_icon_to_rename                 (AnchorCanvasContainer *container);
static void          anchor_canvas_container_stop_monitor_top_left  (AnchorCanvasContainer *container,
								       AnchorCanvasIconData      *data,
								       gconstpointer          client);
static void          anchor_canvas_container_start_monitor_top_left (AnchorCanvasContainer *container,
								       AnchorCanvasIconData      *data,
								       gconstpointer          client,
								       gboolean               large_text);
static void          handle_hadjustment_changed                     (GtkAdjustment         *adjustment,
								     AnchorCanvasContainer *container);
static void          handle_vadjustment_changed                     (GtkAdjustment         *adjustment,
								     AnchorCanvasContainer *container);
static GList *       anchor_canvas_container_get_selected_icons (AnchorCanvasContainer *container);
static void          anchor_canvas_container_update_visible_icons   (AnchorCanvasContainer *container);
static void          reveal_icon                                    (AnchorCanvasContainer *container,
								       AnchorCanvasIcon *icon);

static void	     anchor_canvas_container_set_rtl_positions (AnchorCanvasContainer *container);
static double	     get_mirror_x_position                     (AnchorCanvasContainer *container,
								AnchorCanvasIcon *icon,
								double x);
static void         text_ellipsis_limit_changed_container_callback  (gpointer callback_data);

static int compare_icons_horizontal (AnchorCanvasContainer *container,
				       AnchorCanvasIcon *icon_a,
				       AnchorCanvasIcon *icon_b);

static int compare_icons_vertical (AnchorCanvasContainer *container,
				     AnchorCanvasIcon *icon_a,
				     AnchorCanvasIcon *icon_b);

static void store_layout_timestamps_now (AnchorCanvasContainer *container);

static const char *anchor_canvas_container_accessible_action_names[] = {
	"activate",
	"menu",
	NULL
};

static const char *anchor_canvas_container_accessible_action_descriptions[] = {
	"Activate selected items",
	"Popup context menu",
	NULL
};

G_DEFINE_TYPE (AnchorCanvasContainer, anchor_canvas_container, EEL_TYPE_CANVAS);

/* The AnchorCanvasContainer signals.  */
enum {
	ACTIVATE,
	ACTIVATE_ALTERNATE,
	ACTIVATE_PREVIEWER,
...
/* Functions dealing with AnchorIcons.  */

static void
icon_free (AnchorCanvasIcon *icon)
{
...
static gboolean
icon_is_positioned (const AnchorCanvasIcon *icon)
{
...
static void
icon_set_position (AnchorCanvasIcon *icon,
		     double x, double y)
{	
	AnchorCanvasContainer *container;
	...
	if (anchor_canvas_container_get_is_fixed_size (container)) {
		...
		*/
		...
		icon_bounds = anchor_canvas_item_get_icon_rectangle (icon->item);
...
static void
icon_get_size (AnchorCanvasContainer *container,
		 AnchorCanvasIcon *icon,
		 guint *size)
{
	if (size != NULL) {
		*size = MAX (anchor_get_icon_size_for_zoom_level (container->details->zoom_level)
 ...
 */
static void
icon_set_size (AnchorCanvasContainer *container,
		 AnchorCanvasIcon *icon,
		 guint icon_size,
		 gboolean snap,
		 gboolean update_position)
{
	...
	scale = (double) icon_size /
		anchor_get_icon_size_for_zoom_level
		(container->details->zoom_level);
	anchor_canvas_container_move_icon (container, icon,
					       icon->x, icon->y,
					       scale, FALSE,
...
static void
icon_raise (AnchorCanvasIcon *icon)
{
...
static void
emit_stretch_started (AnchorCanvasContainer *container, AnchorCanvasIcon *icon)
{
...
static void
emit_stretch_ended (AnchorCanvasContainer *container, AnchorCanvasIcon *icon)
{
	g_signal_emit (container,
...
static void
icon_toggle_selected (AnchorCanvasContainer *container,
		      AnchorCanvasIcon *icon)
{		
	end_renaming_mode (container, TRUE);
	...
	if (icon == container->details->stretch_icon) {
		...
		anchor_canvas_item_set_show_stretch_handles (icon->item, FALSE);
		/* snap the icon if necessary */
		if (container->details->keep_aligned) {
			anchor_canvas_container_move_icon (container,
							       icon,
							       icon->x, icon->y,
							       icon->scale,
							       FALSE, TRUE, TRUE);
...
static gboolean
icon_set_selected (AnchorCanvasContainer *container,
		     AnchorCanvasIcon *icon,
		     gboolean select)
{
...
static inline void
icon_get_bounding_box (AnchorCanvasIcon *icon,
		       ...
		       AnchorCanvasItemBoundsUsage usage)
{
	...
	if (usage == BOUNDS_USAGE_FOR_DISPLAY) {
		...
		anchor_canvas_item_get_bounds_for_layout (icon->item,
								   &x1, &y1, &x2, &y2);
	} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
		anchor_canvas_item_get_bounds_for_entire_item (icon->item,
									&x1, &y1, &x2, &y2);
	} else {
...
/* Utility functions for AnchorCanvasContainer.  */

gboolean
anchor_canvas_container_scroll (AnchorCanvasContainer *container,
...
static void
pending_icon_to_reveal_destroy_callback (AnchorCanvasItem *item,
					   AnchorCanvasContainer *container)
{
...
static AnchorCanvasIcon *
get_pending_icon_to_reveal (AnchorCanvasContainer *container)
{
	return container->details->pending_icon_to_reveal;
}

static void
set_pending_icon_to_reveal (AnchorCanvasContainer *container, AnchorCanvasIcon *icon)
{
	AnchorCanvasIcon *old_icon;
...
static void
icon_get_row_and_column_bounds (AnchorCanvasContainer *container,
				AnchorCanvasIcon *icon,
				EelIRect *bounds,
				gboolean safety_pad)
{
	GList *p;
	AnchorCanvasIcon *one_icon;
	EelIRect one_bounds;
...
static void
reveal_icon (AnchorCanvasContainer *container,
	     AnchorCanvasIcon *icon)
{
	...
	if (anchor_canvas_container_is_auto_layout (container)) {
...
static void
process_pending_icon_to_reveal (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *pending_icon_to_reveal;
...
static void
unschedule_keyboard_icon_reveal (AnchorCanvasContainer *container)
{
...
static void
schedule_keyboard_icon_reveal (AnchorCanvasContainer *container,
				 AnchorCanvasIcon *icon)
{
	AnchorCanvasContainerDetails *details;
...
static void
clear_keyboard_focus (AnchorCanvasContainer *container)
{
...
static void inline
emit_atk_focus_tracker_notify (AnchorCanvasIcon *icon)
{
...
static void
set_keyboard_focus (AnchorCanvasContainer *container,
		    AnchorCanvasIcon *icon)
{
	g_assert (icon != NULL);
...
static void
set_keyboard_rubberband_start (AnchorCanvasContainer *container,
			       AnchorCanvasIcon *icon)
{
...
static void
clear_keyboard_rubberband_start (AnchorCanvasContainer *container)
{
	container->details->keyboard_rubberband_start = NULL;
}

/* carbon-copy of eel_canvas_group_bounds(), but
 * for AnchorCanvasContainerItems it returns the
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (child->flags & EEL_CANVAS_ITEM_VISIBLE) {
			...
			if (!NAUTILUS_IS_CANVAS_ITEM (child) ||
				...
				anchor_canvas_item_get_bounds_for_layout (NAUTILUS_CANVAS_ITEM (child),
										   &minx, &miny, &maxx, &maxy);
			} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
				anchor_canvas_item_get_bounds_for_entire_item (NAUTILUS_CANVAS_ITEM (child),
											&minx, &miny, &maxx, &maxy);
			} else {
				g_assert_not_reached ();
			}
...
static void
get_all_icon_bounds (AnchorCanvasContainer *container,
		     double *x1, double *y1,
		     double *x2, double *y2,
		     AnchorCanvasItemBoundsUsage usage)
{
...
void
anchor_canvas_container_reset_scroll_region (AnchorCanvasContainer *container)
{
...
void
anchor_canvas_container_update_scroll_region (AnchorCanvasContainer *container)
{
	double x1, y1, x2, y2;
	double pixels_per_unit;
	GtkAdjustment *hadj, *vadj;
	...
	reset_scroll_region = container->details->reset_scroll_region_trigger
		|| anchor_canvas_container_is_empty (container)
		|| anchor_canvas_container_is_auto_layout (container);
	 ...
	 */
	if (!anchor_canvas_container_is_empty (container)) {
	 ...
	 */
	if (anchor_canvas_container_is_layout_vertical (container)) {
		if (anchor_canvas_container_is_layout_rtl (container)) {
	...
	step_increment = anchor_get_icon_size_for_zoom_level
...
{
	AnchorCanvasContainerClass *klass;
	const AnchorCanvasIcon *icon_a, *icon_b;
...
static void
sort_icons (AnchorCanvasContainer *container,
	      GList                **icons)
{
	AnchorCanvasContainerClass *klass;
...
static void
resort (AnchorCanvasContainer *container)
{
...
#if 0
static double
get_grid_width (AnchorCanvasContainer *container)
{
...
static void
lay_down_one_line (AnchorCanvasContainer *container,
		   GList *line_start,
		   GList *line_end,
		   double y,
		   double max_height,
		   ...
		   gboolean whole_text)
{
	...
	is_rtl = anchor_canvas_container_is_layout_rtl (container);
	...
	for (p = line_start; p != line_end; p = p->next) {
		...
		icon_set_position
			(icon,
			 ...
			 y + y_offset);
		anchor_canvas_item_set_entire_text (icon->item, whole_text);
...
static void
lay_down_icons_horizontal (AnchorCanvasContainer *container,
			     GList *icons,
			     double start_y)
{
	GList *p, *line_start;
...
static void
snap_position (AnchorCanvasContainer *container,
	       AnchorCanvasIcon *icon,
	       int *x, int *y)
{
	int center_x;
	...
	canvas_position = anchor_canvas_item_get_icon_rectangle (icon->item);
...
{
	AnchorCanvasIcon *icon_a, *icon_b;
	int x1, y1, x2, y2;
	int center_a;
	int center_b;

	icon_a = (AnchorCanvasIcon *)a;
	icon_b = (AnchorCanvasIcon *)b;
...
placement_grid_new (AnchorCanvasContainer *container, gboolean tight)
{
	PlacementGrid *grid;
	int width, height;
	int num_columns;
...
static void
placement_grid_mark_icon (PlacementGrid *grid, AnchorCanvasIcon *icon)
{
	EelIRect canvas_pos;
	EelIRect grid_pos;
...
static void
find_empty_location (AnchorCanvasContainer *container,
		     PlacementGrid *grid,
		     AnchorCanvasIcon *icon,
		     int start_x,
		     int start_y,
		     ...
		     int *y)
{
	...
	pixbuf_rect = anchor_canvas_item_get_icon_rectangle (icon->item);
...
static void
align_icons (AnchorCanvasContainer *container)
{
	...
	if (anchor_canvas_container_is_layout_rtl (container)) {
		anchor_canvas_container_set_rtl_positions (container);
	}
}

static double
get_mirror_x_position (AnchorCanvasContainer *container, AnchorCanvasIcon *icon, double x)
{
	EelDRect icon_bounds;
	GtkAllocation allocation;
...
static void
anchor_canvas_container_set_rtl_positions (AnchorCanvasContainer *container)
{
	GList *l;
	AnchorCanvasIcon *icon;
	double x;
...
static void
lay_down_icons_vertical_desktop (AnchorCanvasContainer *container, GList *icons)
{
	...
	if (placed > 0) {
		...
		if (grid) {
			...
			for (p = unplaced_icons; p != NULL; p = p->next) {
				...
				icon_rect = anchor_canvas_item_get_icon_rectangle (icon->item);
	 ...
	 */
	anchor_canvas_container_freeze_icon_positions (container);
}


static void
lay_down_icons (AnchorCanvasContainer *container, GList *icons, double start_y)
{
...
static void
redo_layout_internal (AnchorCanvasContainer *container)
{
	...
	anchor_canvas_container_update_scroll_region (container);

	process_pending_icon_to_reveal (container);
	process_pending_icon_to_rename (container);
	anchor_canvas_container_update_visible_icons (container);
}
...
static void
unschedule_redo_layout (AnchorCanvasContainer *container)
{
...
static void
schedule_redo_layout (AnchorCanvasContainer *container)
{
	if (container->details->idle_id == 0
...
static void
redo_layout (AnchorCanvasContainer *container)
{
...
static void
reload_icon_positions (AnchorCanvasContainer *container)
{
...
static void
invalidate_label_sizes (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		anchor_canvas_item_invalidate_label_size (icon->item);		
	}
}

/* invalidate the entire labels (i.e. their attributes) for all the icons */
static void
invalidate_labels (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		anchor_canvas_item_invalidate_label (icon->item);		
	}
}

static gboolean
select_range (AnchorCanvasContainer *container,
	      AnchorCanvasIcon *icon1,
	      AnchorCanvasIcon *icon2,
	      gboolean unselect_outside_range)
{
	gboolean selection_changed;
	GList *p;
	AnchorCanvasIcon *icon;
	AnchorCanvasIcon *unmatched_icon;
	gboolean select;
...
static gboolean
select_one_unselect_others (AnchorCanvasContainer *container,
			    AnchorCanvasIcon *icon_to_select)
{
	gboolean selection_changed;
	GList *p;
	AnchorCanvasIcon *icon;
...
static gboolean
unselect_all (AnchorCanvasContainer *container)
{
	return select_one_unselect_others (container, NULL);
}

void
anchor_canvas_container_move_icon (AnchorCanvasContainer *container,
				       AnchorCanvasIcon *icon,
				       int x, int y,
				       double scale,
				       gboolean raise,
				       ...
				       gboolean update_position)
{
	...
	if (scale != icon->scale) {
		icon->scale = scale;
		anchor_canvas_container_update_icon (container, icon);
		if (update_position) {
	...
	if (!details->auto_layout) {
		...
		icon->saved_ltr_x = anchor_canvas_container_is_layout_rtl (container) ? get_mirror_x_position (container, icon, icon->x) : icon->x;
	}
...
static void
rubberband_select (AnchorCanvasContainer *container,
		   ...
		   const EelDRect *current_rect)
{
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		is_in = anchor_canvas_item_hit_test_rectangle (icon->item, canvas_rect);
...
rubberband_timeout_callback (gpointer data)
{
	AnchorCanvasContainer *container;
	GtkWidget *widget;
	AnchorCanvasRubberbandInfo *band_info;
	int x, y;
	double x1, y1, x2, y2;
	...
	anchor_canvas_container_scroll (container, x_scroll, y_scroll);
...
static void
get_rubber_color (AnchorCanvasContainer *container,
                  GdkRGBA *bgcolor,
                  GdkRGBA *bordercolor)
{
	Atom         real_type;
	...
	if (anchor_canvas_container_get_is_desktop (container)) {
...
static void
start_rubberbanding (AnchorCanvasContainer *container,
		     GdkEventButton *event)
{
	...
	band_info->selection_rectangle = eel_canvas_item_new
		(eel_canvas_root
		 (EEL_CANVAS (container)),
		 ANCHOR_TYPE_SELECTION_CANVAS_ITEM,
		 "x1", band_info->start_x,
		 "y1", band_info->start_y,
		 "x2", band_info->start_x,
		 "y2", band_info->start_y,
...
static void
stop_rubberbanding (AnchorCanvasContainer *container,
		    guint32 time)
{
	AnchorCanvasRubberbandInfo *band_info;
	GList *icons;
	...
	if (enable_animation) {
		anchor_selection_canvas_item_fade_out (NAUTILUS_SELECTION_CANVAS_ITEM (band_info->selection_rectangle), 150);
	} else {
	...
	icons = anchor_canvas_container_get_selected_icons (container);
...
typedef gboolean (* IsBetterCanvasFunction) (AnchorCanvasContainer *container,
					     AnchorCanvasIcon *start_icon,
					     AnchorCanvasIcon *best_so_far,
					     AnchorCanvasIcon *candidate,
					     void *data);

static AnchorCanvasIcon *
find_best_icon (AnchorCanvasContainer *container,
		  AnchorCanvasIcon *start_icon,
		  IsBetterCanvasFunction function,
		  void *data)
{
	GList *p;
	AnchorCanvasIcon *best, *candidate;
...
static AnchorCanvasIcon *
find_best_selected_icon (AnchorCanvasContainer *container,
			   AnchorCanvasIcon *start_icon,
			   IsBetterCanvasFunction function,
			   void *data)
{
...
static int
compare_icons_by_uri (AnchorCanvasContainer *container,
			AnchorCanvasIcon *icon_a,
			AnchorCanvasIcon *icon_b)
{
	char *uri_a, *uri_b;
	...
	uri_a = anchor_canvas_container_get_icon_uri (container, icon_a);
	uri_b = anchor_canvas_container_get_icon_uri (container, icon_b);
...
static int
get_cmp_point_x (AnchorCanvasContainer *container,
		 EelDRect icon_rect)
{
...
static int
get_cmp_point_y (AnchorCanvasContainer *container,
		 EelDRect icon_rect)
{
	return icon_rect.y1;
}
...
static int
compare_icons_horizontal (AnchorCanvasContainer *container,
			    AnchorCanvasIcon *icon_a,
			    AnchorCanvasIcon *icon_b)
{
	EelDRect world_rect;
	...
	world_rect = anchor_canvas_item_get_icon_rectangle (icon_a->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 ...
		 &ax,
		 NULL);
	world_rect = anchor_canvas_item_get_icon_rectangle (icon_b->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static int
compare_icons_vertical (AnchorCanvasContainer *container,
			  AnchorCanvasIcon *icon_a,
			  AnchorCanvasIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_horizontal_first (AnchorCanvasContainer *container,
				  AnchorCanvasIcon *icon_a,
				  AnchorCanvasIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_vertical_first (AnchorCanvasContainer *container,
				AnchorCanvasIcon *icon_a,
				AnchorCanvasIcon *icon_b)
{
	EelDRect world_rect;
...
static gboolean
leftmost_in_top_row (AnchorCanvasContainer *container,
		     AnchorCanvasIcon *start_icon,
		     AnchorCanvasIcon *best_so_far,
		     AnchorCanvasIcon *candidate,
		     void *data)
...
static gboolean
rightmost_in_top_row (AnchorCanvasContainer *container,
		      AnchorCanvasIcon *start_icon,
		      AnchorCanvasIcon *best_so_far,
		      AnchorCanvasIcon *candidate,
		      void *data)
...
static gboolean
rightmost_in_bottom_row (AnchorCanvasContainer *container,
			 AnchorCanvasIcon *start_icon,
			 AnchorCanvasIcon *best_so_far,
			 AnchorCanvasIcon *candidate,
			 void *data)
...
static int
compare_with_start_row (AnchorCanvasContainer *container,
			AnchorCanvasIcon *icon)
{
	EelCanvasItem *item;
...
static int
compare_with_start_column (AnchorCanvasContainer *container,
			   AnchorCanvasIcon *icon)
{
	EelCanvasItem *item;
...
static gboolean
same_row_right_side_leftmost (AnchorCanvasContainer *container,
			      AnchorCanvasIcon *start_icon,
			      AnchorCanvasIcon *best_so_far,
			      AnchorCanvasIcon *candidate,
			      void *data)
...
static gboolean
same_row_left_side_rightmost (AnchorCanvasContainer *container,
			      AnchorCanvasIcon *start_icon,
			      AnchorCanvasIcon *best_so_far,
			      AnchorCanvasIcon *candidate,
			      void *data)
...
static gboolean
next_row_leftmost (AnchorCanvasContainer *container,
		   AnchorCanvasIcon *start_icon,
	           AnchorCanvasIcon *best_so_far,
		   AnchorCanvasIcon *candidate,
		   void *data)
...
static gboolean
next_row_rightmost (AnchorCanvasContainer *container,
		    AnchorCanvasIcon *start_icon,
		    AnchorCanvasIcon *best_so_far,
		    AnchorCanvasIcon *candidate,
		    void *data)
...
static gboolean
next_column_bottommost (AnchorCanvasContainer *container,
			AnchorCanvasIcon *start_icon,
			AnchorCanvasIcon *best_so_far,
			AnchorCanvasIcon *candidate,
			void *data)
...
static gboolean
previous_row_rightmost (AnchorCanvasContainer *container,
		        AnchorCanvasIcon *start_icon,
			AnchorCanvasIcon *best_so_far,
			AnchorCanvasIcon *candidate,
			void *data)
...
static gboolean
same_column_above_lowest (AnchorCanvasContainer *container,
			  AnchorCanvasIcon *start_icon,
			  AnchorCanvasIcon *best_so_far,
			  AnchorCanvasIcon *candidate,
			  void *data)
...
static gboolean
same_column_below_highest (AnchorCanvasContainer *container,
			   AnchorCanvasIcon *start_icon,
			   AnchorCanvasIcon *best_so_far,
			   AnchorCanvasIcon *candidate,
			   void *data)
...
static gboolean
previous_column_highest (AnchorCanvasContainer *container,
			 AnchorCanvasIcon *start_icon,
			 AnchorCanvasIcon *best_so_far,
			 AnchorCanvasIcon *candidate,
			 void *data)
...
static gboolean
next_column_highest (AnchorCanvasContainer *container,
		     AnchorCanvasIcon *start_icon,
		     AnchorCanvasIcon *best_so_far,
		     AnchorCanvasIcon *candidate,
		     void *data)
...
static gboolean
previous_column_lowest (AnchorCanvasContainer *container,
		        AnchorCanvasIcon *start_icon,
			AnchorCanvasIcon *best_so_far,
			AnchorCanvasIcon *candidate,
			void *data)
...
static gboolean
last_column_lowest (AnchorCanvasContainer *container,
		    AnchorCanvasIcon *start_icon,
		    AnchorCanvasIcon *best_so_far,
		    AnchorCanvasIcon *candidate,
		    void *data)
...
static gboolean
closest_in_90_degrees (AnchorCanvasContainer *container,
		       AnchorCanvasIcon *start_icon,
		       AnchorCanvasIcon *best_so_far,
		       AnchorCanvasIcon *candidate,
		       void *data)
{
	...
	world_rect = anchor_canvas_item_get_icon_rectangle (candidate->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static EelDRect 
get_rubberband (AnchorCanvasIcon *icon1,
		AnchorCanvasIcon *icon2)
{
	EelDRect rect1;
	EelDRect rect2;
...
static void
keyboard_move_to (AnchorCanvasContainer *container,
		  AnchorCanvasIcon *icon,
		  AnchorCanvasIcon *from,
		  GdkEventKey *event)
{
	if (icon == NULL) {
		return;
...
static void
keyboard_home (AnchorCanvasContainer *container,
	       GdkEventKey *event)
{
	AnchorCanvasIcon *from;
	AnchorCanvasIcon *to;
...
static void
keyboard_end (AnchorCanvasContainer *container,
	      GdkEventKey *event)
{
	AnchorCanvasIcon *to;
	AnchorCanvasIcon *from;
	...
	to = find_best_icon (container, NULL,
			       anchor_canvas_container_is_layout_vertical (container) ?
			       last_column_lowest :
			       rightmost_in_bottom_row,
			       NULL);
...
static void
record_arrow_key_start (AnchorCanvasContainer *container,
			AnchorCanvasIcon *icon,
			GtkDirectionType direction)
{
	EelDRect world_rect;

	world_rect = anchor_canvas_item_get_icon_rectangle (icon->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static void
keyboard_arrow_key (AnchorCanvasContainer *container,
		    GdkEventKey *event,
		    GtkDirectionType direction,
...
static void
keyboard_right (AnchorCanvasContainer *container,
		GdkEventKey *event)
{
	...
	keyboard_arrow_key (container,
			    ...
			    GTK_DIR_RIGHT,
			    rightmost_in_bottom_row,
			    anchor_canvas_container_is_layout_rtl (container) ?
...
static void
keyboard_left (AnchorCanvasContainer *container,
	       GdkEventKey *event)
{
	IsBetterCanvasFunction fallback;
...
static void
keyboard_down (AnchorCanvasContainer *container,
	       GdkEventKey *event)
{
	IsBetterCanvasFunction fallback;
...
static void
keyboard_up (AnchorCanvasContainer *container,
	     GdkEventKey *event)
{
	IsBetterCanvasFunction fallback;
...
static void
keyboard_space (AnchorCanvasContainer *container,
		GdkEventKey *event)
{
	AnchorCanvasIcon *icon;
...
static void
select_previous_or_next_icon (AnchorCanvasContainer *container, 
				gboolean next, 
				GdkEventKey *event)
{
	AnchorCanvasIcon *icon;
...
destroy (GtkWidget *object)
{
	...
	container = NAUTILUS_CANVAS_CONTAINER (object);
        ...
        anchor_canvas_container_clear (container);
	...
	GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->destroy (object);
}
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_canvas_container_parent_class)->finalize (object);
}
	       ...
	       GtkAllocation *allocation)
{
	...
	GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->size_allocate (widget, allocation);
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->realize (widget);

	container = NAUTILUS_CANVAS_CONTAINER (widget);

	/* Set up DnD.  */
	anchor_canvas_dnd_init (container);
...
unrealize (GtkWidget *widget)
{
	...
	anchor_canvas_dnd_fini (container);

	GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->unrealize (widget);
}
...
style_updated (GtkWidget *widget)
{
	 ...
	 */
	if (!anchor_canvas_container_get_is_desktop (container)) {
		GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->style_updated (widget);
	}

	if (gtk_widget_get_realized (widget)) {
		invalidate_labels (container);
		anchor_canvas_container_request_update_all (container);
	}
}
		    ...
		    GdkEventButton *event)
{
	...
	clicked_on_icon = GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->button_press_event (widget, event);
...
static void
anchor_canvas_container_did_not_drag (AnchorCanvasContainer *container,
					GdkEventButton *event)
{
	AnchorCanvasContainerDetails *details;
	gboolean selection_changed;
	...
	if (details->drag_icon != NULL &&
		...
		if (click_count == 0 &&
			 ...
			 * AnchorList goes the other way because its "links" seem
			 * much more link-like.
			 */
			if (event->button == MIDDLE_BUTTON) {
				activate_selected_items_alternate (container, NULL);
...
static gboolean
clicked_within_double_click_interval (AnchorCanvasContainer *container)
{
	static gint64 last_click_time = 0;
	static gint click_count = 0;
	gint double_click_time;
...
static void
clear_drag_state (AnchorCanvasContainer *container)
{
...
static gboolean
start_stretching (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	GtkWidget *toplevel;
	...
	if (!anchor_canvas_item_hit_test_stretch_handles (icon->item,
...
static gboolean
update_stretch_at_idle (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	double world_x, world_y;
...
static void
continue_stretching (AnchorCanvasContainer *container,
...
static gboolean
keyboard_stretching (AnchorCanvasContainer *container,
		     GdkEventKey           *event)
{
	AnchorCanvasIcon *icon;
	guint size;
...
static void
ungrab_stretch_icon (AnchorCanvasContainer *container)
{
...
static void
end_stretching (AnchorCanvasContainer *container,
		double world_x, double world_y)
{
	AnchorCanvasPosition position;
	AnchorCanvasIcon *icon;
...
static gboolean
undo_stretching (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *stretched_icon;
	...
	anchor_canvas_item_set_show_stretch_handles
		      ...
		      GdkEventButton *event)
{
	...
	if (event->button == details->drag_button) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			if (!details->drag_started) {
				anchor_canvas_container_did_not_drag (container, event);
			} else {
				anchor_canvas_dnd_end_drag (container);
	...
	return GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->button_release_event (widget, event);
}
		     ...
		     GdkEventMotion *event)
{
	...
	if (details->drag_button != 0) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			...
			if (gtk_drag_check_threshold (widget, 
				...
				anchor_canvas_dnd_begin_drag (container,
								actions,
								details->drag_button,
								event, 
								canvas_x,
	...
	return GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->motion_notify_event (widget, event);
}

static void
anchor_canvas_container_get_icon_text (AnchorCanvasContainer *container,
					   AnchorCanvasIconData      *data,
					   char                 **editable_text,
...
static gboolean
handle_popups (AnchorCanvasContainer *container,
	       GdkEventKey           *event,
	       const char            *signal)
{
		 ...
		 GdkEventKey *event)
{
	...
	if (!handled) {
		handled = GTK_WIDGET_CLASS (anchor_canvas_container_parent_class)->key_press_event (widget, event);
	}
...
static GObject*
anchor_canvas_container_constructor (GType                  type,
				       ...
				       GObjectConstructParam *construct_params)
{
	...
	object = G_OBJECT_CLASS (anchor_canvas_container_parent_class)->constructor
		(type,
		 n_construct_params,
		 construct_params);
...
static void
anchor_canvas_container_class_init (AnchorCanvasContainerClass *class)
{
	GtkWidgetClass *widget_class;
	EelCanvasClass *canvas_class;

	G_OBJECT_CLASS (class)->constructor = anchor_canvas_container_constructor;
	...
	signals[SELECTION_CHANGED]
		= g_signal_new ("selection-changed",
		                ...
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (AnchorCanvasContainerClass,
						 selection_changed),
		                NULL, NULL,
	...
	gtk_widget_class_set_accessible_type (widget_class, anchor_canvas_container_accessible_get_type ());
...
static void
update_selected (AnchorCanvasContainer *container)
{
	GList *node;
	AnchorCanvasIcon *icon;
...
text_ellipsis_limit_changed_callback (gpointer callback_data)
{
	...
	pref = g_settings_get_strv (anchor_icon_view_preferences,
...
desktop_text_ellipsis_limit_changed_callback (gpointer callback_data)
{
	...
	pref = g_settings_get_int (anchor_desktop_preferences, NAUTILUS_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
	desktop_text_ellipsis_limit = pref;
}

static void
anchor_canvas_container_init (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerDetails *details;
	static gboolean setup_prefs = FALSE;

	details = g_new0 (AnchorCanvasContainerDetails, 1);
...
static gboolean
handle_canvas_double_click (AnchorCanvasContainer *container,
			    AnchorCanvasIcon *icon,
			    GdkEventButton *event)
{
	AnchorCanvasContainerDetails *details;
...
/* AnchorCanvasIcon event handling.  */
...
static gboolean
handle_canvas_button_press (AnchorCanvasContainer *container,
			    AnchorCanvasIcon *icon,
			    GdkEventButton *event)
{
	AnchorCanvasContainerDetails *details;
...
GtkWidget *
anchor_canvas_container_new (void)
{
	return gtk_widget_new (ANCHOR_TYPE_CANVAS_CONTAINER, NULL);
}

/* Clear all of the icons in the container. */
void
anchor_canvas_container_clear (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	GList *p;
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		if (icon->is_monitored) {
			anchor_canvas_container_stop_monitor_top_left (container,
									 icon->data,
									 icon);
		}
		icon_free (p->data);
...
gboolean
anchor_canvas_container_is_empty (AnchorCanvasContainer *container)
{
	return container->details->icons == NULL;
}

AnchorCanvasIconData *
anchor_canvas_container_get_first_visible_icon (AnchorCanvasContainer *container)
{
	GList *l;
	AnchorCanvasIcon *icon, *best_icon;
	double x, y;
	double x1, y1, x2, y2;
	double *pos, best_pos;
...
void
anchor_canvas_container_scroll_to_canvas (AnchorCanvasContainer  *container,
					    AnchorCanvasIconData       *data)
{
	GList *l;
	AnchorCanvasIcon *icon;
	...
	anchor_canvas_container_layout_now (container);
...
void
anchor_canvas_container_for_each (AnchorCanvasContainer *container,
				    AnchorCanvasCallback callback,
				    gpointer callback_data)
{
	CallbackAndData callback_and_data;
...
static void
icon_destroy (AnchorCanvasContainer *container,
	      AnchorCanvasIcon *icon)
{
	AnchorCanvasContainerDetails *details;
	gboolean was_selected;
	AnchorCanvasIcon *icon_to_focus;
	GList *item;
...
static void
activate_selected_items (AnchorCanvasContainer *container)
{
	GList *selection;

	g_return_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container));

	selection = anchor_canvas_container_get_selection (container);
	if (selection != NULL) {
	  	g_signal_emit (container,
			       signals[ACTIVATE], 0,
			       selection);
...
static void
preview_selected_items (AnchorCanvasContainer *container)
{
	GList *selection;
	GArray *locations;
	gint idx;
	...
	selection = anchor_canvas_container_get_selection (container);
	locations = anchor_canvas_container_get_selected_icon_locations (container);
...
static void
activate_selected_items_alternate (AnchorCanvasContainer *container,
				   AnchorCanvasIcon *icon)
{
	GList *selection;
...
static AnchorCanvasIcon *
get_icon_being_renamed (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *rename_icon;
...
anchor_canvas_container_get_icon_images (AnchorCanvasContainer *container,
					     AnchorCanvasIconData      *data,
					     int                    size,
					     char                 **embedded_text,
...
static void
anchor_canvas_container_freeze_updates (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerClass *klass;
...
static void
anchor_canvas_container_unfreeze_updates (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerClass *klass;
...
static void
anchor_canvas_container_start_monitor_top_left (AnchorCanvasContainer *container,
						  AnchorCanvasIconData *data,
						  gconstpointer client,
						  gboolean large_text)
{
...
static void
anchor_canvas_container_stop_monitor_top_left (AnchorCanvasContainer *container,
						 AnchorCanvasIconData *data,
						 gconstpointer client)
{
	AnchorCanvasContainerClass *klass;
...
static void
anchor_canvas_container_prioritize_thumbnailing (AnchorCanvasContainer *container,
						   AnchorCanvasIcon *icon)
{
	AnchorCanvasContainerClass *klass;
...
static void
anchor_canvas_container_update_visible_icons (AnchorCanvasContainer *container)
{
	GtkAdjustment *vadj, *hadj;
	double min_y, max_y;
	double min_x, max_x;
	...
	for (node = g_list_last (container->details->icons); node != NULL; node = node->prev) {
		...
		if (icon_is_positioned (icon)) {
			...
			if (visible) {
				anchor_canvas_item_set_is_visible (icon->item, TRUE);
				anchor_canvas_container_prioritize_thumbnailing (container,
										   icon);
			} else {
				anchor_canvas_item_set_is_visible (icon->item, FALSE);
			}
		}
	}
}
...
void 
anchor_canvas_container_update_icon (AnchorCanvasContainer *container,
					 AnchorCanvasIcon *icon)
{
	AnchorCanvasContainerDetails *details;
	guint icon_size;
	...
	icon_info = anchor_canvas_container_get_icon_images (container, icon->data, icon_size,
							       &embedded_text,
	...
	anchor_icon_info_get_attach_points (icon_info, &attach_points, &n_attach_points);
	has_embedded_text_rect = anchor_icon_info_get_embedded_rect (icon_info,
									 &embedded_text_rect);
	...
	if (has_embedded_text_rect && embedded_text_needs_loading) {
		...
		anchor_canvas_container_start_monitor_top_left (container, icon->data, icon, large_embedded_text);
	}
	
	anchor_canvas_container_get_icon_text (container,
						   icon->data,
						   &editable_text,
						   &additional_text,
						   FALSE);
	...
	if (icon == get_icon_being_renamed (container) &&
	    g_strcmp0 (editable_text,
		       anchor_canvas_item_get_editable_text (icon->item)) != 0) {
		end_renaming_mode (container, FALSE);
	}
	...
	anchor_canvas_item_set_image (icon->item, pixbuf);
	anchor_canvas_item_set_attach_points (icon->item, attach_points, n_attach_points);
	anchor_canvas_item_set_embedded_text_rect (icon->item, &embedded_text_rect);
	anchor_canvas_item_set_embedded_text (icon->item, embedded_text);
...
static gboolean
assign_icon_position (AnchorCanvasContainer *container,
			AnchorCanvasIcon *icon)
{
	gboolean have_stored_position;
	AnchorCanvasPosition position;
...
static void
finish_adding_icon (AnchorCanvasContainer *container,
		    AnchorCanvasIcon *icon)
{
	anchor_canvas_container_update_icon (container, icon);
...
static void
finish_adding_new_icons (AnchorCanvasContainer *container)
{
...
static gboolean
is_old_or_unknown_icon_data (AnchorCanvasContainer *container,
			       AnchorCanvasIconData *data)
{
	time_t timestamp;
	gboolean success;
...
/**
 * anchor_canvas_container_add:
 * @container: A AnchorCanvasContainer
 * @data: Icon data.
 * 
 ...
 **/
gboolean
anchor_canvas_container_add (AnchorCanvasContainer *container,
			       AnchorCanvasIconData *data)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	...
	icon = g_new0 (AnchorCanvasIcon, 1);
	icon->data = data;
...
void
anchor_canvas_container_layout_now (AnchorCanvasContainer *container)
{
	if (container->details->idle_id != 0) {
		unschedule_redo_layout (container);
		redo_layout_internal (container);
...
/**
 * anchor_canvas_container_remove:
 * @container: A AnchorCanvasContainer.
 * @data: Icon data.
 * 
 ...
 **/
gboolean
anchor_canvas_container_remove (AnchorCanvasContainer *container,
				  AnchorCanvasIconData *data)
{
	AnchorCanvasIcon *icon;
 ...
 **/
void
anchor_canvas_container_request_update (AnchorCanvasContainer *container,
					  AnchorCanvasIconData *data)
{
	AnchorCanvasIcon *icon;
...
AnchorZoomLevel
anchor_canvas_container_get_zoom_level (AnchorCanvasContainer *container)
{
        return container->details->zoom_level;
}

void
anchor_canvas_container_set_zoom_level (AnchorCanvasContainer *container, int new_level)
{
	AnchorCanvasContainerDetails *details;
        int pinned_level;
	double pixels_per_unit;
	...
	pixels_per_unit = (double) anchor_get_icon_size_for_zoom_level (pinned_level)
...
/**
 * anchor_canvas_container_request_update_all:
 ...
 **/
void
anchor_canvas_container_request_update_all (AnchorCanvasContainer *container)
{
	GList *node;
	AnchorCanvasIcon *icon;
 ...
 */
void
anchor_canvas_container_reveal (AnchorCanvasContainer *container, AnchorCanvasIconData *data)
{
	AnchorCanvasIcon *icon;
...
/**
 * anchor_canvas_container_get_selection:
 * @container: An canvas container.
 * 
 ...
 **/
GList *
anchor_canvas_container_get_selection (AnchorCanvasContainer *container)
{
	GList *list, *p;
...
static GList *
anchor_canvas_container_get_selected_icons (AnchorCanvasContainer *container)
{
	GList *list, *p;
 ...
 **/
void
anchor_canvas_container_invert_selection (AnchorCanvasContainer *container)
{
	GList *p;
...
static GArray *
anchor_canvas_container_get_icon_locations (AnchorCanvasContainer *container,
						GList *icons)
{
	GArray *result;
	GList *node;
	...
	for (index = 0, node = icons; node != NULL; index++, node = node->next) {
	     	g_array_index (result, GdkPoint, index).x =
	     		((AnchorCanvasIcon *)node->data)->x;
	     	g_array_index (result, GdkPoint, index).y =
			((AnchorCanvasIcon *)node->data)->y;
	}
...
/**
 * anchor_canvas_container_get_selected_icon_locations:
 * @container: An canvas container widget.
 * 
 * Returns an array of GdkPoints of locations of the selected icons.
 **/
GArray *
anchor_canvas_container_get_selected_icon_locations (AnchorCanvasContainer *container)
{
	GArray *result;
	GList *icons;
	...
	icons = anchor_canvas_container_get_selected_icons (container);
	result = anchor_canvas_container_get_icon_locations (container, icons);
	g_list_free (icons);
...
/**
 * anchor_canvas_container_select_all:
 * @container: An canvas container widget.
 * 
 * Select all the icons in @container at once.
 **/
void
anchor_canvas_container_select_all (AnchorCanvasContainer *container)
{
	gboolean selection_changed;
	GList *p;
	AnchorCanvasIcon *icon;
...
/**
 * anchor_canvas_container_select_first:
 * @container: An canvas container widget.
 * 
 * Select the first icon in @container.
 **/
void
anchor_canvas_container_select_first (AnchorCanvasContainer *container)
{
	gboolean selection_changed;
	AnchorCanvasIcon *icon;
					 ...
					 GList *selection)
{
	...
	GList *p;
	gboolean res;
	AnchorCanvasIcon *icon, *selected_icon;
...
/**
 * anchor_canvas_container_select_list_unselect_others.
 * @container: An canvas container widget.
 * @selection: A list of AnchorCanvasIcon *.
 * 
 * Set the selection to exactly the icons in @selection.
 **/
void
anchor_canvas_container_select_list_unselect_others (AnchorCanvasContainer *container,
						       GList *selection)
{
	gboolean selection_changed;
	GHashTable *hash;
...
/**
 * anchor_canvas_container_unselect_all:
 * @container: An canvas container widget.
 * 
 * Deselect all the icons in @container.
 **/
void
anchor_canvas_container_unselect_all (AnchorCanvasContainer *container)
{
...
/**
 * anchor_canvas_container_get_icon_by_uri:
 * @container: An canvas container widget.
 ...
 **/
AnchorCanvasIcon *
anchor_canvas_container_get_icon_by_uri (AnchorCanvasContainer *container,
					     const char *uri)
{
	AnchorCanvasContainerDetails *details;
	GList *p;
	...
	*/
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		icon_uri = anchor_canvas_container_get_icon_uri
			(container, icon);
...
static AnchorCanvasIcon *
get_nth_selected_icon (AnchorCanvasContainer *container, int index)
{
	GList *p;
	AnchorCanvasIcon *icon;
	int selection_count;
...
static AnchorCanvasIcon *
get_first_selected_icon (AnchorCanvasContainer *container)
{
        return get_nth_selected_icon (container, 1);
}

static gboolean
has_multiple_selection (AnchorCanvasContainer *container)
{
        return get_nth_selected_icon (container, 2) != NULL;
}

static gboolean
all_selected (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
...
static gboolean
has_selection (AnchorCanvasContainer *container)
{
        return get_nth_selected_icon (container, 1) != NULL;
}

/**
 * anchor_canvas_container_show_stretch_handles:
 * @container: An canvas container widget.
 * 
 * Makes stretch handles visible on the first selected icon.
 **/
void
anchor_canvas_container_show_stretch_handles (AnchorCanvasContainer *container)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	guint initial_size;
	...
	anchor_canvas_item_set_show_stretch_handles (icon->item, TRUE);
...
/**
 * anchor_canvas_container_has_stretch_handles
 * @container: An canvas container widget.
 * 
 * Returns true if the first selected item has stretch handles.
 **/
gboolean
anchor_canvas_container_has_stretch_handles (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *icon;
...
/**
 * anchor_canvas_container_is_stretched
 * @container: An canvas container widget.
 * 
 * Returns true if the any selected item is stretched to a size other than 1.0.
 **/
gboolean
anchor_canvas_container_is_stretched (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
...
/**
 * anchor_canvas_container_unstretch
 * @container: An canvas container widget.
 * 
 * Gets rid of any canvas stretching.
 **/
void
anchor_canvas_container_unstretch (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
...
char *
anchor_canvas_container_get_icon_uri (AnchorCanvasContainer *container,
					  AnchorCanvasIcon *icon)
{
	char *uri;
...
char *
anchor_canvas_container_get_icon_activation_uri (AnchorCanvasContainer *container,
						   AnchorCanvasIcon *icon)
{
	char *uri;
...
char *
anchor_canvas_container_get_icon_drop_target_uri (AnchorCanvasContainer *container,
						      AnchorCanvasIcon *icon)
{
	char *uri;
 ...
 */
static void
reset_scroll_region_if_not_empty (AnchorCanvasContainer *container)
{
	if (!anchor_canvas_container_is_empty (container)) {
		anchor_canvas_container_reset_scroll_region (container);
	}
}
 ...
 */
void
anchor_canvas_container_set_auto_layout (AnchorCanvasContainer *container,
...
gboolean
anchor_canvas_container_is_keep_aligned (AnchorCanvasContainer *container)
{
...
static void
unschedule_align_icons (AnchorCanvasContainer *container)
{
        if (container->details->align_idle_id != 0) {
		g_source_remove (container->details->align_idle_id);
		container->details->align_idle_id = 0;
...
static void
schedule_align_icons (AnchorCanvasContainer *container)
{
	if (container->details->align_idle_id == 0
	    && container->details->has_been_allocated) {
...
void
anchor_canvas_container_set_keep_aligned (AnchorCanvasContainer *container,
...
void
anchor_canvas_container_set_label_position (AnchorCanvasContainer *container,
					      AnchorCanvasLabelPosition position)
{
	g_return_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container));
 ...
 */
void
anchor_canvas_container_freeze_icon_positions (AnchorCanvasContainer *container)
{
	gboolean changed;
	GList *p;
	AnchorCanvasIcon *icon;
...
void
anchor_canvas_container_sort (AnchorCanvasContainer *container)
{
	gboolean changed;
...
gboolean
anchor_canvas_container_is_auto_layout (AnchorCanvasContainer *container)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container), FALSE);
...
static void
pending_icon_to_rename_destroy_callback (AnchorCanvasItem *item, AnchorCanvasContainer *container)
{
...
static AnchorCanvasIcon *
get_pending_icon_to_rename (AnchorCanvasContainer *container)
{
	return container->details->pending_icon_to_rename;
}

static void
set_pending_icon_to_rename (AnchorCanvasContainer *container, AnchorCanvasIcon *icon)
{
	AnchorCanvasIcon *old_icon;
...
static void
process_pending_icon_to_rename (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *pending_icon_to_rename;
	...
	if (pending_icon_to_rename != NULL) {
		if (pending_icon_to_rename->is_selected && !has_multiple_selection (container)) {
			anchor_canvas_container_start_renaming_selected_item (container, FALSE);
		} else {
...
static gboolean
is_renaming_pending (AnchorCanvasContainer *container)
{
	return get_pending_icon_to_rename (container) != NULL;
}

static gboolean
is_renaming (AnchorCanvasContainer *container)
{
 ...
 **/
void
anchor_canvas_container_start_renaming_selected_item (AnchorCanvasContainer *container,
							gboolean select_all)
{
	AnchorCanvasContainerDetails *details;
	AnchorCanvasIcon *icon;
	...
	editable_text = anchor_canvas_item_get_editable_text (icon->item);
	...
	anchor_canvas_container_freeze_updates (container);
	...
	width = anchor_canvas_item_get_max_text_width (icon->item);
	...
	anchor_canvas_item_set_renaming (icon->item, TRUE);
}

static void
end_renaming_mode (AnchorCanvasContainer *container, gboolean commit)
{
	AnchorCanvasIcon *icon;
	...
	anchor_canvas_item_set_renaming (icon->item, FALSE);
	
	anchor_canvas_container_unfreeze_updates (container);
...
gboolean
anchor_canvas_container_has_stored_icon_positions (AnchorCanvasContainer *container)
{
	GList *p;
	AnchorCanvasIcon *icon;
	gboolean have_stored_position;
...
void
anchor_canvas_container_set_single_click_mode (AnchorCanvasContainer *container,
...
gboolean
anchor_canvas_container_get_is_fixed_size (AnchorCanvasContainer *container)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container), FALSE);
...
void
anchor_canvas_container_set_is_fixed_size (AnchorCanvasContainer *container,
...
gboolean
anchor_canvas_container_get_is_desktop (AnchorCanvasContainer *container)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container), FALSE);
...
void
anchor_canvas_container_set_is_desktop (AnchorCanvasContainer *container,
					  gboolean is_desktop)
{
	g_return_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container));
	...
	if (is_desktop) {
		...
		context = gtk_widget_get_style_context (GTK_WIDGET (container));
		gtk_style_context_add_class (context, "anchor-desktop");
	}
}

void
anchor_canvas_container_set_margins (AnchorCanvasContainer *container,
				       int left_margin,
				       int right_margin,
				       int top_margin,
				       int bottom_margin)
...
void
anchor_canvas_container_set_use_drop_shadows (AnchorCanvasContainer  *container,
...
void
anchor_canvas_container_set_font (AnchorCanvasContainer *container,
				    const char *font)
{
	g_return_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container));
...
/**
 * anchor_canvas_container_get_icon_description
 * @container: An canvas container widget.
 * @data: Icon data 
 * 
 ...
 **/
char*
anchor_canvas_container_get_icon_description (AnchorCanvasContainer *container,
						  AnchorCanvasIconData      *data)
{
	AnchorCanvasContainerClass *klass;
...
gboolean
anchor_canvas_container_get_allow_moves (AnchorCanvasContainer *container)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container), FALSE);
...
void
anchor_canvas_container_set_allow_moves	(AnchorCanvasContainer *container,
 ...
 **/
void
anchor_canvas_container_set_highlighted_for_clipboard (AnchorCanvasContainer *container,
...
/* AnchorCanvasContainerAccessible */
typedef struct {
	EelCanvasAccessible parent;
	AnchorCanvasContainerAccessiblePrivate *priv;
} AnchorCanvasContainerAccessible;

typedef EelCanvasAccessibleClass AnchorCanvasContainerAccessibleClass;

#define GET_ACCESSIBLE_PRIV(o) ((AnchorCanvasContainerAccessible *) o)->priv

/* AtkAction interface */
static gboolean
anchor_canvas_container_accessible_do_action (AtkAction *accessible, int i)
{
	GtkWidget *widget;
	AnchorCanvasContainer *container;
	GList *selection;
	...
	case ACTION_MENU :
		...
		break;
	default :
		g_warning ("Invalid action passed to AnchorCanvasContainerAccessible::do_action");
		return FALSE;
	}
	return TRUE;
}
...
static int
anchor_canvas_container_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
anchor_canvas_container_accessible_action_get_description (AtkAction *accessible, 
							     int i)
{
	AnchorCanvasContainerAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return anchor_canvas_container_accessible_action_descriptions[i];
	}
}

static const char *
anchor_canvas_container_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return anchor_canvas_container_accessible_action_names[i];
}

static const char *
anchor_canvas_container_accessible_action_get_keybinding (AtkAction *accessible, 
							    int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
anchor_canvas_container_accessible_action_set_description (AtkAction *accessible, 
							     int i, 
...
static void
anchor_canvas_container_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = anchor_canvas_container_accessible_do_action;
	iface->get_n_actions = anchor_canvas_container_accessible_get_n_actions;
	iface->get_description = anchor_canvas_container_accessible_action_get_description;
	iface->get_name = anchor_canvas_container_accessible_action_get_name;
	iface->get_keybinding = anchor_canvas_container_accessible_action_get_keybinding;
	iface->set_description = anchor_canvas_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void
anchor_canvas_container_accessible_update_selection (AtkObject *accessible)
{
	AnchorCanvasContainer *container;
	AnchorCanvasContainerAccessiblePrivate *priv;
	GList *l;
...
static void
anchor_canvas_container_accessible_selection_changed_cb (AnchorCanvasContainer *container,
							   gpointer data)
{
...
static void
anchor_canvas_container_accessible_icon_added_cb (AnchorCanvasContainer *container,
...
static void
anchor_canvas_container_accessible_icon_removed_cb (AnchorCanvasContainer *container,
							AnchorCanvasIconData *icon_data,
							gpointer data)
{
	AnchorCanvasIcon *icon;
...
static void
anchor_canvas_container_accessible_cleared_cb (AnchorCanvasContainer *container, 
						 gpointer data)
{
...
static gboolean 
anchor_canvas_container_accessible_add_selection (AtkSelection *accessible, 
						    int i)
{
	GtkWidget *widget;
	AnchorCanvasContainer *container;
	...
	if (l) {
		...
		selection = g_list_prepend (selection, 
					    icon->data);
		anchor_canvas_container_set_selection (container, selection);
...
static gboolean
anchor_canvas_container_accessible_clear_selection (AtkSelection *accessible)
{
	GtkWidget *widget;
	AnchorCanvasContainer *container;
	...
	anchor_canvas_container_unselect_all (container);

	return TRUE;
}

static AtkObject *
anchor_canvas_container_accessible_ref_selection (AtkSelection *accessible, 
						    int i)
{
	AnchorCanvasContainerAccessiblePrivate *priv;
	...
	anchor_canvas_container_accessible_update_selection (ATK_OBJECT (accessible));
	priv = GET_ACCESSIBLE_PRIV (accessible);
...
static int
anchor_canvas_container_accessible_get_selection_count (AtkSelection *accessible)
{
	AnchorCanvasContainerAccessiblePrivate *priv;
	int count;
...
static gboolean
anchor_canvas_container_accessible_is_child_selected (AtkSelection *accessible,
							int i)
{
	AnchorCanvasContainer *container;
	GList *l;
...
static gboolean
anchor_canvas_container_accessible_remove_selection (AtkSelection *accessible,
						       int i)
{
	AnchorCanvasContainerAccessiblePrivate *priv;
	AnchorCanvasContainer *container;
...
static gboolean
anchor_canvas_container_accessible_select_all_selection (AtkSelection *accessible)
{
	AnchorCanvasContainer *container;
	GtkWidget *widget;
	...
	anchor_canvas_container_select_all (container);

	return TRUE;
}

void
anchor_canvas_container_widget_to_file_operation_position (AnchorCanvasContainer *container,
							     GdkPoint              *position)
{
	double x, y;
	...
	position->x -= anchor_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
	position->y -= anchor_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
}

static void 
anchor_canvas_container_accessible_selection_interface_init (AtkSelectionIface *iface)
{
	iface->add_selection = anchor_canvas_container_accessible_add_selection;
	iface->clear_selection = anchor_canvas_container_accessible_clear_selection;
	iface->ref_selection = anchor_canvas_container_accessible_ref_selection;
	iface->get_selection_count = anchor_canvas_container_accessible_get_selection_count;
	iface->is_child_selected = anchor_canvas_container_accessible_is_child_selected;
	iface->remove_selection = anchor_canvas_container_accessible_remove_selection;
	iface->select_all_selection = anchor_canvas_container_accessible_select_all_selection;
}


static gint 
anchor_canvas_container_accessible_get_n_children (AtkObject *accessible)
{
	AnchorCanvasContainer *container;
	GtkWidget *widget;
	gint i;
...
static AtkObject* 
anchor_canvas_container_accessible_ref_child (AtkObject *accessible, int i)
{
        AtkObject *atk_object;
        AnchorCanvasContainer *container;
        GList *item;
...
G_DEFINE_TYPE_WITH_CODE (AnchorCanvasContainerAccessible, anchor_canvas_container_accessible,
			 eel_canvas_accessible_get_type (),
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION, anchor_canvas_container_accessible_action_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_SELECTION, anchor_canvas_container_accessible_selection_interface_init))

static void
anchor_canvas_container_accessible_initialize (AtkObject *accessible, 
						 gpointer data)
{
	AnchorCanvasContainer *container;

	if (ATK_OBJECT_CLASS (anchor_canvas_container_accessible_parent_class)->initialize) {
		ATK_OBJECT_CLASS (anchor_canvas_container_accessible_parent_class)->initialize (accessible, data);
	}

	if (GTK_IS_ACCESSIBLE (accessible)) {
		anchor_canvas_container_accessible_update_selection 
			(ATK_OBJECT (accessible));
		
		container = NAUTILUS_CANVAS_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
		g_signal_connect (container, "selection-changed",
				  G_CALLBACK (anchor_canvas_container_accessible_selection_changed_cb), 
				  accessible);
		g_signal_connect (container, "icon-added",
				  G_CALLBACK (anchor_canvas_container_accessible_icon_added_cb), 
				  accessible);
		g_signal_connect (container, "icon-removed",
				  G_CALLBACK (anchor_canvas_container_accessible_icon_removed_cb), 
				  accessible);
		g_signal_connect (container, "cleared",
				  G_CALLBACK (anchor_canvas_container_accessible_cleared_cb), 
				  accessible);
	}
}

static void
anchor_canvas_container_accessible_finalize (GObject *object)
{
	AnchorCanvasContainerAccessiblePrivate *priv;
	int i;
	...
	G_OBJECT_CLASS (anchor_canvas_container_accessible_parent_class)->finalize (object);
}

static void
anchor_canvas_container_accessible_init (AnchorCanvasContainerAccessible *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, anchor_canvas_container_accessible_get_type (),
						  AnchorCanvasContainerAccessiblePrivate);
}

static void
anchor_canvas_container_accessible_class_init (AnchorCanvasContainerAccessibleClass *klass)
{
	AtkObjectClass *atk_class = ATK_OBJECT_CLASS (klass);
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->finalize = anchor_canvas_container_accessible_finalize;

	atk_class->get_n_children = anchor_canvas_container_accessible_get_n_children;
	atk_class->ref_child = anchor_canvas_container_accessible_ref_child;
	atk_class->initialize = anchor_canvas_container_accessible_initialize;

	g_type_class_add_private (klass, sizeof (AnchorCanvasContainerAccessiblePrivate));
}

gboolean
anchor_canvas_container_is_layout_rtl (AnchorCanvasContainer *container)
{
...
gboolean
anchor_canvas_container_is_layout_vertical (AnchorCanvasContainer *container)
{
	g_return_val_if_fail (NAUTILUS_IS_CANVAS_CONTAINER (container), FALSE);
...
int
anchor_canvas_container_get_max_layout_lines_for_pango (AnchorCanvasContainer  *container)
{
	int limit;
...
int
anchor_canvas_container_get_max_layout_lines (AnchorCanvasContainer  *container)
{
	int limit;
...
void
anchor_canvas_container_begin_loading (AnchorCanvasContainer *container)
{
	gboolean dummy;

	if (anchor_canvas_container_get_store_layout_timestamps (container)) {
...
static void
store_layout_timestamps_now (AnchorCanvasContainer *container)
{
	AnchorCanvasIcon *icon;
	GList *p;
	gboolean dummy;
...
void
anchor_canvas_container_end_loading (AnchorCanvasContainer *container,
				       gboolean               all_icons_added)
{
	if (all_icons_added &&
	    anchor_canvas_container_get_store_layout_timestamps (container)) {
...
gboolean
anchor_canvas_container_get_store_layout_timestamps (AnchorCanvasContainer *container)
{
...
void
anchor_canvas_container_set_store_layout_timestamps (AnchorCanvasContainer *container,
...
void
anchor_self_check_canvas_container (void)
{
