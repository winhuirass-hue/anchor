...
} AnchorFileChangeKind;

typedef struct {
	AnchorFileChangeKind kind;
	GFile *from;
	GFile *to;
	GdkPoint point;
	int screen;
...
typedef struct {
	...
	GList *tail;
	GMutex mutex;
} AnchorFileChangesQueue;

static AnchorFileChangesQueue *
anchor_file_changes_queue_new (void)
{
	AnchorFileChangesQueue *result;

	result = g_new0 (AnchorFileChangesQueue, 1);
...
static AnchorFileChangesQueue *
anchor_file_changes_queue_get (void)
{
	static AnchorFileChangesQueue *file_changes_queue;

	if (file_changes_queue == NULL) {
		file_changes_queue = anchor_file_changes_queue_new ();
	}
...
static void
anchor_file_changes_queue_add_common (AnchorFileChangesQueue *queue, 
	AnchorFileChange *new_item)
{
...
void
anchor_file_changes_queue_file_added (GFile *location)
{
	AnchorFileChange *new_item;
	AnchorFileChangesQueue *queue;

	queue = anchor_file_changes_queue_get();

	new_item = g_new0 (AnchorFileChange, 1);
	new_item->kind = CHANGE_FILE_ADDED;
	new_item->from = g_object_ref (location);
	anchor_file_changes_queue_add_common (queue, new_item);
}

void
anchor_file_changes_queue_file_changed (GFile *location)
{
	AnchorFileChange *new_item;
	AnchorFileChangesQueue *queue;
...
void
anchor_file_changes_queue_file_removed (GFile *location)
{
	AnchorFileChange *new_item;
	AnchorFileChangesQueue *queue;
...
void
anchor_file_changes_queue_file_moved (GFile *from,
					GFile *to)
{
	AnchorFileChange *new_item;
	AnchorFileChangesQueue *queue;
	...
	new_item = g_new (AnchorFileChange, 1);
...
void
anchor_file_changes_queue_schedule_position_set (GFile *location, 
						   GdkPoint point,
						   int screen)
{
	AnchorFileChange *new_item;
...
void
anchor_file_changes_queue_schedule_position_remove (GFile *location)
{
	AnchorFileChange *new_item;
	AnchorFileChangesQueue *queue;
...
static AnchorFileChange *
anchor_file_changes_queue_get_change (AnchorFileChangesQueue *queue)
{
	GList *new_tail;
	AnchorFileChange *result;
...
{
	GList *p;
	AnchorFileChangesQueuePosition *item;
 ...
 * in a list to the different anchor_directory_notify calls
 */ 
void
anchor_file_changes_consume_changes (gboolean consume_all)
{
	AnchorFileChange *change;
	GList *additions, *changes, *deletions, *moves;
	GList *position_set_requests;
	GFilePair *pair;
	AnchorFileChangesQueuePosition *position_set;
	guint chunk_count;
	AnchorFileChangesQueue *queue;
	...
	for (chunk_count = 0; ; chunk_count++) {
		change = anchor_file_changes_queue_get_change (queue);
		...
		if (flush_needed) {
			...
			if (position_set_requests != NULL) {
				...
				anchor_directory_schedule_position_set (position_set_requests);
		...
		case CHANGE_POSITION_SET:
			position_set = g_new (AnchorFileChangesQueuePosition, 1);
