...
/* anchor-file-utilities.c - implementation of file manipulation routines.
...
#define NAUTILUS_USER_DIRECTORY_NAME "anchor"
...
gboolean
anchor_uri_parse (const char  *uri,
		    char       **host,
		    guint16     *port,
		    char       **userinfo)
{
...
char *
anchor_compute_title_for_location (GFile *location)
{
	AnchorFile *file;
	char *title;
	...
	if (location) {
		...
		title = anchor_file_get_description (file);
		if (title == NULL) {
...
/**
 * anchor_get_user_directory:
 * 
 * Get the path for the directory containing anchor settings.
 *
 * Return value: the directory path.
 **/
char *
anchor_get_user_directory (void)
{
	...
	if (!g_file_test (user_directory, G_FILE_TEST_EXISTS)) {
		 ...
		 * Note that anchor_application_startup will refuse to launch if this 
		 * directory doesn't get created, so that case is OK. But the directory 
		 * could be deleted after Anchor was launched, and perhaps
...
/**
 * anchor_get_accel_map_file:
 * 
 * Get the path for the filename containing anchor accelerator map.
 * The filename need not exist.
 *
 * Return value: the filename path
 **/
char *
anchor_get_accel_map_file (void)
{
	return g_build_filename (g_get_user_config_dir (), "anchor", "accels", NULL);
}

/**
 * anchor_get_scripts_directory_path:
 *
 * Get the path for the directory containing anchor scripts.
 *
 * Return value: the directory path containing anchor scripts
 **/
char *
anchor_get_scripts_directory_path (void)
{
	return g_build_filename (g_get_user_data_dir (), "anchor", "scripts", NULL);
}
...
static void
xdg_dir_changed (AnchorFile *file,
		 XdgDirEntry *dir)
{
	...
	if (cached_xdg_dirs != NULL) {
		for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
			if (cached_xdg_dirs[i].file != NULL) {
				anchor_file_monitor_remove (cached_xdg_dirs[i].file,
							      &cached_xdg_dirs[i]);
				g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
								      G_CALLBACK (xdg_dir_changed),
								      &cached_xdg_dirs[i]);
				anchor_file_unref (cached_xdg_dirs[i].file);
			}
...
update_xdg_dir_cache (void)
{
	...
	for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++) {
		...
		if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0) {
			...
			cached_xdg_dirs[i].file = anchor_file_get_by_uri (uri);
			anchor_file_monitor_add (cached_xdg_dirs[i].file,
						   &cached_xdg_dirs[i],
...
char *
anchor_get_xdg_dir (const char *type)
{
	int i;
...
{
	return anchor_get_xdg_dir ("DESKTOP");
}

/**
 * anchor_get_desktop_directory:
 * 
 ...
 **/
char *
anchor_get_desktop_directory (void)
{
...
GFile *
anchor_get_desktop_location (void)
{
	char *desktop_directory;
	GFile *res;
...
/**
 * anchor_get_desktop_directory_uri:
 * 
 ...
 **/
char *
anchor_get_desktop_directory_uri (void)
{
	char *desktop_path;
	char *desktop_uri;
	
	desktop_path = anchor_get_desktop_directory ();
...
char *
anchor_get_home_directory_uri (void)
{
...
gboolean
anchor_should_use_templates_directory (void)
{
	char *dir;
	gboolean res;
	
	dir = anchor_get_xdg_dir ("TEMPLATES");
...
char *
anchor_get_templates_directory (void)
{
	return anchor_get_xdg_dir ("TEMPLATES");
}

void
anchor_create_templates_directory (void)
{
	char *dir;

	dir = anchor_get_templates_directory ();
...
char *
anchor_get_templates_directory_uri (void)
{
	char *directory, *uri;

	directory = anchor_get_templates_directory ();
...
char *
anchor_get_searches_directory (void)
{
	char *user_dir;
	char *searches_dir;

	user_dir = anchor_get_user_directory ();
...
gboolean
anchor_is_home_directory_file (GFile *dir,
				 const char *filename)
{
	char *dirname;
...
gboolean
anchor_is_home_directory (GFile *dir)
{
...
gboolean
anchor_is_root_directory (GFile *dir)
{
...
gboolean
anchor_is_desktop_directory_file (GFile *dir,
				    const char *file)
{
...
gboolean
anchor_is_desktop_directory (GFile *dir)
{
...
GMount *
anchor_get_mounted_mount_for_root (GFile *location)
{
...
char *
anchor_ensure_unique_file_name (const char *directory_uri,
...
GFile *
anchor_find_existing_uri_in_hierarchy (GFile *location)
{
	GFileInfo *info;
	GFile *tmp;
...
GIcon *
anchor_special_directory_get_symbolic_icon (GUserDirectory directory)
{
	return special_directory_get_icon (directory, TRUE);
}

GIcon *
anchor_special_directory_get_icon (GUserDirectory directory)
{
	return special_directory_get_icon (directory, FALSE);
}

gboolean
anchor_is_file_roller_installed (void)
{
...
gboolean
anchor_is_in_system_dir (GFile *file)
{
...
GHashTable *
anchor_trashed_files_get_original_directories (GList *files,
						 GList **unhandled_files)
{
	GHashTable *directories;
	AnchorFile *file, *original_file, *original_dir;
	GList *l, *m;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		original_file = anchor_file_get_trash_original_file (file);

		original_dir = NULL;
		if (original_file != NULL) {
			original_dir = anchor_file_get_parent (original_file);
		}
...
locations_from_file_list (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		file = NAUTILUS_FILE (l->data);
		ret = g_list_prepend (ret, anchor_file_get_location (file));
	}
...
void
anchor_restore_files_from_trash (GList *files,
				   GtkWindow *parent_window)
{
	AnchorFile *file;
	GHashTable *original_dirs_hash;
	...
	original_dirs_hash = anchor_trashed_files_get_original_directories (files, &unhandled_files);
	...
	anchor_file_list_unref (unhandled_files);
}
...
void
anchor_get_x_content_types_for_mount_async (GMount *mount,
					      AnchorMountGetContent callback,
					      GCancellable *cancellable,
					      gpointer user_data)
{
	...
	cached = g_object_get_data (G_OBJECT (mount), "anchor-content-type-cache");
	if (cached != NULL) {
		if (callback) {
...
char **
anchor_get_cached_x_content_types_for_mount (GMount *mount)
{
	char **cached;
...
gboolean
anchor_file_selection_equal (GList *selection_a,
			       GList *selection_b)
{
	GList *al, *bl;
	...
	for (al = selection_a; al; al = al->next) {
		GFile *a_location = anchor_file_get_location (NAUTILUS_FILE (al->data));
		gboolean found = FALSE;

		for (bl = selection_b; bl; bl = bl->next) {
			GFile *b_location = anchor_file_get_location (NAUTILUS_FILE (bl->data));
...
void
anchor_self_check_file_utilities (void)
{
}
