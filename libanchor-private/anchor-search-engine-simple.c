...
typedef struct {
	AnchorSearchEngineSimple *engine;
...
struct AnchorSearchEngineSimpleDetails {
	AnchorQuery *query;
...
G_DEFINE_TYPE_WITH_CODE (AnchorSearchEngineSimple,
			 anchor_search_engine_simple,
			 G_TYPE_OBJECT,
...
finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (anchor_search_engine_simple_parent_class)->finalize (object);
}

static SearchThreadData *
search_thread_data_new (AnchorSearchEngineSimple *engine,
			AnchorQuery *query)
{
	...
	uri = anchor_query_get_location (query);
	location = g_file_new_for_uri (uri);
	g_free (uri);

	g_queue_push_tail (data->directories, location);
	data->mime_types = anchor_query_get_mime_types (query);
...
search_thread_done_idle (gpointer user_data)
{
	...
	AnchorSearchEngineSimple *engine = data->engine;
...
search_thread_add_hits_idle (gpointer user_data)
{
	...
	if (!g_cancellable_is_cancelled (data->thread_data->cancellable)) {
		anchor_search_provider_hits_added (ANCHOR_SEARCH_PROVIDER (data->thread_data->engine),
						     data->hits);
	}
...
visit_directory (GFile *dir, SearchThreadData *data)
{
	...
	while ((info = g_file_enumerator_next_file (enumerator, data->cancellable, NULL)) != NULL) {
		...
		if (is_hidden && !anchor_query_get_show_hidden_files (data->query)) {
			goto next;
		}

		child = g_file_get_child (dir, g_file_info_get_name (info));
		match = anchor_query_matches_string (data->query, display_name);
		found = (match > -1);
...
static void
anchor_search_engine_simple_start (AnchorSearchProvider *provider)
{
	AnchorSearchEngineSimple *simple;
	SearchThreadData *data;
	GThread *thread;
	...
	thread = g_thread_new ("anchor-search-simple", search_thread_func, data);
...
static void
anchor_search_engine_simple_stop (AnchorSearchProvider *provider)
{
	AnchorSearchEngineSimple *simple;
...
static void
anchor_search_engine_simple_set_query (AnchorSearchProvider *provider,
					 AnchorQuery          *query)
{
	AnchorSearchEngineSimple *simple;
...
static void
anchor_search_engine_simple_set_property (GObject *object,
					    guint arg_id,
					    const GValue *value,
					    GParamSpec *pspec)
{
...
static void
anchor_search_engine_simple_get_property (GObject *object,
					    guint arg_id,
					    GValue *value,
					    GParamSpec *pspec)
{
...
{
	iface->set_query = anchor_search_engine_simple_set_query;
	iface->start = anchor_search_engine_simple_start;
	iface->stop = anchor_search_engine_simple_stop;
}

static void
anchor_search_engine_simple_class_init (AnchorSearchEngineSimpleClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	gobject_class->finalize = finalize;
	gobject_class->get_property = anchor_search_engine_simple_get_property;
	gobject_class->set_property = anchor_search_engine_simple_set_property;
	...
	g_type_class_add_private (class, sizeof (AnchorSearchEngineSimpleDetails));
}

static void
anchor_search_engine_simple_init (AnchorSearchEngineSimple *engine)
{
	engine->details = G_TYPE_INSTANCE_GET_PRIVATE (engine, ANCHOR_TYPE_SEARCH_ENGINE_SIMPLE,
						       AnchorSearchEngineSimpleDetails);
}

AnchorSearchEngineSimple *
anchor_search_engine_simple_new (void)
{
	AnchorSearchEngineSimple *engine;

	engine = g_object_new (ANCHOR_TYPE_SEARCH_ENGINE_SIMPLE, NULL);
