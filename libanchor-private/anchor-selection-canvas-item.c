...
G_DEFINE_TYPE (AnchorSelectionCanvasItem, anchor_selection_canvas_item, EEL_TYPE_CANVAS_ITEM);

#define DASH_ON 0.8
#define DASH_OFF 1.7
static void
anchor_selection_canvas_item_draw (EelCanvasItem *item,
				     cairo_t *cr,
				     cairo_region_t *region)
{
	AnchorSelectionCanvasItem *self;
	double x1, y1, x2, y2;
	int cx1, cy1, cx2, cy2;
...
static double
anchor_selection_canvas_item_point (EelCanvasItem *item,
				      double x,
				      double y,
				      int cx,
				      int cy,
...
static void
anchor_selection_canvas_item_update (EelCanvasItem *item,
				       double i2w_dx,
				       double i2w_dy,
				       gint flags)
{
	...
	if (EEL_CANVAS_ITEM_CLASS (anchor_selection_canvas_item_parent_class)->update)
		(* EEL_CANVAS_ITEM_CLASS (anchor_selection_canvas_item_parent_class)->update) (item, i2w_dx, i2w_dy, flags);
...
static void
anchor_selection_canvas_item_translate (EelCanvasItem *item,
					  double dx,
					  double dy)
{
	AnchorSelectionCanvasItem *self;
...
static void
anchor_selection_canvas_item_bounds (EelCanvasItem *item,
				       double *x1,
				       double *y1,
				       double *x2,
				       double *y2)
...
{
	AnchorSelectionCanvasItem *self = user_data;
...
void
anchor_selection_canvas_item_fade_out (AnchorSelectionCanvasItem *self,
...
static void
anchor_selection_canvas_item_dispose (GObject *obj)
{
	AnchorSelectionCanvasItem *self = NAUTILUS_SELECTION_CANVAS_ITEM (obj);
	...
	G_OBJECT_CLASS (anchor_selection_canvas_item_parent_class)->dispose (obj);
}

static void
do_set_fill (AnchorSelectionCanvasItem *self,
	     gboolean fill_set)
{
...
static void
do_set_outline (AnchorSelectionCanvasItem *self,
...
static void
anchor_selection_canvas_item_set_property (GObject *object,
					     guint param_id,
					     const GValue *value,
					     GParamSpec *pspec)
{
...
static void
anchor_selection_canvas_item_get_property (GObject *object,
					     guint param_id,
					     GValue *value,
					     GParamSpec *pspec)
{
...
static void
anchor_selection_canvas_item_class_init (AnchorSelectionCanvasItemClass *klass)
{
	...
	gobject_class->set_property = anchor_selection_canvas_item_set_property;
	gobject_class->get_property = anchor_selection_canvas_item_get_property;
	gobject_class->dispose = anchor_selection_canvas_item_dispose;

	item_class->draw = anchor_selection_canvas_item_draw;
	item_class->point = anchor_selection_canvas_item_point;
	item_class->update = anchor_selection_canvas_item_update;
	item_class->bounds = anchor_selection_canvas_item_bounds;
	item_class->translate = anchor_selection_canvas_item_translate;
	...
	g_type_class_add_private (klass, sizeof (AnchorSelectionCanvasItemDetails));
}

static void
anchor_selection_canvas_item_init (AnchorSelectionCanvasItem *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ANCHOR_TYPE_SELECTION_CANVAS_ITEM,
						  AnchorSelectionCanvasItemDetails);
}
