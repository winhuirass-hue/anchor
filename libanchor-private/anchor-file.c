/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-file.c: Anchor file model.
...
#include "anchor-link.h"
#include "anchor-metadata.h"
#include "anchor-module.h"
#include "anchor-search-directory.h"
#include "anchor-search-directory-file.h"
#include "anchor-thumbnails.h"
#include "anchor-ui-utilities.h"
#include "anchor-video-mime-types.h"
#include "anchor-vfs-file.h"
#include "anchor-file-undo-operations.h"
...
#include <libanchor-extension/anchor-extension-private.h>
...
/* Name of Anchor trash directories */
...
typedef void (* ModifyListFunction) (GList **list, AnchorFile *file);
...
static AnchorSpeedTradeoffValue show_file_thumbs;

static AnchorSpeedTradeoffValue show_directory_item_count;
...
static void     anchor_file_info_iface_init                (AnchorFileInfoIface *iface);
static char *   anchor_file_get_owner_as_string            (AnchorFile          *file,
							      gboolean               include_real_name);
static char *   anchor_file_get_type_as_string             (AnchorFile          *file);
static char *   anchor_file_get_detailed_type_as_string    (AnchorFile          *file);
static gboolean update_info_and_name                         (AnchorFile          *file,
							      GFileInfo             *info);
static const char * anchor_file_peek_display_name (AnchorFile *file);
static const char * anchor_file_peek_display_name_collation_key (AnchorFile *file);
static void file_mount_unmounted (GMount *mount,  gpointer data);
static void metadata_hash_free (GHashTable *hash);

G_DEFINE_TYPE_WITH_CODE (AnchorFile, anchor_file, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (ANCHOR_TYPE_FILE_INFO,
						anchor_file_info_iface_init));

static void
anchor_file_init (AnchorFile *file)
{
	file->details = G_TYPE_INSTANCE_GET_PRIVATE ((file), ANCHOR_TYPE_FILE, AnchorFileDetails);

	anchor_file_clear_info (file);
...
static GObject*
anchor_file_constructor (GType                  type,
			   ...
			   GObjectConstructParam *construct_params)
{
  ...
  object = (* G_OBJECT_CLASS (anchor_file_parent_class)->constructor) (type,
...
gboolean
anchor_file_set_display_name (AnchorFile *file,
				...
				gboolean custom)
{
	...
	if (custom && display_name == NULL) {
		...
		if (file->details->got_custom_display_name) {
			...
			anchor_file_invalidate_attributes (file,
...
static void
anchor_file_clear_display_name (AnchorFile *file)
{
	eel_ref_str_unref (file->details->display_name);
...
static void
clear_metadata (AnchorFile *file)
{
...
get_metadata_from_info (GFileInfo *info)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		id = anchor_metadata_get_id (attrs[i] + strlen ("metadata::"));
		if (id == 0) {
			continue;
		}
...
gboolean
anchor_file_update_metadata_from_info (AnchorFile *file,
					 GFileInfo *info)
{
...
void
anchor_file_clear_info (AnchorFile *file)
{
	...
	if (!file->details->got_custom_display_name) {
		anchor_file_clear_display_name (file);
	}
...
static AnchorFile *
anchor_file_new_from_filename (AnchorDirectory *directory,
				 const char *filename,
				 gboolean self_owned)
{
	AnchorFile *file;
	...
	if (NAUTILUS_IS_DESKTOP_DIRECTORY (directory)) {
		if (self_owned) {
			file = NAUTILUS_FILE (g_object_new (ANCHOR_TYPE_DESKTOP_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = NAUTILUS_FILE (g_object_new (ANCHOR_TYPE_VFS_FILE, NULL));
		}
	} else if (NAUTILUS_IS_SEARCH_DIRECTORY (directory)) {
		if (self_owned) {
			file = NAUTILUS_FILE (g_object_new (ANCHOR_TYPE_SEARCH_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = NAUTILUS_FILE (g_object_new (ANCHOR_TYPE_VFS_FILE, NULL));
	...
	file->details->directory = anchor_directory_ref (directory);
...
static void
modify_link_hash_table (AnchorFile *file,
			ModifyListFunction modify_function)
{
	...
	if (file->details->symlink_name == NULL || !anchor_file_is_symbolic_link (file)) {
		return;
	}
	...
	target_uri = anchor_file_get_symbolic_link_target_uri (file);
...
static void
add_to_link_hash_table_list (GList **list, AnchorFile *file)
{
...
static void
add_to_link_hash_table (AnchorFile *file)
{
	modify_link_hash_table (file, add_to_link_hash_table_list);
}

static void
remove_from_link_hash_table_list (GList **list, AnchorFile *file)
{
	if (g_list_find (*list, file) != NULL) {
...
static void
remove_from_link_hash_table (AnchorFile *file)
{
	modify_link_hash_table (file, remove_from_link_hash_table_list);
}

AnchorFile *
anchor_file_new_from_info (AnchorDirectory *directory,
			     GFileInfo *info)
{
	AnchorFile *file;
...
static AnchorFile *
anchor_file_get_internal (GFile *location, gboolean create)
{
	gboolean self_owned;
	...
	directory = anchor_directory_get_internal (parent, create);

	g_object_unref (parent);

	/* Get the name for the file. */
	if (self_owned && directory != NULL) {
		basename = anchor_directory_get_name_for_self_as_new_file (directory);
	} else {
	...
	if (directory == NULL) {
		...
		file = anchor_directory_find_file_by_name (directory, basename);
	}
	...
	if (file != NULL) {
		...
		file = anchor_file_new_from_filename (directory, basename, self_owned);
		if (self_owned) {
...
AnchorFile *
anchor_file_get (GFile *location)
{
	return anchor_file_get_internal (location, TRUE);
}

AnchorFile *
anchor_file_get_existing (GFile *location)
{
	return anchor_file_get_internal (location, FALSE);
}

AnchorFile *
anchor_file_get_existing_by_uri (const char *uri)
{
	GFile *location;
	AnchorFile *file;
	
	location = g_file_new_for_uri (uri);
	file = anchor_file_get_internal (location, FALSE);
...
AnchorFile *
anchor_file_get_by_uri (const char *uri)
{
	GFile *location;
	AnchorFile *file;
	
	location = g_file_new_for_uri (uri);
	file = anchor_file_get_internal (location, TRUE);
	g_object_unref (location);
...
gboolean
anchor_file_is_self_owned (AnchorFile *file)
{
...
finalize (GObject *object)
{
	...
	if (file->details->is_thumbnailing) {
		...
		anchor_thumbnail_remove_from_queue (uri);
		g_free (uri);
	}
	
	anchor_async_destroying_file (file);

	remove_from_link_hash_table (file);

	directory = file->details->directory;
	
	if (anchor_file_is_self_owned (file)) {
		directory->details->as_file = NULL;
	} else {
		if (!file->details->is_gone) {
			anchor_directory_remove_file (directory, file);
		}
	}
	...
	G_OBJECT_CLASS (anchor_file_parent_class)->finalize (object);
}

AnchorFile *
anchor_file_ref (AnchorFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
void
anchor_file_unref (AnchorFile *file)
{
	if (file == NULL) {
		return;
	}
...
/**
 * anchor_file_get_parent_uri_for_display:
 * 
 ...
 */ 
char *
anchor_file_get_parent_uri_for_display (AnchorFile *file) 
{
	GFile *parent;
	char *result;
	...
	parent = anchor_file_get_parent_location (file);
	if (parent) {
 ...
 * Use anchor_file_get_parent_uri_for_display instead if the
 * result is to be displayed on-screen.
 * If the parent is NULL, returns the empty string.
 */ 
char *
anchor_file_get_parent_uri (AnchorFile *file) 
{
	g_assert (NAUTILUS_IS_FILE (file));
	...
	return anchor_directory_get_uri (file->details->directory);
}

GFile *
anchor_file_get_parent_location (AnchorFile *file) 
{
	g_assert (NAUTILUS_IS_FILE (file));
	...
	return anchor_directory_get_location (file->details->directory);
}

AnchorFile *
anchor_file_get_parent (AnchorFile *file)
{
	g_assert (NAUTILUS_IS_FILE (file));
	...
	return anchor_directory_get_corresponding_file (file->details->directory);
}

/**
 * anchor_file_can_read:
 * 
 ...
 */
gboolean
anchor_file_can_read (AnchorFile *file)
{
 ...
 */
gboolean
anchor_file_can_write (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
 ...
 */
gboolean
anchor_file_can_execute (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_mount (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_unmount (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_eject (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_start (AnchorFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
anchor_file_can_start_degraded (AnchorFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
anchor_file_can_poll_for_media (AnchorFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
anchor_file_is_media_check_automatic (AnchorFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
anchor_file_can_stop (AnchorFile *file)
{
	gboolean ret;
	GDrive *drive;
...
GDriveStartStopType
anchor_file_get_start_stop_type (AnchorFile *file)
{
...
void
anchor_file_mount (AnchorFile                   *file,
...
unmount_done (void *callback_data)
{
	...
	anchor_file_unref (data->file);
	g_free (data);
}

void
anchor_file_unmount (AnchorFile                   *file,
		       GMountOperation                *mount_op,
		       GCancellable                   *cancellable,
		       AnchorFileOperationCallback   callback,
		       gpointer                        callback_data)
{
	...
	if (file->details->can_unmount) {
		...
		data->file = anchor_file_ref (file);
		data->callback = callback;
		data->callback_data = callback_data;
		anchor_file_operations_unmount_mount_full (NULL, file->details->mount, NULL, FALSE, TRUE, unmount_done, data);
	} else if (callback) {
...
void
anchor_file_eject (AnchorFile                   *file,
		     GMountOperation                *mount_op,
		     GCancellable                   *cancellable,
		     AnchorFileOperationCallback   callback,
		     gpointer                        callback_data)
{
	...
	if (file->details->can_eject) {
		...
		data->callback = callback;
		data->callback_data = callback_data;
		anchor_file_operations_unmount_mount_full (NULL, file->details->mount, NULL, TRUE, TRUE, unmount_done, data);
	} else if (callback) {
		callback (file, NULL, NULL, callback_data);
	}
}
...
void
anchor_file_start (AnchorFile                   *file,
		    ...
		    gpointer callback_data)
{
	...
	anchor_file_operation_complete (op, NULL, error);
	if (error) {
		g_error_free (error);
	}
}
...
void
anchor_file_stop (AnchorFile                   *file,
		    GMountOperation                *mount_op,
		    GCancellable                   *cancellable,
		    AnchorFileOperationCallback   callback,
		    gpointer                        callback_data)
{
	...
	if (NAUTILUS_FILE_GET_CLASS (file)->stop != NULL) {
		...
		if (drive != NULL && g_drive_can_stop (drive)) {
			...
			op = anchor_file_operation_new (file, callback, callback_data);
			if (cancellable) {
...
void
anchor_file_poll_for_media (AnchorFile *file)
{
	if (file->details->can_poll_for_media) {
		if (NAUTILUS_FILE_GET_CLASS (file)->stop != NULL) {
...
/**
 * anchor_file_is_desktop_directory:
 * 
 ...
 */
gboolean
anchor_file_is_desktop_directory (AnchorFile *file)
{
	GFile *dir;
	...
	return anchor_is_desktop_directory_file (dir, eel_ref_str_peek (file->details->name));
}

static gboolean
is_desktop_file (AnchorFile *file)
{
	return anchor_file_is_mime_type (file, "application/x-desktop");
}

static gboolean
can_rename_desktop_file (AnchorFile *file)
{
	GFile *location;
	gboolean res;
 ...
 */
gboolean
anchor_file_can_rename (AnchorFile *file)
{
	gboolean can_rename;
	...
	if (NAUTILUS_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			can_rename = anchor_desktop_link_can_rename (link);
			g_object_unref (link);
		}
	}
...
gboolean
anchor_file_can_delete (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_trash (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
GFile *
anchor_file_get_location (AnchorFile *file)
{
	GFile *dir;
...
char *
anchor_file_get_uri (AnchorFile *file)
{
	char *uri;
	GFile *loc;
	...
	loc = anchor_file_get_location (file);
...
char *
anchor_file_get_uri_scheme (AnchorFile *file)
{
	GFile *loc;
	char *scheme;
	...
	loc = anchor_directory_get_location (file->details->directory);
...
AnchorFileOperation *
anchor_file_operation_new (AnchorFile *file,
			     AnchorFileOperationCallback callback,
			     gpointer callback_data)
{
	AnchorFileOperation *op;

	op = g_new0 (AnchorFileOperation, 1);
	op->file = anchor_file_ref (file);
...
static void
anchor_file_operation_remove (AnchorFileOperation *op)
{
...
void
anchor_file_operation_free (AnchorFileOperation *op)
{
	anchor_file_operation_remove (op);
	anchor_file_unref (op->file);
	g_object_unref (op->cancellable);
	if (op->free_data) {
	...
	if (op->undo_info != NULL) {
		anchor_file_undo_manager_set_action (op->undo_info);
...
void
anchor_file_operation_complete (AnchorFileOperation *op,
				  GFile *result_file,
				  GError *error)
{
	...
	anchor_file_operation_free (op);
}

void
anchor_file_operation_cancel (AnchorFileOperation *op)
{
			  ...
			  gpointer callback_data)
{
	...
	if (new_info != NULL) {
		 ...
		 */
		existing_file = anchor_directory_find_file_by_name (directory, new_name);
		if (existing_file != NULL) {
			anchor_file_mark_gone (existing_file);
			anchor_file_changed (existing_file);
		}
		
		old_uri = anchor_file_get_uri (op->file);
		...
		new_uri = anchor_file_get_uri (op->file);
		anchor_directory_moved (old_uri, new_uri);
		g_free (new_uri);
		g_free (old_uri);
		...
		if (op->file->details->got_custom_display_name) {
			anchor_file_invalidate_attributes (op->file,
							     NAUTILUS_FILE_ATTRIBUTE_INFO |
		 ...
		 gpointer callback_data)
{
	...
	if (new_file != NULL) {
		if (op->undo_info != NULL) {
			anchor_file_undo_info_rename_set_data (NAUTILUS_FILE_UNDO_INFO_RENAME (op->undo_info),
...
static gboolean
name_is (AnchorFile *file, const char *new_name)
{
	const char *old_name;
...
void
anchor_file_rename (AnchorFile *file,
		      const char *new_name,
		      AnchorFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	if (NAUTILUS_IS_DESKTOP_ICON_FILE (file)) {
		...
		link = anchor_desktop_icon_file_get_link (NAUTILUS_DESKTOP_ICON_FILE (file));
		old_name = anchor_file_get_display_name (file);

		if ((old_name != NULL && strcmp (new_name, old_name) == 0)) {
			success = TRUE;
		} else {
			success = (link != NULL && anchor_desktop_link_rename (link, new_name));
		}
	...
	if (is_renameable_desktop_file) {
		 ...
		 */
		uri = anchor_file_get_uri (file);
		old_name = anchor_link_local_get_text (uri);
		if (old_name != NULL && strcmp (new_name, old_name) == 0) {
			success = TRUE;
			name_changed = FALSE;
		} else {
			success = anchor_link_local_set_text (uri, new_name);
			name_changed = TRUE;
		}
		g_free (old_name);
		g_free (uri);
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		op->undo_info = anchor_file_undo_info_rename_new ();
	}
...
gboolean
anchor_file_rename_in_progress (AnchorFile *file)
{
	GList *node;
	AnchorFileOperation *op;
...
void
anchor_file_cancel (AnchorFile *file,
		      AnchorFileOperationCallback callback,
		      gpointer callback_data)
{
	GList *node, *next;
	...
	for (node = file->details->operations_in_progress; node != NULL; node = next) {
		...
		if (op->callback == callback && op->callback_data == callback_data) {
			anchor_file_operation_cancel (op);
		}
	}
}

gboolean         
anchor_file_matches_uri (AnchorFile *file, const char *match_uri)
{
...
int
anchor_file_compare_location (AnchorFile *file_1,
                                AnchorFile *file_2)
{
	GFile *loc_a, *loc_b;
	gboolean res;

	loc_a = anchor_file_get_location (file_1);
	loc_b = anchor_file_get_location (file_2);
...
gboolean
anchor_file_is_local (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
	
	return anchor_directory_is_local (file->details->directory);
}

static void
update_link (AnchorFile *link_file, AnchorFile *target_file)
{
...
static GList *
get_link_files (AnchorFile *target_file)
{
	char *uri;
	GList **link_files;
	...
	if (symbolic_links == NULL) {
		...
		uri = anchor_file_get_uri (target_file);
		link_files = g_hash_table_lookup (symbolic_links, uri);
		g_free (uri);
	}
	if (link_files) {
		return anchor_file_list_copy (*link_files);
	}
	return NULL;
}

static void
update_links_if_target (AnchorFile *target_file)
{
	GList *link_files, *p;
	...
	anchor_file_list_free (link_files);
}

static gboolean
update_info_internal (AnchorFile *file,
		      GFileInfo *info,
		      gboolean update_name)
{
	...
	changed |= anchor_file_set_display_name (file,
	...
	changed |=
		anchor_file_update_metadata_from_info (file, info);
	...
	if (update_name) {
		...
		if (file->details->name == NULL ||
			...
			node = anchor_directory_begin_file_name_change
			...
			if (!file->details->got_custom_display_name &&
				 ...
				 * anchor_file_set_display_name() did
				 * not unset the display name.
				 */
				anchor_file_clear_display_name (file);
			}

			anchor_directory_end_file_name_change
...
static gboolean
update_info_and_name (AnchorFile *file,
		      GFileInfo *info)
{
...
gboolean
anchor_file_update_info (AnchorFile *file,
			   GFileInfo *info)
{
...
static gboolean
update_name_internal (AnchorFile *file,
		      const char *name,
...
gboolean
anchor_file_update_name (AnchorFile *file, const char *name)
{
	gboolean ret;
...
gboolean
anchor_file_update_name_and_directory (AnchorFile *file, 
					 const char *name,
					 AnchorDirectory *new_directory)
{
	AnchorDirectory *old_directory;
	...
	g_return_val_if_fail (!anchor_file_is_self_owned (file), FALSE);
	...
	monitors = anchor_directory_remove_file_monitors (old_directory, file);
	anchor_directory_remove_file (old_directory, file);

	file->details->directory = anchor_directory_ref (new_directory);
	anchor_directory_unref (old_directory);

	if (name) {
		update_name_internal (file, name, FALSE);
	}

	anchor_directory_add_file (new_directory, file);
	anchor_directory_add_file_monitors (new_directory, file, monitors);
...
void
anchor_file_set_directory (AnchorFile *file,
			     AnchorDirectory *new_directory)
{
	anchor_file_update_name_and_directory (file, NULL, new_directory);
}

static Knowledge
get_item_count (AnchorFile *file,
		guint *count)
{
	gboolean known, unreadable;

	known = anchor_file_get_directory_item_count
...
static int
compare_directories_by_count (AnchorFile *file_1, AnchorFile *file_2)
{
	/* Sort order:
...
static int
compare_files_by_size (AnchorFile *file_1, AnchorFile *file_2)
{
	/* Sort order:
...
static int
compare_by_size (AnchorFile *file_1, AnchorFile *file_2)
{
	/* Sort order:
	 *   Directories with n items
	 *   Directories with 0 items
	...
	is_directory_1 = anchor_file_is_directory (file_1);
	is_directory_2 = anchor_file_is_directory (file_2);
...
static int
compare_by_display_name (AnchorFile *file_1, AnchorFile *file_2)
{
	...
	name_1 = anchor_file_peek_display_name (file_1);
	name_2 = anchor_file_peek_display_name (file_2);
	...
	if (sort_last_1 && !sort_last_2) {
		...
		key_1 = anchor_file_peek_display_name_collation_key (file_1);
		key_2 = anchor_file_peek_display_name_collation_key (file_2);
...
static int
compare_by_directory_name (AnchorFile *file_1, AnchorFile *file_2)
{
	...
	directory_1 = anchor_file_get_parent_uri_for_display (file_1);
	directory_2 = anchor_file_get_parent_uri_for_display (file_2);
...
static int
compare_by_type (AnchorFile *file_1, AnchorFile *file_2)
{
	...
	type_string_1 = anchor_file_get_type_as_string (file_1);
	type_string_2 = anchor_file_get_type_as_string (file_2);
...
static Knowledge
get_search_relevance (AnchorFile *file,
...
static int
compare_by_search_relevance (AnchorFile *file_1, AnchorFile *file_2)
{
	gdouble r_1, r_2;
...
static int
compare_by_time (AnchorFile *file_1, AnchorFile *file_2, AnchorDateType type)
{
	/* Sort order:
...
static int
compare_by_full_path (AnchorFile *file_1, AnchorFile *file_2)
{
	int compare;
...
static int
anchor_file_compare_for_sort_internal (AnchorFile *file_1,
					 AnchorFile *file_2,
 ...
 **/
int
anchor_file_compare_for_sort (AnchorFile *file_1,
				AnchorFile *file_2,
				...
				gboolean reversed)
{
	...
	result = anchor_file_compare_for_sort_internal (file_1, file_2, directories_first, reversed);
...
int
anchor_file_compare_for_sort_by_attribute_q   (AnchorFile                   *file_1,
						 AnchorFile                   *file_2,
						 GQuark                          attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	...
	/* Convert certain attributes into AnchorFileSortTypes and use
	 * anchor_file_compare_for_sort()
	 */
	if (attribute == 0 || attribute == attribute_name_q) {
		return anchor_file_compare_for_sort (file_1, file_2,
	...
	if (result == 0) {
		...
		value_1 = anchor_file_get_string_attribute_q (file_1, 
								attribute);
		value_2 = anchor_file_get_string_attribute_q (file_2, 
								attribute);
...
int
anchor_file_compare_for_sort_by_attribute     (AnchorFile                   *file_1,
						 AnchorFile                   *file_2,
						 const char                     *attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	return anchor_file_compare_for_sort_by_attribute_q (file_1, file_2,
 ...
 **/
int
anchor_file_compare_display_name (AnchorFile *file,
				    const char *pattern)
{
	const char *name;
	int result;
	...
	name = anchor_file_peek_display_name (file);
...
gboolean
anchor_file_is_hidden_file (AnchorFile *file)
{
	return file->details->is_hidden;
}

/**
 * anchor_file_should_show:
 * @file: the file to check
 * @show_hidden: whether we want to show hidden files or not
 * @show_foreign: whether we want to show foreign files or not
 * 
 * Determines if a #AnchorFile should be shown. Note that when browsing
 * a trash directory, this function will always return %TRUE. 
 *
 * Returns: %TRUE if the file should be shown, %FALSE if it shouldn't.
 */
gboolean 
anchor_file_should_show (AnchorFile *file, 
			   gboolean show_hidden,
			   gboolean show_foreign)
{
	/* Never hide any files in trash. */
	if (anchor_file_is_in_trash (file)) {
		return TRUE;
	}

	if (!show_hidden && anchor_file_is_hidden_file (file)) {
		return FALSE;
	}

	if (!show_foreign && anchor_file_is_foreign_link (file)) {
		return FALSE;
	}
...
gboolean
anchor_file_is_home (AnchorFile *file)
{
	GFile *dir;
	...
	return anchor_is_home_directory_file (dir,
						eel_ref_str_peek (file->details->name));
}

gboolean
anchor_file_is_in_desktop (AnchorFile *file)
{
	if (file->details->directory->details->location) {
		return anchor_is_desktop_directory (file->details->directory->details->location);
	}
	return FALSE;
}

gboolean
anchor_file_is_in_search (AnchorFile *file)
{
	char *uri;
	gboolean ret;
				  ...
				  gpointer callback_data)
{
	...
	return anchor_file_should_show (file,
					  options & SHOW_HIDDEN,
					  TRUE);
}

GList *
anchor_file_list_filter_hidden (GList    *files,
				  gboolean  show_hidden)
{
	...
	filtered_files = anchor_file_list_copy (files);
	filtered_files = eel_g_list_partition (filtered_files,
					       ...
					       &removed_files);
	anchor_file_list_free (removed_files);

	return filtered_files;
}

char *
anchor_file_get_metadata (AnchorFile *file,
			    const char *key,
...
GList *
anchor_file_get_metadata_list (AnchorFile *file,
				 const char *key)
{
	GList *res;
	guint id;
...
void
anchor_file_set_metadata (AnchorFile *file,
			    const char *key,
			    const char *default_metadata,
			    const char *metadata)
{
...
void
anchor_file_set_metadata_list (AnchorFile *file,
				 const char *key,
				 GList *list)
{
	char **val;
...
gboolean
anchor_file_get_boolean_metadata (AnchorFile *file,
				    const char   *key,
				    gboolean      default_metadata)
{
	...
	result_as_string = anchor_file_get_metadata
...
int
anchor_file_get_integer_metadata (AnchorFile *file,
				    const char   *key,
...
time_t
anchor_file_get_time_metadata (AnchorFile *file,
				 const char   *key)
{
	time_t time;
	char *time_string;

	time_string = anchor_file_get_metadata (file, key, NULL);
...
void
anchor_file_set_time_metadata (AnchorFile *file,
				 const char   *key,
				 time_t        time)
{
	char time_str[21];
	...
	anchor_file_set_metadata (file, key, NULL, metadata);
}


void
anchor_file_set_boolean_metadata (AnchorFile *file,
				    const char   *key,
				    gboolean      default_metadata,
				    gboolean      metadata)
{
	...
	anchor_file_set_metadata (file, key,
				    default_metadata ? "true" : "false",
				    metadata ? "true" : "false");
}

void
anchor_file_set_integer_metadata (AnchorFile *file,
				    const char   *key,
				    int           default_metadata,
				    int           metadata)
{
...
static const char *
anchor_file_peek_display_name_collation_key (AnchorFile *file)
{
	const char *res;
...
static const char *
anchor_file_peek_display_name (AnchorFile *file)
{
	const char *name;
	char *escaped_name;

	/* FIXME: for some reason we can get a AnchorFile instance which is
	 *        no longer valid or could be freed somewhere else in the same time.
	 *        There's race condition somewhere. See bug 602500.
	 */
	if (file == NULL || anchor_file_is_gone (file))
		return "";
...
char *
anchor_file_get_display_name (AnchorFile *file)
{
	return g_strdup (anchor_file_peek_display_name (file));
}

char *
anchor_file_get_edit_name (AnchorFile *file)
{
	const char *res;
...
char *
anchor_file_get_name (AnchorFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->name));
}

/**
 * anchor_file_get_description:
 * @file: a #AnchorFile.
 * 
 ...
 */
char *
anchor_file_get_description (AnchorFile *file)
{
	return g_strdup (file->details->description);
}
   
void             
anchor_file_monitor_add (AnchorFile *file,
...
void
anchor_file_monitor_remove (AnchorFile *file,
			      gconstpointer client)
{
	g_return_if_fail (NAUTILUS_IS_FILE (file));
	g_return_if_fail (client != NULL);
...
gboolean
anchor_file_is_launcher (AnchorFile *file)
{
	return file->details->is_launcher;
}

gboolean
anchor_file_is_foreign_link (AnchorFile *file)
{
	return file->details->is_foreign_link;
}

gboolean
anchor_file_is_trusted_link (AnchorFile *file)
{
	return file->details->is_trusted_link;
}

gboolean
anchor_file_has_activation_uri (AnchorFile *file)
{
 ...
 * the actual uri if the file is an desktop file or a anchor
 * xml link file.
 */
char *
anchor_file_get_activation_uri (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), NULL);
...
GFile *
anchor_file_get_activation_location (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), NULL);
	...
	return anchor_file_get_location (file);
}


char *
anchor_file_get_drop_target_uri (AnchorFile *file)
{
	char *uri, *target_uri;
	GFile *location;
	AnchorDesktopLink *link;
	...
	if (NAUTILUS_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			location = anchor_desktop_link_get_activation_location (link);
			g_object_unref (link);
			if (location != NULL) {
	...
	if (anchor_file_is_anchor_link (file)) {
		location = anchor_file_get_location (file);
		/* FIXME bugzilla.gnome.org 43020: This does sync. I/O and works only locally. */
		if (g_file_is_native (location)) {
			target_uri = anchor_link_local_get_link_uri (uri);
			if (target_uri != NULL) {
				g_free (uri);
				uri = target_uri;
			}
...
static char *
get_custom_icon_metadata_uri (AnchorFile *file)
{
	char *custom_icon_uri;
	char *uri;
	char *dir_uri;
	
	uri = anchor_file_get_metadata (file, NAUTILUS_METADATA_KEY_CUSTOM_ICON, NULL);
	if (uri != NULL &&
	    anchor_file_is_directory (file) &&
	    is_uri_relative (uri)) {
		dir_uri = anchor_file_get_uri (file);
...
static char *
get_custom_icon_metadata_name (AnchorFile *file)
{
	char *icon_name;

	icon_name = anchor_file_get_metadata (file,
...
static GIcon *
get_custom_icon (AnchorFile *file)
{
...
anchor_file_get_filesystem_use_preview (AnchorFile *file)
{
...
gboolean
anchor_file_should_show_thumbnail (AnchorFile *file)
{
	const char *mime_type;
	GFilesystemPreviewType use_preview;

	use_preview = anchor_file_get_filesystem_use_preview (file);
	 ...
	 */
	if (anchor_thumbnail_is_mimetype_limited_by_size (mime_type) &&
	    file->details->thumbnail_path == NULL &&
	    anchor_file_get_size (file) > cached_thumbnail_limit) {
		return FALSE;
	}
	...
	if (show_file_thumbs == NAUTILUS_SPEED_TRADEOFF_ALWAYS) {
		...
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			...
			/* only local files */
			return anchor_file_is_local (file);
		}
	}
...
static gboolean
anchor_is_video_file (AnchorFile *file)
{
	const char *mime_type;
	guint i;
...
GIcon *
anchor_file_get_gicon (AnchorFile *file,
			 AnchorFileIconFlags flags)
{
	...
	if (file->details->icon) {
		...
		if (flags & NAUTILUS_FILE_ICON_FLAGS_USE_MOUNT_ICON ||
			...
			mount = anchor_file_get_mount (file);
		...
		if (((flags & NAUTILUS_FILE_ICON_FLAGS_EMBEDDING_TEXT) ||
		     ...
		     ((flags & NAUTILUS_FILE_ICON_FLAGS_IGNORE_VISITING) == 0 &&
		      anchor_file_has_open_window (file))) &&
...
static GIcon *
get_default_file_icon (AnchorFileIconFlags flags)
{
...
char *
anchor_file_get_thumbnail_path (AnchorFile *file)
{
	return g_strdup (file->details->thumbnail_path);
}

AnchorIconInfo *
anchor_file_get_icon (AnchorFile *file,
			int size,
			int scale,
			AnchorFileIconFlags flags)
{
	...
	if (gicon != NULL) {
		icon = anchor_icon_info_lookup (gicon, size, scale);
	...
	if (flags & NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS &&
	    anchor_file_should_show_thumbnail (file)) {
		if (file->details->thumbnail) {
			int w, h, s;
			double thumb_scale;
			...
			if (!gdk_pixbuf_get_has_alpha (raw_pixbuf) || s >= 128 * scale) {
				if (anchor_is_video_file (file))
					anchor_ui_frame_video (&scaled_pixbuf);
				else
					anchor_ui_frame_image (&scaled_pixbuf);
			}
			...
			if (modified_size > 128 * 1.25 * scale &&
			    ...
			    anchor_can_thumbnail_internally (file)) {
				/* Invalidate if we resize upward */
				file->details->thumbnail_wants_original = TRUE;
				anchor_file_invalidate_attributes (file, NAUTILUS_FILE_ATTRIBUTE_THUMBNAIL);
			}

			DEBUG ("Returning thumbnailed image, at size %d %d",
			       (int) (w * thumb_scale), (int) (h * thumb_scale));
			
			icon = anchor_icon_info_new_for_pixbuf (scaled_pixbuf, scale);
			...
			if (anchor_can_thumbnail (file)) {
				anchor_create_thumbnail (file);
			}
		}
	}
	...
	if (file->details->is_thumbnailing &&
		...
		gicon = anchor_file_get_gicon (file, flags);
	
	if (gicon) {
		icon = anchor_icon_info_lookup (gicon, size, scale);
		if (anchor_icon_info_is_fallback (icon)) {
			g_object_unref (icon);
			icon = anchor_icon_info_lookup (get_default_file_icon (flags), size, scale);
		}
		g_object_unref (gicon);
		return icon;
	} else {
		return anchor_icon_info_lookup (get_default_file_icon (flags), size, scale);
	}
}

GdkPixbuf *
anchor_file_get_icon_pixbuf (AnchorFile *file,
			       int size,
			       gboolean force_size,
			       int scale,
			       AnchorFileIconFlags flags)
{
	...
	info = anchor_file_get_icon (file, size, scale, flags);
	if (force_size) {
		pixbuf =  anchor_icon_info_get_pixbuf_at_size (info, size);
	} else {
		pixbuf = anchor_icon_info_get_pixbuf (info);
	}
	g_object_unref (info);
...
gboolean
anchor_file_get_date (AnchorFile *file,
...
static char *
anchor_file_get_where_string (AnchorFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
static char *
anchor_file_get_trash_original_file_parent_as_string (AnchorFile *file)
{
	AnchorFile *orig_file, *parent;
	GFile *location;
	char *filename;

	if (file->details->trash_orig_path != NULL) {
		orig_file = anchor_file_get_trash_original_file (file);
		parent = anchor_file_get_parent (orig_file);
		location = anchor_file_get_location (parent);

		filename = g_file_get_parse_name (location);

		g_object_unref (location);
		anchor_file_unref (parent);
		anchor_file_unref (orig_file);
 ...
 * in the Anchor version of strftime that can be
...
/**
 * anchor_file_get_date_as_string:
 * 
 * Get a user-displayable string representing a file modification date. 
 * The caller is responsible for g_free-ing this string.
 * @file: AnchorFile representing the file in question.
 * 
 * Returns: Newly allocated string ready to display to the user.
 * 
 **/
static char *
anchor_file_get_date_as_string (AnchorFile *file, AnchorDateType date_type, gboolean compact)
{
	...
	if (!anchor_file_get_date (file, date_type, &file_time_raw)) {
		return NULL;
	}
...
{
	show_directory_item_count = g_settings_get_enum (anchor_preferences, NAUTILUS_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}

static gboolean
get_speed_tradeoff_preference_for_file (AnchorFile *file, AnchorSpeedTradeoffValue value)
{
	GFilesystemPreviewType use_preview;
...
gboolean
anchor_file_should_show_directory_item_count (AnchorFile *file)
{
...
gboolean
anchor_file_should_show_type (AnchorFile *file)
{
	char *uri;
	gboolean ret;
...
gboolean
anchor_file_should_get_top_left_text (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
/**
 * anchor_file_get_directory_item_count
 * 
 * Get the number of items in a directory.
 * @file: AnchorFile representing a directory.
 ...
 **/
gboolean
anchor_file_get_directory_item_count (AnchorFile *file, 
					guint *count,
					gboolean *count_unreadable)
{
	...
	if (!anchor_file_is_directory (file)) {
		return FALSE;
	}

	if (!anchor_file_should_show_directory_item_count (file)) {
		return FALSE;
	}
...
/**
 * anchor_file_get_deep_counts
 * 
 * Get the statistics about items inside a directory.
 * @file: AnchorFile representing a directory or file.
 ...
 * anchor_file_should_show_directory_item_count returns FALSE
 * for this file.
 * 
 ...
 **/
AnchorRequestStatus
anchor_file_get_deep_counts (AnchorFile *file,
			       ...
			       gboolean force)
{
	...
	if (!force && !anchor_file_should_show_directory_item_count (file)) {
...
void
anchor_file_recompute_deep_counts (AnchorFile *file)
{
	if (file->details->deep_counts_status != NAUTILUS_REQUEST_IN_PROGRESS) {
		file->details->deep_counts_status = NAUTILUS_REQUEST_NOT_STARTED;
		if (file->details->directory != NULL) {
			anchor_directory_add_file_to_work_queue (file->details->directory, file);
			anchor_directory_async_state_changed (file->details->directory);
		}
	}
}
...
/**
 * anchor_file_get_directory_item_mime_types
 * 
 * Get the list of mime-types present in a directory.
 * @file: AnchorFile representing a directory. It is an error to
 ...
 **/
gboolean
anchor_file_get_directory_item_mime_types (AnchorFile *file,
					     GList **mime_list)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
gboolean
anchor_file_can_get_size (AnchorFile *file)
{
 ...
 **/
goffset
anchor_file_get_size (AnchorFile *file)
{
...
time_t
anchor_file_get_mtime (AnchorFile *file)
{
				  ...
				  gpointer callback_data)
{
	...
	new_info = g_file_query_info_finish (G_FILE (source_object), res, &error);
	if (new_info != NULL) {
		if (anchor_file_update_info (op->file, new_info)) {
			anchor_file_changed (op->file);
		}
		g_object_unref (new_info);
	}
...
void
anchor_file_set_attributes (AnchorFile *file, 
...
void
anchor_file_set_search_relevance (AnchorFile *file,
				    gdouble       relevance)
{
...
/**
 * anchor_file_can_get_permissions:
 * 
 ...
 */
gboolean
anchor_file_can_get_permissions (AnchorFile *file)
{
	return file->details->has_permissions;
}

/**
 * anchor_file_can_set_permissions:
 * 
 ...
 */
gboolean
anchor_file_can_set_permissions (AnchorFile *file)
{
	uid_t user_id;

	if (file->details->uid != -1 &&
	    anchor_file_is_local (file)) {
		/* Check the user. */
		user_id = geteuid();
...
guint
anchor_file_get_permissions (AnchorFile *file)
{
	g_return_val_if_fail (anchor_file_can_get_permissions (file), 0);

	return file->details->permissions;
}

/**
 * anchor_file_set_permissions:
 * 
 * Change a file's permissions. This should only be called if
 * anchor_file_can_set_permissions returned TRUE.
 * 
 * @file: AnchorFile representing the file in question.
 ...
 **/
void
anchor_file_set_permissions (AnchorFile *file, 
			       ...
			       gpointer callback_data)
{
	...
	if (!anchor_file_can_set_permissions (file)) {
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		undo_info = anchor_file_undo_info_permissions_new (anchor_file_get_location (file),
								     file->details->permissions,
								     new_permissions);
		anchor_file_undo_manager_set_action (undo_info);
	}

	info = g_file_info_new ();
	g_file_info_set_attribute_uint32 (info, G_FILE_ATTRIBUTE_UNIX_MODE, new_permissions);
	anchor_file_set_attributes (file, info, callback, callback_data);
	
	g_object_unref (info);
}

/**
 * anchor_file_can_get_selinux_context:
 * 
 ...
 */
gboolean
anchor_file_can_get_selinux_context (AnchorFile *file)
{
...
/**
 * anchor_file_get_selinux_context:
 * 
 ...
 **/
char *
anchor_file_get_selinux_context (AnchorFile *file)
{
	char *translated;
	char *raw;
	...
	if (!anchor_file_can_get_selinux_context (file)) {
		return NULL;
	}
 ...
 */
gboolean
anchor_file_can_get_owner (AnchorFile *file)
{
 ...
 */
char *
anchor_file_get_owner_name (AnchorFile *file)
{
	return anchor_file_get_owner_as_string (file, FALSE);
}

/**
 * anchor_file_can_set_owner:
 * 
 * Check whether the current user is allowed to change
 * the owner of a file.
 * 
...
/**
 * anchor_file_set_owner:
 * 
 * Set the owner of a file. This will only have any effect if
 * anchor_file_can_set_owner returns TRUE.
 * 
 * @file: The file in question.
 ...
 */
void
anchor_file_set_owner (AnchorFile *file, 
			 ...
			 gpointer callback_data)
{
	...
	if (!anchor_file_can_set_owner (file)) {
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		current_owner = anchor_file_get_owner_as_string (file, FALSE);

		undo_info = anchor_file_undo_info_ownership_new (NAUTILUS_FILE_UNDO_OP_CHANGE_OWNER,
								   anchor_file_get_location (file),
								   current_owner,
								   user_name_or_id);
		anchor_file_undo_manager_set_action (undo_info);
 ...
 */
gboolean
anchor_file_can_get_group (AnchorFile *file)
{
 ...
 **/
char *
anchor_file_get_group_name (AnchorFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->group));
}

/**
 * anchor_file_can_set_group:
 * 
 * Check whether the current user is allowed to change
 * the group of a file.
 * 
 ...
 */
gboolean
anchor_file_can_set_group (AnchorFile *file)
{
	uid_t user_id;
	 ...
	 * systems.
	 */
	if (!anchor_file_can_get_group (file)) {
		return FALSE;
	}
 ...
 */
static GList *
anchor_get_group_names_for_user (void)
{
	GList *list;
	struct group *group;
	int count, i;
...
/**
 * anchor_get_group_names:
 * 
 * Get a list of all group names.
 */
GList *
anchor_get_all_group_names (void)
{
	GList *list;
	struct group *group;
...
/**
 * anchor_file_get_settable_group_names:
 * 
 * Get a list of all group names that the current user
 * can set the group of a specific file to.
 * 
 * @file: The AnchorFile in question.
 */
GList *
anchor_file_get_settable_group_names (AnchorFile *file)
{
	uid_t user_id;
	GList *result;

	if (!anchor_file_can_set_group (file)) {
		return NULL;
	}	
	...
	if (user_id == 0) {
		...
		result = anchor_get_all_group_names ();
	} else if (user_id == (uid_t) file->details->uid) {
		/* Owner is allowed to set group to any that owner is member of. */
		result = anchor_get_group_names_for_user ();
	} else {
		g_warning ("unhandled case in anchor_get_settable_group_names");
		result = NULL;
	}
...
/**
 * anchor_file_set_group:
 * 
 * Set the group of a file. This will only have any effect if
 * anchor_file_can_set_group returns TRUE.
 * 
 * @file: The file in question.
 ...
 */
void
anchor_file_set_group (AnchorFile *file, 
			 ...
			 gpointer callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		current_group = anchor_file_get_group_name (file);
		undo_info = anchor_file_undo_info_ownership_new (NAUTILUS_FILE_UNDO_OP_CHANGE_GROUP,
								   anchor_file_get_location (file),
								   current_group,
								   group_name_or_id);
		anchor_file_undo_manager_set_action (undo_info);
...
/**
 * anchor_file_get_octal_permissions_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_octal_permissions_as_string (AnchorFile *file)
{
	guint32 permissions;

	g_assert (NAUTILUS_IS_FILE (file));

	if (!anchor_file_can_get_permissions (file)) {
		return NULL;
	}
...
/**
 * anchor_file_get_permissions_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_permissions_as_string (AnchorFile *file)
{
	guint32 permissions;
	...
	permissions = file->details->permissions;
	is_directory = anchor_file_is_directory (file);
	is_link = anchor_file_is_symbolic_link (file);
...
/**
 * anchor_file_get_owner_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_owner_as_string (AnchorFile *file, gboolean include_real_name)
{
	char *user_name;
...
/**
 * anchor_file_get_size_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_size_as_string (AnchorFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	...
	if (anchor_file_is_directory (file)) {
		if (!anchor_file_get_directory_item_count (file, &item_count, &count_unreadable)) {
			return NULL;
		}
...
/**
 * anchor_file_get_size_as_string_with_real_size:
 * 
 * Get a user-displayable string representing a file size. The caller
 * is responsible for g_free-ing this string. The string is an item
 * count for directories.
 ...
 **/
static char *
anchor_file_get_size_as_string_with_real_size (AnchorFile *file)
{
	guint item_count;
	gboolean count_unreadable;
...
static char *
anchor_file_get_deep_count_as_string_internal (AnchorFile *file,
						 gboolean report_size,
						 gboolean report_directory_count,
						 gboolean report_file_count)
{
	AnchorRequestStatus status;
	guint directory_count;
	guint file_count;
	...
	g_assert (NAUTILUS_IS_FILE (file));
	g_assert (anchor_file_is_directory (file));

	status = anchor_file_get_deep_counts 
	 ...
	 * Callers can distinguish them using anchor_file_get_deep_counts
	 * directly if desired.
	 */
	if (report_size) {
...
/**
 * anchor_file_get_deep_size_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_deep_size_as_string (AnchorFile *file)
{
	return anchor_file_get_deep_count_as_string_internal (file, TRUE, FALSE, FALSE);
}

/**
 * anchor_file_get_deep_total_count_as_string:
 * 
 ...
 **/
static char *
anchor_file_get_deep_total_count_as_string (AnchorFile *file)
{
	return anchor_file_get_deep_count_as_string_internal (file, FALSE, TRUE, TRUE);
}

/**
 * anchor_file_get_deep_file_count_as_string:
 * 
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
anchor_file_get_deep_file_count_as_string (AnchorFile *file)
{
	return anchor_file_get_deep_count_as_string_internal (file, FALSE, FALSE, TRUE);
}

/**
 * anchor_file_get_deep_directory_count_as_string:
 * 
 * Get a user-displayable string representing the count of all contained
 ...
 **/
static char *
anchor_file_get_deep_directory_count_as_string (AnchorFile *file)
{
	return anchor_file_get_deep_count_as_string_internal (file, FALSE, TRUE, FALSE);
}

/**
 * anchor_file_get_string_attribute:
 * 
 ...
 * anchor_file_get_string_attribute_with_default if you want a non-NULL
 * default.
 * 
 * @file: AnchorFile representing the file in question.
 ...
 **/
char *
anchor_file_get_string_attribute_q (AnchorFile *file, GQuark attribute_q)
{
	char *extension_attribute;

	if (attribute_q == attribute_name_q) {
		return anchor_file_get_display_name (file);
	}
	if (attribute_q == attribute_type_q) {
		return anchor_file_get_type_as_string (file);
	}
	if (attribute_q == attribute_detailed_type_q) {
		return anchor_file_get_detailed_type_as_string (file);
	}
	if (attribute_q == attribute_mime_type_q) {
		return anchor_file_get_mime_type (file);
	}
	if (attribute_q == attribute_size_q) {
		return anchor_file_get_size_as_string (file);
	}
	if (attribute_q == attribute_size_detail_q) {
		return anchor_file_get_size_as_string_with_real_size (file);
	}
	if (attribute_q == attribute_deep_size_q) {
		return anchor_file_get_deep_size_as_string (file);
	}
	if (attribute_q == attribute_deep_file_count_q) {
		return anchor_file_get_deep_file_count_as_string (file);
	}
	if (attribute_q == attribute_deep_directory_count_q) {
		return anchor_file_get_deep_directory_count_as_string (file);
	}
	if (attribute_q == attribute_deep_total_count_q) {
		return anchor_file_get_deep_total_count_as_string (file);
	}
	if (attribute_q == attribute_trash_orig_path_q) {
		return anchor_file_get_trash_original_file_parent_as_string (file);
	}
	if (attribute_q == attribute_date_modified_q) {
		return anchor_file_get_date_as_string (file, 
	...
	if (attribute_q == attribute_permissions_q) {
		return anchor_file_get_permissions_as_string (file);
	}
	if (attribute_q == attribute_selinux_context_q) {
		return anchor_file_get_selinux_context (file);
	}
	if (attribute_q == attribute_octal_permissions_q) {
		return anchor_file_get_octal_permissions_as_string (file);
	}
	if (attribute_q == attribute_owner_q) {
		return anchor_file_get_owner_as_string (file, TRUE);
	}
	if (attribute_q == attribute_group_q) {
		return anchor_file_get_group_name (file);
	}
	if (attribute_q == attribute_uri_q) {
		return anchor_file_get_uri (file);
	}
	if (attribute_q == attribute_where_q) {
		return anchor_file_get_where_string (file);
	}
	if (attribute_q == attribute_link_target_q) {
		return anchor_file_get_symbolic_link_target_path (file);
	}
	if (attribute_q == attribute_volume_q) {
		return anchor_file_get_volume_name (file);
	}
	if (attribute_q == attribute_free_space_q) {
		return anchor_file_get_volume_free_space (file);
	}
...
char *
anchor_file_get_string_attribute (AnchorFile *file, const char *attribute_name)
{
	return anchor_file_get_string_attribute_q (file, g_quark_from_string (attribute_name));
}


/**
 * anchor_file_get_string_attribute_with_default:
 * 
 * Get a user-displayable string from a named attribute. Use g_free to
 ...
 * anchor_file_get_string_attribute if you want NULL instead of a default
 * result.
 * 
 * @file: AnchorFile representing the file in question.
 * @attribute_name: The name of the desired attribute. See the description of
 * anchor_file_get_string for the set of available attributes.
 * 
 ...
 **/
char *
anchor_file_get_string_attribute_with_default_q (AnchorFile *file, GQuark attribute_q)
{
	char *result;
	guint item_count;
	gboolean count_unreadable;
	...
	result = anchor_file_get_string_attribute_q (file, attribute_q);
	if (result != NULL) {
		return result;
	}
	...
	if (attribute_q == attribute_size_q) {
		...
		count_unreadable = FALSE;
		if (anchor_file_is_directory (file)) {
			anchor_file_get_directory_item_count (file, &item_count, &count_unreadable);
		}
		return g_strdup (count_unreadable ? _("? items") : "...");
	}
	if (attribute_q == attribute_deep_size_q) {
		status = anchor_file_get_deep_counts (file, NULL, NULL, NULL, NULL, FALSE);
...
char *
anchor_file_get_string_attribute_with_default (AnchorFile *file, const char *attribute_name)
{
	return anchor_file_get_string_attribute_with_default_q (file, g_quark_from_string (attribute_name));
}

gboolean
anchor_file_is_date_sort_attribute_q (GQuark attribute_q)
{
...
static char *
get_description (AnchorFile *file,
		 gboolean      detailed)
{
	const char *mime_type;
	...
	if (g_content_type_is_unknown (mime_type)) {
		if (anchor_file_is_executable (file)) {
...
static char *
update_description_for_link (AnchorFile *file, char *string)
{
	char *res;
	
	if (anchor_file_is_symbolic_link (file)) {
		g_assert (!anchor_file_is_broken_symbolic_link (file));
		if (string == NULL) {
...
static char *
anchor_file_get_type_as_string (AnchorFile *file)
{
	if (file == NULL) {
		return NULL;
	}

	if (anchor_file_is_broken_symbolic_link (file)) {
...
static char *
anchor_file_get_detailed_type_as_string (AnchorFile *file)
{
	if (file == NULL) {
		return NULL;
	}
 ...
 **/
GFileType
anchor_file_get_file_type (AnchorFile *file)
{
	if (file == NULL) {
 ...
 **/
char *
anchor_file_get_mime_type (AnchorFile *file)
{
	if (file != NULL) {
		g_return_val_if_fail (NAUTILUS_IS_FILE (file), NULL);
 ...
 **/
gboolean
anchor_file_is_mime_type (AnchorFile *file, const char *mime_type)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
char *
anchor_file_get_extension (AnchorFile *file)
{
	char *name;
	char *extension = NULL;

	name = anchor_file_get_name (file);
	if (name != NULL) {
...
gboolean
anchor_file_is_launchable (AnchorFile *file)
{
	...
	return type_can_be_executable &&
		anchor_file_can_get_permissions (file) &&
		anchor_file_can_execute (file) &&
		anchor_file_is_executable (file) &&
		!anchor_file_is_directory (file);
}
...
static void
clean_up_metadata_keywords (AnchorFile *file,
			    GList **metadata_keywords)
{
	AnchorFile *parent_file;
	GList *l, *res = NULL;
	char *exclude[4];
	char *keyword;
	gboolean found;
	...
	parent_file = anchor_file_get_parent (file);
	if (parent_file) {
		if (!anchor_file_can_write (parent_file)) {
			exclude[i++] = NAUTILUS_FILE_EMBLEM_NAME_CANT_WRITE;
		}
		anchor_file_unref (parent_file);
	}
	exclude[i++] = NULL;
 ...
 **/
static GList *
anchor_file_get_keywords (AnchorFile *file)
{
	...
	metadata_keywords = anchor_file_get_metadata_list (file, NAUTILUS_METADATA_KEY_EMBLEMS);
 ...
 **/
GList *
anchor_file_get_emblem_icons (AnchorFile *file)
{
	GList *keywords, *l;
	GList *icons;
	char *icon_names[2];
	...
	keywords = anchor_file_get_keywords (file);
 ...
 **/
gboolean
anchor_file_is_symbolic_link (AnchorFile *file)
{
	return file->details->is_symlink;
}

gboolean
anchor_file_is_mountpoint (AnchorFile *file)
{
	return file->details->is_mountpoint;
}

GMount *
anchor_file_get_mount (AnchorFile *file)
{
	if (file->details->mount) {
		      ...
		      gpointer data)
{
	...
	anchor_file_invalidate_attributes (file, NAUTILUS_FILE_ATTRIBUTE_MOUNT);
}

void
anchor_file_set_mount (AnchorFile *file,
			 GMount *mount)
{
	if (file->details->mount) {
		g_signal_handlers_disconnect_by_func (file->details->mount, file_mount_unmounted, file);
...
/**
 * anchor_file_is_broken_symbolic_link
 * 
 ...
 **/
gboolean
anchor_file_is_broken_symbolic_link (AnchorFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return anchor_file_get_file_type (file) == G_FILE_TYPE_SYMBOLIC_LINK;
}
...
/**
 * anchor_file_get_volume_free_space
 ...
 */
char *
anchor_file_get_volume_free_space (AnchorFile *file)
{
	GFile *location;
	char *res;
	time_t now;
 ...
 */ 
char *
anchor_file_get_volume_name (AnchorFile *file)
{
	GFile *location;
	char *res;
	GMount *mount;
...
/**
 * anchor_file_get_symbolic_link_target_path
 * 
 ...
 * @file: AnchorFile representing the symbolic link in question.
 * 
 * Returns: newly-allocated copy of the file path of the target of the symbolic link.
 */
char *
anchor_file_get_symbolic_link_target_path (AnchorFile *file)
{
	if (!anchor_file_is_symbolic_link (file)) {
...
/**
 * anchor_file_get_symbolic_link_target_uri
 * 
 ...
 */
char *
anchor_file_get_symbolic_link_target_uri (AnchorFile *file)
{
...
/**
 * anchor_file_is_anchor_link
 * 
 * Check if this file is a "anchor link", meaning a historical
 * anchor xml link file or a desktop file.
 * @file: AnchorFile representing the file in question.
 * 
 * Returns: True if the file is a anchor link.
 * 
 **/
gboolean
anchor_file_is_anchor_link (AnchorFile *file)
{
	if (file->details->mime_type == NULL) {
		return FALSE;
	}
 ...
 **/
gboolean
anchor_file_is_directory (AnchorFile *file)
{
	return anchor_file_get_file_type (file) == G_FILE_TYPE_DIRECTORY;
}

/**
 * anchor_file_is_user_special_directory
 *
 ...
 */
gboolean
anchor_file_is_user_special_directory (AnchorFile *file,
...
gboolean
anchor_file_is_archive (AnchorFile *file)
{
	char *mime_type;
	int i;
 ...
 **/
gboolean
anchor_file_is_in_trash (AnchorFile *file)
{
	g_assert (NAUTILUS_IS_FILE (file));

	return anchor_directory_is_in_trash (file->details->directory);
}

/**
 * anchor_file_is_in_recent
 * 
 ...
 **/
gboolean
anchor_file_is_in_recent (AnchorFile *file)
{
	g_assert (NAUTILUS_IS_FILE (file));

	return anchor_directory_is_in_recent (file->details->directory);
}

/**
 * anchor_file_is_in_network
 * 
 ...
 **/
gboolean
anchor_file_is_in_network (AnchorFile *file)
{
	g_assert (NAUTILUS_IS_FILE (file));

	return anchor_directory_is_in_network (file->details->directory);
}

GError *
anchor_file_get_file_info_error (AnchorFile *file)
{
	if (!file->details->get_info_failed) {
		return NULL;
	}
 ...
 **/
gboolean
anchor_file_contains_text (AnchorFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return anchor_file_is_mime_type (file, "text/plain");
}

/**
 * anchor_file_is_executable
 * 
 ...
 **/
gboolean
anchor_file_is_executable (AnchorFile *file)
{
...
/**
 * anchor_file_peek_top_left_text
 * 
 ...
 **/
char *
anchor_file_peek_top_left_text (AnchorFile *file,
				  gboolean  need_large_text,
				  gboolean *needs_loading)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), NULL);

	if (!anchor_file_should_get_top_left_text (file)) {
		if (needs_loading) {
			*needs_loading = FALSE;
		}
		return NULL;
	...
	if (!file->details->got_top_left_text) {
		...
		if (anchor_file_contains_text (file)) {
			return " ...";
		}
		return NULL;
	}
 ...
 **/
char *
anchor_file_get_top_left_text (AnchorFile *file)
{
	return g_strdup (anchor_file_peek_top_left_text (file, FALSE, NULL));
}

char *
anchor_file_get_filesystem_id (AnchorFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->filesystem_id));
}

AnchorFile *
anchor_file_get_trash_original_file (AnchorFile *file)
{
	GFile *location;
	AnchorFile *original_file;

	original_file = NULL;

	if (file->details->trash_orig_path != NULL) {
		location = g_file_new_for_path (file->details->trash_orig_path);
		original_file = anchor_file_get (location);
		g_object_unref (location);
	}
...
void
anchor_file_mark_gone (AnchorFile *file)
{
	AnchorDirectory *directory;
 ...
 **/
void
anchor_file_changed (AnchorFile *file)
{
	GList fake_list;
	...
	if (anchor_file_is_self_owned (file)) {
		...
		fake_list.next = NULL;
		fake_list.prev = NULL;
		anchor_directory_emit_change_signals
			(file->details->directory, &fake_list);
	}
}

/**
 * anchor_file_updated_deep_count_in_progress
 * 
 * Notify clients that a newer deep count is available for
 * the directory in question.
 */
void
anchor_file_updated_deep_count_in_progress (AnchorFile *file) {
	...
	for (node = link_files; node != NULL; node = node->next) {
		anchor_file_updated_deep_count_in_progress (NAUTILUS_FILE (node->data));
	}
	anchor_file_list_free (link_files);	
}

/**
 * anchor_file_emit_changed
 * 
 ...
 **/
void
anchor_file_emit_changed (AnchorFile *file)
{
	GList *link_files, *p;
	...
	for (p = link_files; p != NULL; p = p->next) {
		if (p->data != file) {
			anchor_file_changed (NAUTILUS_FILE (p->data));
		}
	}
	anchor_file_list_free (link_files);
}
 ...
 **/
gboolean
anchor_file_is_gone (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
/**
 * anchor_file_is_not_yet_confirmed
 * 
 ...
 **/
gboolean
anchor_file_is_not_yet_confirmed (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
 ...
 */
gboolean
anchor_file_check_if_ready (AnchorFile *file,
...
void
anchor_file_call_when_ready (AnchorFile *file,
			       AnchorFileAttributes file_attributes,
...
void
anchor_file_cancel_call_when_ready (AnchorFile *file,
				      AnchorFileCallback callback,
				      gpointer callback_data)
{
	g_return_if_fail (callback != NULL);
...
static void
invalidate_directory_count (AnchorFile *file)
{
	file->details->directory_count_is_up_to_date = FALSE;
}

static void
invalidate_deep_counts (AnchorFile *file)
{
	file->details->deep_counts_status = NAUTILUS_REQUEST_NOT_STARTED;
}

static void
invalidate_mime_list (AnchorFile *file)
{
	file->details->mime_list_is_up_to_date = FALSE;
}

static void
invalidate_top_left_text (AnchorFile *file)
{
	file->details->top_left_text_is_up_to_date = FALSE;
}

static void
invalidate_file_info (AnchorFile *file)
{
	file->details->file_info_is_up_to_date = FALSE;
}

static void
invalidate_link_info (AnchorFile *file)
{
	file->details->link_info_is_up_to_date = FALSE;
}

static void
invalidate_thumbnail (AnchorFile *file)
{
	file->details->thumbnail_is_up_to_date = FALSE;
}

static void
invalidate_mount (AnchorFile *file)
{
	file->details->mount_is_up_to_date = FALSE;
}

void
anchor_file_invalidate_extension_info_internal (AnchorFile *file)
{
	if (file->details->pending_info_providers)
		g_list_free_full (file->details->pending_info_providers, g_object_unref);

	file->details->pending_info_providers =
		anchor_module_get_extensions_for_type (ANCHOR_TYPE_INFO_PROVIDER);
}

void
anchor_file_invalidate_attributes_internal (AnchorFile *file,
					      AnchorFileAttributes file_attributes)
{
	Request request;
	...
	request = anchor_directory_set_up_request (file_attributes);
...
gboolean
anchor_file_has_open_window (AnchorFile *file)
{
	return file->details->has_open_window;
}

void
anchor_file_set_has_open_window (AnchorFile *file,
...
gboolean
anchor_file_is_thumbnailing (AnchorFile *file)
{
	g_return_val_if_fail (NAUTILUS_IS_FILE (file), FALSE);
...
void
anchor_file_set_is_thumbnailing (AnchorFile *file,
...
/**
 * anchor_file_invalidate_attributes
 * 
...
void
anchor_file_invalidate_attributes (AnchorFile *file,
				     AnchorFileAttributes file_attributes)
{
	/* Cancel possible in-progress loads of any of these attributes */
	anchor_directory_cancel_loading_file_attributes (file->details->directory,
							   file,
							   file_attributes);
	
	/* Actually invalidate the values */
	anchor_file_invalidate_attributes_internal (file, file_attributes);
...
AnchorFileAttributes 
anchor_file_get_all_attributes (void)
{
...
void
anchor_file_invalidate_all_attributes (AnchorFile *file)
{
	AnchorFileAttributes all_attributes;

	all_attributes = anchor_file_get_all_attributes ();
	anchor_file_invalidate_attributes (file, all_attributes);
}
 ...
 **/
void
anchor_file_dump (AnchorFile *file)
{
...
/**
 * anchor_file_list_ref
 *
 * Ref all the files in a list.
 * @list: GList of files.
 **/
GList *
anchor_file_list_ref (GList *list)
{
	g_list_foreach (list, (GFunc) anchor_file_ref, NULL);
	return list;
}

/**
 * anchor_file_list_unref
 *
 * Unref all the files in a list.
 * @list: GList of files.
 **/
void
anchor_file_list_unref (GList *list)
{
	g_list_foreach (list, (GFunc) anchor_file_unref, NULL);
}

/**
 * anchor_file_list_free
 *
 * Free a list of files after unrefing them.
 * @list: GList of files.
 **/
void
anchor_file_list_free (GList *list)
{
	anchor_file_list_unref (list);
	g_list_free (list);
}

/**
 * anchor_file_list_copy
 *
 * Copy the list of files, making a new ref of each,
 * @list: GList of files.
 **/
GList *
anchor_file_list_copy (GList *list)
{
	return g_list_copy (anchor_file_list_ref (list));
}

GList *
anchor_file_list_from_uris (GList *uri_list)
{
	GList *l, *file_list;
	const char *uri;
	GFile *file;
...
static gboolean
get_attributes_for_default_sort_type (AnchorFile *file,
				      gboolean *is_recent,
				      ...
				      gboolean *is_search)
{
	...
	*is_search = FALSE;
	...
	if (file && anchor_file_is_directory (file)) {
		is_recent_dir =
			anchor_file_is_in_recent (file);
		is_download_dir =
			anchor_file_is_user_special_directory (file, G_USER_DIRECTORY_DOWNLOAD);
		is_desktop_dir =
			anchor_file_is_user_special_directory (file, G_USER_DIRECTORY_DESKTOP);
		is_trash_dir =
			anchor_file_is_in_trash (file);
		is_search_dir =
			anchor_file_is_in_search (file);
...
AnchorFileSortType
anchor_file_get_default_sort_type (AnchorFile *file,
				     gboolean *reversed)
{
	AnchorFileSortType retval;
...
const gchar *
anchor_file_get_default_sort_attribute (AnchorFile *file,
					  gboolean *reversed)
{
	const gchar *retval;
	gboolean is_recent, is_download, is_trash, is_search, res;
...
/**
 * anchor_file_list_sort_by_display_name
 * 
 * Sort the list of files by file name.
 * @list: GList of files.
 **/
GList *
anchor_file_list_sort_by_display_name (GList *list)
{
...
file_list_ready_data_free (FileListReadyData *data)
{
	...
	if (l != NULL) {
		...
		anchor_file_list_free (data->file_list);
			  ...
			  gpointer callback_data)
{
	...
	data->file_list = anchor_file_list_copy (file_list);
...
static void
file_list_file_ready_callback (AnchorFile *file,
			       gpointer user_data)
{
	FileListReadyData *data;
...
void 
anchor_file_list_call_when_ready (GList *file_list,
				    AnchorFileAttributes attributes,
				    ...
				    gpointer callback_data)
{
	...
	if (handle) {
		*handle = (AnchorFileListHandle *) data;
	}
...
void
anchor_file_list_cancel_call_when_ready (AnchorFileListHandle *handle)
{
	GList *l;
	AnchorFile *file;
	FileListReadyData *data;
...
char *
anchor_extract_top_left_text (const char *text,
				gboolean large,
				int length)
{
        GString* buffer;
...
{
	g_settings_get (anchor_preferences,
...
{
	cached_thumbnail_size = g_settings_get_int (anchor_icon_view_preferences,
...
{
	show_file_thumbs = g_settings_get_enum (anchor_preferences, NAUTILUS_PREFERENCES_SHOW_FILE_THUMBNAILS);
...
static void
real_set_metadata (AnchorFile  *file,
		   const char    *key,
		   const char    *value)
{
	/* Dummy default impl */
...
static void
real_set_metadata_as_list (AnchorFile *file,
			   const char   *key,
			   char         **value)
{
	/* Dummy default impl */
...
static void
anchor_file_class_init (AnchorFileClass *class)
{
	GtkIconTheme *icon_theme;

	anchor_file_info_getter = anchor_file_get_internal;
	...
	G_OBJECT_CLASS (class)->constructor = anchor_file_constructor;
	...
	signals[CHANGED] =
		g_signal_new ("changed",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorFileClass, changed),
		              NULL, NULL,
	...
	signals[UPDATED_DEEP_COUNT_IN_PROGRESS] =
		g_signal_new ("updated-deep-count-in-progress",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AnchorFileClass, updated_deep_count_in_progress),
		              NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);

	g_type_class_add_private (class, sizeof (AnchorFileDetails));
	...
	g_signal_connect (anchor_signaller_get_current (),
			  "mime-data-changed",
...
static void
anchor_file_add_emblem (AnchorFile *file,
...
static void
anchor_file_add_string_attribute (AnchorFile *file,
...
static void
anchor_file_invalidate_extension_info (AnchorFile *file)
{
	anchor_file_invalidate_attributes (file, NAUTILUS_FILE_ATTRIBUTE_EXTENSION_INFO);
}

void
anchor_file_info_providers_done (AnchorFile *file)
{
	g_list_free_full (file->details->extension_emblems, g_free);
...
static void     
anchor_file_info_iface_init (AnchorFileInfoIface *iface)
{
	iface->is_gone = anchor_file_is_gone;
	iface->get_name = anchor_file_get_name;
	iface->get_file_type = anchor_file_get_file_type;
	iface->get_location = anchor_file_get_location;
	iface->get_uri = anchor_file_get_uri;
	iface->get_parent_location = anchor_file_get_parent_location;
	iface->get_parent_uri = anchor_file_get_parent_uri;
	iface->get_parent_info = anchor_file_get_parent;
	iface->get_mount = anchor_file_get_mount;
	iface->get_uri_scheme = anchor_file_get_uri_scheme;
	iface->get_activation_uri = anchor_file_get_activation_uri;
	iface->get_mime_type = anchor_file_get_mime_type;
	iface->is_mime_type = anchor_file_is_mime_type;
	iface->is_directory = anchor_file_is_directory;
	iface->can_write = anchor_file_can_write;
	iface->add_emblem = anchor_file_add_emblem;
	iface->get_string_attribute = anchor_file_get_string_attribute;
	iface->add_string_attribute = anchor_file_add_string_attribute;
	iface->invalidate_extension_info = anchor_file_invalidate_extension_info;
}

#if !defined (NAUTILUS_OMIT_SELF_CHECK)

void
anchor_self_check_file (void)
{
	AnchorFile *file_1;
	AnchorFile *file_2;
	GList *list;
        ...
        EEL_CHECK_INTEGER_RESULT (anchor_directory_number_outstanding (), 0);

	file_1 = anchor_file_get_by_uri ("file:///home/");
	...
	EEL_CHECK_INTEGER_RESULT (G_OBJECT (file_1->details->directory)->ref_count, 1);
        EEL_CHECK_INTEGER_RESULT (anchor_directory_number_outstanding (), 1);

	anchor_file_unref (file_1);

        EEL_CHECK_INTEGER_RESULT (anchor_directory_number_outstanding (), 0);
	
	file_1 = anchor_file_get_by_uri ("file:///etc");
	file_2 = anchor_file_get_by_uri ("file:///usr");
	...
	EEL_CHECK_STRING_RESULT (anchor_file_get_name (file_1), "home");

	EEL_CHECK_BOOLEAN_RESULT (anchor_file_get_by_uri ("file:///home/") == file_1, TRUE);
	anchor_file_unref (file_1);
	 ...
	 * in real life */
	file_1 = anchor_file_get_by_uri (":");
	EEL_CHECK_STRING_RESULT (anchor_file_get_name (file_1), ":");
	anchor_file_unref (file_1);

	file_1 = anchor_file_get_by_uri ("eazel:");
	EEL_CHECK_STRING_RESULT (anchor_file_get_name (file_1), "eazel:///");
	anchor_file_unref (file_1);
	...
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_2, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) < 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_2, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) > 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, TRUE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (anchor_file_compare_for_sort (file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, TRUE, TRUE) == 0, TRUE);

	anchor_file_unref (file_1);
	anchor_file_unref (file_2);
}
