/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-desktop-link.c: Class that handles the links on the desktop
...
struct AnchorDesktopLinkDetails {
	AnchorDesktopLinkType type;
        char *filename;
	char *display_name;
...
G_DEFINE_TYPE(AnchorDesktopLink, anchor_desktop_link, G_TYPE_OBJECT)

static void
create_icon_file (AnchorDesktopLink *link)
{
	link->details->icon_file = anchor_desktop_icon_file_new (link);
}

static void
anchor_desktop_link_changed (AnchorDesktopLink *link)
{
	if (link->details->icon_file != NULL) {
		anchor_desktop_icon_file_update (link->details->icon_file);
	}
}

static void
mount_changed_callback (GMount *mount, AnchorDesktopLink *link)
{
	...
	anchor_desktop_link_changed (link);
}
...
static void
trash_state_changed_callback (AnchorTrashMonitor *trash_monitor,
			      gboolean state,
...
home_name_changed (gpointer callback_data)
{
	...
	g_free (link->details->display_name);
	link->details->display_name = g_settings_get_string (anchor_desktop_preferences,
...
AnchorDesktopLink *
anchor_desktop_link_new (AnchorDesktopLinkType type)
{
	AnchorDesktopLink *link;

	link = NAUTILUS_DESKTOP_LINK (g_object_new (ANCHOR_TYPE_DESKTOP_LINK, NULL));
	...
	case NAUTILUS_DESKTOP_LINK_TRASH:
		...
		g_signal_connect_swapped (anchor_desktop_preferences,
					  ...
					  link);
		link->details->signal_handler_obj = G_OBJECT (anchor_trash_monitor_get ());
...
AnchorDesktopLink *
anchor_desktop_link_new_from_mount (GMount *mount)
{
	AnchorDesktopLink *link;
	GVolume *volume;
	char *name, *filename;
	...
	link->details->filename =
		anchor_desktop_link_monitor_make_filename_unique (anchor_desktop_link_monitor_get (),
								    filename);
	g_free (filename);
	g_free (name);
...
GMount *
anchor_desktop_link_get_mount (AnchorDesktopLink *link)
{
...
anchor_desktop_link_get_link_type (AnchorDesktopLink *link)
{
	return link->details->type;
}

char *
anchor_desktop_link_get_file_name (AnchorDesktopLink *link)
{
	return g_strdup (link->details->filename);
}

char *
anchor_desktop_link_get_display_name (AnchorDesktopLink *link)
{
	return g_strdup (link->details->display_name);
}

GIcon *
anchor_desktop_link_get_icon (AnchorDesktopLink *link)
{
...
GFile *
anchor_desktop_link_get_activation_location (AnchorDesktopLink *link)
{
	if (link->details->activation_location) {
...
char *
anchor_desktop_link_get_activation_uri (AnchorDesktopLink *link)
{
	if (link->details->activation_location) {
...
gboolean
anchor_desktop_link_get_date (AnchorDesktopLink *link,
...
gboolean
anchor_desktop_link_can_rename (AnchorDesktopLink     *link)
{
...
gboolean
anchor_desktop_link_rename (AnchorDesktopLink     *link,
			      const char              *name)
{
	switch (link->details->type) {
	case NAUTILUS_DESKTOP_LINK_HOME:
		g_settings_set_string (anchor_desktop_preferences,
				       NAUTILUS_PREFERENCES_DESKTOP_HOME_NAME,
				       name);
		break;
	case NAUTILUS_DESKTOP_LINK_TRASH:
...
static void
anchor_desktop_link_init (AnchorDesktopLink *link)
{
	link->details = G_TYPE_INSTANCE_GET_PRIVATE (link,
						     ANCHOR_TYPE_DESKTOP_LINK,
						     AnchorDesktopLinkDetails);
}
...
desktop_link_finalize (GObject *object)
{
	...
	if (link->details->icon_file != NULL) {
		anchor_desktop_icon_file_remove (link->details->icon_file);
		anchor_file_unref (NAUTILUS_FILE (link->details->icon_file));
	...
	G_OBJECT_CLASS (anchor_desktop_link_parent_class)->finalize (object);
}

static void
anchor_desktop_link_class_init (AnchorDesktopLinkClass *klass)
{
	...
	g_type_class_add_private (object_class, sizeof(AnchorDesktopLinkDetails));
}
