...
/* anchor-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "anchor-column-chooser.h"

#include <string.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

#include "anchor-column-utilities.h"
...
G_DEFINE_TYPE(AnchorColumnChooser, anchor_column_chooser, GTK_TYPE_BOX);

static void anchor_column_chooser_constructed (GObject *object);

static void
anchor_column_chooser_set_property (GObject *object,
				      guint param_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
	AnchorColumnChooser *chooser;
...
static void
anchor_column_chooser_class_init (AnchorColumnChooserClass *chooser_class)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (chooser_class);

	oclass->set_property = anchor_column_chooser_set_property;
	oclass->constructed = anchor_column_chooser_constructed;

	signals[CHANGED] = g_signal_new
		("changed",
		 G_TYPE_FROM_CLASS (chooser_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AnchorColumnChooserClass,
				  changed),
		 NULL, NULL,
	...
	g_type_class_add_private (chooser_class, sizeof (AnchorColumnChooserDetails));
}

static void 
update_buttons (AnchorColumnChooser *chooser)
{
...
static void
list_changed (AnchorColumnChooser *chooser) 
{
...
static void
toggle_path (AnchorColumnChooser *chooser,
             GtkTreePath *path)
{
	GtkTreeIter iter;
	gboolean visible;
...
static void
add_tree_view (AnchorColumnChooser *chooser)
{
	GtkWidget *scrolled;
	GtkWidget *view;
	GtkListStore *store;
...
static void
add_buttons (AnchorColumnChooser *chooser)
{
	GtkWidget *box;
	GtkWidget *separator;
...
static void
populate_tree (AnchorColumnChooser *chooser)
{
	GList *columns;
	GList *l;

	columns = anchor_get_columns_for_file (chooser->details->file);
...
static void
anchor_column_chooser_constructed (GObject *object)
{
	AnchorColumnChooser *chooser;
...
static void
anchor_column_chooser_init (AnchorColumnChooser *chooser)
{	
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), ANCHOR_TYPE_COLUMN_CHOOSER, AnchorColumnChooserDetails);
...
static void 
set_visible_columns (AnchorColumnChooser *chooser,
...
static char **
get_column_names (AnchorColumnChooser *chooser, gboolean only_visible)
{
	GPtrArray *ret;
	GtkTreeIter iter;
...
static gboolean
get_column_iter (AnchorColumnChooser *chooser, 
		 AnchorColumn *column,
		 GtkTreeIter *iter)
{
	char *column_name;
...
static void
set_column_order (AnchorColumnChooser *chooser,
		  char **column_order)

{
	...
	columns = anchor_get_columns_for_file (chooser->details->file);	
	columns = anchor_sort_columns (columns, column_order);
...
void
anchor_column_chooser_set_settings (AnchorColumnChooser *chooser,
				      char **visible_columns,
				      char **column_order)
{
...
void
anchor_column_chooser_get_settings (AnchorColumnChooser *chooser,
				      char ***visible_columns,
				      char ***column_order)
{
	g_return_if_fail (NAUTILUS_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
anchor_column_chooser_new (AnchorFile *file)
{
	return g_object_new (ANCHOR_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
