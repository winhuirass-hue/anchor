...
/* anchor-tree-view-drag-dest.c: Handles drag and drop for treeviews which 
 *                                 contain a hierarchy of files
 */

#include <config.h>

#include "anchor-tree-view-drag-dest.h"
...
G_DEFINE_TYPE (AnchorTreeViewDragDest, anchor_tree_view_drag_dest,
	       G_TYPE_OBJECT);
...
static void
remove_scroll_timeout (AnchorTreeViewDragDest *dest)
{
...
static void
remove_expand_timer (AnchorTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (AnchorTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (AnchorTreeViewDragDest *dest,
		   GtkTreePath *path)
{
	if (path) {
...
static void
clear_drag_dest_row (AnchorTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (AnchorTreeViewDragDest *dest,
...
static void
remove_hover_timer (AnchorTreeViewDragDest *dest)
{
...
static void
free_drag_data (AnchorTreeViewDragDest *dest)
{
	...
	if (dest->details->drag_list) {
		anchor_drag_destroy_selection_list (dest->details->drag_list);
...
{
	AnchorTreeViewDragDest *dest = user_data;
...
static void
check_hover_timer (AnchorTreeViewDragDest *dest,
		   const char               *uri)
{
...
static void
check_expand_timer (AnchorTreeViewDragDest *dest,
		    GtkTreePath *drop_path,
...
static char *
get_root_uri (AnchorTreeViewDragDest *dest)
{
	char *uri;
...
static AnchorFile *
file_for_path (AnchorTreeViewDragDest *dest, GtkTreePath *path)
{
	AnchorFile *file;
	char *uri;
...
static char *
get_drop_target_uri_for_path (AnchorTreeViewDragDest *dest,
			      GtkTreePath *path)
{
	AnchorFile *file;
	char *target = NULL;
	...
	if (can) {
		target = anchor_file_get_drop_target_uri (file);
	}
...
static void
check_hover_expand_timer (AnchorTreeViewDragDest *dest,
			  GtkTreePath *path,
			  GtkTreePath *drop_path,
			  GtkTreePath *old_drop_path)
{
	gboolean use_tree = g_settings_get_boolean (anchor_list_view_preferences,
...
static GtkTreePath *
get_drop_path (AnchorTreeViewDragDest *dest,
	       GtkTreePath *path)
{
	AnchorFile *file;
	GtkTreePath *ret;
	...
	while (file == NULL /* dummy row */ ||
	       !anchor_drag_can_accept_info (file,
		       			       dest->details->drag_type,
					       dest->details->drag_list)) {
...
static guint
get_drop_action (AnchorTreeViewDragDest *dest, 
		 GdkDragContext *context,
		 GtkTreePath *path)
{
	char *drop_target;
	...
	case NAUTILUS_ICON_DND_GNOME_ICON_LIST :
		anchor_drag_default_drop_action_for_icons
			(context,
			 drop_target,
			 dest->details->drag_list,
			 &action);
...
static char *
get_drop_target_uri_at_pos (AnchorTreeViewDragDest *dest, int x, int y)
{
...
static void
receive_uris (AnchorTreeViewDragDest *dest,
	      GdkDragContext *context,
	      GList *source_uris,
	      int x, int y)
{
	...
	if (real_action == GDK_ACTION_ASK) {
		if (anchor_drag_selection_includes_special_link (dest->details->drag_list)) {
	...
	if (real_action > 0) {
		if (!anchor_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (AnchorTreeViewDragDest *dest,
		       GdkDragContext *context,
		       int x, int y)
{
	GList *source_uris;
	...
	for (l = dest->details->drag_list; l != NULL; l = l->next) {
		source_uris = g_list_prepend (source_uris,
					      ((AnchorDragSelectionItem *)l->data)->uri);
	}
...
static void
receive_dropped_uri_list (AnchorTreeViewDragDest *dest,
			  GdkDragContext *context,
			  int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_text (AnchorTreeViewDragDest *dest,
		      GdkDragContext *context,
		      int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_raw (AnchorTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (AnchorTreeViewDragDest *dest,
			      GdkDragContext *context,
			      int x, int y)
{
	char *drop_target;
...
static gboolean
receive_xds (AnchorTreeViewDragDest *dest,
	     GtkWidget *widget,
	     guint32 time,
	     GdkDragContext *context,
	     int x, int y)
			     ...
			     gpointer data)
{
	...
	if (!dest->details->have_drag_data) {
		...
		if (info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
			dest->details->drag_list = 
				anchor_drag_build_selection_list (selection_data);
		}
	}
...
static gboolean
set_direct_save_uri (AnchorTreeViewDragDest *dest,
		     GdkDragContext *context,
		     int x, int y)
{
	GFile *base, *child;
...
static void
anchor_tree_view_drag_dest_dispose (GObject *object)
{
	AnchorTreeViewDragDest *dest;
	...
	G_OBJECT_CLASS (anchor_tree_view_drag_dest_parent_class)->dispose (object);
}

static void
anchor_tree_view_drag_dest_finalize (GObject *object)
{
	AnchorTreeViewDragDest *dest;
	...
	G_OBJECT_CLASS (anchor_tree_view_drag_dest_parent_class)->finalize (object);
}

static void
anchor_tree_view_drag_dest_init (AnchorTreeViewDragDest *dest)
{
	dest->details = G_TYPE_INSTANCE_GET_PRIVATE (dest, ANCHOR_TYPE_TREE_VIEW_DRAG_DEST,
						     AnchorTreeViewDragDestDetails);
}

static void
anchor_tree_view_drag_dest_class_init (AnchorTreeViewDragDestClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	
	gobject_class->dispose = anchor_tree_view_drag_dest_dispose;
	gobject_class->finalize = anchor_tree_view_drag_dest_finalize;

	g_type_class_add_private (class, sizeof (AnchorTreeViewDragDestDetails));

	signals[GET_ROOT_URI] = 
		g_signal_new ("get-root-uri",
			      G_TYPE_FROM_CLASS (class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AnchorTreeViewDragDestClass,
					       get_root_uri),
			      NULL, NULL,
...
AnchorTreeViewDragDest *
anchor_tree_view_drag_dest_new (GtkTreeView *tree_view)
{
	AnchorTreeViewDragDest *dest;
	GtkTargetList *targets;
	
	dest = g_object_new (ANCHOR_TYPE_TREE_VIEW_DRAG_DEST, NULL);
