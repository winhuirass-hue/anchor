/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   anchor-desktop-directory-file.c: Subclass of AnchorFile to help implement the
   virtual desktop.
...
struct AnchorDesktopDirectoryFileDetails {
	AnchorDesktopDirectory *desktop_directory;

  	AnchorFile *real_dir_file;
...
typedef struct {
	AnchorDesktopDirectoryFile *desktop_file;
	AnchorFileCallback callback;
	gpointer callback_data;

	AnchorFileAttributes delegated_attributes;
	AnchorFileAttributes non_delegated_attributes;
...
G_DEFINE_TYPE (AnchorDesktopDirectoryFile, anchor_desktop_directory_file,
	       ANCHOR_TYPE_FILE);
...
static void
real_file_changed_callback (AnchorFile *real_file,
			    gpointer callback_data)
{
	AnchorDesktopDirectoryFile *desktop_file;
	
	desktop_file = NAUTILUS_DESKTOP_DIRECTORY_FILE (callback_data);
	anchor_file_changed (NAUTILUS_FILE (desktop_file));
}

static AnchorFileAttributes 
...
static void
partition_attributes (AnchorFileAttributes attributes,
		      AnchorFileAttributes *delegated_attributes,
		      AnchorFileAttributes *non_delegated_attributes)
{
	AnchorFileAttributes mask;
...
static void
desktop_directory_file_monitor_add (AnchorFile *file,
				    ...
				    AnchorFileAttributes attributes)
{
	...
	anchor_file_monitor_add (desktop_file->details->real_dir_file,
...
static void
desktop_directory_file_monitor_remove (AnchorFile *file,
				       gconstpointer client)
{
	AnchorDesktopDirectoryFile *desktop_file;
	DesktopMonitor *monitor;
...
desktop_callback_destroy (DesktopCallback *desktop_callback)
{
	...
	anchor_file_unref (NAUTILUS_FILE (desktop_callback->desktop_file));
...
desktop_callback_check_done (DesktopCallback *desktop_callback)
{
	...
	anchor_desktop_update_metadata_from_keyfile (NAUTILUS_FILE (desktop_callback->desktop_file), "directory");
...
static void
ready_callback (AnchorFile *file,
		gpointer callback_data)
{
...
static void
desktop_directory_file_call_when_ready (AnchorFile *file,
					AnchorFileAttributes attributes,
					AnchorFileCallback callback,
					gpointer callback_data)
...
static void
desktop_directory_file_cancel_call_when_ready (AnchorFile *file,
					       AnchorFileCallback callback,
					       gpointer callback_data)
{
	AnchorDesktopDirectoryFile *desktop_file;
...
static gboolean
real_check_if_ready (AnchorFile *file,
		     AnchorFileAttributes attributes)
{
...
static gboolean
desktop_directory_file_check_if_ready (AnchorFile *file,
				       AnchorFileAttributes attributes)
{
	AnchorFileAttributes delegated_attributes, non_delegated_attributes;
	AnchorDesktopDirectoryFile *desktop_file;
	...
	return real_check_if_ready (file, non_delegated_attributes) &&
		anchor_file_check_if_ready (desktop_file->details->real_dir_file,
					      delegated_attributes);
}

static gboolean
desktop_directory_file_get_item_count (AnchorFile *file, 
				       guint *count,
				       gboolean *count_unreadable)
{
	...
	got_count = anchor_file_get_directory_item_count (desktop_file->details->real_dir_file,
							    count,
							    count_unreadable);
...
desktop_directory_file_get_deep_counts (AnchorFile *file,
					...
					goffset *total_size)
{
	...
	status = anchor_file_get_deep_counts (desktop_file->details->real_dir_file,
						directory_count,
						file_count,
...
static gboolean
desktop_directory_file_get_date (AnchorFile *file,
				 ...
				 time_t *date)
{
	...
	return anchor_file_get_date (desktop_file->details->real_dir_file,
				       date_type,
				       date);
}

static char *
desktop_directory_file_get_where_string (AnchorFile *file)
{
...
monitor_destroy (gpointer data)
{
	...
	anchor_file_monitor_remove
		(NAUTILUS_FILE (monitor->desktop_file->details->real_dir_file), monitor);
	g_free (monitor);
}

static void
anchor_desktop_directory_file_set_metadata (AnchorFile           *file,
					      const char             *key,
					      const char             *value)
{
	anchor_desktop_set_metadata_string (file, "directory", key, value);
}

static void
anchor_desktop_directory_file_set_metadata_as_list (AnchorFile           *file,
						      const char             *key,
						      char                  **value)
{
	anchor_desktop_set_metadata_stringv (file, "directory", key, (const gchar **) value);
}

static void
anchor_desktop_directory_file_init (AnchorDesktopDirectoryFile *desktop_file)
{
	AnchorDesktopDirectory *desktop_directory;
	AnchorDirectory *real_dir;
	AnchorFile *real_dir_file;

	desktop_file->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_file,
							     ANCHOR_TYPE_DESKTOP_DIRECTORY_FILE,
							     AnchorDesktopDirectoryFileDetails);

	desktop_directory = NAUTILUS_DESKTOP_DIRECTORY (anchor_directory_get_by_uri (EEL_DESKTOP_URI));
	...
	real_dir = anchor_desktop_directory_get_real_directory (desktop_directory);
	real_dir_file = anchor_directory_get_corresponding_file (real_dir);
	anchor_directory_unref (real_dir);
...
desktop_finalize (GObject *object)
{
	...
	anchor_file_unref (desktop_file->details->real_dir_file);
	anchor_directory_unref (NAUTILUS_DIRECTORY (desktop_directory));

	G_OBJECT_CLASS (anchor_desktop_directory_file_parent_class)->finalize (object);
}

static void
anchor_desktop_directory_file_class_init (AnchorDesktopDirectoryFileClass *klass)
{
	...
	file_class->set_metadata = anchor_desktop_directory_file_set_metadata;
	file_class->set_metadata_as_list = anchor_desktop_directory_file_set_metadata_as_list;

	g_type_class_add_private (klass, sizeof (AnchorDesktopDirectoryFileDetails));
}
