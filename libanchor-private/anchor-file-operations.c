...
/* anchor-file-operations.c - Anchor file operations.
...
#include "anchor-file-changes-queue.h"
...
#include "anchor-file-conflict-dialog.h"
...
typedef struct {
	...
	gchar *target_name;
	AnchorCopyCallback  done_callback;
...
typedef struct {
	...
	AnchorDeleteCallback done_callback;
	gpointer done_callback_data;
} DeleteJob;
...
typedef struct {
	...
	GFile *created_file;
	AnchorCreateCallback done_callback;
	gpointer done_callback_data;
} CreateJob;
...
typedef struct {
	...
	AnchorOpCallback done_callback;
	gpointer done_callback_data;
} EmptyTrashJob;
	     ...
	     GtkWindow *parent_window)
{
	...
	common->progress = anchor_progress_info_new ();
	common->cancellable = anchor_progress_info_get_cancellable (common->progress);
...
{
	anchor_progress_info_finish (common->progress);
	...
	if (common->undo_info != NULL) {
		anchor_file_undo_manager_set_action (common->undo_info);
...
can_delete_without_confirm (GFile *file)
{
	if (g_file_has_uri_scheme (file, "burn") ||
	    ...
	    g_file_has_uri_scheme (file, "x-anchor-desktop")) {
		return TRUE;
	}
		      ...
		      va_list varargs)
{
	...
	anchor_progress_info_pause (job->progress);
	g_io_scheduler_job_send_to_mainloop (job->io_job,
					     do_run_simple_dialog,
					     data,
					     NULL);
	anchor_progress_info_resume (job->progress);
	res = data->result;
...
should_confirm_trash (void)
{
	...
	prefs = g_settings_new ("org.gnome.anchor.preferences");
			...
			TransferInfo *transfer_info)
{
	...
	anchor_progress_info_take_status (job->progress,
					    f (_("Deleting files")));

	elapsed = g_timer_elapsed (job->time, NULL);
	if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE) {

		anchor_progress_info_set_details (job->progress, files_left_s);
	} else {
		...
		anchor_progress_info_take_details (job->progress, details);
	...
	if (source_info->num_files != 0) {
		anchor_progress_info_set_progress (job->progress, transfer_info->num_files, source_info->num_files);
	}
}
 ...
 retry:
	...
	if (!job_aborted (job) &&
		...
		skip:
			...
			anchor_file_changes_queue_file_removed (dir);
	     ...
	     gboolean toplevel)
{
	...
	if (g_file_delete (file, job->cancellable, &error)) {
		anchor_file_changes_queue_file_removed (file);
		transfer_info->num_files ++;
		report_delete_progress (job, source_info, transfer_info);
		return;
	}
		       ...
		       int total_files)
{
	...
	s = f (ngettext ("%'d file left to trash",
			 ...
			 files_left),
	       files_left);
	anchor_progress_info_take_details (job->progress, s);

	if (total_files != 0) {
		anchor_progress_info_set_progress (job->progress, files_trashed, total_files);
	}
}
	...
	for (l = files;
		...
		skip:
			...
			if (job->undo_info != NULL) {
				anchor_file_undo_info_trash_add_file (NAUTILUS_FILE_UNDO_INFO_TRASH (job->undo_info), file);
			}
	    ...
	    gpointer user_data)
{
	...
	anchor_progress_info_start (job->common.progress);
			  ...
			  gpointer                done_callback_data)
{
	...
	if (try_trash && !anchor_file_undo_manager_pop_flag ()) {
		job->common.undo_info = anchor_file_undo_info_trash_new (g_list_length (files));
	}
...
void
anchor_file_operations_trash_or_delete (GList                  *files,
					  GtkWindow              *parent_window,
					  AnchorDeleteCallback  done_callback,
					  gpointer                done_callback_data)
{
...
void
anchor_file_operations_delete (GList                  *files, 
				 GtkWindow              *parent_window,
				 AnchorDeleteCallback  done_callback,
				 gpointer                done_callback_data)
{
...
typedef struct {
	...
	GtkWindow *parent_window;
	AnchorUnmountCallback callback;
...
void
anchor_file_operations_unmount_mount_full (GtkWindow                      *parent_window,
					     GMount                         *mount,
					     GMountOperation                *mount_operation,
					     gboolean                        eject,
...
void
anchor_file_operations_unmount_mount (GtkWindow                      *parent_window,
					GMount                         *mount,
					gboolean                        eject,
					gboolean                        check_trash)
{
	anchor_file_operations_unmount_mount_full (parent_window, mount, NULL, eject,
...
volume_mount_cb (GObject *source_object,
		 ...
		 gpointer user_data)
{
	AnchorMountCallback mount_callback;
	...
	mount_callback = (AnchorMountCallback)
...
void
anchor_file_operations_mount_volume (GtkWindow *parent_window,
				       GVolume *volume)
{
	anchor_file_operations_mount_volume_full (parent_window, volume,
						    NULL, NULL);
}

void
anchor_file_operations_mount_volume_full (GtkWindow *parent_window,
					    GVolume *volume,
					    AnchorMountCallback mount_callback,
					    GObject *mount_callback_data_object)
{
		       ...
		       SourceInfo *source_info)
{
	...
	anchor_progress_info_take_details (job->progress, s);
	anchor_progress_info_pulse_progress (job->progress);
}
		      ...
		      TransferInfo *transfer_info)
{
	...
	anchor_progress_info_set_progress (job->progress, transfer_info->num_bytes, total_size);
}
		 ...
		 char **dest_fs_type)
{
 ...
 retry:
	...
	anchor_file_changes_queue_file_added (*dest);

	if (job->undo_info != NULL) {
		anchor_file_undo_info_ext_add_origin_target_pair (NAUTILUS_FILE_UNDO_INFO_EXT (job->undo_info),
								    src, *dest);
	}
...
do_run_conflict_dialog (gpointer _data)
{
	...
	dialog = anchor_file_conflict_dialog_new (data->parent,
						    data->src,
						    data->dest,
						    data->dest_dir);
	...
	if (response == CONFLICT_RESPONSE_RENAME) {
		data->resp_data->new_name = 
			anchor_file_conflict_dialog_get_new_name (NAUTILUS_FILE_CONFLICT_DIALOG (dialog));
	} else if (response != GTK_RESPONSE_CANCEL ||
		   response != GTK_RESPONSE_NONE) {
		   data->resp_data->apply_to_all =
			   anchor_file_conflict_dialog_get_apply_to_all 
		...
		gboolean readonly_source_fs)
{
 ...
 retry:
	...
	if (res) {
		...
		if (debuting_files) {
			if (position) {
				anchor_file_changes_queue_schedule_position_set (dest, *position, job->screen_num);
			} else {
				anchor_file_changes_queue_schedule_position_remove (dest);
			}
			
			g_hash_table_replace (debuting_files, g_object_ref (dest), GINT_TO_POINTER (TRUE));
		}
		if (copy_job->is_move) {
			anchor_file_changes_queue_file_moved (src, dest);
		} else {
			anchor_file_changes_queue_file_added (dest);
		}
	...
	/* Needs to recurse */
		...
		if (overwrite && would_recurse) {
			...
			anchor_file_changes_queue_file_removed (dest);
		}
...
void
anchor_file_operations_copy_file (GFile *source_file,
				    GFile *target_dir,
...
void
anchor_file_operations_copy (GList *files,
			       ...
			       gpointer done_callback_data)
{
	...
	job = op_job_new (CopyMoveJob, parent_window);
	job->desktop_location = anchor_get_desktop_location ();
	job->done_callback = done_callback;
	job->done_callback_data = done_callback_data;
	job->files = g_list_copy_deep (files, (GCopyFunc) g_object_ref, NULL);
	job->destination = g_object_ref (target_dir);
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		job->common.undo_info = anchor_file_undo_info_ext_new (NAUTILUS_FILE_UNDO_OP_COPY,
									 g_list_length (files),
									 src_dir, target_dir);
...
report_move_progress (CopyMoveJob *move_job, int total, int left)
{
	...
	anchor_progress_info_take_details (job->progress,
...
void
anchor_file_operations_move (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       AnchorCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		if (g_file_has_uri_scheme (g_list_first (files)->data, "trash")) {
			job->common.undo_info = anchor_file_undo_info_ext_new (NAUTILUS_FILE_UNDO_OP_RESTORE_FROM_TRASH,
										 g_list_length (files),
										 src_dir, target_dir);
		} else {
			job->common.undo_info = anchor_file_undo_info_ext_new (NAUTILUS_FILE_UNDO_OP_MOVE,
										 g_list_length (files),
										 src_dir, target_dir);
		}
...
report_link_progress (CopyMoveJob *link_job, int total, int left)
{
	...
	anchor_progress_info_set_progress (job->progress, left, total);
}
	   ...
	   int files_left)
{
 ...
 retry:
	...
	if (path == NULL) {
		...
		if (common->undo_info != NULL) {
			anchor_file_undo_info_ext_add_origin_target_pair (NAUTILUS_FILE_UNDO_INFO_EXT (common->undo_info),
									    src, dest);
		}
		...
		anchor_file_changes_queue_file_added (dest);
		if (position) {
			anchor_file_changes_queue_schedule_position_set (dest, *position, common->screen_num);
		} else {
			anchor_file_changes_queue_schedule_position_remove (dest);
		}
...
void
anchor_file_operations_link (GList *files,
			       GArray *relative_item_points,
			       GFile *target_dir,
			       GtkWindow *parent_window,
			       AnchorCopyCallback  done_callback,
			       gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		src_dir = g_file_get_parent (files->data);
		job->common.undo_info = anchor_file_undo_info_ext_new (NAUTILUS_FILE_UNDO_OP_CREATE_LINK,
									 g_list_length (files),
									 src_dir, target_dir);
		g_object_unref (src_dir);
	}
...
void
anchor_file_operations_duplicate (GList *files,
				    GArray *relative_item_points,
				    GtkWindow *parent_window,
				    AnchorCopyCallback  done_callback,
				    gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		...
		job->common.undo_info =
			anchor_file_undo_info_ext_new (NAUTILUS_FILE_UNDO_OP_DUPLICATE,
							 g_list_length (files),
							 src_dir, src_dir);
		g_object_unref (src_dir);
	}
		      ...
		      GFileInfo *info)
{
	...
	anchor_progress_info_pulse_progress (common->progress);
	...
	if (!job_aborted (common) &&
		...
		if (common->undo_info != NULL) {
			anchor_file_undo_info_rec_permissions_add_file (NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS (common->undo_info),
									  file, current);
		}
		     ...
		     gpointer user_data)
{
	...
	anchor_progress_info_set_status (common->progress,
...
void
anchor_file_set_permissions_recursive (const char *directory,
					 guint32         file_permissions,
					 guint32         file_mask,
					 guint32         dir_permissions,
					 guint32         dir_mask,
					 AnchorOpCallback  callback,
					 gpointer  callback_data)
{
	SetPermissionsJob *job;
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		job->common.undo_info = 
			anchor_file_undo_info_rec_permissions_new (job->file,
...
typedef struct {
	AnchorCopyCallback real_callback;
	gpointer real_data;
} MoveTrashCBData;
...
void
anchor_file_operations_copy_move (const GList *item_uris,
				    GArray *relative_item_points,
				    ...
				    gpointer done_callback_data)
{
	...
	if (copy_action == GDK_ACTION_COPY) {
		...
		if (target_dir == NULL ||
			...
			anchor_file_operations_duplicate (locations,
							    relative_item_points,
							    parent_window,
							    done_callback, done_callback_data);
		} else {
			anchor_file_operations_copy (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		if (g_file_has_uri_scheme (dest, "trash")) {
			...
			anchor_file_operations_trash_or_delete (locations,
								  parent_window,
								  (AnchorDeleteCallback) callback_for_move_to_trash,
								  cb_data);
		} else {

			anchor_file_operations_move (locations,
						       relative_item_points,
						       dest,
						       parent_window,
						       done_callback, done_callback_data);
		...
		anchor_file_operations_link (locations,
					       relative_item_points,
					       dest,
					       parent_window,
					       done_callback, done_callback_data);
 ...
 retry:
	...
	if (job->make_dir) {
		...
		if (res && common->undo_info != NULL) {
			anchor_file_undo_info_create_set_data (NAUTILUS_FILE_UNDO_INFO_CREATE (common->undo_info),
								 dest, NULL, 0);
		}
	...
	if (res) {
		...
		anchor_file_changes_queue_file_added (dest);
		if (job->has_position) {
			anchor_file_changes_queue_schedule_position_set (dest, job->position, common->screen_num);
		} else {
			anchor_file_changes_queue_schedule_position_remove (dest);
		}
	} else {
...
void 
anchor_file_operations_new_folder (GtkWidget *parent_view, 
				     ...
				     gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		job->common.undo_info = anchor_file_undo_info_create_new (NAUTILUS_FILE_UNDO_OP_CREATE_FOLDER);
	}
...
void 
anchor_file_operations_new_file_from_template (GtkWidget *parent_view, 
						 GdkPoint *target_point,
						 const char *parent_dir,
						 ...
						 gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		job->common.undo_info = anchor_file_undo_info_create_new (NAUTILUS_FILE_UNDO_OP_CREATE_FILE_FROM_TEMPLATE);
	}
...
void 
anchor_file_operations_new_file (GtkWidget *parent_view, 
				   GdkPoint *target_point,
				   const char *parent_dir,
				   const char *target_filename,
				   ...
				   gpointer done_callback_data)
{
	...
	if (!anchor_file_undo_manager_pop_flag ()) {
		job->common.undo_info = anchor_file_undo_info_create_new (NAUTILUS_FILE_UNDO_OP_CREATE_EMPTY_FILE);
	}
...
void 
anchor_file_operations_empty_trash (GtkWidget *parent_view)
{
	EmptyTrashJob *job;
	GtkWindow *parent_window;
...
void
anchor_file_mark_desktop_file_trusted (GFile *file,
					 GtkWindow *parent_window,
					 gboolean interactive,
					 AnchorOpCallback done_callback,
					 gpointer done_callback_data)
...
void
anchor_self_check_file_operations (void)
{
