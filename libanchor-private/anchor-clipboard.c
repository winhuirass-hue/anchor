...
/* anchor-clipboard.c
 *
 * Anchor Clipboard support.  For now, routines to support component cut
 * and paste.
 *
			    ...
			    gpointer callback_data)
{
	...
	target_data = g_object_get_data (callback_data, "Anchor:clipboard_target_data");
...
clipboard_items_are_merged_in (GtkWidget *widget)
{
	return GPOINTER_TO_INT (g_object_get_data (G_OBJECT (widget),
						   "Anchor:clipboard_menu_items_merged"));
}
...
static void
anchor_clipboard_real_set_up (gpointer target,
				GtkUIManager *ui_manager,
				gboolean shares_selection_changes,
				SelectAllCallback select_all_callback,
				ConnectCallbacksFunc connect_callbacks,
				ConnectCallbacksFunc disconnect_callbacks)
{
	...
	if (g_object_get_data (G_OBJECT (target), "Anchor:clipboard_target_data") != NULL) {
		return;
	}
	...
	g_object_set_data_full (G_OBJECT (target), "Anchor:clipboard_target_data",
...
void
anchor_clipboard_set_up_editable (GtkEditable *target,
				    GtkUIManager *ui_manager,
				    gboolean shares_selection_changes)
{
	...
	anchor_clipboard_real_set_up (target, ui_manager,
					shares_selection_changes,
...
GList*
anchor_clipboard_get_uri_list_from_selection_data (GtkSelectionData *selection_data,
						     gboolean *cut,
...
GtkClipboard *
anchor_clipboard_get (GtkWidget *widget)
{
...
void
anchor_clipboard_clear_if_colliding_uris (GtkWidget *widget,
					    ...
					    GdkAtom copied_files_atom)
{
	...
	collision = FALSE;
	data = gtk_clipboard_wait_for_contents (anchor_clipboard_get (widget),
						copied_files_atom);
	if (data == NULL) {
		return;
	}

	clipboard_item_uris = anchor_clipboard_get_uri_list_from_selection_data (data, NULL,
										   copied_files_atom);
	...
	if (collision) {
		gtk_clipboard_clear (anchor_clipboard_get (widget));
	}
