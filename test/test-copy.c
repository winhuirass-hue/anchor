...
static void
changed_cb (AnchorProgressInfo *info,
	    gpointer data)
{
	g_print ("Changed: %s -- %s\n",
		 anchor_progress_info_get_status (info),
		 anchor_progress_info_get_details (info));
}

static void
progress_changed_cb (AnchorProgressInfo *info,
		     gpointer data)
{
	g_print ("Progress changed: %f\n",
		 anchor_progress_info_get_progress (info));
}

static void
finished_cb (AnchorProgressInfo *info,
	     gpointer data)
{
	g_print ("Finished\n");
	gtk_main_quit ();
...
main (int argc, char* argv[])
{
	...
	AnchorProgressInfo *progress_info;
	...
	anchor_file_operations_copy (sources,
				       NULL /* GArray *relative_item_points */,
				       dest,
				       GTK_WINDOW (window),
				       copy_done, NULL);
        
	infos = anchor_progress_info_manager_get_all_infos (manager);
