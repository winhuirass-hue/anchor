...
static void
files_added (AnchorDirectory *directory,
	     GList *added_files)
{
#if 0
	GList *list;
	...
	for (list = added_files; list != NULL; list = list->next) {
		AnchorFile *file = list->data;

		g_print (" - %s\n", anchor_file_get_uri (file));
	}
#endif
...
static void
files_changed (AnchorDirectory *directory,
	       GList *changed_files)
{
#if 0
	GList *list;
...
static void
done_loading (AnchorDirectory *directory)
{
...
main (int argc, char **argv)
{
	...
	anchor_directory_file_monitor_add (directory, client1, TRUE,
                                             attributes,
					     NULL, NULL);
