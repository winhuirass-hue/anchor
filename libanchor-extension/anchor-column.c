/*
 *  anchor-column.c - Info columns exported by AnchorColumnProvider
 *                      objects.
 *
...
G_DEFINE_TYPE (AnchorColumn, anchor_column, G_TYPE_OBJECT);
 ...
 *
 * Returns: a newly created #AnchorColumn
 */
AnchorColumn *
anchor_column_new (const char *name,
		     ...
		     const char *description)
{
	...
	column = g_object_new (ANCHOR_TYPE_COLUMN, 
			       "name", name,
			       "attribute", attribute,
			       "label", label,
...
static void
anchor_column_get_property (GObject *object,
			      guint param_id,
			      GValue *value,
			      GParamSpec *pspec)
{
...
static void
anchor_column_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
anchor_column_finalize (GObject *object)
{
	AnchorColumn *column;
	...
	G_OBJECT_CLASS (anchor_column_parent_class)->finalize (object);
}

static void
anchor_column_init (AnchorColumn *column)
{
	column->details = g_new0 (AnchorColumnDetails, 1);
	column->details->xalign = 0.0;
}

static void
anchor_column_class_init (AnchorColumnClass *class)
{
	G_OBJECT_CLASS (class)->finalize = anchor_column_finalize;
	G_OBJECT_CLASS (class)->get_property = anchor_column_get_property;
	G_OBJECT_CLASS (class)->set_property = anchor_column_set_property;
