 ...
 *
 * Creates a new #AnchorPropertyPage from page_widget.
 *
 * Returns: a newly created #AnchorPropertyPage
 */
AnchorPropertyPage *
anchor_property_page_new (const char *name,
			    GtkWidget *label,
			    GtkWidget *page_widget)
{
	...
	page = g_object_new (ANCHOR_TYPE_PROPERTY_PAGE,
			     "name", name,
			     "label", label,
			     "page", page_widget,
			     NULL);
...
static void
anchor_property_page_get_property (GObject *object,
				     guint param_id,
				     GValue *value,
				     GParamSpec *pspec)
{
...
static void
anchor_property_page_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
anchor_property_page_dispose (GObject *object)
{
	AnchorPropertyPage *page;
...
static void
anchor_property_page_finalize (GObject *object)
{
	AnchorPropertyPage *page;
...
static void
anchor_property_page_instance_init (AnchorPropertyPage *page)
{
	page->details = g_new0 (AnchorPropertyPageDetails, 1);
}

static void
anchor_property_page_class_init (AnchorPropertyPageClass *class)
{
	parent_class = g_type_class_peek_parent (class);
	
	G_OBJECT_CLASS (class)->finalize = anchor_property_page_finalize;
	G_OBJECT_CLASS (class)->dispose = anchor_property_page_dispose;
	G_OBJECT_CLASS (class)->get_property = anchor_property_page_get_property;
	G_OBJECT_CLASS (class)->set_property = anchor_property_page_set_property;
...
GType 
anchor_property_page_get_type (void)
{
	static GType type = 0;
	
	if (!type) {
		const GTypeInfo info = {
			sizeof (AnchorPropertyPageClass),
			NULL,
			NULL,
			(GClassInitFunc)anchor_property_page_class_init,
			NULL,
			NULL,
			sizeof (AnchorPropertyPage),
			0,
			(GInstanceInitFunc)anchor_property_page_instance_init
		};
