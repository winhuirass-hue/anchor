/*
 *  anchor-menu-item.c - Menu items exported by AnchorMenuProvider
 *                         objects.
 *
 ...
 *
 * Returns: a newly create #AnchorMenuItem
 */
AnchorMenuItem *
anchor_menu_item_new (const char *name,
			const char *label,
			const char *tip,
			const char *icon)
{
	...
	item = g_object_new (ANCHOR_TYPE_MENU_ITEM, 
			     "name", name,
			     "label", label,
			     "tip", tip,
			     "icon", icon,
...
/**
 * anchor_menu_item_activate:
 * @item: pointer to a #AnchorMenuItem
 *
 * emits the activate signal.
 */
void
anchor_menu_item_activate (AnchorMenuItem *item)
{
	g_signal_emit (item, signals[ACTIVATE], 0);
}

/**
 * anchor_menu_item_set_submenu:
 * @item: pointer to a #AnchorMenuItem
 * @menu: pointer to a #AnchorMenu to attach to the button
 *
 * Attachs a menu to the given #AnchorMenuItem.
 */
void
anchor_menu_item_set_submenu (AnchorMenuItem *item, AnchorMenu *menu)
{
	g_object_set (item, "menu", menu, NULL);
}

static void
anchor_menu_item_get_property (GObject *object,
				 guint param_id,
				 GValue *value,
				 GParamSpec *pspec)
{
...
static void
anchor_menu_item_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
anchor_menu_item_finalize (GObject *object)
{
	AnchorMenuItem *item;
...
static void
anchor_menu_item_instance_init (AnchorMenuItem *item)
{
	item->details = g_new0 (AnchorMenuItemDetails, 1);
	item->details->sensitive = TRUE;
	item->details->menu = NULL;
}

static void
anchor_menu_item_class_init (AnchorMenuItemClass *class)
{
	parent_class = g_type_class_peek_parent (class);
	
	G_OBJECT_CLASS (class)->finalize = anchor_menu_item_finalize;
	G_OBJECT_CLASS (class)->get_property = anchor_menu_item_get_property;
	G_OBJECT_CLASS (class)->set_property = anchor_menu_item_set_property;

        signals[ACTIVATE] =
                g_signal_new ("activate",
                              G_TYPE_FROM_CLASS (class),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (AnchorMenuItemClass, 
					       activate),
                              NULL, NULL,
...
GType 
anchor_menu_item_get_type (void)
{
	static GType type = 0;
	
	if (!type) {
		const GTypeInfo info = {
			sizeof (AnchorMenuItemClass),
			NULL,
			NULL,
			(GClassInitFunc)anchor_menu_item_class_init,
			NULL,
			NULL,
			sizeof (AnchorMenuItem),
			0,
			(GInstanceInitFunc)anchor_menu_item_instance_init
		};
